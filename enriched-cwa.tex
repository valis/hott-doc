...
% TODO: Define type theories in categories with fibrations.
...
there exists a cellular theory $T$ such that a left semi-model structure exists on $\Mod{T'}$ if and only if it exists on $\Mod{T}$ and these structures are Quillen equivalent.
...
The cellular theory of $\Sigma$-types $T_\Sigma$ is defined in a similar way, we just replace the computational rule for the eliminator $\Sigma\text{-}\fs{elim}$ with a propositional equality $\Sigma\text{-}\fs{elim}_\fs{eq}$.
Let $T_{\fs{Id} + \top + \Sigma}$ be the pushout $T_\top \amalg_{T_\fs{Id}} T_\Sigma$.

% TODO: Define weak equivalences in \Mod{T}.
...
% TODO: Define type theories in simplicial categories with fibrations.
