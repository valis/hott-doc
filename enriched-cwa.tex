...
For every sort $s$, we have a model $V_s$ of $T$ that represents the functor $M \mapsto M_s$.
There are obvious maps $V_{(\fs{ctx},n)} \to V_{(\fs{ty},n)}$ and $V_{(\fs{ty},n)} \to V_{(\fs{tm},n)}$.
Let $\I$ be the set of such maps.

\begin{defn}
We will say that a model structure (or a left semi-model structure) on $\Mod{T}$ (resp., $\Mod{A(T)}$) is \emph{standard} if $\I$ (resp., $a_!(\I)$) is a set of generating cofibrations for this model structure.
\end{defn}

In \cite{morita-equiv}, we proved that the category of type theories is locally presentable and defined a set of generating cofibrations on this category.
...
The cellular theory of identity types $T_\fs{Id}$ is defined as follows:
\medskip
\begin{center}
...
The cellular theory of unit types $T_\top$ is the following extension of $T_\fs{Id}$:
\medskip
\begin{center}
\AxiomC{}
...
The cellular theory of $\Sigma$-types $T_\Sigma$ can be defined in a similar way.
Let $T_{\fs{Id} + \top + \Sigma}$ be the pushout $T_\top \amalg_{T_\fs{Id}} T_\Sigma$.
We will define a standard model structure on $\Mod{T_{\fs{Id} + \top + \Sigma}}$.
First, we define a path object functor $P : \Mod{T_{\fs{Id} + \top + \Sigma}} \to \Mod{T_{\fs{Id} + \top + \Sigma}}$.
Let $X$ be a category with attributes.
