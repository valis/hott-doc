...
% 10. Доказательство, что в некоторых локалзациях есть индексированные произведения.

\subsection{Products and exponential objects}

In this subsection, we will prove that certain localizations of the injective structure on $\scat{C}^\scat{J}$ are Cartesian closed and locally small and have products.
...
We will say that a functor $F \in \scat{C}^\scat{J}$ is \emph{anticonstant} if, for every functor $R : \scat{C}^\scat{J}$ and every constant functor $D(X) \scat{C}^\scat{J}$,
every map $F \times R \to D(X)$ factors through the projection $\pi_2 : F \times R \to R$.
...
A map $p : F(-) \cdot 1 \to D(X)$ factors through a map $c : R \to D(X)$ if and only if $p_{j,a} = c$ for all $j$ and $a$.
...
% 11. Кусок про системы факторизаций в контекстуально индексированных контекстуальных категориях.

\section{Factorization systems}

In this section, we describe several examples of factorization systems.
A factorization system consists of a class of dependent types called fibrations closed under identity types and a map from types to fibrations satisfying a universal property.

\begin{lem}
Let $(\scat{C},\Fib)$ be a category with fibrations.
Let $\Fib' \subseteq \Fib$ be a class of fibrations such that every map factors into a map that has the left lifting property with respect to $\Fib'$ followed by a map in $\Fib'$.
Then $\scat{C}_!$ has a factorization system with $\Fib'$ as a class of fibrations.
\end{lem}
\begin{proof}
% TODO
\end{proof}

A large class of factorization systems can be defined as the localization at a finite set of families of maps.
A family of maps is a base type $I$ together with a map indexed by it:
\[ i : I \vdash f_i : \Hom(U_i,V_i). \]

Every class of dependent types $\Fib$ gives rise to a class of maps $\Fib'$.
A map belongs to $\Fib'$ if its fibers belong to $\Fib$.
Conversely, for every class of maps $\Fib'$, we can define a class of dependent types $\Fib$.
A type $\Delta.A$ belongs to $\Fib$ if the projection map $\Sigma(\Delta,A) \to \Sigma(\Delta)$ belongs to $\Fib'$.
These constructions are mutually inverse.

Let $\scat{C}$ be a contextually indexed contextual category.
Let $S$ be a finite set of families of maps of $\scat{C}$.
We will say that a map $p : \Hom(X,Y)$ is \emph{right orthogonal} to $S$ if the following square is a pullback for every $f_i \in S$:
\[ \xymatrix{ \Hom(V_i,X) \ar[r] \ar[d] & \Hom(U_i,X) \ar[d] \\
              \Hom(V_i,Y) \ar[r]        & \Hom(U_i,Y)
            } \]
Equivalently, $p$ is right orthogonal to $S$ if the type of lifts in the following square is contractible for every $f_i \in S$ and all maps $u : \Hom(U_i,X)$ and $v : \Hom(V_i,Y)$:
\[ \xymatrix{ U_i \ar[r]^u \ar[d]_{f_i}     & X \ar[d]^p \\
              V_i \ar[r]_v \ar@{-->}[ur]    & Y
            } \]

Now, let $S$ be a finite set of families of maps of $\scat{C}$ such that the domains and codomains of maps in $S$ are exponentiable.
We will say that a map $p : \Hom(X,Y)$ is \emph{internally right orthogonal} to $S$ if the following square is a pullback for every $f_i \in S$:
\[ \xymatrix{ (V_i \to X) \ar[r] \ar[d] & (U_i \to X) \ar[d] \\
              (V_i \to Y) \ar[r]        & (U_i \to Y)
            } \]
If $\scat{C}$ has a terminal type $1$ and $p$ is internally right orthogonal to $S$, then it is also right orthogonal to $S$.
To prove this, it is enough to apply $\Hom(1,-)$ to the pullback square above.
Since $\Hom(1,-)$ preserves pullbacks and maps $A \to B$ to $\Hom(A,B)$, we see that the required square is a pullback.

We will say that a dependent type is \emph{(internally) right orthogonal} to $S$ if the corresponding map is.

\begin{lem}
The class of types (internally) right orthogonal to a set $S$ is closed under unit types, $\Sigma$-types, identity types, and substitution.
\end{lem}
\begin{proof}
Any unit type $A$ is $S$-local since types $V_i \to A$ and $U_i \to A$ are contractible.
It is easy to see that $S$-local types are closed under substitution.
Indeed, if $\sigma : E \to \Delta$ is a substitution and $e$ is a proof that $f_i^* : (V_i \to A) \to (U_i \to A)$ is an equivalence, then $e[\sigma]$ is a proof that $f_i^* : (V_i \to A[\sigma]) \to (U_i \to A[\sigma])$ is an equivalence.

Let $\Delta.A$ and $\Delta, x : A. B$ be $S$-local types.
Let us prove that $\Sigma_{x : A} B$ is also $S$-local.
\end{proof}

% 12. Модельная структура на обогащенных категориях.
