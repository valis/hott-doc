...
\begin{example}
Let $\scat{C}$ be a complete right proper cofibrantly generated category with fibrations.
...
It follows that the right adjoint to the pullback functor $g^*$ for a $F^*$-fibration $g$ exists by \cite[Theorem~2.12]{comp-fact-tor}.
The fact that this right adjoint preserves fibrations is equivalent to the fact that injective trivial cofibrations are stable under pullbacks along $F^*$-fibrations.
...
\begin{prop}
Let $S$ be a set of maps of a left proper combinatorial model category $\scat{C}$.
Let $\Lambda(S)$ be a set of horns on $S$ as defined in \cite[Definition~3.3.8]{hirschhorn}.
...
\begin{defn}
A model category is \emph{semi-Cartesian} if it is Cartesian closed as a category and, for every pair of cofibrations $f : A \to B$ and $g : C \to D$,
the canonical map $f \square g : A \times D \amalg_{A \times C} B \times C \to B \times D$ is a cofibration.
\end{defn}

\begin{remark}[cartesian]
A model category is \emph{Cartesian} if it is semi-Cartesian, the terminal object is cofibrant, and $f \square g$ is a trivial cofibration whenever $f$ is a cofibration and $g$ is a trivial cofibration.
\end{remark}

\begin{example}
If a model category $\scat{C}$ is semi-Cartesian (resp., Cartesian), then $\scat{C}^\scat{J}$ with the injective structure is also semi-Cartesian (resp., Cartesian).
\end{example}

The main feature of semi-Cartesian model categories is that they have nice cosimplicial resolutions of objects and maps \cite[Definition~16.1.2, Definition~16.1.20]{hirschhorn}.
...
\begin{prop}
Let $\scat{C}$ be a proper combinatorial model category, let $\scat{J}$ be a small category, and let $S$ be a set of maps of $\scat{C}^\scat{J}$.
Then $D^*(L_S(\scat{C}^\scat{J}))$ has dependent products, where $D : \scat{C} \to \scat{C}^\scat{J}$ is the diagonal functor.
\end{prop}
\begin{proof}
% TODO
\end{proof}
