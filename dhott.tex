...
Let $\scat{C}$ be a complete cofibrantly generated type-theoretic semi-fibration category in which fibrations are exponentiable and let $\scat{J}$ be a small category.
Suppose that we have an injective class of fibrations closed under pullbacks, identity morphisms, and compositions such that every map in $(\scat{C}^\scat{J}/\Gamma)_\fs{f}$ for some $\Gamma$ factors into an objectwise trivial cofibration followed by a fibration.
Then this class of fibrations makes $\scat{C}^\scat{J}$ into a type-theoretic semi-fibration category.
\end{prop}
\begin{proof}
First, let us prove that injective fibrations are objectwise fibrations of $\scat{C}$.
...
Since $\scat{C}$ is complete and fibrations of $\scat{C}^\scat{J}$ are objectwise fibrations and fibrations of $\scat{C}$ are exponentiable, \cite[Theorem~2.12]{comp-fact-tor} implies that fibrations of $\scat{C}^\scat{J}$ are also exponentiable.
Since fibrations of $\scat{C}^\scat{J}$ are objectwise fibrations, to prove that other conditions hold, it is enough to prove that a map in $(\scat{C}^\scat{J}/\Gamma)_\fs{f}$ is a trivial cofibration if and only if it is an objectwise trivial cofibration.
The ``if'' direction holds by the assumption that fibrations are injective.
Coversely, if $i : A \to B$ is a map in $(\scat{C}^\scat{J}/\Gamma)_\fs{f}$ that has the left lifting property with respect to fibrations, then we can factor it into an objectwise trivial cofibration $i' : A \to A'$ followed by a fibration $A' \twoheadrightarrow B$.
Then $i$ is a retract of $i'$ which implies that it is also an objectwise trivial cofibration.
\end{proof}

\begin{cor}
Let $\scat{C}$ be a combinatorial type-theoretic semi-fibration category in which fibrations are exponentiable and let $\scat{J}$ be a small category.
Then $\scat{C}^\scat{J}$ is a combinatorial injective type-theoretic semi-fibration category in which fibrations are exponentiable.
