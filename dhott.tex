...
Let $(\scat{C},\Fib)$ be a type-theoretic semi-fibration category and let $\Fib'$ be a subclass of $\Fib$.
...
\begin{enumerate}
\item Every map in $((\scat{C},\Fib')/\Gamma)_\fs{f}$ factors into a trivial cofibration of $(\scat{C},\Fib')$ followed by a fibration in $\Fib'$.
\item A map in $((\scat{C},\Fib')/\Gamma)_\fs{f}$ is a trivial cofibration of $(\scat{C},\Fib')$ if and only if it is a trivial cofibration of $(\scat{C},\Fib)$.
\end{enumerate}
\end{lem}
\begin{proof}
...
Let $(\scat{C},\Fib)$ be a type-theoretic semi-fibration category and let $\Fib'$ be a subclass of $\Fib$ which contains identity morphisms and closed under compositions and pullbacks.
...
Let $\Fib'$ be a subclass of fibrations of $\scat{C}$ closed under identity types.
...
Let $\Fib$ be a class of injective fibrations closed under identity types.
...
% \begin{prop}
% Let $S$ be a class of maps of a model category closed under coidentity types.
...
Let $\scat{J}$ be a small category and let $S$ be a set of maps of $\sSet^\scat{J}$.
...
We will use the cosimplicial resolution of the form $Q(X) \times D(\Delta^n) \to Q(Y) \times D(\Delta^n)$.
Consider the following diagram:
\[ \xymatrix{ W \ar[r] \ar[d] \pb & Q(X) \times D(\Delta^n) \amalg_{Q(X) \times D(\partial \Delta^n)} Q(Y) \times D(\partial \Delta^n) \ar[d] \\
              V \ar[r] \ar[d] \pb & Q(Y) \times D(\Delta^n) \ar[d] \\
              D(B) \ar[r]_{D(p)} & D(A)
            } \]
where $p : B \to A$ is a fibration and $Q(f) : Q(X) \to Q(Y)$ is a cofibrant replacement of a map $f \in S$.
We need to prove that $W \to V$ is a trivial cofibration in $L_S(\scat{C}^\scat{J})$.
\Rlem{anticonstant-factor} implies that the map $Q(Y) \times D(\Delta^n) \to D(A)$ factor as the second projection followed by a map of the form $D(\Delta^n) \to D(A)$ for some map $c : \Delta^n \to A$.
Let us denote by $T$ the pullback of $p$ and $c$.
Then $V \simeq Q(Y) \times D(T)$.

Since $\sSet^\scat{J}$ is locally Cartesian closed, pullbacks along the map $Q(Y) \times D(T) \to Q(Y) \times D(\Delta^n)$ preserve colimits.
Thus, $W \to V$ is the following map:
\[ (Q(X) \times D(T)) \amalg_{(Q(X) \times D(T \times_{\Delta^n} \partial \Delta^n))} (Q(Y) \times D(T \times_{\Delta^n} \partial \Delta^n)) \to Q(Y) \times D(T). \]
That is, this map is $Q(f) \square D(g)$, where $g$ is the map $T \times_{\Delta^n} \partial \Delta^n \to T$.

The set $\Lambda(S)$ consists of maps of the form $Q(f) \square D(i)$ for all $f \in S$ and $i : \partial \Delta^n \to \Delta^n$.
By the usual argument, maps of the form $Q(f) \square i'$ belong to $\Icof[\Lambda(S)]$, where $i' \in \Icof[D(i)]$ and
...
% This might be false unless we assume some additional hypotheses on $\scat{C}$ or weaken the conclusion.
% \begin{remark}
% Let $\scat{C}$ be a proper type-theoretic combinatorial model category and let $S$ be a set of maps of $\scat{C}$.
% Then $L_S(\scat{C})$ is right proper if and only if pullbacks of maps in $\Lambda(S)$ along fibrations in $L_S(\scat{C})$ are trivial cofibrations in $L_S(\scat{C})$.
% An argument similar to the one that we used in the proof of \rprop{localized-products} shows that if $\scat{C}$ is semi-Cartesian closed, then $L_S(\scat{C})$ is right proper if and only if
% each map in $S$ has a cofibrant replacement such that its pullbacks along fibrations in $L_S(\scat{C})$ are trivial cofibrations in $L_S(\scat{C})$.
% \end{remark}
