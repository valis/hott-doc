...
\title{Complete Segal objects in contextually indexed categories}
...
Then
\begin{enumerate}
\item If $F^*((\scat{C},\Fib)_!)$ is locally small or has dependent $\Hom$-types, then this is also true for $F^*((\scat{C},\Fib')_!)$.
\item If $F^*((\scat{C},\Fib)_!)$ has extensional identity types, then identity types are also extensional in $F^*((\scat{C},\Fib')_!)$ if they exist there.
\item If $F^*((\scat{C},\Fib)_!)$ has extensional $\Pi$-types, then $\Pi$-types are also extensional in $F^*((\scat{C},\Fib')_!)$ if they exist there.
\end{enumerate}
\end{prop}
\begin{proof}
% TODO
...
% It should be possible to prove that identity types are extensional even if $\scat{C}$ is not right proper.
\end{prop}
\begin{proof}
...
By \rprop{localized}, it is enough to prove that any Cisinski model structure is a left Bousfield localization of a right proper model structure.
...
% If $\scat{C}$ is not right proper, but $F$ has a left adjoint, then the conclusion of the theorem still should be true.
\end{thm}
\begin{proof}
Unit types, $\Sigma$-types, and identity types exist by \rprop{indexed-locally-small}.
By \rcor{injective-combinatorial}, $(\scat{C}^\scat{J},\Fib)$ is a localization of a right proper extensional type-theoretic semi-fibration category.
By \rprop{localized}, it has dependent $\Hom$-types and extensional identity types.
\end{proof}
