...
\begin{example}
If $\scat{M}$ is the category of simplicial presheaves on a small category with its Cisinski model structure \cite{cisinski} and $\scat{J}$ is a small category, then $\scat{M}^\scat{J}$ with its injective model structure is a good model category.
\end{example}
...
Let $\scat{B}$ be a contextual category, let $\scat{C}$ be a category with fibrations, and let $F : \scat{B} \to \scat{C}$ be a functor between them.
A convenient way to define a full indexed subcategory of $F^*(\scat{C}_!)$ is to specify a subclass $\mathcal{F}$ of fibrations of $\scat{C}$.
But this actually defines not only full subcategory of $\scat{C}$, but also full subcategories of all slice categories $(\scat{C}/\Gamma)_\fs{f}$.
We will say that $\mathcal{F}$ is \emph{locally reflective} if it the right class in a weak factorization system.
...
We will say that a class $\mathcal{F}$ is \emph{representable} if there is a class of fibrations $\fs{Fib}$ as defined in \cite[Subsection~9.2]{indexed-tt} such that $\fs{Fib}(f)$ is inhabited if and only if $f$ belongs to $\mathcal{F}$.
In this case, we will say that $\fs{Fib}$ \emph{represents} $\mathcal{F}$ and that $\mathcal{F}$ is \emph{represented} by $\fs{Fib}$.
