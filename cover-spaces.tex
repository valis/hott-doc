...
\begin{example}
If the distance function of a metric space takes values in Dedekind real numbers, then it is strongly regular as a cover space.
We will call such metric spaces {Dedekind metric spaces}.

If the distance function takes values only in upper real numbers, then the metric space is still a cover space, even though not a strongly regular one.
\end{example}
...
With this definition, every strongly regular uniform space is a strongly regular cover space.
\end{example}
...
If $M$ is a locale, then a set is a Cauchy cover if and only if it is refined by the cover $\{ \varepsilon^*(a) \mid a \in C \}$
for some set $C \subseteq O_M$ satisfying $\bigvee C = \top$.
\end{lem}
\begin{proof}
Suppose $\bigvee C = \top$.
...
Since $\bigvee D' = \top$ and $\{ \varepsilon^*(a) \mid a \in D' \}$ refines $D$, this completes the proof.
\end{proof}
...
\begin{example}
The collection of uniform covers of a Dedekind metric space is a strongly regular base.
\end{example}
...
\begin{cor}
In a totally bounded Dedekind metric space, Cauchy covers coincide with uniform covers.
\end{cor}
