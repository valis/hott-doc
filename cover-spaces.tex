...
We will establish that there exists a full reflective subcategory of complete cover spaces (\rcor{complete-reflective}),
and that a certain full subcategory of locales is equivalent to a full subcategory of complete cover spaces,
encompassing a broad class of complete metric spaces (\rthm{locale-equiv}).
This serves as an alternative to localic completion \cite{localic-completion}.

The definition of cover spaces also enables us to generalize the Cauchy condition for sequences and, more broadly, nets (\rprop{conv-char}).
...
\end{prop}
\begin{proof}
This follows from \rlem{closure-filter}.
\end{proof}
...
Now, we can prove a version of \rprop{dense-unique} for strongly Hausdorff spaces:
...
We have an analogue of \rlem{cauchy-filter-rb}:
...
\end{prop}
\begin{proof}
The proof is the same as in \rprop{cauchy-filter-equiv}.
\end{proof}
...
This follows from \rlem{rbs-point}.
\end{example}
...
\end{prop}
\begin{proof}
The proof is the same as in \rprop{regular-minimal}.
\end{proof}
...
\end{prop}
\begin{proof}
The proof is the same as in \rprop{regular-filter-repr}.
\end{proof}
...
\end{lem}
\begin{proof}
The proof is the same as in \rlem{filter-point-char}.
\end{proof}
...
\end{lem}
\begin{proof}
We already proved in \rlem{filter-lift} that this filter satisfies the Cauchy condition.
...
\begin{proof}
The ``only if'' direction is obvious.
The coverse follows from \rlem{weak-filter-lift}.
\end{proof}
...
The proof of existence is the same as in \rthm{dense-lift} using \rlem{weak-filter-point-char} and \rlem{weak-filter-lift}.
\end{proof}
...
\rthm{weakly-dense-lift} implies that completion is unique up to isomorphism.
...
The set of Cauchy covers of $C(X)$ is defined as the set of those $C$ which are refined by the set $\{ \widetilde{U'} \mid U' \in C' \}$ for some Cauchy cover $C'$ in $X$.
The proof that $C(X)$ is a precover space is the same as in \rthm{completion}.
...
We already proved that the set $\{ x \in X \mid \eta_X(x) \in V \}$ belongs to $F$ in \rthm{completion}, but this set is empty by assumption, which contradicts weak properness of $F$.
This complete the proof that $\eta_X$ is dense.

Finally, completeness of $C(X)$ is proved as in \rthm{completion} using \rlem{strongly-complete-part}.
\end{proof}
...
\rlem{rbs-point} implies that, for every open $U$, we have $U = \bigcup \{ V \mid V \rb^s_X U \}$.
...
Since $G \subseteq F$ and $F$ is strongly regular, \rprop{strongly-regular-minimal} implies that $F \subseteq G$, which finishes the proof.
\end{proof}
...
\end{thm}
\begin{proof}
\rprop{u-strongly-regular}, \rprop{u-sober}, and \rprop{u-fully-faithful} imply that we indeed have such a functor and that it is fully faithful.
\rprop{t-complete} implies that it is essentially surjective on objects.
\end{proof}
...
Finally, if $X$ is strongly complete, then \rthm{weakly-dense-lift} implies that $\eta_X$ is an isomorphism since it is a weakly dense embedding.
This concludes the proof.
\end{proof}
...
\rlem{rat-gen} and \rcor{complete-strongly-complete} also imply that $\mathbb{R}$ is strongly complete and \rthm{locale-equiv} implies that $\mathbb{R} \simeq P(L(\mathbb{R})) \simeq P(\mathbb{R}_l)$.
\end{proof}
