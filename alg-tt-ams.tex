...
We define morphisms of theories $\mathbb{T}$ and $\mathbb{T}'$ as equivalence classes of certain functions.
Such a function $f$, for every function symbol $\sigma : s_1 \times \ldots \times s_k \to s$ of $\mathbb{T}$,
defines a restricted term $f(\sigma)$ of $\mathbb{T}'$ of sort $s$ such that $FV(f(\sigma)) = \{ x_1 : s_1, \ldots x_k : s_k \}$
and, for every predicate symbol $P : s_1 \times \ldots \times s_k$, defines a formula $f(P)$ of $\mathbb{T}'$ such that $FV(f(P)) = \{ x_1 : s_1, \ldots x_k : s_k \}$.
We will say that two such functions $f$ and $f'$ are equivalent if,
