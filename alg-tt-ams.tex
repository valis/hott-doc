...
for every $\sigma \in \mathcal{F}$, $\sigma : (p_1,q_1) \times \ldots \times (p_m,q_m) \to (p,q)$
and every $R \in \mathcal{P}$, $R : (p_1,q_1) \times \ldots \times (p_m,q_m)$, the following sequents are derivable in it:
\medskip
\begin{center}
\AxiomC{$x_1 : A_1, \ldots x_k : A_k, \Delta\!\uparrow^k\ \vdash \sigma_k(b_1, \ldots b_m)$}
\AxiomC{$\Gamma \vdash ctx$}
\AxiomC{$J$}
\TrinaryInfC{$\Gamma, \Delta\!\uparrow^n\ \vdash subst^q(\sigma_k(b_1, \ldots b_m), a_1, \ldots a_k) \deq \sigma_n(b_1', \ldots b_m')$}
\DisplayProof
\end{center}
\medskip
\begin{center}
\AxiomC{$x_1 : A_1, \ldots x_k : A_k, \Delta_i \vdash b_i$}
\AxiomC{$R_k(b_1, \ldots b_m)$}
\AxiomC{$\Gamma \vdash ctx$}
\AxiomC{$J$}
\QuaternaryInfC{$R_n(b_1', \ldots b_m')$}
\DisplayProof
\end{center}
where $J = \{ \Gamma \vdash a_i : A_i[x_1 \mapsto a_1, \ldots x_{i-1} \mapsto a_{i-1}]\ |\ 1 \leq i \leq k \}$ and $b_i' = subst^{q_i}(b_i, a_1, \ldots a_k)$.
...
\begin{enumerate}
\item $a \deq a'$, where $\Gamma \in Ctx(V)$ and $a,a' \in CTerm(V)_{(p,n)}$.
...
A \emph{sequent} is an expression of the form $\varphi \sststile{}{V} \psi$, where $\varphi$ and $\psi$ are contextual formulas with free variables in $V$ such that every variable in $V$ appears in $\varphi$.
The set of axioms of a theory consists of sequents.
...
\medskip
\begin{center}
$(\Gamma \vdash x \deq y) \land \varphi \sststile{}{V,x,y} \varphi[y/x]$ \axlabel{c2}
\qquad
...
\medskip
\begin{center}
$\Gamma \vdash  R(\ldots, (A^i_1, \ldots A^i_{n_i}).\,x_i, \ldots) \sststile{}{\Gamma, A^1_1, \ldots A^k_{n_k}, x_1, \ldots x_k} \Gamma, A^i_1, \ldots A^i_{n_i} \vdash x_i\!\downarrow$ \axlabel{c4}
\end{center}

\medskip
\begin{center}
$\Gamma \vdash x \deq y \sststile{}{\Gamma,x,y} \Gamma \vdash x\!\downarrow$ \axlabel{c4'}
\end{center}

\medskip
\begin{center}
$\Gamma \vdash \sigma(\ldots, (A^i_1, \ldots A^i_{n_i}).\,x_i, \ldots)\!\downarrow\ \sststile{}{\Gamma, A^1_1, \ldots A^k_{n_k}, x_1, \ldots x_k} \Gamma, A^i_1, \ldots A^i_{n_i} \vdash x_i\!\downarrow$ \axlabel{c5}
\end{center}

\medskip
\begin{center}
$A_1, \ldots A_n \vdash \varphi \sststile{}{A_1, \ldots A_n, V} A_1, \ldots A_{n-1} \vdash A_n\!\downarrow$ \axlabel{c6}
\end{center}

\medskip
\begin{center}
$(\Gamma \vdash \varphi) \land (\Gamma \vdash A\!\downarrow) \sststile{}{\Gamma, A, V} A, \Gamma \vdash \varphi$ \axlabel{c7}
\end{center}
...
Partial contextual terms $t$ and $t'$ are equivalent if and only if $FV(t) = FV(t')$ and sequents $\sststile{}{FV(t)} t \cong t'$ are derivable.
Contextual formulas $\varphi$ and $\psi$ are equivalent if and only if $FV(\varphi) = FV(\psi)$ and sequents $\varphi \ssststile{}{FV(\varphi)} \psi$ are derivable.

Morphisms of contextual theories $\mathbb{T}$ and $\mathbb{T}'$ are equivalence classes of functions $f$ such that for every function symbol $\sigma : (p_1,n_1) \times \ldots \times (p_k,n_k) \to p$ of $\mathbb{T}$, $f(\sigma)$ is a partial contextual term of $\mathbb{T}'$ of sort $(p,0)$ with free variables in $\{ A^1_1 , \ldots A^1_{n_1}, x_1, \ldots A^k_1, \ldots A^k_{n_k}, x_k \}$, where $A^i_j : (ty,j)$ and $x_i : (p,n_i)$, and for every predicate symbol $R : (p_1,n_1) \times \ldots \times (p_k,n_k)$, $f(R)$ is a contextual formula of $\mathbb{T}'$ with free variables in the same set as above.
...
\begin{align*}
\Gamma \vdash a\!\downarrow\ & \ssststile{}{\Gamma,a} \Gamma \vdash ty(a)\!\downarrow \\
A_1, \ldots A_n \vdash ctx & \sststile{}{A_1, \ldots A_n} A_1, \ldots A_n \vdash v_i : A_{n-i},
\end{align*}
where $\Gamma \vdash a : A$ stands for $\Gamma \vdash ty(a) \deq A$.
We will also write $\Gamma \vdash a \deq a' : A$ for $(\Gamma \vdash a \deq a') \land (\Gamma \vdash a : A)$.
...
\begin{align*}
\Gamma, ty(a) \vdash b\!\downarrow\ & \ssststile{}{\Gamma, a, b} \Gamma \vdash b[a]\!\downarrow \\
\Gamma, ty(a) \vdash b\!\downarrow\ & \sststile{}{\Gamma, a, b} \Gamma \vdash ty(b[a]) \deq ty(b)[a] \\
(\Gamma \vdash a_0\!\downarrow) \land \ldots \land (\Gamma \vdash a_i\!\downarrow)\ & \sststile{}{\Gamma, a_0, \ldots a_i} \Gamma \vdash v_i[a_0] \ldots [a_i] \deq a_i
\end{align*}
...
C(\Gamma, \sigma(\ldots, (A^i_1, \ldots A^i_{n_i}).\,t_i, \ldots)) & = \sigma_n(\ldots, C(C(\Gamma, \ldots C(\Gamma, A^i_1) \ldots A^i_{n_i}), t_i), \ldots)
\end{align*}
...
C(\Gamma, R(\ldots, (A^i_1, \ldots A^i_{n_i}).\,t_i, \ldots)) & = R_n(\ldots, C(C(\Gamma, \ldots C(\Gamma, A^i_1) \ldots A^i_{n_i}), t_i), \ldots)
\end{align*}
...
The set of axioms of $C(\mathbb{T})$ consists of axioms of $\mathbb{T}_1$, axiom $C(S)$ for every axiom $S$ of $\mathbb{T}$, and the following axioms:
\begin{align*}
...
For every theory $\mathbb{T}$, we can define a function $\alpha : L(C(\mathbb{T})) \to C(\mathbb{T})$ as follows:
\begin{align*}
...
\[ \bigwedge_{1 \leq i \leq n} \Gamma_i \vdash \varphi_i \sststile{}{V} \bigwedge_{1 \leq i \leq k} \Delta_i \vdash \psi_i, \]
the following sequent is derivable in $\mathbb{T}$:
\[ \bigwedge_{1 \leq i \leq n} A, \Gamma_i \vdash \varphi_i \sststile{}{A,V} \bigwedge_{1 \leq i \leq k} A, \Delta_i \vdash \psi_i, \]
...
\end{prop}
\begin{proof}
First, let us prove that $C$ preserves substitution in a weak sence.
Let $t$ be a term, and let $x$ be a variable.
Then for every occurrence of $x$ in $t$, we can define a context $\Delta$ such that $\Gamma \vdash t\!\downarrow\ \sststile{}{V} ctx(x) = \Delta$.
...
where $\Delta$ is a context corresponding to an occurence of $x$ in $\varphi$.
...
\UnaryInfC{$C(\varphi)[C(\Delta,t)/x] \sststile{}{V,V'} C(\psi)[C(\Delta',t)/x]$}
\DisplayProof
\end{center}
Note that $C(\varphi) \sststile{}{V,x} C(\psi)$ implies that $C(\Delta) = C(\Delta')$.
Hence this rule is admissible.
Finally, it is easy to see that rules $\axref{c4}-\axref{c5}$ are also preserved.

It follows that $C$ is well-defined.
Let us prove that $C$ is surjective.
First, we need to define functions $D : Term_{C(\mathbb{T})}(V)_{(p,n)} \to CTerm_\mathbb{T}(D(V))_{(p,n)}$ and $D : Term_{C(\mathbb{T})}(V)_{(ctx,n)} \to Ctx(V)_n$,
where $D(V)$ is the union of $V$ and the set of variables $x^i$ for every $x : (p,n)$ in $V$, $p \in \{ty,tm\}$ and $0 \leq i < n$.
They are defined recursively:
\begin{align*}
D(x) & = x \\
D(\sigma_n(\Gamma, t_1, \ldots t_k)) & = \sigma(\ldots, ctx_{\leq n}(t_i).\,D(t_i), \ldots)
\end{align*}
\end{proof}

\subsection{Named representation of terms}

We can use usual named representation of terms for stable theories.
Let $var$ be a set of variables.
Let $\mathbb{T} = ((\mathcal{C}, \mathcal{F}, \mathcal{P}), \mathcal{A})$ be a stable theory.
Then for every set $V$, we define the set $NTerm(V)$ of named preterms of theory $T$ inductively as follows:
\begin{enumerate}
\item For every $x \in var$, $x$ is a preterm.
\item For every $X \in V$, $X$ is a preterm.
\item For every term $t$, $ft(t)$ and $ctx_{p,n}(t)$ are preterms.
\item For every $\sigma \in \mathcal{F}$, $\sigma : (ctx,n) \times (p_1,n_1) \times \ldots \times (p_k,n_k) \to (p,n)$,
variables $x^1_1$, \ldots $x^1_{n_1}$, \ldots $x^k_1$, \ldots $x^k_{n_k}$, preterms $A^1_1$, \ldots $A^1_{n_1}$, \ldots $A^k_1$, \ldots $A^k_{n_k}$
and preterms $t_1$, \ldots $t_k$, the following expression is a preterm:
\[ \sigma(x^1_1 : A^1_1 \ldots x^1_{n_1} : A^1_{n_1}.\ t_1, \ldots x^k_1 : A^k_1 \ldots x^k_{n_k} : A^k_{n_k}.\ t_k). \]
\end{enumerate}

Let $Ctx(V)$ be the set of precontext, that is finite sequences of expressions of the form $x : t$, where $x \in var$ and $t \in NTerm(V)$.
Now, we can define partial functions $E : Ctx(V) \times NTerm(V) \to \coprod_{s \in \mathcal{C}} PTerm(V)_s$ and $E : Ctx(V) \to \coprod_{s \in \mathcal{C}} PTerm(V)_s$ as follows:
\begin{align*}
E(\cdot) & = \emptyCtx \text{, where $\cdot$ is the empty precontext} \\
E(x_1:A_1, \ldots x_n:A_n) & = E((x_1:A_1, \ldots x_{n-1}:A_{n-1}), A_n) \\
E((x_1:A_1, \ldots x_n:A_n), x_i) & = v_{n,n-i}(E((x_1:A_1, \ldots x_{n-1}:A_{n-1}), A_n)) \\
E(\Gamma,ft^n(X)) & = ft^n(X)|_{ft^{n+1}(X) = E(\Gamma)} \\
E(\Gamma,\sigma(t_1', \ldots t_k')) & = \sigma_{|\Gamma|}(E(\Gamma), t_1'', \ldots t_k'')
\end{align*}
where $t_i' = x^i_1 : A^i_1, \ldots x^i_{n_i} : A^i_{n_i}.\ t_i$, $t_i'' = E((\Gamma, x^i_1 : A^i_1, \ldots x^i_{n_i} : A^i_{n_i}), t_i)$,
and $\sigma_m : (ctx,m) \times (p_1,n_1+m) \times \ldots \times (p_k,n_k+m) \to (p,n+m)$ is defined inductively as follows:
\begin{align*}
...
We say that a preterm $t$ is a \emph{correct term} in a context $\Gamma$ and write $\Gamma \vdash t$ if $E(\Gamma,t)$ is defined.
If we associate to every $X \in V_{(p,n)}$ a context $x_1, \ldots x_n$ in which it is defined,
then we can also use the ordinary syntax for substitutions, that is write $X[y_1 \mapsto t_1, \ldots y_k \mapsto t_k]$,
where $\{ y_1, \ldots y_k \}$ is a subset of $\{ x_1, \ldots x_n \}$.

If $A$ and $A'$ are terms of sort $(ty,n)$, $a$ and $a'$ are terms of sort $(tm,n)$, and $\Gamma$ is a context of length $n$, then we use the following abbreviations:
\begin{align*}
\Gamma \vdash ctx & \text{ means } E(\Gamma)\!\downarrow \\
\Gamma \vdash a & \text{ means } E(\Gamma,a)\!\downarrow \\
\Gamma \vdash A\ type & \text{ means } E(\Gamma,A)\!\downarrow \\
\Gamma \vdash a : A & \text{ means } ty(E(\Gamma,a) = E(\Gamma,A) \\
\Gamma \vdash A \deq A' & \text{ means } E(\Gamma,A) = E(\Gamma,A') \\
\Gamma \vdash a \deq a' : A & \text{ means } E(\Gamma,a) = E(\Gamma,a') \land (\Gamma \vdash a : A)
\end{align*}
...
\AxiomC{$\ldots$}
\AxiomC{$J_n$}
\TrinaryInfC{$\Gamma \vdash a : A$}
\DisplayProof
\end{center}
denotes the conjunction of two sequents $E(\Gamma,a)\!\downarrow\ \sststile{}{V} J_1 \land \ldots \land J_n$ and $J_1 \land \ldots \land J_n \sststile{}{V} \Gamma \vdash a : A$,
where $V$ is the set of variables that occur in $J_1$, \ldots $J_n$, $\Gamma$, $a$, and $A$.
That is, when we write such rule we mean that this sequents are derivable in the theory.
...
\AxiomC{$\ldots$}
\AxiomC{$J_n$}
\TrinaryInfC{$\Gamma \vdash A\ type$}
\DisplayProof
\end{center}
denotes sequents $J_1 \land \ldots \land J_n \ssststile{}{V} \Gamma \vdash A\ type$.

Finally, other rules of the form
\medskip
\begin{center}
\AxiomC{$J_1$}
\AxiomC{$\ldots$}
...
\DisplayProof
\end{center}
denote sequent $J_1 \land \ldots \land J_n \sststile{}{V} J$.

If our theory has substitutions, then we can also use weakening in named terms.
The operations of weakening $wk^m_{p,n} : (ctx,n+m) \times (p,n) \to (p,n+m)$ are defined as follows:
\[ wk^m_{p,n}(\Gamma,a) = subst_{p,n+m,n}(\Gamma, a, v_{n+m-1}, \ldots v_m) \]
We can extend the definition of preterms to include the following clause.
If $t$ is a preterm and $m \in \mathbb{N}$, then $t\!\uparrow^m$ is also a preterm.
The definition of $E$ is extended as follows:
\[ E(\Gamma, t\!\uparrow^m) = wk^m_{p,n}(E(\Gamma), E(\Gamma, t)) \]
We can also omit the first argument in $subst_{p,n,k}$ since they can be recovered as follows:
\[ E(\Gamma, subst_{p,n,k}(B, a_1, \ldots a_k)) = subst_{p,n,k}(E(\Gamma), E(\Gamma,B), E(\Gamma,a_1), \ldots E(\Gamma,a_k)) \]

\begin{example}
The weakening operations satisfy the following rules:
\medskip
\begin{center}
\AxiomC{$\Gamma, y_1 : B_1, \ldots y_m : B_m \vdash ctx$}
\AxiomC{$\Gamma \vdash A\ type$}
\BinaryInfC{$\Gamma, y_1 : B_1, \ldots y_m : B_m \vdash A\!\uparrow^m type$}
\DisplayProof
\end{center}
...
Often, we left the weakening operations implicit since they can be easily recovered.
...
\medskip
\begin{center}
\AxiomC{$x : A \vdash B\ type$}
\UnaryInfC{$\vdash \Sigma(x : A.\ B)\ type$}
\DisplayProof
\quad
\AxiomC{$x : A \vdash B\ type$}
\AxiomC{$\vdash a : A$}
\AxiomC{$\vdash b : B[x \mapsto a]$}
\TrinaryInfC{$\vdash pair(x : A.\ B, a, b) : \Sigma(x : A.\ B)$}
\DisplayProof
\end{center}

\medskip
\begin{center}
\AxiomC{$\vdash p : \Sigma(x : A.\ B)$}
\UnaryInfC{$\vdash proj_1(x : A.\ B, p) : A$}
\DisplayProof
\quad
\AxiomC{$\vdash p : \Sigma(x : A.\ B)$}
\UnaryInfC{$\vdash proj_2(x : A.\ B, p) : B[x \mapsto proj_1(x : A.\ B, p)]$}
\DisplayProof
\end{center}
...
\AxiomC{$\vdash a : A$}
\AxiomC{$\vdash b : B[x \mapsto a]$}
\TrinaryInfC{$\vdash proj_1(x : A.\ B, pair(x : A.\ B, a, b)) \deq a$}
\DisplayProof
\end{center}
...
\AxiomC{$\vdash a : A$}
\AxiomC{$\vdash b : B[x \mapsto a]$}
\TrinaryInfC{$\vdash proj_2(x : A.\ B, pair(x : A.\ B, a, b)) \deq b$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$\vdash p : \Sigma(x : A.\ B)$}
\UnaryInfC{$\vdash pair(x : A.\ B, proj_1(x : A.\ B, p), proj_2(x : A.\ B, p)) \deq p$}
\DisplayProof
\end{center}
\end{example}
...
\Sigma & : (ty,1) \to (ty,0) \\
pair & : (ty,1) \times (tm,0) \times (tm,0) \to (tm,0) \\
\Sigma\text{-}elim & : (ty,1) \times (ty,1) \times (tm,2) \times (tm,0) \to (tm,0)
\end{align*}
The axioms for $\Sigma$ and $pair$ are the same, and the axioms for $\Sigma\text{-}elim$ are
\medskip
\begin{center}
\def\extraVskip{1pt}
\Axiom$\fCenter z : \Sigma(x : A.\ B) \vdash D\ type$
\noLine
\UnaryInf$\fCenter x : A, y : B \vdash d : D[z \mapsto pair(x : A.\ B, x, y)]$
\noLine
\UnaryInf$\fCenter \vdash p : \Sigma(x : A.\ B)$
\def\extraVskip{2pt}
\UnaryInf$\fCenter \vdash \Sigma\text{-}elim(x : A.\ B, z. D, x y. d, p) : D[z \mapsto p]$
\DisplayProof
\end{center}
...
\noLine
\UnaryInf$\fCenter \vdash b : B[x \mapsto a]$
\def\extraVskip{2pt}
\UnaryInf$\fCenter \vdash \Sigma\text{-}elim(x : A. B, z. D, x y. d, pair(x : A.\ B, a, b)) \deq d[x \mapsto a, y \mapsto b]$
\DisplayProof
\end{center}
\end{example}
...
\begin{center}
\AxiomC{$x : A \vdash B\ type$}
\UnaryInfC{$\vdash \Pi(x : A.\ B)\ type$}
\DisplayProof
\quad
\AxiomC{$x : A \vdash b : B$}
\UnaryInfC{$\vdash \lambda(x : A.\ b) : \Pi(x : A.\ B)$}
\DisplayProof
\end{center}

\medskip
\begin{center}
\AxiomC{$\vdash f : \Pi(x : A.\ B)$}
\AxiomC{$\vdash a : A$}
\BinaryInfC{$\vdash app(x : A.\ B, f, a) : B[x \mapsto a]$}
\DisplayProof
\end{center}
...
\AxiomC{$\vdash a : A$}
\AxiomC{$x : A \vdash b : B$}
\BinaryInfC{$\vdash app(x : A.\ B, \lambda(x : A.\ b), a) \deq b[x \mapsto a]$}
\DisplayProof
\quad
\AxiomC{$\vdash b : \Pi(x : A.\ B)$}
\UnaryInfC{$\vdash \lambda(x : A.\ app(x : A.\ B, b, x)) \deq b$}
\DisplayProof
\end{center}
\end{example}
...
\medskip
\begin{center}
\AxiomC{$x : A, y : A, z : Id(A, x, y) \vdash D\ type$}
\AxiomC{$x : A \vdash d : D'$}
\AxiomC{$\vdash p : Id(A, a, a')$}
\TrinaryInfC{$\vdash J(A, x y z. D, x. d, a, a', p) : D[x \mapsto a, y \mapsto a', z \mapsto p]$}
\DisplayProof
\end{center}
...
\AxiomC{$x : A \vdash d : D'$}
\AxiomC{$\vdash a : A$}
\TrinaryInfC{$\vdash J(A, x y z. D, x. d, a, a, refl(A, a)) \deq d[x \mapsto a]$}
\DisplayProof
\end{center}
\medskip

where $D' = D[y \mapsto x, z \mapsto refl(A, x)]$.
\end{example}
...
\begin{example}
Let $\mathbb{T}$ be a theory in $\algtt^1$, and let $\mathcal{F}'$ be a subset of the set $\mathcal{F}$ of function symbols of $\mathbb{T}$.
We want to define a theory that contains a universe that is closed under functions in $\mathcal{F}'$.
...
Then theory $U(\mathbb{T})$ has the same function symbols, predicate symbols and axioms as $\mathbb{T}$, and also the following symbols:
\begin{align*}
U & : (ty,n) \\
El & : (tm,n) \to (ty,n) \\
\Sigma_U & : (tm,n) \times (tm,n+1) \to (tm,n) \\
\Pi_U & : (tm,n) \times (tm,n+1) \to (tm,n) \\
Id_U & : (tm,n) \times (tm,n) \times (tm,n) \to (tm,n)
\end{align*}
...
\begin{center}
\AxiomC{$\Gamma \vdash A : U$}
\AxiomC{$\Gamma, x : El(A) \vdash B : U$}
\BinaryInfC{$\Gamma \vdash \Sigma_U(A, x.B) : U$}
\DisplayProof
\end{center}
...
\AxiomC{$\Gamma \vdash A : U$}
\AxiomC{$\Gamma, x : El(A) \vdash B : U$}
\BinaryInfC{$\Gamma \vdash \Pi_U(A, x.B) : U$}
\DisplayProof
\end{center}
...
\AxiomC{$\Gamma \vdash a' : El(A)$}
\TrinaryInfC{$\Gamma \vdash Id_U(A, a, a') : U$}
\DisplayProof
\end{center}
...
\BinaryInfC{$\Gamma \vdash El(\Sigma_U(A, x.B)) \deq \Sigma(x : El(A).\ El(B))$}
\DisplayProof
\end{center}
...
\AxiomC{$\Gamma \vdash A : U$}
\AxiomC{$\Gamma, x : A \vdash B : U$}
\BinaryInfC{$\Gamma \vdash El(\Pi_U(A, x.B)) \deq \Pi(x : El(A).\ El(B))$}
\DisplayProof
\end{center}
...
\AxiomC{$\Gamma \vdash a : El(A)$}
\AxiomC{$\Gamma \vdash a' : El(A)$}
\TrinaryInfC{$\Gamma \vdash El(Id_U(A, a, a')) \deq Id(El(A), a, a')$}
\DisplayProof
\end{center}
\end{example}
