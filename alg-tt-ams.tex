...
\UnaryInfC{$\varphi[t/x] \sststile{}{} \psi[t/x]$}
\DisplayProof
\end{center}

\medskip
\begin{center}
$\Gamma \vdash  R(\ldots, (A^i_1, \ldots A^i_{n_i}).\,x_i, \ldots) \sststile{}{} \Gamma, A^i_1, \ldots A^i_{n_i} \vdash x_i\!\downarrow$ \axlabel{c4}
\end{center}
...
\medskip
\begin{center}
$\Gamma \vdash \sigma(\ldots, (A^i_1, \ldots A^i_{n_i}).\,x_i, \ldots)\!\downarrow\ \sststile{}{} \Gamma, A^i_1, \ldots A^i_{n_i} \vdash x_i\!\downarrow$ \axlabel{c5}
\end{center}

\medskip
\begin{center}
$A_1, \ldots A_n \vdash \varphi \sststile{}{} A_1, \ldots A_{n-1} \vdash A_n\!\downarrow$ \axlabel{c6}
\end{center}

\medskip
\begin{center}
$(\Gamma \vdash \varphi) \land (\Gamma \vdash A\!\downarrow) \sststile{}{} A, \Gamma \vdash \varphi$ \axlabel{c7}
\end{center}
...
Partial contextual terms $t$ and $t'$ are equivalent if and only if $FV(t) = FV(t')$ and sequents $\sststile{}{} t \cong t'$ are derivable.
...
Morphisms of contextual theories $\mathbb{T}$ and $\mathbb{T}'$ are equivalence classes of functions $f$ such that for every function symbol
$\sigma : (p_1,n_1) \times \ldots \times (p_k,n_k) \to p$ of $\mathbb{T}$, $f(\sigma)$ is a partial contextual term of $\mathbb{T}'$ of sort $(p,0)$
with free variables in $\{ A^1_1 , \ldots A^1_{n_1}, x_1, \ldots A^k_1, \ldots A^k_{n_k}, x_k \}$, where $A^i_j : (ty,j)$ and $x_i : (p,n_i)$,
and for every predicate symbol $R : (p_1,n_1) \times \ldots \times (p_k,n_k)$, $f(R)$ is a contextual formula of $\mathbb{T}'$ with free variables in the same set as above.
...
\medskip
\begin{center}
\AxiomC{$\Gamma, A \vdash \sigma(\ldots, (B^i_1, \ldots B^i_{n_i}).\,x_i, \ldots)\!\downarrow$}
...
\medskip
\begin{center}
\AxiomC{$\Gamma, A \vdash R(\ldots, (B^i_1, \ldots B^i_{n_i}).\,x_i, \ldots)$}
\AxiomC{$\Gamma \vdash a : A$}
\BinaryInfC{$\Gamma \vdash R(\ldots, x_i', \ldots)$}
\DisplayProof
\end{center}
