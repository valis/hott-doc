...
First, we need to define functions $F_p : Term_{C(\mathbb{T})}(V)_{(p,n)} \to PCTerm_\mathbb{T}(F(V))_{(p,n)}$ and $G : Term_{C(\mathbb{T})}(V)_{(ctx,n)} \to PCtx(F(V))_n$,
...
\begin{align*}
F_p(x) & = x^0, \ldots x^{n-1} \vdash x \\
F_p(\sigma_n(\Gamma, t_1, \ldots t_k)) & = G(\Gamma) \vdash \sigma(F_{p_1}(t_1), \ldots F_{p_k}(t_k)) \\
F_{tm}(v_{n,i}(\Gamma)) & = v_i \\
F_p(subst(\Gamma, b, t_1, \ldots t_k)) & = G(\Gamma) \vdash F_p(b)[F_{tm}(t_1)] \ldots [F_{tm}(t_k)] \\
F_{ty}(ty_n(t)) & = ty(F_{tm}(t)) \\
F_{ty}(ft^k(ctx(x))) & = x^0, \ldots x^{n-k-2} \vdash x^{n-k-1} \\
F_{ty}(ft^k(ctx(\sigma_n(\Gamma, t_1, \ldots t_k)))) & = F_{ty}(ft^k(\Gamma)) \\
G(t) & = G(ft(t)), F_{ty}(t)
\end{align*}
where $\Gamma \vdash t$ stands for $t|_{\Gamma \vdash t \downarrow}$
...
\[ g(\sigma(\ldots, (x_i^0, \ldots x_i^{n_i-1}).\,x_i, \ldots)) = F_p(f(\sigma_0(\emptyCtx, x_1, \ldots x_k)))|_{\bigwedge_{1 \leq i \leq k} x_i^0, \ldots x_i^{n_i-1} \vdash x_i \downarrow} \]
...
% \[ \sststile{}{x_1, \ldots x_k} C(\Gamma, F_p(t))[ft^{n_i-1-j}(ctx(x_i))/x_i^j] \cong t|_{ctx(t) = \Gamma} \]
\begin{align*}
...
It is easy to see that $F_p$ preserves inference rules.
It follows that $F_p$ preserves theorems in the sense that if $\varphi \sststile{}{x_1, \ldots x_k} \psi$ is a theorem,
...
\begin{align*}
F(t = t') & = G(ctx(t)) \vdash F_p(t) \deq F_p(t') \land G(ctx(t)) = G(ctx(t')) \\
F(R_n(\Gamma, t_1, \ldots t_k)) & = \Gamma' \vdash R(\ldots, G(ctx(t_i))_{\geq n}.\,F_{p_i}(t_i), \ldots)|_{\bigwedge_{1 \leq i \leq k} \Gamma' = G(ctx(t_i))_{<n}},
\end{align*}
where $\Gamma' = F_{ty}(\Gamma)$.
...
\[ \bigwedge_{1 \leq i \leq k} \varphi_i \sststile{}{}\ \vdash F_p(C(f)(\sigma_0(\emptyCtx, x_1, \ldots x_k))) \cong F_p(C(g)(\sigma_0(\emptyCtx, x_1, \ldots x_k))) \]
...
\[ \bigwedge_{1 \leq i \leq k} \varphi_i \sststile{}{} \Gamma \vdash t \cong F_p(C(C(\Gamma),t)) \]
