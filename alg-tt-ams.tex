...
In section 5, we define contextual theories which can be represented in a way that does not involve explicit weakening operations.
In section 6, we give a few standard examples of algebraic dependent type theories.
...
Each function symbol $\sigma \in \mathcal{F}_0$ of a contextual theory is equipped with a signature:
...
\item $\sigma(t_1, \ldots t_k) \in CTerm(V)_{(p,n)}$
for every $\sigma \in \mathcal{F}_0$, $\sigma : (p_1,n_1) \times \ldots \times (p_k,n_k) \to p$ and $t_i \in CTerm(V)_{(p_i,n+n_i)}$.
\end{enumerate}
...
\item $R(t_1, \ldots t_k)$, where $R : (p_1,n_1) \times \ldots \times (p_k,n_k)$ and $t_i \in CTerm(V)_{(p,n+n_i)}$.
\end{enumerate}
...
if $e = x \in V_{(p,m)}$ is a term of sort $(p,n)$, then we define $x[t/x]$ as $t\!\uparrow^{n-m}$, where $\uparrow^i = \uparrow^i_0$ and
$\uparrow^i_j : CTerm(V)_{(p,m)} \to CTerm(V)_{(p,m+i)}$ is the usual operation of lifting de Bruijn indices defined as follows:
\begin{align*}
v_k\!\uparrow^i_j & =
  \begin{cases}
    v_k         & \text{if } k < j \\
    v_{k+i}     & \text{if } k \geq j \\
  \end{cases} \\
x\!\uparrow^i_j & = x \\
\sigma(t_1, \ldots t_k)\!\uparrow^i_j & = \sigma(t_1\!\uparrow^i_{j+n_1}, \ldots t_k\!\uparrow^i_{j+n_k})
\end{align*}

A \emph{contextual theory} consists consists of sets $\mathcal{F}_0$ and $\mathcal{P}_0$ of function and predicate symbols respectively and a set of axioms $\mathcal{A}$.
Moreover, every function symbol $\sigma : (p_1,n_1) \times \ldots \times (p_k,n_k) \to p$ is equipped with
a context $D_i(\sigma) \in Ctx(\{ x_1, \ldots x_{i-1} \})$ of length $n_i$ for every $1 \leq i \leq k$.
Similarly, every predicate symbol $R : (p_1,n_1) \times \ldots \times (p_k,n_k)$ is equipped with
a context $D_i(R) \in Ctx(\{ x_1, \ldots x_{i-1} \})$ of length $n_i$ for every $1 \leq i \leq k$.
Note that for every occurrence of a variable in a term or a formula, we can define a context that corresponds to this occurrence repeatedly applying functions $D_i$.

We need to assume that contexts $D_i(\sigma)$ satisfies two technical assumptions.
First, we assume that every variable occurs at most once in $D_i(\sigma)$.
Second, let $\prec$ be the relation on the set of function symbols which is defined as follows:
$\tau \prec \sigma$ if and only if $\tau$ occurs in $D_i(\sigma)$ for some $i$.
We assume that this relation is well-founded.
...
\medskip
\begin{center}
$(\Gamma \vdash x \deq y) \land \varphi \sststile{}{} \varphi[y/x]$ \axlabel{c2}
\end{center}

\medskip
\begin{center}
\AxiomC{$\varphi \sststile{}{} \psi$}
\RightLabel{\axlabel{c3}}
\UnaryInfC{$\bigwedge_{i \in I} E = E_i[t/x] \land \varphi[t/x] \sststile{}{} \psi[t/x]$}
\DisplayProof
\end{center}

\medskip
\begin{center}
$\Gamma \vdash  R(x_1, \ldots x_k) \sststile{}{} \Gamma, D_i(R) \vdash x_i\!\downarrow$ \axlabel{c4}
\qquad
$\Gamma \vdash x \deq y \sststile{}{} \Gamma \vdash x\!\downarrow$ \axlabel{c4'}
\end{center}

\medskip
\begin{center}
$\Gamma \vdash \sigma(x_1, \ldots x_k)\!\downarrow\ \sststile{}{} \Gamma, D_i(\sigma) \vdash x_i\!\downarrow$ \axlabel{c5}
\qquad
$\Gamma \vdash x\!\downarrow \land \Delta \vdash x\!\downarrow\ \sststile{}{} \Gamma = \Delta$ \axlabel{c6}
\end{center}

\medskip
\begin{center}
\AxiomC{$\Gamma_1 \vdash \varphi_1 \land \ldots \land \Gamma_n \vdash \varphi_n \sststile{}{} \Gamma \vdash \psi$}
\RightLabel{\axlabel{c7}}
\UnaryInfC{$A \vdash ctx \land A, \Gamma_1 \vdash \varphi_1 \land \ldots \land A, \Gamma_n \vdash \varphi_n \sststile{}{} A, \Gamma \vdash \psi$}
\DisplayProof
\end{center}
where $E$ is a context corresponding to the leftmost occurrence of $x$ in $\varphi$.
Since $E$ corresponds to the leftmost occurrence of $x$, $x \notin FV(E)$.
Set $\{E_i\}_{i \in I'}$ consists of contexts that correspond to occurrences of $x$ in $\varphi$ and in every context $E_i$.
Since every variable occurs at most once in $D_i(\sigma)$, this set is finite.
Set $\{E_i\}_{i \in I}$ consists of contexts from $\{E_i\}_{i \in I'}$ except for $E$.
In particular, if $\{E_i\}_{i \in I'}$ consists of a single context (which is often the case in practice), then \axref{c3} looks exactly as \axref{a3}.

Now, we define partial contextual terms as pairs of contextual terms and contextual formulas.
As before, we define equivalence relations on partial contextual terms and contextual formulas.
Partial contextual terms $t$ and $t'$ of sort $(p,0)$ are equivalent if and only if $FV(t) = FV(t')$ and sequents $\sststile{}{}\ \vdash t \cong t'$ are derivable.
Contextual formulas $\varphi$ and $\psi$ are equivalent if and only if $FV(\varphi) = FV(\psi)$ and sequents $\varphi \ssststile{}{} \psi$ are derivable.
...
such that $FV(f(\sigma)) = \{ x_1, \ldots x_k \}$, where $x_i : (p,n_i)$ and, for every predicate symbol $R : (p_1,n_1) \times \ldots \times (p_k,n_k)$,
$f(R)$ is a contextual formula of $\mathbb{T}'$ with the same free variables as above.
...
\begin{align*}
\vdash f(\sigma)\!\downarrow\ & \sststile{}{} f(D_i(\sigma)) \vdash x_i\!\downarrow \\
\vdash f(R) & \sststile{}{} f(D_i(R)) \vdash x_i\!\downarrow
\end{align*}
Compositions of morphisms are defined in the obvious way.

\begin{lem}
Morphisms of theories preserve substitutions and theorems.
\end{lem}
\begin{proof}
First, note that morphisms preserve liftings.
For every term $t$, $f(t\!\uparrow^i_j) = f(t)\!\uparrow^i_j$.
If $t = v_k$ or $t = x$, then this is obvious, and if $t = \sigma(t_1, \ldots, t_k)$,
then this follows from the fact that $f(\sigma)$ is a closed term and for every closed term $t'$, $t'\!\uparrow^i_j = t'$.
This implies that $f(x[t/x]) = f(x)[f(t)/x]$, and now it is easy to prove by induction that $f$ preserves substitutions.

It is easy to see that morphisms preserve all inference rules of the logic except for \axref{c3}.
To show that morphisms nevertheless preserve theorem, we define another inference rule:
\begin{center}
\AxiomC{$\bigwedge_{i \in I} E = E_i \land \varphi \sststile{}{} \psi$}
\RightLabel{\axlabel{c3'}}
\UnaryInfC{$\bigwedge_{i \in I} E = E_i[t/x] \land \varphi[t/x] \sststile{}{} \psi[t/x]$}
\DisplayProof
\end{center}

It is easy to see that \axref{c3'} implies \axref{c3}.
To see that the converse is also true, we need to prove that for every contexts $E_1$ and $E_2$ which correspond to some occurrences of a variable $x$ in a formula $\varphi$,
sequent $\varphi \sststile{}{} E_1 = E_2$ is derivable.
First, note that for every term $t$ and every context $E$ corresponding to an occurrence of a variable $x$ in $t$,
we have a derivable sequent $\Gamma \vdash t\!\downarrow\ \sststile{}{} E \vdash x\!\downarrow$.
It follows that analogous sequent is derivable for every formula $\varphi$: $\varphi \sststile{}{} E \vdash x\!\downarrow$.
Now, \axref{c6} implies the required property.

Since \axref{c3} and \axref{c3'} are equivalent, to complete the proof we just need to note that
if a sequent $\varphi \sststile{}{} \psi$ is derivable in the logic with \axref{c3'} instead of \axref{c3},
then $f(\varphi) \sststile{}{} f(\psi)$ is derivable in the logic with \axref{c3}.
\end{proof}
...
\medskip
\begin{center}
\AxiomC{$\Gamma \vdash a\!\downarrow$}
\doubleLine
\UnaryInfC{$\Gamma \vdash ty(a)\!\downarrow$}
\DisplayProof
\qquad
\AxiomC{$A_1, \ldots A_n \vdash ctx$}
\UnaryInfC{$A_1, \ldots A_n \vdash v_i : A_{n-i}$}
\DisplayProof
\end{center}
A \emph{$\mathbb{T}_1$-theory} is a $\mathbb{T}_0$-theory with two distinguished function symbols $-[-] : (p,1) \times (tm,0) \to p$ such that the following sequents are derivable:
\medskip
\begin{center}
\AxiomC{$\Gamma, ty(a) \vdash b\!\downarrow$}
\doubleLine
\UnaryInfC{$\Gamma \vdash b[a]\!\downarrow$}
\DisplayProof
\qquad
\AxiomC{$\Gamma, ty(a) \vdash b\!\downarrow$}
...
\DisplayProof
\qquad
\AxiomC{$\Gamma \vdash a_j\!\downarrow$, $0 \leq j \leq i$}
\UnaryInfC{$\Gamma \vdash v_i[a_0] \ldots [a_i] \deq a_i$}
\DisplayProof
\end{center}
Morphisms of $\mathbb{T}_0$-theories are morphisms of underlying theories which preserve $ty$
and morphisms of $\mathbb{T}_1$-theories are morphisms of underlying $\mathbb{T}_0$-theories which preserve $-[-]$.

A \emph{regular} contextual theory is a $\mathbb{T}_1$-theory $\mathbb{T}$ such that,
for every function symbol $\sigma : (p_1,n_1) \times \ldots \times (p_k,n_k) \to p$ and
every predicate symbol $R : (p_1,n_1) \times \ldots \times (p_k,n_k)$, the following sequents are derivable:
\medskip
\begin{center}
\AxiomC{$\Gamma, A \vdash \sigma(x_1, \ldots x_k)\!\downarrow$}
\AxiomC{$\Gamma \vdash a : A$}
\BinaryInfC{$\Gamma \vdash \sigma(x_1, \ldots x_k)[a] \deq \sigma(x_1[v_0] \ldots [v_{n_1-1}][a], \ldots x_k[v_0] \ldots [v_{n_k-1}][a])$}
\DisplayProof
\end{center}

\medskip
\begin{center}
\AxiomC{$\Gamma, A \vdash R(x_1, \ldots x_k)$}
\AxiomC{$\Gamma \vdash a : A$}
\BinaryInfC{$\Gamma \vdash R(x_1[v_0] \ldots [v_{n_1-1}][a], \ldots x_k[v_0] \ldots [v_{n_k-1}][a])$}
\DisplayProof
\end{center}
...
Now, let us define a function $C : Term_{C(\mathbb{T})}(V)_{(ctx,n)} \times CTerm_\mathbb{T}(V)_{(p,n)} \to PTerm_{C(\mathbb{T})}(V)_{(p,n)}$ as follows:
\begin{align*}
C(\Gamma, v_i) & = v_{n,i}(\Gamma) \\
C(\Gamma, x) & = x|_{ctx(x) = \Gamma} \\
C(\Gamma, \sigma(t_1, \ldots t_k)) & = \sigma_n(\Gamma, t_1', \ldots t_k')
\end{align*}
where $t_i' = C(C(\ldots C(\Gamma, A_1) \ldots, A_{n_i})[t_1'/x_1, \ldots t_{i-1}'/x_{i-1}], t_i)$ and $(A_1, \ldots A_{n_i}) = D_i(\sigma)$.
Since relation $\prec$ is well-founded, this definition is correct.
...
C(\Gamma, R(t_1, \ldots t_k)) & = R_n(\Gamma, t_1', \ldots t_k')
\end{align*}
where $t_i' = C(C(\Gamma, D_i(R)[t_1/x_1, \ldots t_{i-1}/x_{i-1}]), t_i)$.
...
Let $\overline{\mathcal{A}}$ be the set of sequents of the form
\[ \Gamma \vdash \land \bigwedge_{1 \leq i \leq n} \Gamma, \Gamma_i \vdash \varphi_i \sststile{}{} \bigwedge_{1 \leq i \leq k} \Gamma, \Delta_i \vdash \psi_i \]
for every axiom $\bigwedge_{1 \leq i \leq n} \Gamma_i \vdash \varphi_i \sststile{}{} \bigwedge_{1 \leq i \leq k} \Delta_i \vdash \psi_i$
of $\mathbb{T}$ and every sequence $\Gamma$ of variables.
The set of axioms of $C(\mathbb{T})$ consists of axioms of $\mathbb{T}_1$, regularity axioms, axiom $C(S)$ for every axiom $S \in \overline{\mathcal{A}}$, and the following axioms:
\begin{align*}
\sigma_n(\Gamma, x_1, \ldots x_k)\!\downarrow\ & \sststile{}{\Gamma, x_1, \ldots x_k} ctx(\sigma_n(\Gamma, x_1, \ldots x_k)) = \Gamma \\
\sigma_n(\Gamma, x_1, \ldots x_k)\!\downarrow\ & \sststile{}{\Gamma, x_1, \ldots x_k} \bigwedge_{1 \leq i \leq k} ctx(x_i) = C(\Gamma, D_i(\sigma)) \\
R_n(\Gamma, x_1, \ldots x_k) & \sststile{}{\Gamma, x_1, \ldots x_k} \bigwedge_{1 \leq i \leq k} ctx(x_i) = C(\Gamma, D_i(R)) \\
...
\end{align*}
It is easy to see that $\alpha$ preserves axioms, so $\mathbb{T}$ is a regular theory.

For every morphism $f : \mathbb{T} \to \mathbb{T}'$ of contextual theories, we can define a stable morphism $C(f) : C(\mathbb{T}) \to C(\mathbb{T}')$.
For every function or predicate symbol $\sigma$, we define $C(f)(\sigma_n(\Gamma, x_1, \ldots x_k))$ as $C(\Gamma, f(\sigma(x_1, \ldots x_k)))$.
...
Thus $C$ is a fully faithful functor from the category of regular contextual $\mathbb{T}_1$-theories to the category of regular algebraic dependent type theories.
\end{prop}
\begin{proof}
First, let us prove that $C$ preserves substitution in a weak sense.
Let $t$ be a contextual term, and let $x$ be a variable.
Then, for every occurrence of $x$ in $t$, we can define a context $\Delta$ such that $\Gamma \vdash t\!\downarrow\ \sststile{}{} ctx(x) = \Delta$.
It is easy to see that if $x \in FV(t)$, $\Gamma$ is a term such that $x \notin FV(\Gamma)$, then the following sequent is derivable:
\[ \bigwedge_{i \in I} \Delta = \Delta_i[s/x] \sststile{}{V} C(\Gamma, t[s/x]) \cong C(\Gamma, t)[C(\Delta,s)/x], \label{eq:C-subst} \tag{*} \]
where $\{ \Delta_i \}_{i \in I}$ is the set of contexts corresponding to all occurrences of $x$ in $t$.
...
\medskip
\begin{center}
\AxiomC{$C(\varphi) \sststile{}{V,x} C(\psi)$}
\UnaryInfC{$\bigwedge_{i \in I} C(\Delta) = C(\Delta_i[t/x]) \land C(\varphi[t/x]) \sststile{}{V,V'} C(\psi[t/x])$}
\DisplayProof
\end{center}
To prove that this rule is admissible, consider the rule analogous to \axref{c3'},
but where the set $\{ E_i \}_{i \in I}$ of contexts contains contexts corresponding to all occurrences of $x$ in $\psi$.
The same argument that we used to prove that \axref{c3'} is equivalent to \axref{c3} can be used to prove that the modified version of \axref{c3'} and \axref{c3} are equivalent.
Thus we may assume that the set $\{ E_i \}_{i \in I}$ in the rule above also contains contexts corresponding to occurrences of $x$ in $\psi$.
Now, \eqref{eq:C-subst} implies that this rule is derivable.

It is easy to see that rules \axref{c4}-\axref{c6} are also preserved.
The fact that $C$ preserve \axref{c7} follows from the stability.
We just need to prove that for every $\Gamma$ and $\varphi$, sequents $\alpha(L(A,C(C(\Gamma),\varphi))) \ssststile{}{A,V} C(C(A,\Gamma),\varphi)$ are derivable.
To do this, it is enough to prove that for every $\Gamma$ and $t$, sequents $\sststile{}{A,V} \alpha(L(A,C(C(\Gamma),t))) \cong C(C(A,\Gamma),t)$ are derivable,
which is easy to do by induction on $t$.

Thus $C$ preserves theorems.
It follows that $C$ is well-defined.
It is obvious that $C$ preserves identity morphisms.
To prove that it preserves compositions, it is enough to show that sequent $\sststile{}{V} C(f)(C(\Gamma,t)) \cong C(\Gamma, f(t))$ is derivable.
But this follows from \eqref{eq:C-subst} and the fact that sequents $\vdash f(\sigma)\!\downarrow\ \sststile{}{} f(D_i(\sigma)) \vdash x_i\!\downarrow$
and $\vdash f(R) \sststile{}{} f(D_i(R)) \vdash x_i\!\downarrow$ are derivable.

Let us prove that $C$ is surjective and injective.
First, we need to define functions $F : Term_{C(\mathbb{T})}(V)_{(p,n)} \to PCTerm_\mathbb{T}(F(V))_{(p,n)}$ and $G : Term_{C(\mathbb{T})}(V)_{(ctx,n)} \to PCtx(F(V))_n$,
where $F(V)$ is the union of $V$ and the set of variables $x^i : (ty,i)$ for every $x : (p,n)$ in $V$, $p \in \{ty,tm\}$ and $0 \leq i < n$.
They are defined recursively:
\begin{align*}
F(x) & = x \\
F(\sigma_n(\Gamma, t_1, \ldots t_k)) & = \sigma(F(t_1), \ldots F(t_k))|_{G(\Gamma) \vdash ctx \land \bigwedge_{1 \leq i \leq k} \varphi_i} \\
F(v_{n,i}(\Gamma)) & = v_i|_{G(\Gamma) \vdash ctx} \\
F(subst(\Gamma, b, t_1, \ldots t_k)) & = F(b)[F(t_1)] \ldots [F(t_k)]|_{G(\Gamma) \vdash ctx \land \bigwedge_{1 \leq i \leq k} \varphi_i} \\
F(ty_n(t)) & = ty(F(t)) \\
F(ft^k(ctx(x))) & = x^{n-k-1} \\
F(ft^k(ctx(\sigma_n(\Gamma, t_1, \ldots t_k)))) & = F(ft^k(\Gamma))|_{\bigwedge_{1 \leq i \leq k} \varphi_i} \\
G(t) & = G(ft(t)), F(t)
\end{align*}
where $\varphi_i$ equals to $G(ctx(t_i)) = G(\Gamma), D_i(\sigma)[F(t_1)/x_1, \ldots F(t_{i-1})/x_{i-1}]$.

Note that $F$ preserves compositions in the sense that the following sequent is derivable:
\[ \Gamma \vdash F(t[s/x]) \cong F(t)[F(s)/x]|_{\bigwedge_{i \in I} \Gamma, E_i[s/x] = \Gamma, G(ctx(s))}, \]
where $\{ E_i \}_{i \in I}$ is the set of contexts corresponding to all occurrences of $x$ in $F(t)$.
It follows that $F$ preserves inference rules.

Now, let us prove that $C$ is surjective.
If $f : C(\mathbb{T}) \to C(\mathbb{T}')$ is a stable morphism, then we define a contextual morphism $g : \mathbb{T} \to \mathbb{T}'$ as follows:
\[ g(\sigma(\ldots, (x_i^0, \ldots x_i^{n_i-1}).\,x_i, \ldots)) = F(f(\sigma_0(\emptyCtx, x_1, \ldots x_k)))|_{\bigwedge_{1 \leq i \leq k} x_i^0, \ldots x_i^{n_i-1} \vdash x_i \downarrow} \]
...
% \[ \sststile{}{x_1, \ldots x_k} C(\Gamma, F(t))[ft^{n_i-1-j}(ctx(x_i))/x_i^j] \cong t|_{ctx(t) = \Gamma} \]
\begin{align*}
& \sststile{}{x_1, \ldots x_k} C(C(G(ctx(t))), F_p(t))[ft^{n_i-1-j}(ctx(x_i))/x_i^j] \cong t \\
& \sststile{}{x_1, \ldots x_k} C(G(t))[ft^{n_i-1-j}(ctx(x_i))/x_i^j] \cong t
\end{align*}
...
It is easy to see that $F$ preserves inference rules.
It follows that $F$ preserves theorems in the sense that if $\varphi \sststile{}{x_1, \ldots x_k} \psi$ is a theorem,
then $\bigwedge_{1 \leq i \leq k} x_i^0, \ldots x_i^{n_i-1} \vdash x_i\!\downarrow \land F(\varphi) \sststile{}{} F(\psi)$ is also a theorem, where $F$ is defined as follows:
\begin{align*}
F(t = t') & = G(ctx(t)) \vdash F(t) \deq F(t') \land G(ctx(t)) = G(ctx(t')) \\
F(R_n(\Gamma, t_1, \ldots t_k)) & = \Gamma' \vdash R(\ldots, G(ctx(t_i))_{\geq n}.\,F(t_i), \ldots)|_{\bigwedge_{1 \leq i \leq k} \Gamma' = G(ctx(t_i))_{<n}},
\end{align*}
where $\Gamma' = F(\Gamma)$.
...
\[ \bigwedge_{1 \leq i \leq k} \varphi_i \sststile{}{}\ \vdash F(C(f)(\sigma_0(\emptyCtx, x_1, \ldots x_k))) \cong F(C(g)(\sigma_0(\emptyCtx, x_1, \ldots x_k))) \]
...
\[ \bigwedge_{1 \leq i \leq k} \varphi_i \sststile{}{} \Gamma \vdash t \cong F(C(C(\Gamma),t)) \]
...
For every function symbol $\sigma$, there exists a (unique) rule either of the form
$\varphi \sststile{}{} \Gamma \vdash \sigma(x_1, \ldots x_k)$ or $\varphi \sststile{}{} \Gamma \vdash \sigma(x_1, \ldots x_k) : A$.
Moreover, for every $i$, the set of contexts that correspond to occurrences of $x_i$ in $\varphi$ consists of a unique context.
Then we define $D_i(\sigma)$ to be this context.

\begin{example}
The theory of unit types with eta rules has function symbols $\top : ty$ and $unit : tm$ and the following axioms:
\medskip
\begin{center}
\AxiomC{}
...
\begin{example}
The theory of unit types without eta rules has function symbols $\top : ty$, $unit : tm$ and $\top\text{-}elim : (ty,1) \times (tm,0) \times (tm,0) \to tm$.
...
\begin{align*}
\Sigma & : (ty,0) \times (ty,1) \to ty \\
pair & : (ty,1) \times (tm,0) \times (tm,0) \to tm \\
proj_1 & : (ty,0) \times (ty,1) \times (tm,0) \to tm \\
proj_2 & : (ty,0) \times (ty,1) \times (tm,0) \to tm
\end{align*}
...
\AxiomC{$\vdash a : A$}
\AxiomC{$\vdash b : B[a]$}
\TrinaryInfC{$\vdash pair(B, a, b) : \Sigma(A, B)$}
\DisplayProof
\end{center}
...
\AxiomC{$\vdash a : A$}
\AxiomC{$\vdash b : B[a]$}
\TrinaryInfC{$\vdash proj_1(A, B, pair(B, a, b)) \deq a$}
\DisplayProof
\end{center}
...
\AxiomC{$\vdash a : A$}
\AxiomC{$\vdash b : B[a]$}
\TrinaryInfC{$\vdash proj_2(A, B, pair(B, a, b)) \deq b$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$\vdash p : \Sigma(A, B)$}
\UnaryInfC{$\vdash pair(B, proj_1(A, B, p), proj_2(A, B, p)) \deq p$}
\DisplayProof
\end{center}
\end{example}
...
\Sigma & : (ty,0) \times (ty,1) \to ty \\
pair & : (ty,1) \times (tm,0) \times (tm,0) \to tm \\
\Sigma\text{-}elim & : (ty,0) \times (ty,1) \times (ty,1) \times (tm,2) \times (tm,0) \to tm
\end{align*}
...
\begin{align*}
\Pi & : (ty,0) \times (ty,1) \to ty \\
\lambda & : (ty,0) \times (tm,1) \to tm \\
app & : (ty,1) \times (tm,0) \times (tm,0) \to tm
\end{align*}
and the following axioms:
\medskip
\begin{center}
...
\begin{align*}
Id & : (ty,0) \times (tm,0) \times (tm,0) \to ty \\
refl & : (ty,0) \times (tm,0) \to tm \\
J & : (ty,0) \times (ty,3) \times (tm,1) \times (tm,0) \times (tm,0) \times (tm,0) \to tm
\end{align*}
...
\medskip
\begin{center}
\AxiomC{$A, A, Id(A, v_1, v_0) \vdash D\ type$}
\AxiomC{$A \vdash d : D'$}
...
where $D' = D[v_0, v_0, refl(A, v_0)]$.
\end{example}

\begin{example}[Id-simpl]
We can define a theory which is isomorphic to the previous one, but terms of this theory contain less redundant information.
\begin{align*}
Id & : (tm,0) \times (tm,0) \to ty \\
refl & : (tm,0) \to tm \\
J & : (ty,3) \times (tm,1) \times (tm,0) \times (tm,0) \times (tm,0) \to tm
\end{align*}
...
\medskip
\begin{center}
\AxiomC{$A, A, Id(v_1, v_0) \vdash D\ type$}
