...
with the set of free variables equal to $\{ A^1_1 , \ldots A^1_{n_1}, x_1, \ldots A^k_1, \ldots A^k_{n_k}, x_k \}$, where $A^i_j : (ty,j)$ and $x_i : (p,n_i)$,
...
Then $f$ can be extended in an obvious way to a function on terms, formulas, and contexts.
Morphisms of contextual theories $\mathbb{T}$ and $\mathbb{T}'$ are equivalence classes of functions $f$ as above which preserve axioms.
...
\begin{align*}
D_p(x) & = x \\
D_p(\sigma_n(\Gamma, t_1, \ldots t_k)) & = \sigma(\ldots, D_{ctx}(t_i)_{\geq n}.\,D_p(t_i), \ldots)|_{\bigwedge_{1 \leq i \leq k} D_{ty}(\Gamma) = D_{ctx}(t_i)_{<n}} \\
