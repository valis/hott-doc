...
First, we need to define functions $D_p : Term_{C(\mathbb{T})}(V)_{(p,n)} \to CTerm_\mathbb{T}(D(V))_{(p,n)}$ and $D_{ctx} : Term_{C(\mathbb{T})}(V)_{(p,n)} \to Ctx(D(V))_n$,
...
\begin{align*}
D_p(x) & = x \\
D_p(\sigma_n(\Gamma, t_1, \ldots t_k)) & = \sigma(\ldots, D_{ctx}(t_i)_{\geq n}.\,D_{p_i}(t_i), \ldots)|_{\bigwedge_{1 \leq i \leq k} D_{ty}(\Gamma) = D_{ctx}(t_i)_{<n}} \\
D_p(v_{n,i}(\Gamma)) & = v_i \\
D_p(subst(\Gamma, b, a_1, \ldots a_k)) & = D_p(b)[D_{tm}(a_1)] \ldots [D_{tm}(a_k)]|_{\bigwedge_{1 \leq i \leq k} D_{ty}(\Gamma) = D_{ctx}(a_i)} \\
D_{ty}(ty_n(t)) & = ty(D_{tm}(t)) \\
D_p(ft^k(ctx(x))) & = x^{n-k-1} \\
D_{ty}(ctx(\sigma_n(\Gamma, t_1, \ldots t_k))) & = D_{ty}(\Gamma) \\
D_{ctx}(t) & = D_{ctx}(ft(ctx(t))), D_{ty}(ctx(t))
\end{align*}
...
Unfolding the definitions, we can see that we just need to show that for every term $t$, the following sequent is derivable:
\[ \sststile{}{x_1, \ldots x_k} C(ctx(t), D_p(t)) \cong t[ft^{n_i-1-j}(ctx(x_i))/x_i^j] \]
...
\begin{align*}
D(t = t') & = D_{ctx}(t) \vdash D_p(t) \deq D_p(t') \land D_{ctx}(t) = D_{ctx}(t') \\
D(R_n(\Gamma, t_1, \ldots t_k)) & = \Gamma' \vdash R(\ldots, D_{ctx}(t_i)_{\geq n}.\,D_{p_i}(t_i), \ldots)|_{\bigwedge_{1 \leq i \leq k} \Gamma' = D_{ctx}(t_i)_{<n}},
\end{align*}
where $\Gamma' = D_{ty}(\Gamma)$.
If $f,g : \mathbb{T} \to \mathbb{T}'$ are morphisms of contextual theories such that $\sststile{}{V} C(f)(\sigma_0(\emptyCtx, x_1, \ldots x_k)) \cong C(g)(\sigma_0(\emptyCtx, x_1, \ldots x_k))$ is derivable,
then
...
Thus we only need to prove that for every context $\Gamma$ and every contextual term $t$ with free variables in $D(\{ x_1, \ldots x_k \})$, the following sequent is derivable:
\[ \bigwedge_{1 \leq i \leq k} x^0_i, \ldots x^{n_i-1}_i \vdash x_i\!\downarrow\ \sststile{}{} \Gamma \vdash t \cong D_p(C(\Gamma,t)) \]
\end{proof}

\subsection{Examples}

Now, let us describe a few examples of algebraic dependent type theories with substitution.
We use the named representation, which can be translated to the contextual representation as noted in \rremark{named}.
