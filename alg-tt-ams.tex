...
In particular, we have the following stabilization functors: $st^0 : \algtt^0 \to \algtt^0_{st}$ and $st^1 : \algtt^1 \to \algtt^1_{st}$.

\subsection{Regular theories}

Regular theories are prestable theories in which substitution commutes with all function symbols.
...
\begin{defn}
A prestable algebraic dependent type theory with substitution is \emph{regular} if,
...
The inclusion functor $\algtt^1_{reg} \to \algtt^1_{st}$ has a left adjoint $reg : \algtt^1_{st} \to \algtt^1_{reg}$,
which is defined as follows: $reg(\mathbb{T}) = \mathbb{T} \cup \mathcal{R}$, where $\mathcal{R}$ is the set of regularity axioms.
...
\begin{enumerate}
\item There exists a set of function symbols $\mathcal{F}_0$ such that the set of funtion symbols of $\mathbb{T}$
consists of symbols $\sigma_m : (ctx,m) \times (p_1,n_1+m) \times \ldots \times (p_k,n_k+m) \to (p,n+m)$
for every $\sigma : (p_1,n_1) \times \ldots \times (p_k,n_k) \to (p,n) \in \mathcal{F}_0$ and $m \in \mathbb{N}$.
\item There exists a set of predicate symbols $\mathcal{P}_0$ such that the set of predicate symbols of $\mathbb{T}$
consists of symbols $R_m : (ctx,m) \times (p_1,n_1+m) \times \ldots \times (p_k,n_k+m)$
...
We often omit first arguments to function symbols in a contextual theories since they usually can be inferred.
...
In practice, we often define prestable theories in the contextual form.
To do this, we just need to specify sets $\mathcal{F}_0$ and $\mathcal{P}_0$ and the set of axioms.
