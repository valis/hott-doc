...
\begin{defn}
We say that a theory is \emph{contextual} if (it is isomorphic to a theory in which) the following conditions hold:
\begin{enumerate}
\item For every $\sigma \in \mathcal{F}$, $\sigma : (p_1,n_1) \times \ldots \times (p_k,n_k) \to (p,n)$,
...
\item For every $R \in \mathcal{P}$, $R : s_1 \times \ldots \times s_k$, $1 \leq i \leq k$, $s_i$ does not equal to $(ctx,0)$.
\end{enumerate}
\end{defn}
