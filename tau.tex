...
\UnaryInfC{$\Gamma \vdash \bigsqcap_n i$}
\DisplayProof
\end{center}
...
\RightLabel{, $j_1 \ebst i$ or $\exists k \geq 2 (j_k \ebst left \text{ or } j_k \ebst right)$}
\BinaryInfC{$\Gamma \vdash (j_1, \ldots j_n) : \bigsqcap_n i$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$\Gamma \vdash i : I$}
\AxiomC{$\Gamma \vdash h : \bigsqcap_n i$}
...
\AxiomC{$\Gamma \vdash i : I$}
\noLine
\UnaryInfC{$\Gamma, x : \bigsqcap_{n+1} i \vdash A$}
\AxiomC{$\Gamma, y : \bigsqcap_n i, j : I \vdash a : A[x := (\pi_1(y), \ldots \pi_n(y), j)]$}
\noLine
...
\def\extraVskip{2pt}
\AxiomC{$\Gamma \vdash h : \bigsqcap_{n+1} i$}
\TrinaryInfC{$\Gamma \vdash (\bigsqcap_{n+1} i) \text{-} elim_{\lambda x. A}\ (\lambda y j. a)\ (\lambda y_1 \ldots y_n. a_1)\ (\lambda y_1 \ldots y_n. a_2)\ h : A[x := h]$}
\DisplayProof
\end{center}
...
\AxiomC{$\Gamma, h : \bigsqcap_n i \vdash a : A[x_k := \pi_k(h)]$}
