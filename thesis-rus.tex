...
Так как категории $\Th^\lambda$ являются локально представимыми, то любой функтор из $\Th^\lambda$ в локально малую категорию, сохраняющий малые копределы, является левым сопряженным.
Для категории $\Th$ мы можем доказать более слабую версию этого утверждения.
Пусть $F : \cat{C} \to \cat{D}$ -- некоторый функтор.
Мы будем говорить, что объект $Y$ категории $\cat{D}$ удовлетворяет SSC (solution set condition) по отношению к классу $C$ объектов категории $\cat{C}$,
если существует множество морфизмов $\{ f_i : F(X_i) \to Y \}_{i \in I}$ такое,
что для любого объекта $X$ из $C$ любой морфизм $F(X) \to Y$ факторизуется как $f_i \circ F(g)$ для некоторых $f_i : F(X_i) \to Y$ и $g : X \to X_i$.

\begin{lem}[cat-aft]
Пусть $F : \cat{C} \to \cat{D}$ -- некоторый функтор, сохраняющий копределы.
Допустим, что класс регулярных факторов (с точностью до изоморфизма) любого объекта категории $\cat{C}$ образует множество.
Если некоторый объект $Y$ категории $\cat{D}$ удовлетворяет SSC по отношению к некоторому классу $C$ объектов $\cat{C}$, то он удовлетворяет этому условию и по отношению к классу, состоящему из копределов и регулярных факторов объектов из $C$.
\end{lem}
\begin{proof}
Пусть $\{ f_i : F(X_i) \to Y \}$ -- множество морфизмов, которое существует по предположению.
Так как $F$ сохраняет копроизведения, то морфизмы $f_i$ порождают морфизм $f : F(\coprod_{i \in I} X_i) \to Y$.
В качестве множества $\{ f_i' : F(X_i') \to Y \}$ мы возьмем все такие функции $f_i' : F(X_i') \to Y$, что $f_i' \circ F(e) = f$ для некоторого регулярного эпиморфизма $e : X_i \to X_i'$.
По предположению класс таких эпиморфизмов является множеством.
Так как $F$ сохраняет копределы, то $F(e)$ тоже является регулярным эпиморфизмом, а значит для любого $e$ и $f_i$ существует не более одного морфизма $f_i'$.

По предположению для любого объекта $X$ из $C$ любой морфизм $F(X) \to Y$ факторизуется через $F(g)$ и $f$ для некоторого $g$.
Тот факт, что $F$ сохраняет копроизведения, влечет, что это верно не только для объектов из $C$, но и для любых их копроизведений.
Так как любой копредел объектов из $C$ является регулярным фактором копроизведений объектов из $C$, нам осталось проверить, что требуемое свойство верно для любого регулярного фактора копроизведения объектов из $C$.
Пусть $e : X \to X'$ -- такой фактор и $h : F(X') \to Y$ -- некоторый морфизм.
Тогда $h = f \circ F(g)$ для некоторого $g$.
Пусть $e'$ -- пушаут $e$ вдоль $g$.
Так как $F$ сохраняет копределы, то $F(e')$ является пушаутом $F(e)$ и регулярным эпиморфизмом.
По универсальному свойству пушаутов $h$ факторизуется через $F(g')$ и некоторый $f_i'$, где $g'$ -- пушаут $g$ вдоль $e$, что и требовалось доказать.
\end{proof}

Применив предыдущую лемму к категории теорий, мы получим следующее утверждение:

\begin{prop}[th-aft]
Пусть $F : \Th \to \cat{D}$ -- функтор, сохраняющий малые копределы.
Тогда $F$ является левым сопряженным тогда и только тогда, когда любой объект $\cat{D}$ удовлетворяет SCC по отношению к теориям вида $(V \amalg s, \{ \sigma : \prod V \to s \}, \varnothing, \varnothing)$ и $(V, \varnothing, \{ R : \prod V \}, \varnothing)$.
\end{prop}
\begin{proof}
По теореме о сопряженных функторах $F$ является левым сопряженным тогда и только тогда, когда любой объект $\cat{D}$ удовлетворяет SCC по отношению ко всем теориям.
Следовательно, нам достаточно доказать, что если некоторый объект $\cat{D}$ удовлетворяет SCC по отношению к теориям, приведенным в условии, то он удовлетворяет этому условию по отношению ко всем теориям.
Любой объект удовлетворяет SCC по отношению к одноэлементному множеству $\{ (\{s\},\varnothing,\varnothing,\varnothing) \}$.
По \dlem{cat-aft} это верно и для теорий вида $(\mathcal{S},\varnothing,\varnothing,\varnothing)$.
Так как теории вида $,\{ \sigma \},\varnothing,\varnothing)$ и $(\mathcal{S},\varnothing,\{ R \},\varnothing)$ являются пушаутами теорий из условия утверждения и теорий вида $(\mathcal{S}',\varnothing,\varnothing,\varnothing)$,
то по той же лемме SCC верно для всех таких теорий.
Теории вида $(\mathcal{S},\mathcal{F},\mathcal{P},\varnothing)$ являются копределами теорий вида, описанного выше, а произвольные теории являются регулярными факторами таких теорий по \dprop{th-epi}.
Таким образом, по \dlem{cat-aft} любой объект $\cat{D}$ удовлетворяет SCC по отношению ко всем теориям.
\end{proof}
...
\begin{prop}
Тензорное произведение теорий задает структуру симметричной замкнутой моноидальной категории на $\Th$ и $\Th^\lambda$.
\end{prop}
\begin{proof}
...
Нам осталось проверить замкнутость категорий $\Th$ и $\Th^\lambda$.
Докажем, что для любой теории $B = (\mathcal{S},\mathcal{F},\mathcal{P},\mathcal{A})$ функтор $B \otimes -$ сохраняет копределы.
...
\begin{comment}
Так как $\Th^\lambda$ локально представима, то $B \otimes - : \Th^\lambda \to \Th^\lambda$ является левым сопряженным.
Чтобы доказать, что это верно и для $B \otimes - : \Th \to \Th$, воспользуемся утверждением~\nprop{th-aft}.
Пусть $X$ -- некоторая теория.
Пусть $\lambda$ -- регулярный кардинал такой, что $X$ является $\lambda$-достижимой, $|\mathcal{S}| < \lambda$, $|\mathcal{F}| < \lambda$ и $|\mathcal{P}| < \lambda$.
Тогда определим теорию $Y$ вместе с морфизмом $e : B \otimes Y \to X$.
Сорта $Y$ -- это морфизмы $B \to X$.
Пусть $I$ -- множество такое, что $|I| < \lambda$ и пусть $f$ -- функция $I \to \Hom(B,X)$.
Для каждого (представителя) такого множества, каждой $f$ и каждой функции $g$, сопоставляющей сорту $s$ теории $B$ атомарную формулу с переменными в $\{ x_i : f(i)(s) \mid i \in I \}$, мы добавляем предикатный символ $(f,g)$ в $Y$.
Для каждых $I$ и $f$ и каждой функции $p$, сопоставляющей предикатному символу $R : \prod_{j \in J} s_j$ теории $B$ атомарную формулу с переменными в $\{ z_{i,j} : f(i)(s_j) \mid i \in I, j \in J \}$, мы добавляем предикатный символ $(f,p)$ в $Y$.
Для каждых $I$ и $f$ и каждой функции $t$, сопоставляющей функциональному символу $\sigma : \prod_{j \in J} s_j \to s$ теории $B$ атомарную формулу с переменными в $\{ z_{i,j} : f(i)(s_j) \mid i \in I, j \in J \}$, мы добавляем предикатный символ $(f,t)$ в $Y$.
Для каждых $I$ и $f$, каждого морфизма $c : B \to X$ и каждой функции $d$, сопоставляющей сорту $s$ теории $B$ терм с переменными в $\{ x_i : f(i)(s) \mid i \in I \}$ сорта $c(s)$, мы добавляем функциональный символ $(f,c,d)$ в $Y$.

Морфизм $e : B \otimes Y \to X$ определяется следующим образом.
Для каждого сорта $[s,f]$ мы определяем $e([s,f])$ как $f(s)$.
Для каждого символа $p$ теории $B$ и каждого сорта $f$ теории $Y$ мы определяем $e([p,f])$ как $f(p)$.
Для каждого сорта $s$ теории $B$ и каждого функционального символа $\sigma' = (f,c,d)$ теории $Y$ мы определяем $e([s,\sigma'])$ как $d(s)$.
Для каждого предикатного символа $R'$ вида $(f,g)$ теории $Y$ мы определяем $e([s,R'])$ как $g(s)$.
Для остальных предикатных символов $R'$ мы определяем $e([s,R'])$ как $\top$.
Для каждого предикатного символа $R$ теории $B$ и каждого предикатного символа $R' = (f,p)$ теории $Y$ мы определяем $e([R,R'])$ как $p(R)$.
Для остальных предикатных символов $R'$ мы определяем $e([R,R'])$ как $\top$.
Для каждого функционального символа $\sigma$ теории $B$ и каждого предикатного символа $R' = (f,t)$ теории $Y$ мы определяем $e([\sigma,R'])$ как $t(\sigma)$.
Для остальных предикатных символов $R'$ мы определяем $e([\sigma,R'])$ как $\top$.
Для каждого символа $r$ теории $B$ и каждого функционального символа $\sigma'$ теории $Y$ мы определяем $e([r,\sigma'])$ как $\top$.
\end{comment}
...
Тогда определим теорию $Y$ вместе с морфизмом $e : B \otimes Y \to X$.
Сорта $Y$ -- это морфизмы $B \to X$.
Пусть $I$ -- множество такое, что $|I| < \lambda$.
Для каждого (представителя) такого множества, каждой функции $f : I \to \Hom(B,X)$ и каждой тройки функций $g$, $p$ и $t$, описанных ниже, мы добавляем предикатный символ в $Y$.
Функция $f$ задает сигнатуру предикатного символа.
Функция $g$ сопоставляет каждому сорту $s$ теории $B$ формулу с переменными в $\{ x_i : f(i)(s) \mid i \in I \}$.
Функция $p$ сопоставляет каждому предикатному символу $R : \prod_{j \in J} s_j$ теории $B$ формулу с переменными в $\{ z_{i,j} : f(i)(s_j) \mid i \in I, j \in J \}$.
Функция $t$ сопоставляет каждому функциональному символу $\sigma : \prod_{j \in J} s_j \to s$ теории $B$ формулу с переменными в $\{ z_{i,j} : f(i)(s_j) \mid i \in I, j \in J \}$.
Для каждого $I$ такого, что $|I| < \lambda$, каждой функции $f : I \to \Hom(B,X)$, каждого морфизма $c : B \to X$ и каждой тройки функций $g$, $p$ и $t$ мы добавляем функицональный символ в $Y$.
Функция $f$ и морфизм $c$ задают сигнатуру функционального символа.
Функция $g$ сопоставляет каждому сорту $s$ теории $B$ суженный терм с переменными в $\{ x_i : f(i)(s) \mid i \in I \}$ сорта $c(s)$.
Функции $p$ и $t$ действуют так же, как и раньше.

Морфизм $e : B \otimes Y \to X$ определяется следующим образом.
Для каждого сорта $[s,f]$ мы определяем $e([s,f])$ как $f(s)$.
Для каждого символа $p$ теории $B$ и каждого сорта $f$ теории $Y$ мы определяем $e([p,f])$ как $f(p)$.
Для каждого сорта $s$ теории $B$ и каждого функционального символа $\sigma = (f,c,g,p,t)$ теории $Y$ мы определяем $e([s,\sigma])$ как $g(s)$.
Для каждого предикатного символа $R = (f,g,p,t)$ теории $Y$ мы определяем $e([s,R])$ как $g(s)$.
Для каждого предикатного символа $R$ теории $B$ и каждого предикатного символа $R' = (f,g,p,t)$ теории $Y$ мы определяем $e([R,R'])$ как $p(R)$.
Для каждого функционального символа $\sigma$ теории $B$ и каждого предикатного символа $R' = (f,g,p,t)$ теории $Y$ мы определяем $e([\sigma,R'])$ как $t(\sigma)$.
Для каждого предикатного символа $R$ теории $B$ и каждого функционального символа $\sigma' = (f,c,g,p,t)$ теории $Y$ мы определяем $e([R,\sigma'])$ как $p(R)$.
Для каждого функционального символа $\sigma$ теории $B$ и каждого функционального символа $\sigma' = (f,c,g,p,t)$ теории $Y$ мы определяем $e([\sigma,\sigma'])$ как $t(\sigma)$.

Пусть теперь $h : B \otimes (\{ s_i \}_{i \in I}, \varnothing, \{ R' : \prod_{j \in J} s_j' \}, \varnothing)$ -- некоторый морфизм.
Нам нужно показать, что существует морфизм $h' : (\{ s_i \}_{i \in I}, \varnothing, \{ R' : \prod_{j \in J} s_j' \}, \varnothing) \to Y$ такой, что $e \circ (\fs{id}_B \otimes h') = h$.
Для каждого $i \in I$ мы определяем $h'(s_i)$ как $h([-,s_i])$.
Допустим, что $h([s,R']) = \bigwedge_{k \in K_1} g_k(s)$, $h([R,R']) = \bigwedge_{k \in K_2} p_k(R)$ и $h([\sigma,R']) = \bigwedge_{k \in K_3} t_k(\sigma)$, где $g_k(s)$, $p_k(R)$ и $t_k(\sigma)$ -- атомарные формулы.
Так как теория $X$ является $\lambda$-достижимой, $|\mathcal{S}| < \lambda$, $|\mathcal{F}| < \lambda$ и $|\mathcal{P}| < \lambda$, то формулы $g_k(s)$, $p_k(R)$ и $t_k(\sigma)$ используют менее $\lambda$ переменных.
Мы можем предположить, что все $K_i$ не пусты, так как иначе мы можем определить соответствующую формулу просто как $\top$.
Для каждой тройки $(k_1,k_2,k_3) \in K_1 \times K_2 \times K_3$ мы определим множество $I_{k_1,k_2,k_3} \subseteq I$ такое, что $|I_{k_1,k_2,k_3}| < \lambda$ и все свободные переменные этих формул входят в это подмножество.
Мы определяем формулу $h'(R')$ как $\bigwedge_{(k_1,k_2,k_3) \in K_1 \times K_2 \times K_3} (f_{k_1,k_2,k_3},g_{k_1},p_{k_2},t_{k_3})$, где $f_{k_1,k_2,k_3} : I_{k_1,k_2,k_3} \to \Hom(B,X)$ каждому $i$ сопоставляет $h'(s_i)$.
Проверить, что равенство $e \circ (\fs{id}_B \otimes h') = h$ выполнено легко, перебрав все сорта и символы теории $B \otimes (\{ s_i \}_{i \in I}, \varnothing, \{ R' : \prod_{j \in J} s_j' \}, \varnothing)$.

Пусть теперь $h : B \otimes (\{ s_i \}_{i \in I}, \{ \sigma' : \prod_{j \in J} s_j' \to s' \}, \varnothing, \varnothing)$ -- некоторый морфизм.
Нам нужно показать, что существует морфизм $h' : (\{ s_i \}_{i \in I}, \{ \sigma' : \prod_{j \in J} s_j' \to s' \}, \varnothing, \varnothing) \to Y$ такой, что $e \circ (\fs{id}_B \otimes h') = h$.
Допустим, что $h([s,R']) = \bigwedge_{k \in K_1} g_k(s)$, $h([R,R']) = \bigwedge_{k \in K_2} p_k(s)$ и $h([\sigma,R']) = \bigwedge_{k \in K_3} t_k(s)$, где $g_k(s)$, $p_k(s)$ и $t_k(s)$ -- атомарные формулы.
Для каждого $i \in I$ мы определяем $h'(s_i)$ как $h([-,s_i])$.
Допустим, что $h([s,\sigma']) = q(s)|_{\bigwedge_{k \in K_1} g_k(s)}$, $h([R,\sigma']) = \bigwedge_{k \in K_2} p_k(R)$ и $h([\sigma,\sigma']) = \bigwedge_{k \in K_3} t_k(\sigma)$, где $g_k(s)$, $p_k(R)$ и $t_k(\sigma)$ -- атомарные формулы.
Так как теория $X$ является $\lambda$-достижимой, $|\mathcal{S}| < \lambda$, $|\mathcal{F}| < \lambda$ и $|\mathcal{P}| < \lambda$, то термы $q(s)$ и формулы $g_k(s)$, $p_k(R)$ и $t_k(\sigma)$ используют менее $\lambda$ переменных.
Мы можем предположить, что все $K_i$ не пусты, так как иначе мы можем определить соответствующую формулу просто как $\top$.
Для каждой тройки $(k_1,k_2,k_3) \in K_1 \times K_2 \times K_3$ мы определим множество $I_{k_1,k_2,k_3} \subseteq I$ такое, что $|I_{k_1,k_2,k_3}| < \lambda$ и все свободные переменные этих термов и формул входят в это подмножество.

Мы определяем терм $h'(\sigma')$ как $q_{\bigwedge_{(k_1,k_2,k_3) \in K_1 \times K_2 \times K_3} (f_{k_1,k_2,k_3},g_{k_1},p_{k_2},t_{k_3})}$, где $f_{k_1,k_2,k_3} : I_{k_1,k_2,k_3} \to \Hom(B,X)$ каждому $i$ сопоставляет $h'(s_i)$.
Проверить, что равенство $e \circ (\fs{id}_B \otimes h') = h$ выполнено легко, перебрав все сорта и символы теории $B \otimes (\{ s_i \}_{i \in I}, \varnothing, \{ R' : \prod_{j \in J} s_j' \}, \varnothing)$.

% TODO: Проверить, что e сохраняет все аксиомы B \otimes Y.
\end{proof}
