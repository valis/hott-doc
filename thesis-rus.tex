...
Система является \emph{слабо нормализующей}, если все ее элементы имеют нормальную форму.
\item Элемент $a$ \emph{сильно нормалищуем}, если не существует бесконечной последовательности редукций, начинающейся в $a$.
Система является \emph{сильно нормалиющей}, если все ее элементы сильно нормализуемы.
...
что секвенция $\sststile{}{} \psi[\rho] \land c[t[\rho]/x] = c[s[\rho]/x]$ выводима,
...
\begin{enumerate}
\item \label{it:trs-dir-first} Для каждой подстановки $\rho$ и каждой пары термов $t$ и $s$ таких, что $(t,s) \in R$, если $\sststile{T}{} t[\rho]\!\downarrow$, то $\sststile{T}{} t[\rho] = s[\rho]$.
\item \label{it:trs-dir-second} Для каждой подстановки $\rho$ и каждой аксиомы вида $\psi \sststile{}{V} t = s$ в $\mathcal{A}_e$ таких,
что секвенция $\sststile{}{} \psi[\rho]$ выводима, термы $t[\rho]$ и $s[\rho]$ эквивалентны в системе $(\Term_T^d,\Rightarrow_T)$.
\end{enumerate}
\end{remark}
...
\circ(\fs{id}(c(f)),f) & \Rightarrow_R f \\
\circ(f,\fs{id}(d(f))) & \Rightarrow_R f \\
...
\begin{comment}
\begin{example}
На теории конечно полных категорий тоже есть естественная система редукций.
...
\fs{pair}(a,b,f,g)\!\downarrow & \ssststile{}{f,g,a,b} d(a) = d(b) \land c(a) = d(f) \land c(b) = d(g) \land c(f) = c(g) \\
\end{align*}
Отношение $R$ расширяется следующими парами:
\begin{align*}
d(!(x)) & \Rightarrow_R x \\
c(!(x)) & \Rightarrow_R 1 \\
c(\pi_1(f,g)) & \Rightarrow_R d(f) \\
c(\pi_2(f,g)) & \Rightarrow_R d(g) \\
d(\pi_1(f,g)) & \Rightarrow_R \times(f,g) \\
d(\pi_2(f,g)) & \Rightarrow_R \times(f,g) \\
d(\fs{pair}(a,b,f,g)) & \Rightarrow_R d(a) \\
c(\fs{pair}(a,b,f,g)) & \Rightarrow_R d(\pi_1(f,g)) \\
\circ(\pi_1(f,g),\fs{pair}(a,b,f,g)) & \Rightarrow_R a \\
\circ(\pi_2(f,g),\fs{pair}(a,b,f,g)) & \Rightarrow_R b \\
\fs{pair}(\circ(\pi_1(f,g),h),\circ(\pi_2(f,g),h),f,g) & \Rightarrow_R h
\end{align*}
% Если $c(f) \Rightarrow_T^* 1$, то $f = !(d(f))$.
\end{example}
\end{comment}
...
что секвенция $\sststile{}{} \psi[\rho]$ выводима, $t_i = c[a[\rho]/y]$ и $t_{i+1} = c[b[\rho]/y]$ (или наоборот).
