...
Пусть $U^\lambda : \Th \to \Th^\lambda$ -- правый сопряженный функтор к функтору вложения, который существует по \dprop{th-adj}.
Тогда мы можем доказать еще одно свойство Морита-эквивалентностей:

\begin{prop}[th-lambda-morita]
Пусть $f : T \to T'$ -- морфизм теорий.
Тогда $U^\lambda(f)$ является Морита-эквивалентностью тогда и только тогда, когда $f_! : \cat{C}_T^\lambda \to \cat{C}_{T'}^\lambda$ является эквивалентностью категорий.
\end{prop}
\begin{proof}
По \dprop{th-morita-char} нам достаточно доказать, что $\fs{Cl}^\lambda(\epsilon_T)$ является эквивалентностью категорий, где $\epsilon_T : U^\lambda(T) \to T$ -- коединица сопряжения.
Сорта теории $U^\lambda(T)$ -- это сорта $T$.
Функциональные символы этой теории -- это $\lambda$-достижимые термы теории $T$, предикатные символы -- $\lambda$-достижимые формулы, а аксиомы -- $\lambda$-достижимые теоремы.
Так как в определении $\fs{Cl}^\lambda$ используются только $\lambda$-достижимые формулы и теоремы, то отсюда следует, что функтор $\fs{Cl}^\lambda(\epsilon_T)$ сюръективен на объектах, полон и строг.
\end{proof}
...
Модельная структура на $\Th^\lambda$ может быть перенесена на категорию $\Th$ вдоль функтора вложения $i^\lambda : \Th^\lambda \to \Th$.
У этого функтора есть правый сопряженный $U^\lambda$ по \dprop{th-adj}.
По \dprop{transferred} нам достаточно проверить, что $U^\lambda(\Icell[\J^\lambda])$ являются Морита-эквивалентностями.
Это следует из \oprop{th-lambda-morita} и \olem{th-triv-cof}.
Кроме того, сопряжение $i^\lambda \dashv U^\lambda$ является Квиллен-эквивалентностью, так как $U^\lambda$ отражает и сохраняет слабые эквивалентности по определению, а единица сопряжения является изоморфизмом.
Таким образом, мы получили следующее утверждение:

\begin{prop}
На категории $\Th$ существует корасслоено порожденная модельная структура с множеством $\I^\lambda$ в качестве порождающих корасслоений и множеством $\J^\lambda$ в качестве порождающих тривиальных корасслоений.
Морфизм в этой модельной категории является слабой эквивалентностью тогда и только тогда, когда соответствующий ему функтор между $\lambda$-классифицирующими категориями является эквивалентностью категорий.
Это модельная категория Квиллен-эквивалентна модельной категории на $\Th^\lambda$.
Мы будем называть эту модельную структуру $\lambda$-достижимой.
\end{prop}

Предыдущее утверждение позволяет сравнивать модельные структуры для различных $\lambda$.
Конкретно, оно показывает, что если $\lambda < \lambda'$, то $\lambda$-достижимая модельная структура является локализацией $\lambda'$-достижимой.
Кроме того, мы можем построить модельную структуру на $\Th$ такую, что все $\lambda$-достижимые модельные структуры являются ее локализацией.
Тождественный функтор является левым Квилленовским функтором между $\lambda$-достижимой и этой модельными структурами.
...
Теперь эта теорема следует из \othm{th-l-model-str}.
\end{proof}

\begin{remark}
Эта модельная структура не является корасслоено порожденной, так как класс $\lambda$-достижимых теорий замкнут относительно копределов и ретрактов,
а значит в любой корасслоено порожденной модельной структуре на $\Th$ корасслоенные объекты будут $\lambda$-достижимы для некоторого $\lambda$.
\end{remark}
