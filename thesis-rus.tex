...
Это определение не удобно с практической точки зрения, поэтому ниже мы приведем эквивалентное определение.

Нам понадобится ввести новое определение.
...
\item Пусть $\sigma \in \mathcal{F}$ -- некоторый функциональный символ такой, что $\sigma : \prod_{i \in I} s_i \to s$.
Тогда $f(s_i) = \{ \prod_{j \in J_i} s^j_i \mid \varphi_i \}$ и $f(s) = \{ \prod_K s'_k \mid \psi \}$.
Каждому такому $\sigma$ функция $f$ сопоставляет $K$-множество суженных термов $\{ f(\sigma)_k \}_{k \in K}$ в $\Sigma'$ с переменными в $\amalg_{i \in I} \FV(f(s_i))$ таких, что $f(\sigma)_k$ имеет сорт $s'_k$.
\end{enumerate}

TODO: Закончить определение категории и доказать ее свойства.
