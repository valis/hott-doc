...
В этом подразделе для каждой достижимой теории $T$ мы определим конечно полную категорию $\cat{C}_T$, которая называется классифицирующей категорией этой теории.
...
Мы будем записывать сорт, соответствующий такой паре как $\{ x : \prod V \mid \varphi(x) \}$ или более коротко как $\{ x \mid \varphi(x) \}$.
Мы будем говорить, что производный сорт является \emph{$\lambda$-достижимым} для некоторого регулярного кардинала $\lambda$, если $|\coprod_{s \in \mathcal{S}} V_s| < \lambda$.

Пусть $\{ s_i \}_{i \in I}$ -- множество производных сортов таких, что $s_i = \{ x : \prod V_i \mid \varphi_i(x) \}$ и $s = \{ x : \prod W \mid \psi(x) \}$ -- еще один производный сорт.
...
Каждой теории $T$ можно сопоставить ее \emph{классифицирующую категорию} $\cat{C}_T$.
Объекты этой категории -- это производные сорта $T$.
Морфизм между объектами $s$ и $s'$ -- это класс эквивалентности суженных термов $\{ t_k \}_{k \in W}|_\chi$ сигнатуры $s \to s'$ таких,
что выводима секвенция $\bigwedge_{i \in I} \varphi_i \sststile{}{\amalg_{i \in I} \FV(s_i)} \bigwedge_{k \in W} t_k\!\downarrow \land \chi$.
Два терма $\{ t_k \}_{k \in W}$ и $\{ t'_k \}_{k \in W'}$ эквивалентны, если существует биекция $h : W \simeq W'$ такая, что выводима секвенция $\bigwedge_{i \in I} \varphi_i \sststile{}{\amalg_{i \in I} \FV(s_i)} \bigwedge_{k \in W'} t_k[h] = t'_k$.

Тождественный морфизм на объекте $\{ x : \prod V \mid \varphi \}$ -- это терм $\{ x_i \}_{i \in V}|_\varphi$.
Композиция морфизмов $\{ t_i \}_{i \in I}|_\chi : \{ x \mid \varphi \} \to \{ y \mid \varphi' \}$
и $\{ t'_j \}_{j \in J}|_\chi' : \{ y \mid \varphi' \} \to \{ z \mid \varphi'' \}$ -- это терм $\{ t'_j[\rho] \}_j|_{\chi'[\rho] \land \chi \land \bigwedge_{i \in I} t_i \downarrow}$, где $\rho(i) = t_i$.
Легко видеть, что это определение корректно и действительно задает категорию.

Категория $\cat{C}_T$ не является малой.
Для каждого регулярного кардинала $\lambda$ мы можем определить малую полную подкатегорию $\cat{C}_T^\lambda$ категории $\cat{C}_T$, состоящую из $\lambda$-достижимых производных сортов.
Часто под классифицирующей категорией $\lambda$-достижимой теории подразумевают $\cat{C}_T^\lambda$, но мы будем называть такую каетегорию \emph{$\lambda$-классифицирующей},
