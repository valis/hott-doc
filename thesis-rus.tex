...
\expandafter\newcommand\csname p#2\endcsname[1]{#5~\ref{#2:##1}}
...
\newref[section]{thm}{теорема}{Теорема}{теореме}
\newref{lem}{лемма}{Лемма}{лемме}
\newref{prop}{утверждение}{Утверждение}{утверждении}
\newref{cor}{следствие}{Следствие}{следствии}

\theoremstyle{definition}
\newref{defn}{определение}{Определение}{определении}
\newref{example}{пример}{Пример}{примере}

\theoremstyle{remark}
\newref{remark}{замечание}{Замечание}{замечании}
...
\item Существенно алгебраическая теория \cite[Definition~3.34]{LPC} -- это теория, в которой некоторые функциональные символы могут интерпретироваться как \emph{частичные} функции.
Мы не будем использовать это понятие, поэтому точное определение нам не понадобится.
\item Частичные хорновские теории \cite{PHL} являются обобщением существенно алгебраических теорий.
Это основное понятие с которым мы будем работать.
Его определение будет дано в следующем подразделе.
...
В этом разделе мы приведем определение частичных хорновских теорий, зададим структуру 2-категории на них и докажем, что она эквивалентна 2-категории конечно полных малых категорий.
...
Для каждого сорта $s$ теории $T$ мы добавляем константу $\upgamma^s : \fs{ob}$.
...
то их послойное определение можно определить как $\{ \overline{x}, \overline{y} \mid \varphi \land \psi \land t_1 = t_1' \land \ldots \land t_k = t_k' \}$.
...
Следовательно существует уникальный функтор $\cat{C}_T' \to \cat{C}_T$, являющийся морфизмом моделей.
\end{proof}
