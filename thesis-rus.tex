...
Тогда \emph{суженный терм} $t$ сигнатуры $\prod_{i \in I} s_i \to s$ -- это пара, состоящая из $W$-множества термов $\{ t_k \}_{k \in W}$ и формулы $\chi$ с переменными в $\amalg_{i \in I} \FV(s_i)$, такая, что $t_k$ имеет сорт $s'_k$,
...
Морфизм между объектами $s = \{ \overline{x} : \prod V \mid \varphi \}$ и $s' = \{ \overline{x} : \prod W \mid \psi \}$ -- это класс эквивалентности суженных термов $\{ t_k \}_{k \in W}|_\chi$ сигнатуры $s \to s'$ таких,
...
Пусть $t$ -- суженный терм сигнатуры $\prod_{i \in I} s_i \to s$.
...
Пусть $t,t'$ -- пара суженных термов сигнатуры $s_i \to s$ для некоторого $i \in I$ таких, что $\varphi \sststile{T}{\amalg_{i \in I} \FV(s_i)} t = t'$.
...
\subsection{2-категория теорий}

В этом подразделе мы определим строгую 2-категорию теорий $\Th$ и ее различные 2-подкатегории.
Эти определения являются основными для данного текста.
...
Если $t$ -- терм в $\Sigma$ сорта $s$ с переменными в $\{ i \in I \mid s_i \}$, то мы можем определить суженный терм $f(t)$ сигнатуры $\prod_{i \in I} f(s_i) \to f(s)$ в $\Sigma'$.
Если $t = i$ -- переменная сорта $s_i$, то $f(t) = \{ (i,x) \}_{x \in \FV(f(s_i))}$.
...
Тогда мы определяем $f(s)$ как $\{ \prod_{(i,j) \in \coprod_{i \in I} J_i} s^j_i \mid \bigwedge_{i \in I} \varphi_i \land f(\varphi) \}$.

\emph{Интерпретация} теории $T$ в теории $T'$ -- это интерпретация $f$ сигнатуры $T$ в сигнатуре $T'$ такая,
...
Мы будем говорить, что интерпретации $f$ и $f'$ \emph{эквивалентны}, если для всех функциональных символов $\sigma$ термы $f(\sigma)$ и $f'(\sigma)$ эквивалентны и для всех предикатных символов $R$ формулы $f(R)$ и $f(R')$ эквивалентны.
...
Таким образом, мы получили категорию теорий, которую мы будем обозначать $\Th$.

Каждому морфизму теорий $f : T \to T'$ можно сопоставить функтор $\cat{C}(f) : \cat{C}_T \to \cat{C}_{T'}$.
Для каждого производного сорта $s$ теории $T$ мы определяем $\cat{C}(f)(s)$ как $f(s)$.
Если $t = \{ t_i \}_i|_\varphi$ -- терм сигнатуры $s \to s'$, то мы определяем $\cat{C}(f)(t)$ как $\{ f(t_i)_j \}_{(i,j)}|_{f(\varphi)}$.
Легко видеть, что $\cat{C}(f)$ корректно определен, $\cat{C}(f)$ действительно является функтором, и $\cat{C}$ сохраняет тождественные морфизмы и композицию морфизмов теорий.
Категорию теорий можно доопределить до строгой 2-категории.
Мы определяем 2-ячейки между морфизмами $f,g : T \to T'$ как естественные преобразования между функторами $\cat{C}(f),\cat{C}(g) : \cat{C}_T \to \cat{C}_{T'}$.
Эту 2-категорию мы будем обозначать тоже как $\Th$.

\begin{prop}
Пусть $\mathrm{Cart}$ -- строгая 2-категория полных категорий и функторов, сохраняющих малые пределы.
Тогда $\cat{C}$ устанавливает биэквивалентность между $\Th$ и полной 2-подкатегорией $\mathrm{Cart}$, состоящей из категорий, дуальных к локально представимым категориям.
\end{prop}
\begin{proof}
Легко видеть, что $\cat{C}(f)$ сохраняет малые пределы, а значит $\cat{C}$ является 2-функтором $\Th \to \mathrm{Cart}$.
Так как 2-ячейки в $\Th$ определяются как 2-ячейки в $\mathrm{Cart}$, чтобы доказать, что $\Th$ является полной 2-подкатегорией $\mathrm{Cart}$,
нам достаточно показать, что любой функтор $G : \cat{C}_T \to \cat{C}_{T'}$, сохраняющий малые пределы, изоморфен функтору $F(f)$ для некоторого морфизма $f : T \to T'$.
Мы можем определить $f(s)$ как $G(s)$.
TODO:
Чтобы определить $f(\sigma)$ и $f(P)$ нужно заметить, что $G$ сохраняет мономорфизмы как правый сопряженный.
Еще нужно показать, что мономорфизм в классифицирующей категории всегда изоморфен формуле.
Тогда $f(P)$ можно определить как формулу, соответствующую мономорфизму $G(\{ \overline{x} \mid P(\overline{x}) \} \to \{ \overline{x} \mid \top \})$.
Аналогично для $f(\sigma)$.
Потом еще доказывать, что $F(f) \simeq G$.

Еще нужно доказать, что любая дуальная категория к локально представимой представима в виде $\cat{C}_T$.
\end{proof}

\begin{cor}
Пусть $\mathrm{LPC}$ -- строгая 2-категория локально представимых категорий и правых сопряженных функторов.
Тогда 2-категории $\mathrm{LPC}$ и $\Th$ дуальны.
\end{cor}
\begin{proof}
Во-первых, заметим, что 2-категория $\mathrm{LPC}$ дуальна 2-категории локально представимых категорий и левых сопряженных функторов.
Функтор между локально представимыми категориями является левым сопряженным тогда и только тогда, когда он сохраняет копределы.
Таким образом, $\mathrm{LPC}$ дуальна полной 2-подкатегорией 2-категории кополных категорий и функторов, сохраняющих копределы.
Последняя 2-категория эквивалентна 2-категории $\mathrm{Cart}$ полных категорий и функторов, сохраняющих пределы.
Следовательно $\mathrm{LPC}$ дуальна полной 2-подкатегории $\mathrm{Cart}$, состоящей из категорий, дуальных к локально представимым категориям.
\end{proof}

% Пусть $\lambda$ является либо регулярным кардиналом, либо $\lambda = \infty$.
% Этот факт мы будем записывать как $\lambda \leq \infty$.
% Для любых $\lambda \leq \infty$ и $\mu \leq \infty$ мы можем определить строгую 2-категорию $(\lambda,\mu)\text{-}\Th'$, объекты которой -- это $\lambda$-достижимые теории,
% а категория морфизмов между теориями $T$ и $T'$ -- это категория функторов между их $\mu$-классифицирующими категориями, сохраняющих $\mu$-малые пределы.
% Если $\lambda = \infty$, то объекты этой 2-категории -- это произвольные теории.
% Если $\mu = \infty$, то морфизмы между $T$ и $T'$ -- это функторы между их классифицирующими категориями, сохраняющие малые пределы.
% Ниже мы дадим определения строгих 2-категорий $(\lambda,\mu)\text{-}\Th$, биэквивалентных $(\lambda,\mu)\text{-}\Th'$, но имеющих более явно описание категорий морфизмов.
