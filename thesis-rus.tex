...
Если $t$ -- терм сорта $s$ в сигнатуре $\Sigma$ такой, что $\FV(t) \subseteq V$, и $M$ -- интерпретация $\Sigma$, то мы можем определить частичную функцию $M(t) : M_{s_1} \times \ldots \times M_{s_n} \to M_s$ рекурсией по структуре $t$.
...
Если $\varphi$ -- формула в сигнатуре $\Sigma$ такая, что $\FV(t) \subseteq V$, и $M$ -- интерпретация $\Sigma$, то мы можем определить отношение $M(\varphi) \subseteq M_{s_1} \times \ldots \times M_{s_n}$.
...
Мы добавляем функциональные символы $1 : \fs{ob}$, $! : \fs{ob} \to \fs{hom}$, $\times : \fs{hom} \times \fs{hom} \to \fs{ob}$, $\pi_1,\pi_2 : \fs{hom} \times \fs{hom} \to \fs{hom}$, $\fs{pair} : \fs{hom} \times \fs{hom} \times \fs{hom} \times \fs{hom} \to \fs{hom}$ и следующие аксиомы:
\begin{align*}
& \sststile{}{x} d(!(x)) = x \land c(!(x)) = 1 \\
c(f) = 1 & \sststile{}{f} f =\ !(d(f)) \\
c(f) = c(g) & \ssststile{}{f,g} \times(f,g)\!\downarrow \\
...
c(f) = c(g) & \sststile{}{f,g} d(\pi_1(f,g)) = \times(f,g) \land d(\pi_2(f,g)) = \times(f,g) \\
...
c(h) = \times(f,g) & \sststile{}{f,g,h} \fs{pair}(\circ(\pi_1(f,g),h),\circ(\pi_2(f,g),h),f,g) = h
\end{align*}
...
\begin{lem}[mcf]
Секвенция $\varphi \sststile{}{x_1, \ldots x_n} \psi$ доказуема в теории $T$ тогда и только тогда,
...
\begin{example}[fcat-red]
На теории конечно полных категорий тоже можно задать систему редукций.
Мы расширяем множество $\mathcal{A}_f \cup \mathcal{A}_d$ следующими аксиомами:
\begin{align*}
& \sststile{}{} 1\!\downarrow \\
...
\fs{pair}(a,b,f,g)\!\downarrow & \ssststile{}{f,g,a,b} d(a) = d(b) \land c(a) = d(f) \land c(b) = d(g) \land c(f) = c(g) \\
\end{align*}
Отношение $R$ из предыдущего примера удобно разбить на два подмножества.
Все пары вида $(d(t),s)$ и $(c(t),s)$ будут принадлежать подмножеству $R_1$, а остальные пары подмножеству $R_0$.
Мы добавляем следующие пары к $R_1$:
\begin{align*}
d(!(x)) & \Rightarrow_{R_1} x \\
c(!(x)) & \Rightarrow_{R_1} 1 \\
c(\pi_1(f,g)) & \Rightarrow_{R_1} d(f) \\
c(\pi_2(f,g)) & \Rightarrow_{R_1} d(g) \\
d(\pi_1(f,g)) & \Rightarrow_{R_1} \times(f,g) \\
d(\pi_2(f,g)) & \Rightarrow_{R_1} \times(f,g) \\
d(\fs{pair}(a,b,f,g)) & \Rightarrow_{R_1} d(a) \\
c(\fs{pair}(a,b,f,g)) & \Rightarrow_{R_1} \times(f,g)
\end{align*}
Множество $R_0$ расширяется следующими парами:
\begin{align*}
\circ(\pi_1(f,g),\fs{pair}(a,b,f,g)) & \Rightarrow_{R_0} a \\
\circ(\pi_2(f,g),\fs{pair}(a,b,f,g)) & \Rightarrow_{R_0} b \\
\circ(\pi_1(f,g),\circ(\fs{pair}(a,b,f,g),h)) & \Rightarrow_{R_0} \circ(a,h) \\
\circ(\pi_2(f,g),\circ(\fs{pair}(a,b,f,g),h)) & \Rightarrow_{R_0} \circ(b,h) \\
\fs{pair}(\circ(\pi_1(f,g),h),\circ(\pi_2(f,g),h),f,g) & \Rightarrow_{R_0} h
\end{align*}
Кроме того, мы добавляем следующую редукцию к $R_0$ для всех термов $f$ и $x$ таких, что $c(f) \Rightarrow_{R_1}^* 1$ и $d(f) \Rightarrow_{R_1}^\nf x$:
\[ f \Rightarrow_{R_0} !(x) \]
\end{example}
...
В следующем подразделе мы увидим, что теория конечно полных категорй тоже конфлюэнтна.
...
\emph{Производный сорт} теории -- это пара, состоящая из упорядоченного конечного множества переменных $V = \{ x_1 : s_1, \ldots x_n : s_n \}$ и формулы $\varphi$ такой, что $\FV(\varphi) \subseteq V$.
...
Для каждого предикатного символа $R : s_1 \times \ldots \times s_n$ теории $T$ мы добавляем константы $\upgamma^R_o : \fs{ob}$ и $\upgamma^R_m : \fs{hom}$
и аксиому $\sststile{}{} d(\upgamma^R_m) = \upgamma^R_o \land c(\upgamma^R_m) = \upgamma^{s_1} \times \ldots \times \upgamma^{s_n} \land \fs{Mon}(\upgamma^R_m)$,
...
Для каждого функционального символа $\sigma : s_1 \times \ldots \times s_n \to s$ теории $T$ мы добавляем константы $\upgamma^\sigma_o : \fs{ob}$ и $\upgamma^\sigma_d, \upgamma^\sigma_m : \fs{hom}$ и аксиому
$\sststile{}{} c(\upgamma^\sigma_m) = \upgamma^{s_1} \times \ldots \times \upgamma^{s_n} \land c(\upgamma^\sigma_d) = \upgamma^s \land d(\upgamma^\sigma_d) = \upgamma^\sigma_o \land d(\upgamma^\sigma_m) = \upgamma^\sigma_o \land \fs{Mon}(\upgamma^\sigma_m)$.
...
Ниже мы покажем, что категории $\cat{C}_T'$ и $\cat{C}_T$ эквивалентны.
Для этого нам нужно доказать, что $\mathrm{Cart} \overline{\omega} T$ конфлюэнтна.
Мы определяем систему редукций на этой теории как расширение системы, приведенной в \pexample{fcat-red}.
Мы добавляем следующие аксиомы к $\mathcal{A}_f \cup \mathcal{A}_d$:
$\sststile{}{} \upgamma^s\!\downarrow$,
$\sststile{}{} \upgamma^R_o\!\downarrow$,
$\sststile{}{} \upgamma^R_m\!\downarrow$,
$\sststile{}{} \upgamma^\sigma_o\!\downarrow$,
$\sststile{}{} \upgamma^\sigma_m\!\downarrow$,
$\sststile{}{} \upgamma^\sigma_d\!\downarrow$.
$\sststile{}{} \upgamma^a\!\downarrow$.
Отношение $R_1$ расширяется следующими парами:
\begin{align*}
d(\upgamma^R_m) & \Rightarrow_{R_1} \upgamma^R_o \\
c(\upgamma^R_m) & \Rightarrow_{R_1} \upgamma^{s_1} \times \ldots \times \upgamma^{s_n} \\
d(\upgamma^\sigma_m) & \Rightarrow_{R_1} \upgamma^\sigma_o \\
c(\upgamma^\sigma_m) & \Rightarrow_{R_1} \upgamma^{s_1} \times \ldots \times \upgamma^{s_n} \\
d(\upgamma^\sigma_d) & \Rightarrow_{R_1} \upgamma^\sigma_o \\
c(\upgamma^\sigma_d) & \Rightarrow_{R_1} \upgamma^s
\end{align*}
Наконец, отношение $R_0$ расширяется следующим образом:
\begin{align*}
\pi_2(\upgamma^R_m,\upgamma^R_m) & \Rightarrow_{R_0} \pi_1(\upgamma^R_m,\upgamma^R_m) \\
\pi_2(\upgamma^\sigma_m,\upgamma^\sigma_m) & \Rightarrow_{R_0} \pi_1(\upgamma^\sigma_m,\upgamma^\sigma_m) \\
\circ(\upgamma^\psi,\upgamma^a) & \Rightarrow_{R_0} \upgamma^\varphi \\
\circ(\upgamma^\psi,\circ(\upgamma^a,h)) & \Rightarrow_{R_0} \circ(\upgamma^\varphi,h) \\
\fs{pair}(\circ(\pi_1(\upgamma^R_m,\upgamma^R_m),h),\circ(\pi_1(\upgamma^R_m,\upgamma^R_m),h),\upgamma^R_m,\upgamma^R_m) & \Rightarrow_{R_0} h \\
\fs{pair}(\circ(\pi_1(\upgamma^\sigma_m,\upgamma^\sigma_m),h),\circ(\pi_1(\upgamma^\sigma_m,\upgamma^\sigma_m),h),\upgamma^\sigma_m,\upgamma^\sigma_m) & \Rightarrow_{R_0} h
\end{align*}
Эти правила добавляются для всех предикатных символов $R$, функциональных символов $\sigma$ и аксиом $a$, где мы используем те же обозначение, что и в определении теории.

\begin{lem}
Для любой теории $T$ теория $\mathrm{Cart} \overline{\omega} T$ конфлюэнтна и является SN.
\end{lem}
\begin{proof}
Легко видеть, что теория $\mathrm{Cart} \overline{\omega} T$ является SN.
Действительно, мы можем сопоставить каждому терму $t$ ординал $|t|$, меньший $\omega^3$ так, чтобы $t \Rightarrow_{R_0 \cup R_1} s$ влекло, что $|t| > |s|$.
Пусть $|\!\circ\!(f,g)| = |f| \oplus |f| \oplus |g| \oplus 1$, $|\pi_2(f,g)| = |f| \oplus |g| \oplus 2$, $|\upgamma^R_m| = |\upgamma^\sigma_m| = \omega$, $|\upgamma^a| = \omega^2$, и для остальных функциональных символов $\sigma$ мы определим $|\sigma(t_1, \ldots t_k)|$ как $|t_1| \oplus \ldots \oplus |t_k| \oplus 1$.

Тогда $|\upgamma^{s_1} \times \ldots \times \upgamma^{s_n}| < \omega$.
Следовательно для правил $c(\upgamma^R_m) \Rightarrow_{R_1} \upgamma^{s_1} \times \ldots \times \upgamma^{s_n}$ и $c(\upgamma^\sigma_m) \Rightarrow_{R_1} \upgamma^{s_1} \times \ldots \times \upgamma^{s_n}$ неравенство выполнено.
Кроме того, термам вида $\upgamma^\varphi$ соответствуют ординалы, меньшие $\omega^2$.
Следовательно неравенства верны и для правил $ \circ(\upgamma^\psi,\upgamma^a) \Rightarrow_{R_0} \upgamma^\varphi$ и $\circ(\upgamma^\psi,\circ(\upgamma^a,h)) \Rightarrow_{R_0} \circ(\upgamma^\varphi,h)$.
Для остальных правил неравенства также очевидны верны.

Чтобы доказать конфлюэнтность, определим вспомогательное отношение $\Rightarrow_{R_0}'$, которое будет подмножеством $\Rightarrow_{R_0}$.
В конечном итоге мы покажем, что эти отношения совпадают.
Так как $\Rightarrow_{R_0}'$ является подмножеством $\Rightarrow_{R_0 \cup R_1}$ и второе отношение является SN, то это верно и для первого отношения.
Чтобы доказать, что $\Rightarrow_{R_0}'$ конфлюэнтно, по \dlem{newman} достаточно проверить локальную конфлюэнтность.
\end{proof}
...
\begin{itemize}
\item Если $t$ имеет сорт $\fs{ob}$ и определен, то $\alpha_t$ -- это изоморфизм $GF(t) \to t$, то есть терм сорта $\fs{hom}$ такой, что $\sststile{T}{} d(\alpha_t) = GF(t) \land c(\alpha_t) = t$ и что существует обратный к нему морфизм.
\item Если $t$ имеет сорт $\fs{hom}$ и определен, то $\alpha_t$ -- это пара изоморфизмов $\alpha_t^d : GF(d(t)) \to d(t)$ и $\alpha_t^c : GF(c(t)) \to c(t)$.
\end{itemize}
