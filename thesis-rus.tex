...
По \dprop{tensor-mod}, модель $M$ теории $T \otimes T^\kappa_s$ -- это просто $V$-индексированное множество $\{ M_x \}_{x \in V}$ моделей $T$ вместе с подмоделью $M_P$ модели $\prod_{x \in V} M_x$.
...
Функтор $F : \Cat \to \Th^\lambda$ сохраняет корасслоения по \dprop{th-cof}.
...
Категория $\Th$ также является модельной категорией, обогащенной над $\Cat$.
Определим сначала обогащение.
Пусть $N(\Delta^1)$ -- это категория с двумя объектами $[0]$ и $[1]$ и единственным нетождественным морфизмом $[0] \to [1]$.
\emph{Естественное преобразование} между морфизмами теорий $f,g : T \to T'$ -- это морфизм $\alpha : T \otimes FN(\Delta^1) \to T'$ такой, что $\alpha(-,[0]) = f$ и $\alpha(-,[1]) = g$.
...
Тождественное естественное преобразование и композиция естественных преобразований определяются очевидным образом.
Таким образом, для каждой пары теорий $T$ и $T'$ у нас есть категория морфизмов $\Th(T,T')$, где объекты -- это морфизмы $T \to T'$, а морфизмы -- естественные преобразования.
Горизонтальная композиция определяется обычным образом.
Таким образом, $\Th$ обогащена над $\Cat$.

Докажем, что в $\Th$ существуют тензоры над $\Cat$.
Функтор тензорного произведения определяется как $F(-) \otimes - : \Cat \times \Th \to \Th$.
Нам достаточно доказать, что существует естественная биекция $\alpha_{\C,T,T'} : \Hom(F(\C) \otimes T, T') \simeq \Hom(\C, \Th(T,T'))$.
Действительно, по лемме Йонеды, чтобы доказать, что существует естественный изоморфизм $\Th(F(\C) \otimes T, T') \simeq \Cat(\C, \Th(T,T'))$, достаточно доказать, что для каждой малой категории $\D$
существует естественная биекция $\Hom(\D, \Th(F(\C) \otimes T, T')) \simeq \Hom(\D, \Cat(\C, \Th(T,T')))$.
Такую биекцию можно определить следующим образом:
\begin{align*}
\Hom(\D, \Th(F(\C) \otimes T, T'))  \simeq \Hom(F(\D) \otimes F(\C), T') & \simeq \\
\Hom(F(\D \times \C) \otimes T, T') \simeq \Hom(\D \times \C, \Th(T,T')) & \simeq \\
\Hom(\D, \Cat(\C, \Th(T,T'))) & &
\end{align*}

Морфизм $F(\C) \otimes T \to T'$ состоит из функции $\alpha : \fs{Ob}(\C) \to \Hom(T,T')$ и функции $\beta$, сопоставляющей каждому сорту $s$ теории $T$ и морфизму $f \in \Hom_\C(a,b)$ морфизм $\beta(s,f) : \Hom_{U(T')}(\alpha(a)(s), \alpha(b)(s))$,
таких, что $\beta(s,-)$ сохраняет тождественные морфизмы и композицию и $\beta(-,f)$ естественен для всех $f \in \Hom_\C(a,b)$.
Эти данные в точности соответствуют функтору $\C \to \Th(T,T')$.

Теперь докажем, что в $\Th$ существуют котензоры над $\Cat$.
...
По \dprop{cart-mod-dual} множество морфизмов категории $\Mod{T}$ можно представить в виде множества 
