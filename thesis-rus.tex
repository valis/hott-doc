...
\begin{defn}
(Полу)модельная категория $\C$ называется \emph{корасслоено поражденной}, если она кополна и в качестве классов $\I$ и $\J$ в \pprop{model-cat} (в \pprop{semimodel-cat}) можно выбрать множества, допускающие аргумент малых объектов.
...
\begin{lem}[th-morita-prop]
Морита-эквивалентности удовлетворяют свойству 2-из-3 и замкнуты относительно ретрактов.
В категории $\Th^\lambda$ Морита-эквивалентности замкнуты относительно $\lambda$-трансфинитных композиций.
\end{lem}
\begin{proof}
По \dprop{th-func-mod} соответствие $T \mapsto \Mod{T}$ является функториальным.
Отсюда следуют первые два требуемых свойства.
Докажем третье.
Так как эквивалентности категорий замкнуты относительно трансфинитных композиций, достаточно проверить, что функтор $\fs{Cl}^\lambda : \Th^\lambda \to \bcat{Cat}$ сохраняет $\lambda$-направленные копределы.
Пусть $F : J \to \Th^\lambda$ -- $\lambda$-направленная диаграмма.
Пусть $T$ -- копредел этой диаграммы.
Так как каждый терм теории $T$ принадлежит теории $F_j$ для некоторого $j \in J$ и каждая теорема выводима в теории $F_k$ для некоторого $k \in J$,
то функтор $\colim_{j \in J} \fs{Cl}^\lambda(F_j) \to \fs{Cl}^\lambda(T)$ задает биекцию на морфизмах.

Для любого $\lambda$-малого множества сортов в $\fs{Cl}^\lambda(T)$ существует $j \in J$ такой, что эти сорта принадлежат $F_j$.
Кроме того, для любая $\lambda$-достижимая формула принадлежит $F_k$ для некоторого $k \in J$.
Эти факты влекут, что функтор $\colim_{j \in J} \fs{Cl}^\lambda(F_j) \to \fs{Cl}^\lambda(T)$ сюръективен на объектах.
Если $S$ и $S'$ -- пара $\lambda$-малых множеств сортов в теориях $F_j$ и $F_{j'}$, соответственно, таких, что $S = S'$ в $\fs{Cl}^\lambda(T)$, то $S = S'$ в теории $F_k$ для некоторого $k \in J$.
Аналогично, если две формулы равны в $\fs{Cl}^\lambda(T)$, то они равны и в $F_k$ для некоторого $k \in J$.
Отсюда следует, что функтор $\colim_{j \in J} \fs{Cl}^\lambda(F_j) \to \fs{Cl}^\lambda(T)$ инъективен на объектах.
\end{proof}
...
\begin{prop}[th-cof]
Морфизм в модельной категории, сконструированной в \pprop{th-l-model-str}, является корасслоением тогда и только тогда, когда он инъективен на объектах.
\end{prop}
\begin{proof}
...
Класс Морита-эквивалентностей удовлетворяет свойству 2-из-3 по \dlem{th-morita-prop}.
По этой же лемме, чтобы доказать, что $\Icof[\J^\lambda] \subseteq \we$, достаточно доказать, что пушауты морфизмов из $\J^\lambda$ являются Морита-эквивалентностями,
так как любой морфизм из $\Icof[\J^\lambda]$ является ретрактом $\lambda$-трансфинитной композиции таких морфизмов.
...
Морфизмы в $\J^\lambda$ являются корасслоениями по \dprop{th-cof}.
Таким образом, $\Icof[\J^\lambda] \subseteq \we \cap \Icof[\I^\lambda]$.
