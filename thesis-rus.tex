...
\item \label{it:sep-e} Для каждой аксиомы $\varphi \sststile{}{V} \psi$ в $\mathcal{A}_e$, каждой аксиомы $\sigma(x_1, \ldots x_k)\!\downarrow\ \sststile{}{x_1, \ldots x_k} \chi$ в $\mathcal{A}_f$, каждого подтерма вида $\sigma(t_1, \ldots t_k)$ формулы $\psi$,
секвенция $\varphi \sststile{}{V} \chi[t_1/x_1, \ldots t_k/x_k]$ выводима из аксиом $\mathcal{A}_e$.
\end{enumerate}

Большинство теорий, встречающихся на практике, эквивалентны теориям с разделенными аксиомами.
...
Для правила \axref{na} это верно по поредположению.
...
Предыдущую лемму можно усилить.
...
Мы будем говорить, что \emph{функциональные символы посылки этой секвенции определены} в $T$,
если секвенция $\varphi_1 \land \ldots \land \varphi_i \sststile{}{V} t\!\downarrow$ выводима для любого подтерма $t$ формулы $\varphi_{i+1}$.
Мы будем говорить, что \emph{функциональные символы этой секвенции определены} в $T$,
если функциональные символы ее посылки определены и $\varphi_1 \land \ldots \land \varphi_n \sststile{}{V} t\!\downarrow$ выводима для любого подтерма $t$ формулы $\psi$.

\begin{prop}[der-separated]
Пусть $T$ -- теория с разделенными аксиомами.
Если секвенция $\varphi \sststile{}{V} \psi$ выводима в $T$ и функциональные символы ее посылки определены в $T$, то она выводима из $\mathcal{A}_e$.
\end{prop}
\begin{proof}
...
Так как функциональные символы $\varphi$ определены, то по \dlem{mcf} секвенция $\sststile{}{} \sigma(t_1, \ldots t_k)\!\downarrow$ выводима в $T_j$.
...
Многие теории с разделенными аксиомами удовлетворяют более сильному условию, сформулированному в следующем утверждении.
Оно говорит, что все функциональные символы, которые используются в некоторой аксиоме, должны быть определены при помощи заданных ранее аксиом.
Мы будем говорить, что \emph{аксиомы теории фундированы}, если это условие выполнено.

\begin{prop}[wf-sep]
Пусть аксиомы некоторой теории $T$ состоят из трех подмножеств $\mathcal{A}_f$, $\mathcal{A}_d$ и $\mathcal{A}_c$, удовлетворяющих следующим условиям:
\begin{itemize}
\item Секвенции в $\mathcal{A}_f$ имеют вид $\sigma(x_1, \ldots x_k)\!\downarrow\ \sststile{}{x_1, \ldots x_k} \chi$.
\item Множество $\mathcal{A}_d$ состоит из секвенций вида $\chi \sststile{}{x_1, \ldots x_k} \sigma(x_1, \ldots x_k)\!\downarrow$ таких,
что если $\mathcal{A}_f$ содержит аксиому вида $\sigma(x_1, \ldots x_k)\!\downarrow\ \sststile{}{x_1, \ldots x_k} \chi'$, то $\chi = \chi'$.
\item Множество $\mathcal{A}_c$ фундировано, и функциональные символы каждой аксиомы $\mathrm{ax}$ из $\mathcal{A}_c$ определены в $\mathcal{A}_f \cup \mathcal{A}_d \cup \mathcal{A}_c|_{\mathrm{ax}}$,
где $\mathcal{A}_c|_{\mathrm{ax}}$ -- это множество аксиом из $\mathcal{A}_c$, меньших $\mathrm{ax}$.
\end{itemize}
Тогда аксиомы $T$ разделены на множества $\mathcal{A}_f$ и $\mathcal{A}_e = \mathcal{A}_d \cup \mathcal{A}_c$.
\end{prop}
\begin{proof}
Если $X$ -- фундированное множество, то множество подмножеств $X$ тоже фундировано в следующем порядке.
Подмножество $S$ меньше, чем $S'$, если в $S'$ существует элемент, больший, чем все элементы в $S$.
Мы докажем утверждение индукцией по подмножествам $\mathcal{A}_c$.
Пусть $S$ -- подмножество $\mathcal{A}_c$.
Мы хотим доказать, что множества $\mathcal{A}_f$ и $\mathcal{A}_d \cup S$ разделяют теорию $\mathcal{A}_f \cup \mathcal{A}_d \cup S$.
Условие \eqref{it:sep-f} верно по предположению.
Условие \eqref{it:sep-e} верно для аксиом из $\mathcal{A}_d$ так как левая часть аксиомы из $\mathcal{A}_d$ и правая часть соответствующей ей аксиомы из $\mathcal{A}_f$ совпадают по предположению.

Докажем, что \eqref{it:sep-e} верно для аксиом из $\mathcal{A}_c$.
Пусть $\varphi \sststile{}{V} \psi$ -- аксиома из $\mathcal{A}_c$, которую мы будем обозначать $\mathrm{ax}$
Пусть $\sigma(x_1, \ldots x_k)\!\downarrow\ \sststile{}{x_1, \ldots x_k} \chi$ -- аксиома в $\mathcal{A}_f$, и $\sigma(t_1, \ldots t_k)$ -- подтерм $\psi$.
Тогда секвенция $\varphi \sststile{}{V} \sigma(t_1, \ldots t_k)\!\downarrow$ выводима в $\mathcal{A}_f \cup \mathcal{A}_d \cup \mathcal{A}_c|_{\mathrm{ax}}$, так как функциональные символы $\mathrm{ax}$ определены в этой теории.
По индукционному предположению аксиомы этой теории разделены.
Следовательно в этой теории выводима секвенция $\varphi \sststile{}{V} \chi[t_1/x_1, \ldots t_k/x_k]$.
По \dprop{der-separated} эта секвенция выводима в $\mathcal{A}_d \cup \mathcal{A}_c|_{\mathrm{ax}}$, что и требовалось доказать.
\end{proof}
