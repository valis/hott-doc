...
\medskip
\begin{center}
\AxiomC{$(A_1, \ldots A_k, \Delta\!\uparrow^k \vdash \sigma_k(b_1, \ldots b_m))\!\downarrow$}
\AxiomC{$\Gamma \vdash ctx$}
\AxiomC{$J$}
...
\medskip
\begin{center}
\AxiomC{$(A_1, \ldots A_k, \Delta_i \vdash b_i)\!\downarrow$}
...
\DisplayProof
\end{center}
where $J = (\Gamma \vdash a_i : subst(A_i, a_1, \ldots a_{i-1}))$ and $b_i' = subst^{q_i}(b_i, a_1, \ldots a_k)$.
...
\begin{center}
\AxiomC{$\Gamma \vdash ctx$}
\UnaryInfC{$\Gamma \vdash Type_n$}
\DisplayProof
\quad
\AxiomC{$\Gamma \vdash A : Type$}
\UnaryInfC{$\Gamma \vdash El_n(A)\ type$}
\DisplayProof
\end{center}
Also, we add the following axiom for every $\sigma \in \mathcal{F}'$, $\sigma : s_1 \times \ldots \times s_k \to (ty,n)$:
\medskip
\begin{center}
\AxiomC{$\Gamma \vdash \sigma_U(x_1, \ldots x_k)\!\downarrow$}
...
\begin{center}
\AxiomC{$\vdash A : Type$}
\AxiomC{$El_0(A) \vdash B : Type$}
\BinaryInfC{$\vdash \Sigma_U(A, B) : Type$}
\DisplayProof
\quad
\AxiomC{$\vdash A : Type$}
\AxiomC{$El_0(A) \vdash B : Type$}
\BinaryInfC{$\vdash \Pi_U(A, B) : Type$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$\vdash A : Type$}
\AxiomC{$\vdash a : El_0(A)$}
\AxiomC{$\vdash a' : El_0(A)$}
\TrinaryInfC{$\vdash Id_U(A, a, a') : Type$}
\DisplayProof
\end{center}

Note that this construction of a theory with a universe does not work well in general.
For instance, we should use $Id$ types of \rexample{Id} instead of $Id$ types from \rexample{Id-simpl}.
