...
\end{align*}
where $\varphi = \bigwedge_{1 \leq i \leq k} (G^{n-1}(\Gamma), \ldots G^0(\Gamma)) \equiv (G^n(t_i), \ldots G^1(t_i))$, $\sigma : (p_1,n_1) \times \ldots \times (p_k,n_k) \to (p,0)$,
and $\psi = \bigwedge_{1 \leq i \leq k} (G^{n-1}(\Gamma), \ldots G^0(\Gamma)) \equiv (G^{n_i+n}(t_i), \ldots G^{n_i+1}(t_i))$.
...
\begin{proof}
The proof is by induction on $F(\Gamma,t)$.
If $t = x$ is a variable, then both sequents are obvious since $\Gamma \vdash G^0(F(\Gamma,x))$ is equal to $(\Gamma \vdash x) \land \chi$ and $\Gamma \vdash G^{i+1}(F(\Gamma,x))$ is equal to $(\Gamma \vdash x^{i+1}) \land \chi$ and the equality $B_1, \ldots B_{n-i-1} \vdash x^{i+1} \equiv B_{n-i}$ follows from $\chi$.
...
G(R_n(\Gamma, t_1, \ldots t_k)) & = G(\Gamma) \vdash R(G^0(t_1), \ldots G^0(t_k)) \\
G(\varphi_1 \land \ldots \land \varphi_n) & = G(\varphi_1) \land \ldots \land G(\varphi_n)
\end{align*}

\begin{lem}
The following sequents are derivable for every $\ft$-free formula $\varphi$:
\begin{align*}
\chi \land \varphi & \sststile{}{G(V)} G(F(\varphi)) \\
\chi \land G(F(\varphi)) & \sststile{}{G(V)} \varphi
\end{align*}
...
By \rlem{gmap-term}, $\Gamma_1$ and $\Gamma_2$ are equal to $\Gamma$ and $G^0(F(\Gamma,t_i))$ is equivalent to $t_i$ in $\Gamma$, which implies that $\varphi$ and $G(F(\varphi))$ are equivalent.
\end{proof}
