...
Sequents are defined differently.
...
The derivation system for the $\subst$-free syntax is similar to that of $\ft$-free theories.
Rules \axref{ch}, \axref{cd}, \axref{cx}, \axref{cr}, \axref{cs}, \axref{ct}, \axref{cp}, \axref{cf}, \axref{cf'}, \axref{cxl}, \axref{cxr}, and \axref{ca} are the same.
Other rules are replaced with the following ones:

\medskip
\begin{center}
\AxiomC{$\varphi \sststile{}{V} \Gamma \vdash$}
\RightLabel{\axlabel{fv}}
\UnaryInfC{$\varphi \sststile{}{V} \Gamma \vdash v_i$}
\DisplayProof
\qquad
\AxiomC{$\varphi \sststile{}{V} A_1, \ldots A_m \vdash$}
\RightLabel{\axlabel{fvt}}
...
\begin{center}
\AxiomC{$\varphi \sststile{}{V} \Gamma \vdash$}
\AxiomC{$\varphi \sststile{}{V} \Gamma \vdash a_i : \subst(A_i, a_1, \ldots a_{i-1})$}
\RightLabel{, $(A_1, \ldots A_k \vdash X) \in V$ \axlabel{fs}}
\doubleLine
\BinaryInfC{$\varphi \sststile{}{V} \Gamma \vdash X(a_1, \ldots a_k)$}
\DisplayProof
\end{center}
...
\noLine
\UnaryInf$\fCenter \varphi \sststile{}{V} \Gamma \vdash a_i : \subst(A_i, a_1, \ldots a_{i-1})$
\def\extraVskip{2pt}
\RightLabel{, $(A_1, \ldots A_k \vdash X) \in V$ \axlabel{fst}}
...
\RightLabel{\axlabel{fsc}}
\BinaryInfC{$\varphi \sststile{}{V} \Gamma \vdash X(a_1, \ldots a_k) \equiv X(a_1, \ldots a_i', \ldots a_k)$}
\DisplayProof
\end{center}

\begin{defn}[subst-free]
A $\subst$-free \emph{theory} $(\mathcal{F},\mathcal{P},\mathcal{A})$ consists of a set of function symbols $\mathcal{F}$, a set of predicate symbols $\mathcal{P}$, and a set of axioms $\mathcal{A}$
together with a context $\Gamma^S_i(x_1, \ldots x_{i-1})$ for every symbol $S$ with $n$ parameters and every $1 \leq i \leq n$ such that the following conditions hold:
\begin{enumerate}
\item \label{it:subst-ax-wf} The following relation on the set of function symbols is well-founded: $\tau \prec \sigma$ if and only if $\tau$ appears in $\Gamma^\sigma_i$ for some $i$.
\item \label{it:subst-ax-cond} For every axiom $\varphi \sststile{}{V} \psi$ and every contexted subterm $(\Gamma,t)$ of $\psi$, the sequent $\varphi \sststile{}{V} \Gamma \vdash t$ is derivable.
\end{enumerate}
