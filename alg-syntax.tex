...
\[ \Gamma \vdash t \qquad \Gamma \vdash t \equiv t' \qquad \Gamma \vdash R(\overline{A_1}.\,t_1, \ldots \overline{A_k}.\,t_k) \]
where $\Gamma$ is a context of length $m$, $t$ and $t'$ are terms of sort $(p,m)$ (where $p \in \{ \ty, \tm \}$), $R : (p_1,n_1) \times \ldots \times (p_k,n_k)$ is a predicate symbol,
...
Judgements play the role of atomic formulas in the $\ft$-free syntax.
...
\AxiomC{$\Gamma \vdash \ty(f) \equiv \Pi(A.B)$}
\AxiomC{$\Gamma \vdash \ty(a) \equiv A$}
\TrinaryInfC{$\Gamma \vdash \ty(\fs{app}(A.B,f,a)) \equiv B[a]$}
\DisplayProof
\end{center}
\medskip
...
\AxiomC{$\Gamma, A \vdash b$}
\AxiomC{$\Gamma \vdash \ty(a) \equiv A$}
\BinaryInfC{$\Gamma \vdash \fs{app}(A.\ty(b),\lambda(A.b),a) \equiv b[a]$}
\DisplayProof
\end{center}
\medskip
\end{example}
...
\[ \bigcup_{1 \leq i \leq k} \{ x_i : (p_i,n_i) \} \cup \{ x^j_i : (\ty,j) \mid 0 \leq j < n_i \}. \]
Note that all of these variables must occur in the interpretation.
This is not a serious restriction because we can always add formulas $x^0_i, \ldots x^{n_i-1}_i \vdash x_i$ to it.
...
\medskip
\begin{center}
\AxiomC{$\varphi \sststile{}{V} \Gamma, \Gamma^\sigma(t_1, \ldots t_k) \vdash \sigma(t_1, \ldots t_k)$}
\AxiomC{$\varphi \sststile{}{V} \Gamma, \Gamma^\sigma_i(t_1, \ldots t_k) \vdash t_i \equiv t_i'$}
\RightLabel{\axlabel{cf}}
\BinaryInfC{$\varphi \sststile{}{V} \Gamma, \Gamma^\sigma(t_1, \ldots t_k) \vdash \sigma(t_1, \ldots t_k) = \sigma (t_1, \ldots t_{i-1}, t_i', t_{i+1}, \ldots t_k)$}
\DisplayProof
\end{center}
...
\begin{enumerate}
\item For every symbol $S$ and every possible length of $\Gamma$, there is unique axiom of the form
...
The only non-trivial case is \axref{ca}.
...
\begin{enumerate}
\item \label{it:interp-symb} For every symbol $S$ of $T$, if $x_i$ has context $\Delta$ in $(f(\Gamma^S), f(S(x_1, \ldots x_k)))$, then $\Gamma, f(\Gamma^S) \vdash f(S(x_1, \ldots x_k)) \sststile{}{\Gamma, x_1, \ldots x_k} (\Gamma, \Delta) \equiv (\Gamma, f(\Gamma^S_i))$ is derivable in $T'$.
...
Interpretations $f$ and $f'$ are \emph{equivalent} if $f(S(x_1, \ldots x_k))$ and $f'(S(x_1, \ldots x_k))$ are equivalent in $f(\Gamma^S(x_1, \ldots x_k))$ for every symbol $S$.
A \emph{morphism} of $\ft$-free theories $T$ and $T'$ is an equivalence class of interpretations of $T$ in $T'$.
The identity morphism is defined in the obvious way: $\id(\sigma(x_1, \ldots x_k)) = \sigma(x_1, \ldots x_k)|_\top$ and $\id(R(x_1, \ldots x_k)) = R(x_1, \ldots x_k)$.
The composition of morphisms is defined as follows:
\[ (g \circ f)(S(x_1, \ldots x_k)) = g(f(S(x_1, \ldots x_k))) \]
