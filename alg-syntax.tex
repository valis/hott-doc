...
\sub(\Gamma,\sigma(t_1, \ldots t_k)) & = \{ (\Gamma,t) \} \cup \bigcup_{1 \leq i \leq k} \sub((\Gamma_{\leq n},\Gamma^\sigma_i(\Gamma_{\leq n}, t_1, \ldots t_k)),t_i)
\end{align*}
...
\sub(\Gamma, R(t_1, \ldots t_k)) & = \bigcup_{1 \leq i \leq k} \sub((\Gamma,\Gamma^R_i(\Gamma, t_1, \ldots t_k)),t_i) \\
...
\AxiomC{$\varphi \sststile{}{V} \Gamma, \Gamma^R_i(\Gamma, t_1, \ldots t_k) \vdash t_i \equiv t_i'$}
...
\AxiomC{$\varphi \sststile{}{V} \Gamma, \Gamma^\sigma_i(\Gamma, t_1, \ldots t_k) \vdash t_i \equiv t_i'$}
...
Two interpretations $f$ and $f'$ are equivalent if restricted terms $f(S(x_1, \ldots x_k))$ and $f'(S(x_1, \ldots x_k))$ are equivalent for every function or predicate symbol $S$.
...
A restricted term $t|_\varphi$ is \emph{valid} in a context $\Gamma$ if $(\Gamma,t)$ and $\varphi$ are valid with respect to $(\varphi \land (\Gamma \vdash t), \FV(t|_\varphi) \cup \FV(\Gamma))$.
...
Then we can extend its definition to all terms, formulas, and restricted terms.
% TODO: Define this.

% TODO: Потребовать, чтобы \Gamma^S_i зависели только от x_1, ... x_{i-1}, т.к. иначе мы не сможем восстановить полные термы.
% TODO: Потребовать, чтобы все \Gamma^S_i и \Gamma^S совпадали для всех m. И еще что они не зависят от контекста, только от x_1, ... x_k.
%       Так будет проще формулировать всякие штуки, а категория не изменится, т.к. те ft-свободные теории, которые мы будем строить из свободных и так удовлетворяют этому условию.
% TODO: После упрощения \Gamma^S можно определить валидность интерпретаций как валидность f(\sigma(x_1, \ldots x_k)) в контексте \Gamma^\sigma(x_1, \ldots x_k).
% TODO: Можно определить замыкание на формулах и restricted термах, которое будет давать всегда валидную их версию. Хотя это лучше в виде ремарки оставить, т.к. опасно с этим работать, можно забыть при подстановки.
