...
A sequent $\varphi \sststile{}{V} \psi$ is \emph{valid} if $\FV(\psi) \subseteq \FV(\varphi)$ and $\varphi$ and $\psi$ are valid with respect to $(\varphi,V)$.
...
\begin{proof}
TODO
% It seems we might need \rlem{ftmap-subst} for this, but only when there is a unique occurrence of $x$ in $t$, which can be proved without this lemma.
% I'm not sure we need the general case of \rlem{ftmap-subst} anyway.
\end{proof}
