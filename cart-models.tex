...
\begin{lem}
Let $f : T_1 \to T_2$ and $g : T_2 \to T_1$ be morphisms of theories under $I$ such that $g \circ f = id_{T_1}$.
Suppose that the following conditions hold:
\begin{enumerate}
\item \label{it:mel-nfe} There is a function $\nf : Term_{T_2} \to Term_{T_2}$ such that for every term $t$,
    sequent $t\!\downarrow\ \sststile{}{V} \nf(t) = t$ is derivable in $T_2$.
\item \label{it:mel-nfc} There is a subtheory $T_2' \subseteq T_2$ which has the same sets of function and predicate symbols as $T_2$ and
    such that for every term $t$, sequent $\nf(t)\!\downarrow\ \sststile{}{V} \nf(e_p(t)) = e_p(\nf(t))$ is derivable in $T_2'$.
\item \label{it:mel-nfw} For every term $t$ of $T_1$, sequent $f(t)\!\downarrow\ \sststile{}{V} \nf(f(t)) = f(t)$ is derivable in $T_2'$.
\item \label{it:mel-nfa} For every theorem $\varphi \sststile{}{V} \psi$ of $T_2$, sequent $\nf(\varphi) \sststile{}{V} \nf(\psi)$ is derivable in $T_2'$.
\item \label{it:mel-hd} There is a function $h : Term_{T_2}(V)_{(p,n)} \to Term_{T_2}(L(V))_{(p,n+1)}$ which preserves $ft$, $ty$, variables and substitutions.
\item \label{it:mel-hp} For every axiom $\varphi \sststile{}{V} \psi$ of $T_2'$, sequent $h(\varphi) \sststile{}{L(V)} h(\psi)$ is a theorem of $T_2$.
\item \label{it:mel-ht} For every term $t$ of $T_1$, sequent $f(t)\!\downarrow\ \sststile{}{V} h_V(f(t)) = f(t)$ is derivable in $T_2$,
    where $h_V(a) = h(a)[\rho_V]$ for every term $a$, and $\rho_V(x) = I \times x$ for every $x \in V$.
\item \label{it:mel-he} For every term $a$, sequents
\begin{align*}
h_V(a)\!\downarrow\ & \sststile{}{V} e_p(h_V(a)) = I \times e_p(a) \\
h_V(a)\!\downarrow\ & \sststile{}{V} left^*(h_V(a)) = f(g(a)) \\
h_V(a)\!\downarrow\ & \sststile{}{V} right^*(h_V(a)) = a
\end{align*}
are derivable.
\end{enumerate}
...
\end{lem}
\begin{proof}
Let $a$ be a term of $T_2$, let $A$ be a variable, and let $\varphi$ be a formula of $T_1$ such that $f(\varphi) \sststile{}{V} e_p(a) = A$ is derivable in $T_2$.
Condition~\eqref{it:mel-nfe} implies that sequent $f(\varphi) \sststile{}{V} e_p(\nf(a)) = A$ is also derivable in $T_2$.
...
Let $h' = h_V(\nf(a))$.
By conditions \eqref{it:mel-nfa} and \eqref{it:mel-hp}, sequent $h_V(\nf(f(\varphi))) \sststile{}{V} h_V(\nf(e_p(a))) = h_V(\nf(A))$ is derivable in $T_2$.
Conditions \eqref{it:mel-nfw}, \eqref{it:mel-hp} and \eqref{it:mel-ht} imply that sequent $f(t)\!\downarrow\ \sststile{}{V} h_V(\nf(f(t))) = f(t)$ is also derivable in $T_2$.
...
Since $f(\varphi) \sststile{}{V} a\!\downarrow$, conditions \eqref{it:mel-nfa} and \eqref{it:mel-hp} imply that $h_V(\nf(f(\varphi))) \sststile{}{V} h_V(\nf(a))\!\downarrow$.
Conditions \eqref{it:mel-nfc} and \eqref{it:mel-hp} imply that $h_V(\nf(a))\!\downarrow\ \sststile{}{V} h_V(\nf(e_p(a))) = h_V(e_p(\nf(a)))$.
Hence the following sequent is a theorem of $T_2$:
\[ f(\varphi) \sststile{}{V} h_V(\nf(e_p(a))) = h_V(e_p(\nf(a))). \]
Condition~\eqref{it:mel-hd} implies that sequent
\[ f(\varphi) \sststile{}{V} h_V(e_p(\nf(a))) = e_p(h_V(\nf(a))) \]
is also a theorem of $T_2$.
By condition \eqref{it:mel-nfw}, sequent $f(\varphi) \sststile{}{V} h_V(\nf(A)) = h_V(A)$ is derivable in $T_2$.
Since $h_V(A) = I \times A$, these facts imply that $f(\varphi) \sststile{}{V} e_p(h') = I \times A$.
Thus $h'$ satisfies condition~\eqref{it:mcs} of \rprop{mor-eq}.
Condition~\eqref{it:mel-nfe} implies that $f(\varphi) \sststile{}{V} \nf(a) = a$.
This fact and condition~\eqref{it:mel-he} imply that $h'$ also satisfies the other two conditions of \rprop{mor-eq}.
\end{proof}
