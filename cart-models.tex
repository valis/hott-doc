...
\newcommand{\face}[1]{\mathrm{c}_{#1}}
\newcommand{\degen}[1]{\mathrm{e}_{#1}}
...
\face{f} & : (tm,1) \text{, for every } f \in \Delta^{>0}_+ \\
\degen{f} & : (tm,1) \text{, for every } f \in \Delta^{>0}_- \\
...
The identity morphism is represented by $x : \Delta^n \vdash x : \Delta^n$ and morphisms $\Delta^{\{i\}} : \Delta^0 \to \Delta^n$ are represented by terms $\vdash \vtx{n}{i} : \Delta^n$.
...
\UnaryInfC{$\Gamma, x : \Delta^m \vdash \face{f} : \Delta^n$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$\Gamma \vdash$}
\RightLabel{, for every $g : \Delta^m \to \Delta^n \in \Delta^{>0}_-$}
\UnaryInfC{$\Gamma, x : \Delta^m \vdash \degen{g} : \Delta^n$}
\DisplayProof
\end{center}
...
\RightLabel{, for every $0 \leq i \leq 1$}
\def\extraVskip{2pt}
...
\noLine
\UnaryInf$\fCenter \Gamma, y : \Delta^{n-1} \vdash a_j : A[x \repl \face{\delta^n_j}(y)] \text{, for every } 0 \leq j \leq n, j \neq i$
\noLine
...
and for $n > 2$, $S^n_i$ consists of equations $\Gamma, z : \Delta^{n-2} \vdash a_j[y \repl \face{\delta^{n-1}_{k-1}}(z)] \deq a_k[y \repl \face{\delta^{n-1}_j}(z)]$
...
\RightLabel{, $f : \Delta^m \to \Delta^n \in \Delta^{>0}_+$, $f' : \Delta^n \to \Delta^k \in \Delta^{>0}_+$}
\UnaryInfC{$\Gamma \vdash \face{f'}(\face{f}(a)) \deq \face{f' \circ f}(a) : \Delta^k$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$\Gamma \vdash a : \Delta^m$}
\RightLabel{, $g : \Delta^m \to \Delta^n \in \Delta^{>0}_-$, $g' : \Delta^n \to \Delta^k \in \Delta^{>0}_-$}
\UnaryInfC{$\Gamma \vdash \degen{g'}(\degen{g}(a)) \deq \degen{g' \circ g}(a) : \Delta^k$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$\Gamma \vdash$}
\RightLabel{, $f : \Delta^m \to \Delta^n \in \Delta^{>0}_+$, $0 \leq i \leq m$}
\UnaryInfC{$\Gamma \vdash \face{f}(\vtx{m}{i}) \deq \vtx{n}{f(i)} : \Delta^n$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$\Gamma \vdash$}
\RightLabel{, $g : \Delta^m \to \Delta^n \in \Delta^{>0}_-$, $0 \leq i \leq m$}
\UnaryInfC{$\Gamma \vdash \degen{g}(\vtx{m}{i}) \deq \vtx{n}{g(i)} : \Delta^n$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$\Gamma \vdash a : \Delta^m$}
\RightLabel{, $f : \Delta^m \to \Delta^n \in \Delta^{>0}_+$, $g : \Delta^n \to \Delta^k \in \Delta^{>0}_-$}
\UnaryInfC{$\Gamma \vdash \degen{g}(\face{f}(a)) \deq \face{f'}(\degen{g'}(a)) : \Delta^k$}
\DisplayProof
\end{center}
where $f'$ and $g'$ are (unique) maps such that $f' \in \Delta^{>0}_+ \cup \{id\}$, $g' \in \Delta^{>0}_- \cup \{id\}$ and $g \circ f = f' \circ g'$.
We let $\face{id}(x) = \degen{id}(x) = x$.

\medskip
\begin{center}
\AxiomC{$\Gamma, x : \Delta^1 \vdash A\ type$}
\AxiomC{$\Gamma \vdash a : A[x \repl \vtx{1}{1-i}]$}
\RightLabel{, $0 \leq i \leq 1$}
\BinaryInfC{$\Gamma \vdash fill^1_i(x.A, a, \vtx{1}{1-i}) \deq a : A[x \repl \vtx{1}{1-i}]$}
\DisplayProof
\end{center}
\medskip

Finally, for every $0 \leq i \leq n$ and $f : \Delta^m \to \Delta^n \in \Delta^{>0}_+$, $f \neq \delta^n_i$, we have the following axiom:
\medskip
\begin{center}
\def\extraVskip{1pt}
\Axiom$\fCenter \Gamma, x : \Delta^n \vdash A\ type$
...
\UnaryInf$\fCenter \Gamma, y : \Delta^{n-1} \vdash a_j : A[x \repl \face{\delta^n_j}(y)] \text{, for every } 0 \leq j \leq n, j \neq i$
\noLine
\UnaryInf$\fCenter \Gamma \vdash a : \Delta^m$
\noLine
\UnaryInf$\fCenter S^n_i$
\def\extraVskip{2pt}
\UnaryInf$\fCenter \Gamma \vdash fill^n_i(x.A, a_0, \ldots a_{i-1}, a_{i+1}, \ldots a_n, \face{f}(a)) \deq a_k[y \repl \face{f'}(a)] : A[x \repl \face{f}(a)]$
\DisplayProof
\end{center}
where $k$ and $f'$ are such that $k \neq i$ and $f = \delta^n_k \circ f'$, and $k$ is minimal such number.
