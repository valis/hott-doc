...
We also consider several equivalent descriptions of Morita equivalences.
...
\begin{prop}
Let $f : T_1 \to T_2$ be a morphism of stable theories under $I$.
Then $f$ is a Morita equivalence if and only if for every $\mathcal{S}$-set of variables $V$, every formula $\varphi \in Form_{T_1}(V)$,,
every term $A \in Term_{T_1}(V)_{(d_p,n)}$, and every term $a \in Term_{T_2}(V)_{(p,n)}$ such that $f(\varphi) \sststile{}{V} e_p(a) = f(A)$,
there exists a term $a' \in Term_{T_1}(V)_{(p,n)}$ and a term $h \in Term_{T_2}(V)_{(p,n+1)}$, which satisfy the following conditions:
\begin{enumerate}
\item $\varphi \sststile{}{V} e_p(a') = A$,
\item $f(\varphi) \sststile{}{V} e_p(h) = I \times f(A)$,
\item $f(\varphi) \sststile{}{V} left^*(h) = f(a')$,
\item $f(\varphi) \sststile{}{V} right^*(h) = a$.
\end{enumerate}
\end{prop}
\begin{proof}
First, assume that $f$ is a Morita equivalence.
There is a unique morphism of theories $u : 0 \to T_1$, where $0$ is the initial theory.
This morphism induces a pair of ajoint functor $u^* : \Mod{T_1} \to \Set^\mathcal{S}$ and $u_! : \Set^\mathcal{S} \to \Mod{T_1}$.
For every $\mathcal{S}$-set $V$, there is a natural bijection $Term_{T_2}(V) \simeq Term_{T_2 \amalg_{T_1} Lang(u_!(V))}(\varnothing)$.

Let $V$, $\varphi$, $A$, and $a$ be data as in the statement of the proposition.
\end{proof}
