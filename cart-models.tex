...
\begin{thm}[mark-main]
Let $\C$ be a model category, and let $\mathcal{K}$ be a set of cofibrant objects of $\C$.
...
It is easy to see that weak equivalences closed under retracts.
...
Now, $U$ preserve cofibrations and weak equivalences by definition.
...
Then factors through either $U(Z)$ or $U(Y)$.
...
Category $\csSet$ represents $(\infty,1)$-category of $(\infty,1)$-categories in which
for every $K \in \mathcal{K}$ limits of shape $K$ exist, and functors between these categories preserve them.
