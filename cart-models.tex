...
\begin{lem}
Let $f : T_1 \to T_2$ and $g : T_2 \to T_1$ be morphisms of regular theories under $I$ such that $g \circ f = id_{T_1}$.
...
\item \label{it:mel-nfa} There is a subtheory $T_2' \subseteq T_2$ which has the same sets of function and predicate symbols such that
for every theorem $\varphi \sststile{}{V} \psi$ of $T_2$, sequent $\nf(\varphi) \sststile{}{V} \nf(\psi)$ is derivable in $T_2'$.
\item There is a function $h : Term_{T_2}(V)_{(p,n)} \to Term_{T_2}(L(V))_{(p,n+1)}$ which preserves $ft$, $ty$, variables and substitutions.
...
\item \label{it:mel-ht} For every function symbol $\sigma$ of $T_1$, sequent $t\!\downarrow\ \sststile{}{x_1, \ldots x_k} h_V(t) = t$ is derivable in $T_2$,
    where $t = f(\sigma(x_1, \ldots x_k))$, $h_V(a) = h(a)[\rho_V]$ for every term $a$, and $\rho_V(x) = I \times x$ for every $x \in V$.
...
If we define $a'$ as $g(a)$, then sequent $\varphi \sststile{}{V} e_p(a') = A$ is derivable in $T_1$.
We just need to define a term $h'$ which satisfies conditions \eqref{it:mcs}-\eqref{it:mcl} of \rprop{mor-eq}.

Let $h' = h_V(a)$.
By conditions \eqref{it:mel-nfa} and \eqref{it:mel-hp}, sequent
\[ h_V(\nf(f(\varphi))) \sststile{}{V} h_V(\nf(e_p(a))) = h_V(\nf(A)) \]
is derivable in $T_2$.
Since $T_1$ is regular and $h$ preserves substitutions, condition~\eqref{it:mel-ht} implies that sequent $f(t)\!\downarrow\ \sststile{}{V} h_V(f(t)) = f(t)$ is derivable in $T_2$ for every term $t$.
Condition~\eqref{it:mel-nfe} implies (???) that sequent $f(t)\!\downarrow\ \sststile{}{V} h_V(\nf(f(t))) = f(t)$ is also derivable in $T_2$.
It follows that sequent $f(\varphi) \sststile{}{V} h_V(\nf(f(\varphi)))$ is also a theorem of $T_2$ for every formula $\varphi$.
Hence, sequent $f(\varphi) \sststile{}{V} h_V(\nf(e_p(a))) = h_V(\nf(A))$ is also a theorem of $T_2$, and by condition~\eqref{it:mel-nfe}, so is sequent $f(\varphi) \sststile{}{V} h_V(e_p(a)) = I \times A$.
Since $h$ preserves $e_p$, it follows that $h'$ satisfies condition~\eqref{it:mcs} of \rprop{mor-eq}.
Condition~\eqref{it:mel-he} implies that it also satisfies the other two conditions.
\end{proof}
