...
Now, let us prove a simple lemma:

\begin{lem}[term-form]
Let $T_1$ and $T_2$ be theories and let $f,g : Term_{T_1}(V)_s \to Term_{T_2}(V)_{s'}$ be two functions.
If $f(t)\!\downarrow\ \sststile{T_2}{V} f(t) = g(t)$ for every term $t$, then $f(\varphi) \sststile{T_2}{V} g(\varphi)$ for every formula $\varphi$.
If $f$ and $g$ preserve variables, then the converse holds.
\end{lem}
\begin{proof}
We can assume that $\varphi$ is atomic.
Then there exists a predicate symbol $R$ (including =) such that $\varphi = R(t_1, \ldots t_k)$.
It is easy to see by induction on $i$ that $R(g(t_1), \ldots g(t_i), f(t_{i+1}), \ldots f(t_k)) \sststile{T_2}{V} R(g(t_1), \ldots g(t_{i+1}), f(t_{i+1}), \ldots f(t_k))$.
Conversely, let $\varphi = (x = t)$.
Then $x = f(t) \sststile{T_2}{V,x} x = g(t)$, and the required property easily follows from this.
\end{proof}
...
But sometimes it is enough to know that $g$ and $h$ preserve only some axioms.
These ideas are formalized in the following lemma:

\begin{lem}
Let $f : T_1 \to T_2$ be a morphism of theories under $I$.
Suppose that there are a subtheory $T_2' \subseteq T_2$ which has the same sets of function and predicate symbols as $T_2$ and
a function $\nf : Term_{T_2} \to Term_{T_2}$ which satisfies the following conditions:
\begin{description}
\item[(nf1)\label{it:mel-nfc}] $\nf(t)\!\downarrow\ \sststile{T_2'}{V} \nf(e_p(t)) = e_p(\nf(t))$ for every term $t$.
\item[(nf2)\label{it:mel-nfa}] $\nf(\varphi) \sststile{T_2'}{V} \nf(\psi)$ for every theorem $\varphi \sststile{}{V} \psi$ of $T_2$.
\item[(nf3)\label{it:mel-nft}] $f(t)\!\downarrow\ \sststile{T_2'}{V} \nf(f(t)) = f(t)$ for every term $t$.
\item[(nf4)\label{it:mel-nfe}] $t\!\downarrow\ \sststile{T_2}{V} \nf(t) = t$ for every term $t$.
\end{description}

Suppose that there is a function $g : Term_{T_2}(V)_{(p,n)} \to Term_{T_1}(V)_{(p,n)}$ which satisfies the following conditions:
\begin{description}
\item[(g1)\label{it:mel-gc}] $g(t)\!\downarrow\ \sststile{T_1}{V} g(e_p(t)) = e_p(g(t))$ for every term $t$.
\item[(g2)\label{it:mel-gp}] $g(\varphi) \sststile{T_1}{V} g(\psi)$ for every theorem $\varphi \sststile{}{V} \psi$ of $T_2'$.
\item[(g3)\label{it:mel-gt}] $t\!\downarrow\ \sststile{T_1}{V} g(f(t)) = t$ for every term $t$.
\end{description}

Suppose that there is a function $h : Term_{T_2}(V)_{(p,n)} \to Term_{T_2}(V)_{(p,n+1)}$ which satisfies the following conditions:
\begin{description}
\item[(h1)\label{it:mel-hc}] $h(t)\!\downarrow\ \sststile{T_2}{V} h(e_p(t)) = e_p(h(t))$ for every term $t$.
\item[(h2)\label{it:mel-hp}] $h(\varphi) \sststile{T_2}{V} h(\psi)$ for every theorem $\varphi \sststile{}{V} \psi$ of $T_2'$.
\item[(h3)\label{it:mel-ht}] $f(t)\!\downarrow\ \sststile{T_2}{V} h(f(t)) = I \times f(t)$ for every term $t$.
\item[(h4)\label{it:mel-he}] $h(t)\!\downarrow\ \sststile{T_2}{V} left^*(h(t)) = f(g(t)) \land right^*(h(t)) = t$ for every term $t$.
\end{description}
...
Conditions \ref{it:mel-nfa} and \ref{it:mel-gp} imply that $g(\nf(f(\varphi))) \sststile{T_1}{V} g(\nf(e_p(a))) = g(\nf(f(A)))$.
Conditions \ref{it:mel-nft} and \ref{it:mel-gp} imply that $g(f(t))\!\downarrow\ \sststile{T_1}{V} g(\nf(f(t))) = g(f(t))$.
Now, condition~\ref{it:mel-gt} implies that $t\!\downarrow\ \sststile{T_1}{V} g(\nf(f(t))) = t$.
By lemma~\rlem{term-form}, $\varphi \sststile{T_1}{V} g(\nf(f(\varphi)))$.
Thus we obtain the following theorem:
\[ \varphi \sststile{T_1}{V} g(\nf(e_p(a))) = g(\nf(f(A))) \]

Conditions \ref{it:mel-nft} and \ref{it:mel-gp} imply that $g(f(A))\!\downarrow\ \sststile{T_1}{V} g(\nf(f(A))) = g(f(A))$.
Condition~\ref{it:mel-gt} implies that $A\!\downarrow\ \sststile{T_1}{V} g(\nf(f(A))) = A$.
Thus we have the following theorem:
\[ \varphi \sststile{T_1}{V} g(\nf(e_p(a))) = A \]

Condition~\ref{it:mel-nfe} implies that sequent $f(\varphi) \sststile{}{V} e_p(\nf(a)) = A$ is also derivable in $T_2$.
Hence, if we define $a'$ as $g(\nf(a))$, then sequent $\varphi \sststile{}{V} e_p(a') = A$ is derivable in $T_1$.
We just need to define a term $h'$ which satisfies conditions \eqref{it:mcs}-\eqref{it:mcl} of \rprop{mor-eq}.

Let $h' = h_V(\nf(a))$.
By conditions \ref{it:mel-nfa} and \ref{it:mel-hp}, sequent $h_V(\nf(f(\varphi))) \sststile{}{V} h_V(\nf(e_p(a))) = h_V(\nf(A))$ is derivable in $T_2$.
Conditions \ref{it:mel-nft}, \ref{it:mel-hp} and \ref{it:mel-ht} imply that sequent $f(t)\!\downarrow\ \sststile{}{V} h_V(\nf(f(t))) = f(t)$ is also derivable in $T_2$.
It follows that sequent $f(\varphi) \sststile{}{V} h_V(\nf(f(\varphi)))$ is a theorem of $T_2$ for every formula $\varphi$.
Hence, sequent
\[ f(\varphi) \sststile{}{V} h_V(\nf(e_p(a))) = h_V(\nf(A)) \]
is also a theorem of $T_2$.
Since $f(\varphi) \sststile{}{V} a\!\downarrow$, conditions \ref{it:mel-nfa} and \ref{it:mel-hp} imply that $h_V(\nf(f(\varphi))) \sststile{}{V} h_V(\nf(a))\!\downarrow$.
Conditions \ref{it:mel-nfc} and \ref{it:mel-hp} imply that $h_V(\nf(a))\!\downarrow\ \sststile{}{V} h_V(\nf(e_p(a))) = h_V(e_p(\nf(a)))$.
Hence the following sequent is a theorem of $T_2$:
\[ f(\varphi) \sststile{}{V} h_V(\nf(e_p(a))) = h_V(e_p(\nf(a))). \]
Condition~\ref{it:mel-hc} implies that sequent
\[ f(\varphi) \sststile{}{V} h_V(e_p(\nf(a))) = e_p(h_V(\nf(a))) \]
is also a theorem of $T_2$.
Since $f(\varphi) \sststile{}{V} h_V(\nf(A)) = h_V(A)$ and $h_V(A) = I \times A$, these facts imply that $f(\varphi) \sststile{}{V} e_p(h') = I \times A$.
Thus $h'$ satisfies condition~\eqref{it:mcs} of \rprop{mor-eq}.
Condition~\ref{it:mel-nfe} implies that $f(\varphi) \sststile{}{V} \nf(a) = a$.
This fact and condition~\ref{it:mel-he} imply that $h'$ also satisfies the other two conditions of \rprop{mor-eq}.
\end{proof}
