...
\begin{defn}
An \emph{\lc-marked simplicial set} is a pair $(X,\mathcal{E})$, where $X$ is a simplicial set and $\mathcal{E}$ is a subset of $Hom(\Delta^0, X) \cup Hom(\Delta^1 \times \Delta^1, X)$.
Vertices and squares that belong to $\mathcal{E}$ will be caled \emph{marked}.
...
For every simplicial set $X$, $X^\flat$ is the \lc-marked simplicial set in which no vertices or squares are marked (that is, $X^\flat = (X,\varnothing)$),
and $X^\sharp$ is the \lc-marked simplicial set in which all vertices and squares are marked.

We will say that a map $f : X \to Y$ of \lc-marked simplicial sets is a \emph{cofibration} if and only if $U(f)$ is a monomorphism of simplicial sets. 
The class of cofibrations is generated by the set $\lcI$ which consists of the following maps:
\begin{enumerate}
\item For every $n \geq 0$, inclusions $(\partial \Delta^n)^\flat \to (\Delta^n)^\flat$.
\item For every $K \in \{ \Delta^0, \Delta^1 \times \Delta^1 \}$, maps $K^\flat \to (K,\{ id : K \to K \})$.
\end{enumerate}
Let $\lcJ$ be the set which consists of the following maps:
\begin{enumerate}
\item For every $0 < i < n$, inclusions $(\partial \Lambda^n_i)^\flat \to (\Delta^n)^\flat$.
\item For every $K \in \{ \varnothing, \Lambda^2_2 \}$, maps $K^\flat \to (\Delta^0 \join K, \{ id : \Delta^0 \join K \to \Delta^0 \join K \})$,
where $\Delta^0 \join \varnothing$ and $\Delta^0 \join \Lambda^2_2$ are identified with $\Delta^0$ and $\Delta^1 \times \Delta^1$ respectively.
\item For every $n > 0$ and $K \in \{ \varnothing, \Lambda^2_2 \}$, maps $(\partial \Delta^n \join K, \{ \Delta^{\{n\}} \join K : \Delta^0 \join K \to \partial \Delta^n \join K \}) \to (\Delta^n \join K, \{ \Delta^{\{n\}} \join K : \Delta^0 \join K \to \Delta^n \join K \})$.
\item For every $K \in \{ \varnothing, \Lambda^2_2 \}$, maps $(N(\{ 0 \cong 1 \}) \join K,\{ f_1 \}) \to (N(\{ 0 \cong 1 \}) \join K,\{ f_0, f_1 \})$, where
