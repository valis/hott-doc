...
We also consider morphisms of theories which have a right inverse, and give some lemmas that can be used to prove that such a morphism is a Morita equivalence.

Consider the category $\Mod{T}$ of models of some theory $T$.
Recall that the set $\I$ of morphisms in $\Mod{T}$ consists of the following maps:
\begin{align*}
i_{(tm,n)} & : F(\{\,\Gamma \vdash A\ type\,\}) \to F(\{\,\Gamma \vdash a : A\,\}) \\
i_{(ty,n)} & : F(\{\,\Gamma \vdash ctx\,\}) \to F(\{\,\Gamma \vdash A\ type\,\})
\end{align*}
We will say that a morphism in $\Mod{T}$ is a weak equivalence if it has RLP up to $\sim_i$ with respect every $i \in \I$.

\begin{defn}
A \emph{Morita equivalence} between theories $T_1$ and $T_2$ is a morphism $f : T_1 \to T_2$ such that for every model $X$ of $T_1$,
the unit $\eta_X : X \to f^*(f_!(X))$ of the adjunction $f_! \dashv f^* : \Mod{T_2} \to \Mod{T_1}$ is a weak equivalence.
\end{defn}
...
then there is a model structure on $\Mod{T}$ with this class of weak equivalences.
...
\begin{center}
\AxiomC{$\Gamma \vdash$}
\RightLabel{, $f : \Delta^m \to \Delta^n$, $0 \leq i \leq m$}
\UnaryInfC{$\Gamma \vdash \cmap{f}(\vtx{m}{i}) \deq \vtx{n}{f(i)} : \Delta^n$}
\DisplayProof
\end{center}
...
\UnaryInf$\fCenter S^n_i$
\def\extraVskip{2pt}
\UnaryInf$\fCenter \Gamma \vdash fill^n_i(x.A, a_0, \ldots a_{i-1}, a_{i+1}, \ldots a_n, \cmap{f}(a)) \deq a_k[y \repl \cmap{f'}(a)] : A[x \repl \cmap{f}(a)]$
\DisplayProof
\end{center}
where $k$ and $f'$ are such that $k \neq i$, $f = \delta^n_k \circ f'$, and $k$ is minimal such number.
