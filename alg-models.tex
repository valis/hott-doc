...
Note that for every $i \in \I$, map $[\cyli_0,\cyli_1] : V \amalg_U V \to C_U(V)$ belongs to $\Icof$.
\todo{Finish this}
\[ \xymatrix{ U^0_{(tm,n)} \ar[r]^{A \mapsto } \ar[d]_{i^0_{(tm,n)}} & V^0_{(tm,n)} \amalg_{U^0_{(tm,n)}} V^0_{(tm,n)} \ar[d]^{[\cyli_0,\cyli_1]} \\
              V^0_{(tm,n)} \ar[r] & C^0_{(tm,n)}
            } \]
