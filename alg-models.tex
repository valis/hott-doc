...
Since $f(\pi_1(a'))$ and $f(v_0)$ are equal in the homotopy category and $Ho(f)$ is faithful,
there exists a term $\Sigma(\Gamma), I \vdash p' : \Sigma(\Gamma)\!\uparrow^2$ such that $p'[left] = \pi_1(a')$ and $p'[right] = v_0$.
Note that $\Sigma(\Gamma) \vdash coe_0(p'^*d^*(A), \pi_2(a')) : d^*(A)$.
Thus we can define a term $\Gamma \vdash a'' : A$ as $c^*(coe_0(p'^*d^*(A), \pi_2(a')))$.
