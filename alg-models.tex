...
If $p \in \{ ty, tm\}$, then a \emph{relative cylinder object} of sort $(p,n)$ is $F(\{\,h : (p,n+1)\,\}, \{\,ctx(h) = I(ctx^2(h))\,\})$.
We will denote this object by $RCyl_{(p,n)}(h)$.
We have two inclusions $i^0_{(p,n)}, i^1_{(p,n)} : F(\{\,a : (p,n)\,\}) \to RCyl_{(p,n)}(h)$, where $i^0_{(p,n)}(a) = h[left]$ and $i^1_{(p,n)}(a) = h[right]$.

A \emph{homotopy between} maps $f,g : F(\{\,a : (ctx,n)\,\}) \to M$ is a map $h : Cyl_{(ctx,n)}(h) \allowbreak \to M$ such that $h \circ i^0_{(p,n)} = f$ and $h \circ i^1_{(p,n)} = g$.
Maps $f$ and $g$ are \emph{homotopic} if there is a homotopy between them.
In this case we will write $f \sim g$.
If $p \in \{ ty, tm \}$, then a \emph{relative homotopy between} maps $f,g : F(\{\,a : (p,n)\,\}) \to M$ is a map $h : RCyl_{(p,n)}(h) \to M$
    such that $h \circ i^0_{(p,n)} = f$ and $h \circ i^1_{(p,n)} = g$.
Maps $f$ and $g$ are \emph{homotopic relative to $(ctx,n)$} if there is a relative homotopy between them.
...
If $p \in \{ ty, tm \}$, then let $\I_p$ be the set of arrows of the form $F(\{\,A : (e(p),n)\,\}) \to F(\{\,a : (p,n)\,\})$, where $A$ is mapped to $d_p(a)$.
