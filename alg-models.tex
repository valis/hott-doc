...
\UnaryInfC{$\Gamma \vdash proj_1(A, B, p) : A$}
\DisplayProof
\quad
\AxiomC{$\Gamma \vdash p : \Sigma(A, B)$}
\UnaryInfC{$\Gamma \vdash proj_2(A, B, p) : B[proj_1(A, B, p)]$}
\DisplayProof
\end{center}
...
where $b_{n-1} = proj_2(\ldots proj_2(v_0) \ldots )$, where $proj_2$ is repeated $n-1$ times, and
for every $0 \leq i < n-1$, $b_i = proj_1(proj_2(\ldots proj_2(v_0) \ldots ))$, where $proj_2$ is repeated $i$ times,
