...
\end{lem}
\begin{proof}
Maps $v$ and $v'$ are induced by maps $w : C_U(V) \to C'_U(V)$ and $w' : C'_U(V) \to C_U(V)$.
To define these maps, we need to introduce several auxiliary constructions.
% Let $f(x,k) = coe_2(\lambda i.\,H, left, x, k)$ and $g(y,k) = coe_2(\lambda i.\,H, right, y, k)$.
% If $\Delta \vdash k : I$, $\Delta \vdash x : H[i \repl left]$ and $\Delta \vdash y : H[r \repl right]$,
% then $\Delta \vdash f(x,k) : H[i \repl k]$ and $\Gamma \vdash g(y,k) : H[i \repl k]$.
Let $f(k) = coe_2(\lambda i.\,H, left, x, k)$ and $g(k) = coe_2(\lambda i.\,H, right, y, k)$.
If $\Gamma \vdash k : I$, then $\Gamma, x : H[i \repl left] \vdash f(k) : H[i \repl k]$ and $\Gamma, y : H[i \repl right] \vdash g(k) : H[i \repl k]$.
Now, we can define $w$ and $w'$:
\begin{align*}
w(A) & = H[i \repl left] \\
w(B) & = H[i \repl right] \\
w(b) & = f(right) \\
w(a_1) = w(a_2) & = g(left) \\
w(p) & = coe_2(\lambda j.\,coe_2(\lambda i.\,H, j, f(j), left) = x, left, refl(x), right) \\
w(q) & = coe_2(\lambda j.\,coe_2(\lambda i.\,H, j, g(j), right) = y, right, refl(x), left) \\
w'(H) & = iso(A, B, \lambda x.\,b, \lambda y.\,a_1, \lambda x.\,p, \lambda y.\,q', i)
\end{align*}
where $q'$ is the concatenation of paths $path(\lambda j.\,b[x \repl at(a_1, a_2, p',j)])$ and $q$;
$p'$ is a path between $a_1$ and $a_2$, which can be obtained from $p$ and $q$ as the following concatenation:
\[ a_1 \equiv a_1[y \repl y] = a_1[y \repl b[x \repl a_2]] \equiv a_1[y \repl b][x \repl a_2] = x[x \repl a_2] \equiv a_2. \]

Note that $w'(w(A)) = A$, $w'(w(B)) = B$ and $w'(w(b)) = b$.
To prove that $v' \circ v \sim^r id_Y$, we just need to construct a term $\Delta, i : I \vdash h(x)$ for every symbol $x \in \{\,a_1, a_2, p, q\,\}$,
so that $h(x)[left] = w'(w(x))$ and $h(x)[right] = x$.
Then we can define a homotopy $h' : Y \to P(Y)$ between $v' \circ v$ and $id_Y$ as follows: $h'(y) = I \times y$ for every $y \in Y_{(p,k)}$,
$h'(A) = I \times A$, $h'(B) = I \times B$, $h'(b) = I \times b$, and $h'(x) = (i : I, \Delta \vdash subst(h(x), v_{n-1}, \ldots v_0, v_n))$ for every $x \in \{\,a_1, a_2, p, q\,\}$.
\end{proof}
