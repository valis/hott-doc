...
Here we define theories $Fill^l_{tm}$, $Fill^l_{ty}$, $Fill_{tm}$ and $Fill_{ty}$ of local and global filler operations for terms and types.
We will also construct morphisms between $Fill_{tm} + Path$ and $coe_1 + Path$;
we do not have analogous constructions for $Fill_{ty}$.
It might be possible to construct maps between $Fill_{tm} + \sigma + Path$ and $coe_1 + \sigma + Path$, but it is more complicated and we will not need this construction.
...
\UnaryInf$\fCenter [i_1 = c]^*(D_{[i_2 = c']}) = [i_2-1=c']^*(D_{[i_1=c]}),\ 0 \leq i_1 < i_2 \leq n-1$
\def\extraVskip{2pt}
\UnaryInf$\fCenter I^n, \Delta \vdash Fill^n_{(ty,n+k)}((I^n, \Delta \vdash), D_{[0=left]}, D_{[0=right]}, \ldots D_{[n-1=left]})\ type$
\DisplayProof
\end{center}
...
\UnaryInf$\fCenter [i_1 = c]^*(D_{[i_2 = c']}) = [i_2-1=c']^*(D_{[i_1=c]}),\ 0 \leq i_1 < i_2 \leq n-1$
\def\extraVskip{2pt}
\UnaryInf$\fCenter [i=c]^*(Fill^n_{(ty,n+k)}((I^n, \Delta \vdash), D_{[0=left]}, D_{[0=right]}, \ldots D_{[n-1=left]})) = D_{[i=c]}$
\DisplayProof
\end{center}

Global theories $Fill_{tm}$ and $Fill_{ty}$ are subtheories of $Fill^l_{tm}$ and $Fill^l_{ty}$ respectively which have only function symbols of the form $Fill^n_{(p,n+0)}$.
We will denote such function symbols by $Fill_{(p,n)}$.
...
\section{Global model structure on models of theories with heterogeneous path types and univalence}

For every regular theory $T$ under $coe_0 + Path + UA$, we define a (global) model structure on the category of models of $T$.
Every object of this model structure is fibrant and weak equivalences have several equivalent descriptions.
The set $\I$ of generating cofibrations consists of the following morphisms:
\begin{align*}
i^0_{(tm,n)} & : F(\{\,\Gamma \vdash A\ type\,\}) \to F(\{\,\Gamma \vdash a : A\,\}) \\
i^0_{(ty,n)} & : F(\{\,\Gamma \vdash ctx\,\}) \to F(\{\,\Gamma \vdash A\ type\,\})
\end{align*}
...
Thus for every $i : U \to V \in \I$, we need to define a relative cylinder object $C_U(V)$.
...
Let $C_{F(\{\,\Gamma \vdash ctx\,\})}(F(\{\,\Gamma \vdash A\ type\,\}))$ be equal to
\[ F(\{\,\Gamma, I \vdash H\ type\,\}), \]
$\cyli_1(A) = H[left]$, $\cyli_1(A) = H[right]$, and let $s : C_U(V) \to V$ be defined as $s(H) = A\!\uparrow$.
...
If $T$ is under $coe^l_2$, then $p_0 : P(X) \to X$ has RLP with respect to $\I$.
...
First, note that $F(\{\,(\Gamma \vdash A\ type), (\Gamma, I \vdash h : A\!\uparrow)\,\})$ is isomorphic to $F(\{\,\Gamma \vdash A\ type, ty(h) = I \times A)\,\})$.
Thus, given a term $I, \Delta \vdash h_0 : A$ and terms $h_1$ and $h_2$ such that $ty(h_1) = I \times left^*(A)$, and $ty(h_2) = I \times right^*(A)$,
we need to define a term $h$ such that $ty(h) = I \times A$, $[1 = left]^*(h) = h_1$, $[1 = right]^*(h) = h_2$ and $[0 = left]^*(h) = h_0$.
We can define $h$ as $Fill_{(tm,2)}(I \times A, h_1, h_2, h_0)$.
