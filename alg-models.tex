...
, and $coe : (ty,1) \times (tm,0) \times (tm,0) \to (tm,0)$
\medskip
\begin{center}
\AxiomC{$x : I \vdash D\ type$}
\AxiomC{$\vdash d : D[x \repl left]$}
...
Now, we can define $J(A, \lambda x y z.\,D, \lambda x.\,d, a, a', p)$ as $coe(\lambda i.\,D', d[x \repl a], right)$, where $D'$ is defined as
\[ D[x \repl a, y \repl at(a, a', p, squeeze(i,right)), z \repl path(\lambda j.\,at(a, a', p, squeeze(i,j)))]. \]
If $\Gamma, x : A, y : A, z : Id(A, x, y) \vdash D\ type$, $\Gamma, x : A \vdash d : D[y \repl x, z \repl refl(x)]$, and $\Gamma \vdash p : Id(A, a, a')$,
then $\Gamma \vdash J(A, \lambda x y z.\,D, \lambda x.\,d, a, a', p) : D[x \repl a, y \repl a', z \repl p]$.
