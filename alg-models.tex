...
the model structure will usually have the same class of weak equivalences.
...
For every $p \in \{ tm, ty, ctx \}$, let $\I^0_p$ be the set of maps of the form $i^0_{(p,n)}$.
Also, let $\I^0_{(p,0)} = \{ i^0_{(p,0)} \}$.
Usually, these sets are too small to generate all cofibrations,
but sometimes they can be used to classify weak equivalences, as we will see later.
