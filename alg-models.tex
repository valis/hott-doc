...
We will denote this object by $Cyl_{(ctx,n)}(H)$.
We have two inclusions $i^0_{(ctx,n)}, i^1_{(ctx,n)} : F(\{\,\Gamma : (ctx,n)\,\}) \to Cyl_{(ctx,n)}(H)$.
If $k \in \{0,1\}$, then $i^k_{(ctx,n)}(\Gamma)$ is defined as
\[ subst(ft^{n-1}(H), c_k), \ldots subst(ft(H), c_k, v_{n-3}, \ldots v_0) \vdash subst(H, c_k, v_{n-2}, \ldots v_0), \]
where $c_0 = left$ and $c_1 = right$.

A \emph{relative cylinder object} of sort $(ty,n)$ is $F(\{\,H : (ty,n+1)\,\}, \{\,ft(H) = I(ft^2(H))\,\})$.
We will denote this object by $RCyl_{(ty,n)}(H)$ (or simply $RCyl_{(ty,n)}$).
We have two inclusions $i^0_{(ty,n)}, i^1_{(ty,n)} : F(\{\,A : (ty,n)\,\}) \to RCyl_{(ty,n)}(H)$, where $i^0_{(ty,n)}(A) = H[left]$ and $i^1_{(ty,n)}(A) = H[right]$.

A \emph{relative cylinder object} of sort $(tm,n)$ is $F(\{\,h : (tm,n+1), A : (ty,n)\,\}, \{\,ty(h) \allowbreak = (I(ft(A)) \vdash A\!\!\uparrow)\,\})$.
We will denote this object by $RCyl_{(tm,n)}(h)$ (or simply $RCyl_{(tm,n)}$).
We have two inclusions $i^0_{(tm,n)}, i^1_{(tm,n)} : F(\{\,a : (tm,n)\,\}) \to RCyl_{(tm,n)}(h)$, where $i^0_{(tm,n)}(a) = h[left]$ and $i^1_{(tm,n)}(a) = h[right]$.

Let $e : \{ ty, tm \} \to \{ ctx, ty \}$ be the function defined as $e(ty) = ctx$, $e(tm) = ty$.
Also we define $d_p : (p,n) \to (e(p),n)$ as $d_{ty} = ft$, $d_{tm} = ty$.
If $p \in \{ ty, tm \}$, then let $\I_p$ be the set of arrows of the form $\delta_{(p,n)} : F(\{\,A : (e(p),n)\,\}) \to F(\{\,a : (p,n)\,\})$, $\delta_{(p,n)}(A) = d_p(a)$.
...
A \emph{homotopy between} maps $f,g : F(\{\,a : (ctx,n)\,\}) \to X$ is a map $h : Cyl_{(ctx,n)}(H) \allowbreak \to X$ such that $h \circ i^0_{(p,n)} = f$ and $h \circ i^1_{(p,n)} = g$.
Maps $f$ and $g$ are \emph{homotopic} if there is a homotopy between them.
In this case we will write $f \sim g$.
If $p \in \{ ty, tm \}$, then a \emph{relative homotopy between} maps $f,g : F(\{\,a : (p,n)\,\}) \to X$ is a map $h : RCyl_{(p,n)} \to X$
    such that $h \circ i^0_{(p,n)} = f$ and $h \circ i^1_{(p,n)} = g$.
Thus a relative homotopy between $f$ and $g$ exists only if $f \circ \delta_{(p,n)} = g \circ \delta_{(p,n)}$, and in this case a homotopy $h$ is a map such that the following triangle commutes:
\[ \xymatrix{ F(\{\,a : (p,n)\,\}) \amalg_{F(\{\,A : (e(p),n)\,\})} F(\{\,a : (p,n)\,\}) \ar[d]_{[i^0,i^1]} \ar[r]^-{[f,g]} & X \\
              RCyl_{(p,n)} \ar@{-->}[ur]_h
            } \]
Maps $f$ and $g$ are \emph{homotopic relative to $(e(p),n)$} if there is a relative homotopy between them.
In this case we will write $f \sim_r g$.
