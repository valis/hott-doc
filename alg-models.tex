...
We will prove that if a theory $T$ has enough additional structure, then the category $\Mod{T}$ of its models carries the structure of a model category.
We will also show that if $T$ has $\Sigma$ types, then there exists an exact functor from $\Mod{T}$
to the category of fibration categories which preserves and reflects weak equivalences.
\end{abstract}
...
We conjecture that the model category of models of $T_\Sigma = coe_1 + \sigma + Path + wUA + \Sigma$
presents the $\infty$-category of finitely complete $\infty$-categories.
Thus it is natural to stduy the relationship between the category of models of $T_\Sigma$ and the category of fibration categories.
...
If $X$ is a model of $T_\Sigma$, then we define category $U(X)$, which has contexts of length $\leq 1$ as objects and context morphisms as morphisms.
Note that since we have $\Sigma$ types, this category is equivalent to the category which has all contexts as objects.
...
Indeed, first let $Ho(U(X))$ be the category which has the same objects as $U(X)$ and morphisms of $Ho(U(X))$
are equivalence classes of maps of $U(X)$ with respect to the homotopy relation.
Then $Ho(U(X))$ is equivalent to the localization of $U(X)$, which is proved as usual (see, for example, \cite[Corollary~1.2.9]{hovey}).
Then $Ho(X)$ is a full subcategory of $Ho(U(X))$.
...
(both of them are terminal objects in $Ho(U(X))$), so $Ho(X)$ is equivalent to $Ho(U(X))$.

Since every object of $\Mod{T_\Sigma}$ is fibrant, $\Mod{T_\Sigma}$ is a fibration category.
Then $U : \Mod{T_\Sigma} \to \cat{FibCat}$ is a functor to the category of fibration categories and exact functors.
