...
We will also consider stable theory $UA$ under $I$, which has additional symbols
\[ iso : (ty,n)^2 \times (tm,n+1)^2 \times (tm,n+2)^2 \times (tm,n) \to (ty,n) \]
...
\noLine
\UnaryInfC{$\Gamma \vdash B\ type$}
\AxiomC{$\Gamma, A \vdash f : B$}
\noLine
\UnaryInfC{$\Gamma, B \vdash g : A$}
\AxiomC{$\Gamma, A, I \vdash p : A$}
\noLine
\UnaryInfC{$\Gamma, B, I \vdash q : B$}
...
The univalence axiom is equivalent to the statement that a universe is closed under $iso$.

Let $\mathbb{T}$ be a theory under $I \cup UA$.
...
Let $\I_{tm}$ be the set of arrows of the form $F(\{\,\Gamma \vdash A\ type\,\}) \to F(\{\,\Gamma \vdash a : A\,\})$, where $A$ is mapped to $A$.
...
Let $a,b \in M_{(tm,n)}$ be elements such that $ty(a) = ty(b)$.
% Then a \emph{homotopy} between $a$ and $b$ is a morphism $h : F(\{\,\Gamma, I \vdash a : A\,\}) \to M$ such that $ctx(h) = I(ctx(a))$, $ty(h) = ty(a)$
Then a \emph{homotopy} between $a$ and $b$ is a morphism $h \in M_{(tm,n+1)}$ such that $ctx(h) = I(ctx(a))$, $ty(h) = ty(a)$
