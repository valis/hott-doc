...
Let $b$ and $b'$ be terms such that $A \vdash b : B\!\uparrow$, $A \vdash b' : B\!\uparrow$ and there exists a term $p$ such that $f(A) \vdash p : f(b) = f(b')$.
Then there exists a term $p'$ such that $A \vdash p' : b = b'$.
...
Since $f$ is full, there exists a term $I \vdash a' : A\!\uparrow$ in $X$ and a term $I \vdash p : f(a') = a\!\uparrow$ in $Y$.
If we define $a''$ as $a'[left]$, then $\vdash a'' : A$ and $\vdash p[left] : f(a'') = a$.
...
a term $\Sigma(f(\Gamma)) \vdash p : f(a') = pair(v_0,d^*(a))$ in $Y$.
