...
We will also consider theories $coe_3$ and $coe_4$.
Theory $coe_3$ is $coe_2$ together with the following axiom:
\medskip
\begin{center}
\AxiomC{$\Gamma, x : I \vdash D\ type$}
\AxiomC{$\Gamma \vdash d : D[x \repl left]$}
...
\DisplayProof
\end{center}
Theory $coe_4$ is $coe_2$ together with the following axiom:
\medskip
\begin{center}
\AxiomC{$\Gamma, x : I \vdash D\ type$}
\AxiomC{$\Gamma \vdash d : D[x \repl left]$}
...
Theory $coe_4$ is slightly stronger than other theories.
To make them equivalent to $coe_4$, we need to assume additional operations.
...
\[ \xymatrix{ & Id_- \ar[rr] \ar[d] & & Id \ar[d] \\
              coe'_0 + sq \ar[r] & coe'_1 \ar[r] & coe'_2 \ar[r] \ar[d] & coe'_0 + sq + \sigma \ar[r] & coe'_1 + \sigma \ar[dd] \\
                                 &               & coe'_3 \ar[d]        &                             & \\
              sq \ar[r]          & dc \ar[r]     & coe'_4 \ar[r]        & coe'_0 + \sigma + dc \ar[r] & coe'_1 + \sigma + dc
            } \]
...
But this is not the case for $coe$ operations; it turns out that $coe^l$ follow from $coe$ even without $\Pi$ types.
