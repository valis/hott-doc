...
We also show that if $T$ has $\Sigma$ and unit types, then there exists an exact functor from $\Mod{T}$
to the category of fibration categories which reflects weak equivalences.
\end{abstract}
...
\section{Theories with sigma types}
...
\begin{center}
\AxiomC{$\Gamma \vdash p : \Sigma(A, B)$}
\UnaryInfC{$\Gamma \vdash pair(A, B, proj_1(A, B, p), proj_2(A, B, p)) \deq p$}
\DisplayProof
\end{center}
We will usually omit first two arguments to $pair$, $\pi_1$ nad $\pi_2$.
...
If $n = 0$, then there exists terms $I \vdash a' : A\!\uparrow$ and $I \vdash p : f(a') = a\!\uparrow$.
Then $\vdash a'[left] : A$ and $\vdash p[left] : f(a'[left]) = a$.
If $n > 0$, then there exists terms $\Sigma(\Gamma) \vdash a' : d^*(A)$ and $f(\Sigma(\Gamma)) \vdash p : f(a') : d^*(a)$.
Then $\Gamma \vdash c^*(a') : A$ and $f(\Gamma) \vdash c^*(p) : f(c^*(a')) = a$.
...
We will say that terms $a,a' \in X_{(tm,n)}$ are equivalent if $ty(a) = ty(a')$ and there exists a term $p$ such that $ctx(a) \vdash p : a = a'$.
...
If $x : A \vdash p : b = b'$ and $y : B \vdash q : c = c'$, then $x : A \vdash path(\lambda i.\,at(c,c',q,i)[y \repl at(b,b',p,i)]) : c[y \repl b] = c'[y \repl b']$.
...
We need to find a term $s$ such that $A \vdash s : \pi_1(b') = v_0$ and $f(s)$ is homotopic to $refl(v_0)$.
If such term exists, then since $f$ is a fibration, there exists a term $s'$ such that $A \vdash s' : \pi_1(b') = v_0$ and $f(s') = refl(v_0)$.
...
then it is easy to see that there exist terms $\Gamma \vdash q_1 : \pi_1(p) = \pi_1(p')$ and $\Gamma \vdash q_2 : sym(q_1) * \pi_2(p) * q_1 \idtype \pi_2(p')$.
...
\subsection{Fibration categories}

Fibration categories were introduced in \cite{brown}.
It was shown in \cite{szumilo} that the category of fibration categories itself carries the structure of a fibration category,
and this fibration category is equivalent to the fibration category of finitely complete quasicategories.

We conjecture that the model category of models of $coe_1 + \sigma + Path + wUA + \Sigma$
presents the $\infty$-category of finitely complete $\infty$-categories.
Thus it is natural to study the relationship between the category of models of $coe_1 + \sigma + Path + wUA + \Sigma$ and the category of fibration categories.

It turns out that if we want to construct an exact functor from the category of models to the category of fibration categories,
then we need to assume that our theory has unit types.
Recall that theory $\top$ is defined as follows:
\begin{center}
\AxiomC{}
...
Let $T_{\Sigma + \top} = coe_1 + \sigma + Path + wUA + \Sigma + \top$.
Every model of $T_{\Sigma + \top}$ carries the structure of a fibration category, which was proved in \cite{tt-fibr-cat}.
The proofs there work in our case too.
Let us briefly describe this construction.
If $X$ is a model of $T_{\Sigma + \top}$, then we define category $U(X)$, which has types from $X_{(ty,0)}$ as objects and terms of the form $A \vdash b : B\!\uparrow$ as morphisms between $A$ and $B$.
...
Note that the homotopy category $Ho(X)$ is equivalent to the localization of $U(X)$ with respect to homotopy equivalences.
This can be proved in the usual way (see, for example, \cite[Corollary~1.2.9]{hovey}).

Usually, category $U(X)$ is defined in such a way that it has all contexts as objects.
Since we have $\Sigma$ and unit types, these definitions give equivalent categories,
but if we want $U$ to be exact, then we must use our definition, since otherwise $U$ does not preserve finite limits.

Since every object of $\Mod{T_{\Sigma + \top}}$ is fibrant, $\Mod{T_{\Sigma + \top}}$ is a fibration category.
Then $U : \Mod{T_{\Sigma + \top}} \to \cat{FibCat}$ is a functor to the category of fibration categories and exact functors.
We already defined $U$ on objects and it is easy to see that every morphism $f : X \to Y$ of models determines morphism $U(f) : U(X) \to U(Y)$ of fibration categories.

\begin{prop}
Functor $U : \Mod{T_{\Sigma + \top}} \to \cat{FibCat}$ is exact.
\end{prop}
\begin{proof}
Limits in $\Mod{T_{\Sigma + \top}}$ are computed pointwise.
This implies that $U$ sends limits of models to limits of categories.
Since there is a unique structure of a fibration category on the terminal category, this implies that $U$ preserves terminal objects.
The structure of a fibration category on a pullback of (fibration) categories is also defined pointwise, which implies that $U$ preserves pullbacks.

\Rprop{sigma-we-ho} implies that $U$ preserves weak equivalences.
Thus we only need to prove that $U$ preserves fibrations.
\end{proof}
