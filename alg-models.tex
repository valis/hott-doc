...
Theory $Id$ is the regularization of a theory with the following function symbols:
\begin{align*}
...
\medskip
\begin{center}
\AxiomC{$x : ty(a), y : ty(a), z : Id(x,y) \vdash D\ type$}
\AxiomC{$x : ty(a) \vdash d : D'$}
...
\begin{center}
\AxiomC{$x : ty(a), y : ty(a), z : Id(x,y) \vdash D\ type$}
\AxiomC{$x : ty(a) \vdash d : D[y \repl x, z \repl refl(x)]$}
...
Theory $I$ is the regularization of a theory with function symbols $I : (ty,0)$, $left : (tm,0)$, $right : (tm,0)$, and the following axioms:
\begin{center}
\AxiomC{}
...
\BinaryInfC{$\vdash coe_0(\lambda x.\,D, d) : D[x \repl right]$}
\DisplayProof
\end{center}
...
\AxiomC{$\vdash d : D[x \repl left]$}
\AxiomC{$\vdash i : I$}
\TrinaryInfC{$\vdash coe_1(\lambda x.\,D, d, i) : D[x \repl i]$}
\DisplayProof
\end{center}
...
\AxiomC{$x : I \vdash D\ type$}
\AxiomC{$\vdash d : D[x \repl left]$}
\BinaryInfC{$\vdash coe_1(\lambda x.\,D, d, left) \deq d$}
\DisplayProof
\end{center}
...
\AxiomC{$x : I \vdash D\ type$}
\AxiomC{$\vdash i : I$}
\AxiomC{$\vdash d : D[x \repl i]$}
\AxiomC{$\vdash j : I$}
\QuaternaryInfC{$\vdash coe_2(\lambda x.\,D, i, d, j) : D[x \repl j]$}
\DisplayProof
\end{center}
...
\AxiomC{$x : I \vdash D\ type$}
\AxiomC{$\vdash d : D[x \repl left]$}
\BinaryInfC{$\vdash coe_2(\lambda x.\,D, left d, left) \deq d$}
\DisplayProof
\end{center}

It turns out that $coe_0$ is too weak.
To make it equivalent to other two theories, we need to add theory $sq$ to it,
which is the regularization of a theory with one function symbol $sq : (tm,0) \times (tm,0) \to (tm,0)$
with the following axioms:
\medskip
\begin{center}
\AxiomC{$\vdash i : I$}
\AxiomC{$\vdash j : I$}
\BinaryInfC{$\vdash sq(i,j) : I$}
\DisplayProof
\qquad
\AxiomC{$\vdash i : I$}
\UnaryInfC{$\vdash sq(i,left) \deq left$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$\vdash j : I$}
\UnaryInfC{$\vdash sq(left,j) \deq left$}
\DisplayProof
\qquad
\AxiomC{$\vdash j : I$}
\UnaryInfC{$\vdash sq(right,j) \deq j$}
\DisplayProof
\end{center}
...
Let $HPath$ be the regularization of a theory with function symbols $Path : (ty,0) \times (tm,0) \times (tm,0) \to (ty,0)$,
    $path : (tm,1) \to (tm,0)$, and $at : (tm,0) \times (tm,0) \times (tm,0) \times (tm,0) \to (tm,0)$, and the following axioms:
\begin{center}
\AxiomC{$\vdash a : A$}
\AxiomC{$\vdash a' : A$}
\BinaryInfC{$\vdash Path(A, a, a')\ type$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$\vdash A\ type$}
\AxiomC{$x : I \vdash a : A$}
\BinaryInfC{$\vdash path(\lambda x.\,a) : Path(A, a[x \repl left], a[x \repl right])$}
\DisplayProof
\end{center}

\smallskip
\begin{center}
\AxiomC{$\vdash p : Path(A, a, a')$}
\AxiomC{$\vdash i : I$}
\BinaryInfC{$\vdash at(a, a', p, i) : A$}
\DisplayProof
\end{center}
...
\AxiomC{$x : I \vdash a : A$}
\AxiomC{$\vdash i : I$}
\TrinaryInfC{$\vdash at(a[x \repl left], a[x \repl right], path(\lambda x.\,a), i) \deq a[x \repl i]$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$\vdash p : Path(A, a, a')$}
\UnaryInfC{$\vdash path(\lambda x.\,at(a, a', p, x)) \deq p$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$\vdash p : Path(A, a, a')$}
\UnaryInfC{$\vdash at(a, a', p, left) \deq a$}
\DisplayProof
\quad
\AxiomC{$\vdash p : Path(A, a, a')$}
\UnaryInfC{$\vdash at(a, a', p, right) \deq a'$}
\DisplayProof
\end{center}
...
\[ \xymatrix{ & Id_- \ar[rr] \ar[d] & & Id \ar[d] \\
              coe_0 + sq \ar[r] & coe_1 \ar[r] & coe_2 \ar[r] & coe_0 + sq + \sigma \ar[r] & coe_1 + \sigma
            } \]
where each of the theories in the bottom row are theories with homogeneous path types.
Note that we have maps going in both directions between theories $coe_0 + sq + \sigma + HPath$ and $coe_1 + \sigma + HPath$,
which means that we can define all of the constructions that we discussed in either of these theories.

Arrows $Id_- \to Id$ and $coe_1 + HPath \to coe_2 + HPath$ are obvious.
...
If we have $coe_1$, then we can define $Jeq(D,d,a)$ as $coe_0(\lambda j.\,Path(D'', coe_1(\lambda i. D'', d[a], j), d[a]), refl(d[a]))$.

Map $sq \to coe_1 + HPath$ can be defined as follows:
\[ sq(i,j) = at(left, j, coe_1(\lambda x.\,Path(I, left, x), refl(left), j), i). \]

Finally, let us define arrow $coe_2 \to coe_0 + sq + \sigma + HPath$.
First, note that we can define a map $coe_1 \to coe_0 + sq + \sigma$ as follows: $coe_1(\lambda x.\,A, a, j) = coe_0(\lambda i.\,A[x \repl sq(i,j)], a)$.
Then let $Ic(i) = path\ (\lambda j.\,sq(j,i))$.
Since $Ic(i) : left = i$, we can define a term $pp(i,j)$ of type $i = j$.
Now, let $coe_2(\lambda x.\,A, i, a, j)$ be equal to $coe_0(\lambda x.\,A[x \repl at(i, j, pp(i,j), x)], a)$.
Note that $\vdash pp(left,left) \deq refl(left)$; hence $\vdash coe_2(\lambda x.\,A, left, a, left) \deq a$.
...
\[ iso : (ty,0)^2 \times (tm,1)^4 \times (tm,0) \to (ty,0) \]
...
y : B & \vdash g : A \\
x : A, i : I & \vdash p : A \\
x : A & \vdash p[i \repl left] \deq g[y \repl f] \\
x : A & \vdash p[i \repl right] \deq x \\
y : B, i : I & \vdash q : B \\
y : B & \vdash q[i \repl left] \deq f[x \repl g] \\
y : B & \vdash q[i \repl right] \deq y
\end{align*}
If we have path types, then the last six axioms can be replaced with the following two:
\begin{align*}
x : A & \vdash p : Path(A, g[y \repl f], x) \\
y : B & \vdash q : Path(B, f[x \repl g], y)
\end{align*}
...
\AxiomC{$S$}
\AxiomC{$\vdash j : I$}
\BinaryInfC{$\vdash iso(A, B, \lambda x.\,f, \lambda y.\,g, \lambda x i.\,p, \lambda y i.\,q, j)\ type$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$S$}
\UnaryInfC{$\vdash iso(A, B, \lambda x.\,f, \lambda y.\,g, \lambda x i.\,p, \lambda y i.\,q, left) \deq A$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$S$}
\UnaryInfC{$\vdash iso(A, B, \lambda x.\,f, \lambda y.\,g, \lambda x i.\,p, \lambda y i.\,q, right) \deq B$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$S$}
\UnaryInfC{$\vdash coe_0(\lambda j.\,iso(A, B, \lambda x.\,f, \lambda y.\,g, \lambda x i.\,p, \lambda y i.\,q, j), a) \deq f[x \repl a]$}
\DisplayProof
\end{center}
\medskip
...
Now, we are ready to define our first examples of generating cofibrations.
These examples are particularly important since they will be used to characterize weak equivalences.
Let $e : \{ ty, tm \} \to \{ ctx, ty \}$ be the function defined as $e(ty) = ctx$, $e(tm) = ty$.
Also we define $d_p : (p,n) \to (e(p),n)$ as $d_{ty} = ft$, $d_{tm} = ty$.
Let $U^0_{(p,n)} = F(\{\,A : (e(p),n)\,\})$ and $V^0_{(p,n)} = F(\{\,a : (p,n)\,\})$.
Then we define $i^0_{(p,n)} : U^0_{(p,n)} \to V^0_{(p,n)}$ as $i^0_{(p,n)}(A) = d_p(a)$.
Let $\I^0_p$ be the set of maps of the form $i^0_{(p,n)}$, and let $\I^0 = \I^0_{ty} \cup \I^0_{tm}$.
...
Let $i^{k+1}_{p,n,m} : U^{k+1}_{p,n,m} \to V^{k+1}_{p,n,m}$ be the following map: $i^{k+1}_{p,n,m}(\Gamma) = \Gamma$, $i^{k+1}_{p,n,m}(a_{[i = c]}) = [i = c]^*(a)$.

Finally, let us define $V^{k+1}_{tm,n,-1}$ as
\[ F(\{\,A : (ty,n), a : (tm,n+k+1)\,\}, \{\,ty(a) = (ft(A), I^{k+1} \vdash A\!\!\uparrow^{k+1})\,\}), \]
and let $U^{k+1}_{tm,n,-1}$ be generated by $A : (ty,n)$ and $a_{[i = c]} : (tm,n+k)$ for every $0 \leq i \leq k$ and $c \in \{ left, right \}$,
with relations $ty(a_{[i = c]}) = (ft(A), I^k \vdash A\!\!\uparrow^k)$ for every $0 \leq i \leq k$ and $c$,
and $[i_1 = c_1]^*(a_{[i_2 = c_2]}) = [i_2 - 1 = c_2]^*(a_{[i_1 = c_1]})$ for every $0 \leq i_1 < i_2 \leq k$ and $c_1$, $c_2$.
Let $i^{k+1}_{tm,n,-1} : U^{k+1}_{tm,n,-1} \to V^{k+1}_{tm,n,-1}$ be the following map: $i^{k+1}_{tm,n,-1}(A) = A$, $i^{k+1}_{tm,n,-1}(a_{[i = c]}) = [i = c]^*(a)$.
Let $\I$ be the set of maps of the form $i^{k+1}_{p,n,m}$ together with maps from $\I^0$.
