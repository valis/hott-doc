...
\quad
\AxiomC{}
\UnaryInfC{$\Gamma \vdash refl(a) : Id(ty(a), a, a)$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$\Gamma, x : ty(a), y : ty(a), z : Id(x,y) \vdash D\ type$}
\AxiomC{$\Gamma, x : ty(a) \vdash d : D[y \repl x, z \repl refl(x)]$}
...
\AxiomC{$\Gamma \vdash i : I$}
\BinaryInfC{$\Gamma \vdash at(A, a, a', p, i) : A[x \repl i]$}
\DisplayProof
\end{center}
...
\AxiomC{$\Gamma, x : I \vdash a : A$}
\AxiomC{$\Gamma \vdash i : I$}
\BinaryInfC{$\Gamma \vdash at(A, a[x \repl left], a[x \repl right], path(\lambda x.\,a), i) \deq a[x \repl i]$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$\Gamma \vdash p : Path(\lambda x.\,A, a, a')$}
\UnaryInfC{$\Gamma \vdash path(\lambda x.\,at(A, a, a', p, x)) \deq p$}
\DisplayProof
\end{center}
...
There are a few different model structures on categories of models;
thus now we define several sets of morphisms which can be used as generating cofibrations.
