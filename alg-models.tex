...
Let $C_{F(\{\,\Gamma \vdash ctx\,\})}(F(\{\,\Gamma \vdash A\ type\,\}))$ be the theory generated by the following relations:
\begin{align*}
\Gamma & \vdash A\ type \\
\Gamma & \vdash B\ type \\
\Gamma, x : A & \vdash b : B \\
\Gamma, y : B & \vdash a_1 : A \\
\Gamma, x : A & \vdash p_1 : a_1[y \repl b] = x \\
\Gamma, y : B & \vdash q_1 : b[x \repl a_1] = y \\
\Gamma, y : B & \vdash a_2 : A \\
\Gamma, x : A & \vdash p_2 : a_2[y \repl b] = x \\
\Gamma, y : B & \vdash q_2 : b[x \repl a_2] = y
\end{align*}
...
To do this, we need to establish a few lemmas.

We will consider maps of the form
\[ F(\{\,\Gamma \vdash A\ type\,\}) \to F(\{\,\Gamma, I \vdash H\ type\,\}), \]
where $A$ is mapped to $H[left]$.
We will denote the set of such maps by $\J'_{\I_{ty}}$.
