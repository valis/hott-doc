...
Now, let us show how to construct a set of generating trivial cofibrations from a set of generating cofibrations satisfying some mild hypotheses.
Let $\C$ be a cocomplete category and $\I$ a class of maps of $\C$ together with a relative cylinder object $C_U(V)$ for every $i : U \to V$ in $\I$.
Let us denote by $\J_\I$ the class of maps $\cyli_0 : V \to C_U(V)$ for each $U \to V \in \I$.

\begin{prop}[triv-fib-iinj]
Let $\C$ be a category and $\I$ a class of maps of $\C$ such that
...
Often the class of cofibrations is generated by some smaller class $\I$.
We want to prove that to check that a map is a weak equivalences, it is enough to show that it has RLP up to $\sim_i$ with respect to every $i \in \I$.
Since $\I$ is usually much smaller than the whole class of cofibrations, this condition is easier to verify.
To do this, we introduce some notation and prove a technical lemma.
...
% \todo{Construct a system of cylinders on a model category}

% \todo{Prove that this implies the characterization of weak equivalences}
