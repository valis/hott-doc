...
Now, we need to formulate a lemma, which we will prove later in \rlem{we-bot-path-proof}.

\begin{lem}[we-bot-path]
...
By \rlem{we-bot-path}, $g$ has RLP up to $\sim_i$ with respect to every $i \in \I$.
...
\end{enumerate}
Then conditions of \rlem{we-bot} are satisfied.
\end{prop}
\begin{proof}
Let $H(U,B) = Hom(U,P(B))$ and $H(V,B) = H'(V,B) = Hom(V,P(B))$.
Then by \rlem{hom-ext}, $i^* : H(V,B) \to H(U,B)$ has right homotopy extension property.
...
\begin{enumerate}
\item For every $i : U \to V \in \I$ conditions of \rlem{we-bot} are satisfied.
...
By \rlem{we-bot}, $g$ is a weak equivalence.
By \rprop{triv-fib-iinj}, $g$ is a trivial fibration.
...
Now we can prove \rlem{we-bot-path}:

\begin{lem}[we-bot-path-proof]
Let $\C$ be a model category, and let $Y$ and $Z$ be fibrant objects of $\C$.
Let $f : X \to Y$ and $g : Y \to Z$ be maps of $\C$ such that $f$ has RLP up to $\sim^r$ with respect to $U$, and $g \circ f$ has RLP with respect to $i$ up to $\sim_i$.
Then $g$ also has RLP with respect to $i$ up to $\sim_i$.
\end{lem}
