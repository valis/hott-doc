...
If pushout $V \amalg_U V$ exists and $s$ is pure with respect to $[\cyli_0,\cyli_1] : V \amalg_U V \to C_U(V)$, then the homotopy relation is also symmetric.
We will say that a (relative) cylinder object $C(V)$ is \emph{proper} if the corresponding homotopy relation on every set $\C(V,X)$ is an equivalence relation.

Let $\C$ be a category and let $X$ be an object of $\C$.
A \emph{path object} for $X$ is an object $P(X)$ together with maps $t : X \to P(X)$
and $p_0,p_1 : P(X) \to X$ such that $p_0 \circ t = p_1 \circ t = id_X$.
...
We say that a right homotopy $h : V \to P(X)$ between $f,g : V \to X$
is constant on $i : U \to V$ if $h \circ i = t \circ f \circ i$.
...
and a path object $t : X \to P(X)$, $p_0,p_1 : P(X) \to X$ for every $X$, such that $p_0$ is pure with respect to $[\cyli_0,\cyli_1] : V \amalg_U V \to C_U(V)$,
...
Every object $X$ in a model category $\C$ has a path object which is defined as a factorization of the diagonal $\langle id_X, id_X \rangle : X \to X \times X$.
...
Let $\C$ be a finitely complete and finitely cocomplete category, and let $i : U \to V$ be a morphism of $\C$.
Suppose that for an object $X$ of $\C$, there exists a path object $t : X \to P(X)$, $p_0,p_1 : P(X) \to X$ such that $p_1$ has RLP with respect to $i$.

Let $i : U \to V \in \cof$, $u : U \to X$, and $v : V \to X$ be maps, and let $h : U \to P(X)$ be a homotopy between $u$ and $v \circ i$.
Then there exists a map $v' : V \to X$ and a homotopy $h' : V \to P(X)$ between $v' : V \to X$ and $v$ such that $h = h' \circ i$.
\end{lem}
\begin{proof}
Let $h : U \to P(X)$ be a homotopy between $u$ and $v \circ i$.
Consider the following diagram:
\[ \xymatrix{ U \ar[r]^-h \ar[d]_i & P(X) \ar[d]^{p_1} \\
              V \ar[r]_v & X
            } \]
...
\begin{lem}[we-bot]
Let $\C$ be a finitely complete and finitely cocomplete category, and let $i : U \to V$ be a map of $\C$ which satisfies conditions of \rlem{path-cyl}.
