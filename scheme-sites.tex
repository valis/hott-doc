...
We call opens of the form $P(a,x)$ \emph{principal opens}.
...
We now construct a scheme site associated to a scheme. In fact, the construction is more general and applies to any ringed locale.

First, we prove two simple lemmas about principal opens.

\begin{lem}[po-restrict]
Let $(L,\mathcal{O}^L)$ be a ringed locale.
Then for all $b,a \in O_L$ such that $b \leq a$, and every $x \in \mathcal{O}^L_a$, we have $P(a,x) \wedge b = P(b, \mathcal{O}^L_{b,a}(x))$.
\end{lem}
\begin{proof}
Recall that
\[ P(a,x) = \bigvee \{ c \in O_L \mid c \leq a, \mathcal{O}^L_{c,a}(x) \text{ is invertible} \}. \]

We first prove that
$P(b, \mathcal{O}^L_{b,a}(x)) \leq P(a,x) \wedge b$.
It suffices to show that for every $c \leq b$ such that $\mathcal{O}^L_{c,b}(\mathcal{O}^L_{b,a}(x))$ is invertible, we have $c \leq P(a,x) \wedge b$.
The inequality $c \leq b$ holds by assumption, and $c \leq P(a,x)$ follows from the fact that $\mathcal{O}^L_{c,a}(x)$ is invertible.
Thus $c \leq P(a,x) \wedge b$.

We now prove the opposite inequality.
It suffices to show that for every $c \leq a$ such that $\mathcal{O}^L_{c,a}(x)$ is invertible, we have $c \wedge b \leq P(b, \mathcal{O}^L_{b,a}(x))$.
This follows from the fact that $\mathcal{O}^L_{c \wedge b, b}(\mathcal{O}^L_{b,a}(x))$ is invertible.
\end{proof}

\begin{lem}[po-meet]
Let $(L,\mathcal{O}^L)$ be a ringed locale.
Then for all $a \in O_L$ and $x,y \in \mathcal{O}^L_a$, we have $P(a,x) \wedge P(a,y) = P(a, x y)$.
\end{lem}
\begin{proof}
This follows directly from the definition of principal opens.
\end{proof}

Let $(L,\mathcal{O}^L)$ be a ringed locale, and let $C \subseteq O_L$ be a set of affine opens such that
for all $a \in C$ and $x \in \mathcal{O}^L_a$, the principal open $P(a,x)$ also belongs to $C$.
Then we construct a scheme site $S(L,\mathcal{O}^L,C)$.
Its underlying set is $C$.
The preorder is defined by declaring that
\[
b \leq a \text{ if and only if } \exists x \in \mathcal{O}^L_a, b = P(a,x).
\]
The structure presheaf is given by the restriction of $\mathcal{O}^L$ to $C$.
Condition~\axref{SE} follows from \rdefn{affine-open}.\eqref{affine:loc}.
Condition~\axref{SC} follows from \rlem{po-restrict}.

We now show that affine opens are stable under taking principal opens.

\begin{lem}[po-affine]
Let $(L,\mathcal{O}^L)$ be a ringed locale.
Then for every affine open $a \in \mathcal{O}_L$ and every $z \in \mathcal{O}^L_a$, the principal open $P(a,z)$ is also affine.
\end{lem}
\begin{proof}
We verify conditions of \rdefn{affine-open}.

\eqref{affine:loc} This follows from \rlem{loc-factor}.

\eqref{affine:surj} Let $b$ be an open such that $b \leq P(a,z)$. Then we have % For every $b \leq a$, we have \[ b \leq \bigvee \{ c \mid c \leq b, \exists x \in \mathcal{O}^L_a, c = P(a,x) \}. \]
\[ b \leq \bigvee \{ c \mid c \leq b, \exists x \in \mathcal{O}^L_a, c = P(a,x) \}. \]
Thus, it is enough to show that for every $x \in \mathcal{O}^L_a$ such that $P(a,x) \leq b$, we have $P(P(a,z), \mathcal{O}^L_{P(a,z),a}(x)) \leq b$.
By \rlem{po-restrict}, we have $P(P(a,z), \mathcal{O}^L_{P(a,z),a}(x)) = P(a,x) \wedge P(a,z)$.
This implies the required inequality.

\eqref{affine:cover}
Let $x \in \mathcal{O}^L_{P(a,z)}$ and $U \subseteq O_L$ be such that $P(P(a,z),x) \leq \bigvee U$.
We need to show that the ideal of $\mathcal{O}^L_{P(P(a,z),x)}$ generated by the following set is the unit ideal:
\[ \{ y \mid \exists c \in U, \exists b, b \leq P(P(a,z),x), b \leq c, \mathcal{O}^L_{b,P(P(a,z),x)} \text{ is a localization at } y \}. \]

We can assume that $x = \mathcal{O}^L_{P(a,z),a}(x')$ for some $x' \in \mathcal{O}^L_a$.
By \rlem{po-restrict} and \rlem{po-meet}, we have $P(P(a,z),x) = P(a,x') \wedge P(a,z) = P(a, x' z)$.
By condition~\eqref{affine:cover} for $a$, the ideal of $\mathcal{O}^L_{P(a, x' z)}$ generated by the following set is the unit ideal:
\[ \{ y \mid \exists c \in U, \exists b, b \leq P(a, x' z), b \leq c, \mathcal{O}^L_{b,P(a, x' z)} \text{ is a localization at } y \}. \]

This concludes the proof since the rings $\mathcal{O}^L_{P(P(a,z),x)}$ and $\mathcal{O}^L_{P(a, x' z)}$, and the ideals described above coincide.
\end{proof}

This lemma implies that we can take $C$ to be the set of all affine opens in the construction of $S(L,\mathcal{O}^L,C)$.
In this case, we denote this scheme site by $S(L,\mathcal{O}^L)$.

\begin{prop}[surj-obj]
Every scheme $(L,\mathcal{O}^L)$ is isomorphic to $\widetilde{S(L,\mathcal{O}^L)}$.
\end{prop}
\begin{proof}
We first construct a presentation morphism $f : (L,\mathcal{O}^L) \to S(L,\mathcal{O}^L)$.
Since the set of opens of $S(L,\mathcal{O}^L)$ is a subset of $O_L$, we can define $f^*$ as the inclusion of this subset.
The homomorphism $f^\sharp_a$ is defined as the identity map.
Now, we verify conditions of \rdefn{pres-map}.

\eqref{pres:top}
Since $(L,\mathcal{O}^L)$ is a scheme, we have that $\top$ in $O_L$ is covered by the set of affine opens.

\eqref{pres:meet}
Let $a,b \in O_L$ be affine opens. We need to show that
\[ a \wedge b \leq \bigvee \{ c \in O_L \mid c \text{ is affine}, c \leq a, c \leq b \}. \]
By \rdefn{affine-open}.\eqref{affine:surj}, we know that
\[ a \wedge b \leq \bigvee \{ a' \wedge b' \mid a',b' \leq a \wedge b, \exists x \in \mathcal{O}^L_a, \exists y \in \mathcal{O}^L_b, a' = P(a,x), b' = P(b,y) \}. \]
Thus, it is enough to show that if $P(a,x),P(b,y) \leq a \wedge b$, then $P(a,x) \wedge P(b,y)$ is affine.
By \rlem{po-affine}, $P(b,y)$ is affine.
By \rlem{po-restrict}, we have $P(a,x) \wedge P(b,y) = P(P(b,y),\mathcal{O}^L_{P(b,y),a}(x))$.
Thus, by \rlem{po-affine}, $P(a,x) \wedge P(b,y)$ is also affine.

\eqref{pres:loc}
This follows immediately from the definition of principal opens.

Next, we prove that $\psi(f) : (L,\mathcal{O}^L) \to \widetilde{S(L,\mathcal{O}^L)}$ is an isomorphism.
To do this, it is enough to show that both $\psi(f)^*$ and $\psi(f)^\sharp$ are bijective.

To prove injectivity of $\psi(f)^*$, it is enough to show that $f^*$ reflects covers.
That is, we need to show that given an affine open $a \in O_L$ and a set of affine opens $U \subseteq O_L$, if $a \leq \bigvee U$, then $a$ is covered by $U$ in $S(L,\mathcal{O}^L)$.
This follows from \rdefn{affine-open}.\eqref{affine:cover} applied to $x = 1$.

To prove surjectivity of $\psi(f)^*$, we need to show that every open $b \in O_L$ is a join of affine opens.
Since $(L,\mathcal{O}^L)$ is a scheme, $\top$ is covered by a set of affine opens $\{ a_i \}_{i \in I}$.
It follows that $b = \bigvee_{i \in I} b \wedge a_i$.
Thus, it is enough to show that $b \wedge a_i$ is a join of affine opens, but this follows from \rdefn{affine-open}.\eqref{affine:surj}. 

Finally, $\psi(f)^\sharp$ is the identity map on basic opens.
It follows that it is an isomorphism on all opens by the sheaf condition.
\end{proof}

Finally, we are ready to prove the main result of this section.

\begin{thm}
The categories $\cat{SchSite}$ and $\cat{Sch}$ are equivalent.
\end{thm}
\begin{proof}
By \rprop{fff}, we have a fully faithful functor $\cat{SchSite} \to \cat{Sch}$.
By \rprop{surj-obj}, this functor is essentially surjective on objects.
Thus, it is an equivalence of categories.
\end{proof}
