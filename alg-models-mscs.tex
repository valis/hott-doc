...
Let us define the substitution function for restricted terms.
...
In particular, we will show that theories with an interval type and path types also have $\IdT$-types.
...
The theory $\IdT$ has the following function symbols:
\begin{align*}
...
We will also need slightly weaker version of $\IdT$ which we will denote by $\IdT_-$.
It has all of the function symbols of $\IdT$ together with the following one:
\[ \Jeq : (tm,n+3) \times (tm,n+1) \times (tm,n) \to (tm,n) \]
The theory $\IdT_-$ has all of the axioms of $\IdT$ except the last one; instead it has the following additional axiom:
\begin{center}
\AxiomC{$\Gamma, x : A, y : A, z : \Id(x,y) \vdash D\ \type$}
\AxiomC{$\Gamma, x : A \vdash d : D'$}
\AxiomC{$\Gamma \vdash a : A$}
...
\end{center}
where $D' = D[y \repl x, z \repl \refl(x)]$.
The idea is that the last axiom of $\IdT$ holds in $\IdT_-$ only propositionally.
...
The theory $I$ has function symbols $I : (ty,n)$, $\leftI : (tm,n)$, $\rightI : (tm,n)$ and the following axioms:
\begin{center}
\AxiomC{$\Gamma \vdash$}
...
In $\coeT_0$, we can only take $i = \leftI$ and $j = \rightI$.

\medskip
\begin{center}
\AxiomC{$\Gamma, x : I \vdash D\ \type$}
\AxiomC{$\Gamma \vdash d : D[x \repl \leftI]$}
\BinaryInfC{$\Gamma \vdash \coe_0(D, d) : D[x \repl \rightI]$}
\DisplayProof
\end{center}

In $\coeT_1$, we can take $i = \leftI$ and arbitrary $j$.
...
\AxiomC{$\Gamma, x : I \vdash D\ \type$}
\AxiomC{$\Gamma \vdash d : D[x \repl \leftI]$}
\BinaryInfC{$\Gamma \vdash \coe_1(D, d, \leftI) \deq d$}
\DisplayProof
\end{center}

In $\coeT_2$, both $i$ and $j$ may be arbitrary.
...
\AxiomC{$\Gamma, x : I \vdash D\ \type$}
\AxiomC{$\Gamma \vdash d : D[x \repl \leftI]$}
\BinaryInfC{$\Gamma \vdash \coe_2(D, \leftI, d, \leftI) \deq d$}
\DisplayProof
\end{center}

It turns out that $\coeT_0$ is too weak.
To make it equivalent to other two theories, we need to add the theory $\sqT$ to it,
...
\qquad
\AxiomC{$\Gamma \vdash i : I$}
\UnaryInfC{$\Gamma \vdash \sq(i,\leftI) \deq \leftI$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$\Gamma \vdash j : I$}
\UnaryInfC{$\Gamma \vdash \sq(\leftI,j) \deq \leftI$}
\DisplayProof
\qquad
\AxiomC{$\Gamma \vdash j : I$}
\UnaryInfC{$\Gamma \vdash \sq(\rightI,j) \deq j$}
\DisplayProof
\end{center}

Also, these theories correspond to $\IdT_-$.
To get theories that correspond to $\IdT$, we need to add one additional rule to each of them:
\medskip
\begin{center}
\AxiomC{$\Gamma \vdash a : A$}
...
\quad
\AxiomC{$\Gamma \vdash a : A$}
\UnaryInfC{$\Gamma \vdash \coe_1(A, a, \rightI) \deq a$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$\Gamma \vdash a : A$}
\UnaryInfC{$\Gamma \vdash \coe_2(A, \leftI, a, \rightI) \deq a$}
\DisplayProof
\end{center}
\smallskip
We denote these theories by $\coeT_0 + \sigma$, $\coeT_1 + \sigma$ and $\coeT_2 + \sigma$.

We will also consider additional axioms $\beta_1$ and $\beta_2$ for $\coeT_2$.
The axiom $\beta_1$ is defined as follows:
\medskip
\begin{center}
\AxiomC{$\Gamma, x : I \vdash D\ \type$}
\AxiomC{$\Gamma \vdash d : D[x \repl \leftI]$}
\BinaryInfC{$\Gamma \vdash \coe_2(D, \rightI, d, \rightI) \deq d$}
\DisplayProof
\end{center}
\medskip
...
\DisplayProof
\end{center}
Obviously, we have maps from $\coeT_2$ to $\coeT_2 + \beta_1$ and from $\coeT_2 + \beta_1$ to $\coeT_2 + \beta_2$.

The theory $\coeT_2 + \beta_2$ is slightly stronger than other theories.
To make them equivalent to $\coeT_2 + \beta_2$, we need to assume additional operations.
For example, we can consider the theory $\dcT$, which has one function symbol
...
\AxiomC{$\Gamma \vdash i : I$}
\AxiomC{$\Gamma \vdash j : I$}
\BinaryInfC{$\Gamma \vdash \dc(i,j,\leftI) \deq i$}
\DisplayProof
\end{center}
...
\AxiomC{$\Gamma \vdash i : I$}
\AxiomC{$\Gamma \vdash j : I$}
\BinaryInfC{$\Gamma \vdash \dc(i,j,\rightI) \deq j$}
\DisplayProof
\qquad
\AxiomC{$\Gamma \vdash i : I$}
\AxiomC{$\Gamma \vdash k : I$}
...
To define maps between theories with $\IdT$-types and theories with an interval type,
...
$\pathI : (ty,n) \times (tm,n+1) \to (tm,n)$, and $\at : (tm,n) \times (tm,n) \times (tm,n) \times (tm,n) \to (tm,n)$ and the following axioms:
\begin{center}
\AxiomC{$\Gamma \vdash a : A$}
\AxiomC{$\Gamma \vdash a' : A$}
...
\BinaryInfC{$\Gamma \vdash \pathI(A, a) : a[x \repl \leftI] \idtype a[x \repl \rightI]$}
\DisplayProof
\end{center}
...
\AxiomC{$\Gamma, x : I \vdash a : A$}
\AxiomC{$\Gamma \vdash i : I$}
\TrinaryInfC{$\Gamma \vdash \at(a[x \repl \leftI], a[x \repl \rightI], \pathI(A, a), i) \deq a[x \repl i]$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$\Gamma \vdash p : a \idtype a'$}
\UnaryInfC{$\Gamma \vdash \pathI(ty(a),\,x.\,\at(a, a', p, x)) \deq p$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$\Gamma \vdash p : a \idtype a'$}
\UnaryInfC{$\Gamma \vdash \at(a, a', p, \leftI) \deq a$}
\DisplayProof
\quad
\AxiomC{$\Gamma \vdash p : a \idtype a'$}
\UnaryInfC{$\Gamma \vdash \at(a, a', p, \rightI) \deq a'$}
\DisplayProof
\end{center}

We can summarize the relationship between different theories in the following diagram of theories:
\[ \xymatrix{ & \IdT_- \ar[rr] \ar[d] & & \IdT \ar[d] \\
              \coeT'_0 + \sqT \ar[r] & \coeT'_1 \ar[r] & \coeT'_2 \ar[d]    \ar[r] & \coeT'_0 + \sigma + \sqT \ar@<1.5pt>[r] \ar[d] & \coeT'_1 + \sigma \ar[d] \ar@<1.5pt>[l] \\
              \sqT           \ar[r]  & \dcT     \ar[r] & \coeT'_2 + \beta_2 \ar[r] & \coeT'_0 + \sigma + \dcT \ar@<1.5pt>[r]        & \coeT'_1 + \sigma + \dcT \ar@<1.5pt>[l]
            } \]
where $\coeT'_\alpha = \coeT_\alpha + \HPath$ and each arrow $T \to T'$ represents an interpretation of $T$ in $T'$.

Interpretations $\IdT_- \to \IdT$ and $\coeT'_1 \to \coeT'_2$ are obvious.
...
\[ D[a, \at(a, a', p, \sq(i,\rightI)), \pathI(ty(a),\,j.\,\at(a, a', p, \sq(i,j)))]. \]
...
We can define an interpretation $\sqT \to \coeT'_1$ as follows:
\[ \sq(i,j) = \at(\leftI, j, \coe_1(x.\,\leftI \idtype x, \refl(\leftI), j), i). \]

Now, let us define an interpretation $\coeT_2 \to \coeT'_0 + \sqT + \sigma$.
First, note that we can define an interpretation $\coeT_1 \to \coeT_0 + \sqT + \sigma$ as follows: $\coe_1(D, d, j) = \coe_0(i.\,D[\sq(i,j)], d)$.
Then let $\Ic(i) = \pathI(I, j.\,\sq(j,i))$.
Since $\Ic(i) : \leftI \idtype i$, we can define a term $\dc'(i,j)$ of type $i \idtype j$.
Now, let $\coe_2(D, i, d, j)$ be equal to $\coe_0(x.\,D[\at(i, j, \dc'(i,j), x)], d)$.
Note that $\dc'(\leftI,\leftI) \deq \refl(\leftI)$; hence we have the following equality:
\[ \coe_2(D, \leftI, d, \leftI) \deq d. \]
...
Let $\PathT$ be the theory with function symbols $\Path : (ty,n+1) \times (tm,n) \times (tm,n) \to (ty,n)$,
$\pathI : (tm,n+1) \to (tm,n)$, and $\at : (ty,n+1) \times (tm,n) \times (tm,n) \times (tm,n) \times (tm,n) \to (tm,n)$, and the following axioms:
\begin{center}
\AxiomC{$\Gamma, x : I \vdash A\ \type$}
\AxiomC{$\Gamma \vdash a : A[x \repl \leftI]$}
\AxiomC{$\Gamma \vdash a' : A[x \repl \rightI]$}
\TrinaryInfC{$\Gamma \vdash \Path(A, a, a')\ \type$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$\Gamma, x : I \vdash a : A$}
\UnaryInfC{$\Gamma \vdash \pathI(a) : \Path(A, a[x \repl \leftI], a[x \repl \rightI])$}
\DisplayProof
\end{center}
...
\AxiomC{$\Gamma, x : I \vdash a : A$}
\AxiomC{$\Gamma \vdash i : I$}
\BinaryInfC{$\Gamma \vdash \at(A, a[x \repl \leftI], a[x \repl \rightI], \pathI(a), i) \deq a[x \repl i]$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$\Gamma \vdash p : \Path(A, a, a')$}
\UnaryInfC{$\Gamma \vdash \pathI(y.\,\at(A, a, a', p, y)) \deq p$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$\Gamma \vdash p : \Path(A, a, a')$}
\UnaryInfC{$\Gamma \vdash \at(A, a, a', p, \leftI) \deq a$}
\DisplayProof
\quad
\AxiomC{$\Gamma \vdash p : \Path(A, a, a')$}
\UnaryInfC{$\Gamma \vdash \at(A, a, a', p, \rightI) \deq a'$}
\DisplayProof
\end{center}

We will often omit the first three arguments of $\at$ since it is easy to infer them from the type of the fourth argument.
We can interpret $\HPath$ in $\PathT$ as follows: $a \idtype a' = \Path(x.\,ty(a), a, a')$.
...
If $M$ is a model of $\coeT_1 + \PathT$, then these cubical sets are fibrant; that is, have fillers for all boxes (a box in a cubical set is an analog of a horn in a simplicial set).
...
\AxiomC{$\Gamma, x : I, \Delta \vdash D\ \type$}
\AxiomC{$\Gamma, \Delta[x \repl \leftI] \vdash d : D[x \repl \leftI]$}
\BinaryInfC{$\Gamma, \Delta[x \repl \leftI] \vdash \coe^l_1(D, d, \leftI) \deq d$}
\DisplayProof
\end{center}
...
\AxiomC{$\Gamma, x : I, \Delta \vdash D\ \type$}
\AxiomC{$\Gamma, \Delta[x \repl \leftI] \vdash d : D[x \repl \leftI]$}
\BinaryInfC{$\Gamma, \Delta[x \repl \leftI] \vdash \coe^l_2(D, \leftI, d, \leftI) \deq d$}
\DisplayProof
\end{center}

We can also consider theory $\coeT^l_2 + \beta^l_2$ which is $\coeT^l_2$ together with the following axiom:
\begin{center}
\AxiomC{$\Gamma, x : I, \Delta \vdash D\ \type$}
\AxiomC{$\Gamma \vdash i : I$}
\AxiomC{$\Gamma, \Delta[x \repl i] \vdash d : D[x \repl i]$}
...
But this is not the case for $\coeT_2 + \beta_2$; it turns out that $\coeT^l_2 + \beta^l_2$ can be defined in $\coeT_2 + \beta_2$ even without $\Pi$-types.
...
For example, if $\Delta$ equals to $y_1 : B_1, \ldots y_k : B_k$, then we can rewrite $\coeT^l_2$ as follows:
\medskip
\begin{center}
\def\extraVskip{1pt}
...
\DisplayProof
\end{center}
Then theories $\coeT^l_2$ and $\coeT^{l'}_2$ are isomorphic; that is, we can interpret $\coeT^l_2$ in $\coeT^{l'}_2$, and vice versa, and the interpretations are mutually inverse.
...
Now, we can define an interpretation $\coeT^{l'}_2 + \beta^l_2 \to \coeT_2 + \beta_2$.
...
Theories $\coeT^{l'}_0$ and $\coeT^{l'}_1$ are defined similarly to $\coe^{l'}_2$.
Theory $\coeT^{l'}_1 + \sigma^l$ is $\coeT^{l'}_1$ together with the following axiom:
\medskip
\begin{center}
\def\extraVskip{1pt}
...
\AxiomC{$S$}
\AxiomC{$\Gamma \vdash j : I$}
\BinaryInfC{$\Gamma \vdash \iso(A, B, b, a, p, q, j)\ \type$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$S$}
\UnaryInfC{$\Gamma \vdash \iso(A, B, b, a, p, q, \leftI) \deq A$}
\DisplayProof
\end{center}
...
\begin{center}
\AxiomC{$S$}
\UnaryInfC{$\Gamma \vdash \iso(A, B, b, a, p, q, \rightI) \deq B$}
\DisplayProof
\end{center}
...
\begin{thm}[main]
For every $I$-stable theory $T$ in which the theory $\coeT_1 + \sigma + \PathT + \wUA$ can be interpreted,
...
Indeed, given a type $i : I, \Delta \vdash H$ and a term $\Delta[i \repl \leftI] \vdash a : H[i \repl \leftI]$ in $X$,
we need to find a term $i : I, \Delta \vdash h : H$ such that $h[i \repl \leftI] = a$.
We can define $h$ as $\Fill^1_{(tm,1+n)}(H, a, i)$.
Given a context $j : I, \Delta$, where $\Delta = (x_1 : B_1, \ldots x_n : B_n)$, and a type $\Delta[j \repl \leftI] \vdash A$,
we need to find a type $j : I, \Delta \vdash H$ such that $\Delta[j \repl \leftI] \vdash H[j \repl \leftI] \deq A$.
We can define $H$ as $j : I, \Delta \vdash A[x_1 \repl b_1, \ldots x_n \repl b_n)$, where $b_i = \coe^l_2(j.\,B_i, j, x_i, \leftI)$.
...
\bibitem[Isaev 2013]{f-model-structures}
V.~{Isaev}, \emph{On fibrant objects in model categories}, unpublished, (2013),
  \href {http://arxiv.org/abs/1312.4327} {\path{arXiv:1312.4327}}.

\bibitem[Isaev 2016]{alg-tt}
V.~{Isaev}, \emph{{Algebraic Presentations of Dependent Type Theories}},
  unpublished, (2016), \href {http://arxiv.org/abs/1602.08504} {\path{arXiv:1602.08504}}.
...
C.~{Kapulkin}, \emph{{Locally Cartesian Closed Quasicategories from Type
  ...
  {\path{arXiv:1507.02648}}.

\bibitem[Lumsdaine and Warren 2015]{local-universes}
P.~L.~{Lumsdaine} and M.~A.~{Warren}, \emph{The local universes model:
...
M.~{Shulman}, \emph{Univalence for inverse diagrams and homotopy
  canonicity}, Mathematical Structures in Computer Science \textbf{25} (2015),
  Special issue 05, 1203--1277.

\bibitem[Szumi{\l}o 2014]{szumilo}
K.~{Szumi{\l}o}, \emph{{Two Models for the Homotopy Theory of Cocomplete
  Homotopy Theories}}, Ph.D. thesis, University of Bonn, (2014)
...
C.~{Kapulkin} and P.~L.~{Lumsdaine}, \emph{{The homotopy theory of type
  theories}}, unpublished, (2016), \href {http://arxiv.org/abs/1610.00037}
  {\path{arXiv:1610.00037}}.

\bibitem[Voevodsky 2014]{b-systems}
V.~{Voevodsky}, \emph{{B-systems}}, unpublished, (2014), \href
