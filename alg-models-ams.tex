...
Independently, Krzysztof Kapulkin and Peter LeFanu Lumsdaine constructed a semi-model structure on such category in \cite{kap-lum-model}.
...
Theory $Id$ is a theory with the following function symbols:
\begin{align*}
...
Theory $I$ is a theory with function symbols $I : (ty,n)$, $left : (tm,n)$, $right : (tm,n)$, and the following axioms:
\begin{center}
\AxiomC{$\Gamma \vdash$}
...
For example, we can consider theory $dc$ which has one function symbol
...
Let $HPath$ be a theory with function symbols $\idtype\ : (tm,n) \times (tm,n) \to (ty,n)$,
...
Now we can define theory $Fill^l_{tm}$.
...
We will consider theory $wUA$ under $coe_0$, which has additional symbol
...
Define $t : X \to P(X)$ as $t(a) = I \times a$ and $p_0,p_1 : P(X) \to X$ as $p_0(a) = left^*(a)$ and $p_1(a) = right^*(a)$.
...
Thus to prove that for every theorem $\varphi \sststile{}{V} \psi$ of $T'$, $h(\varphi) \land \bigwedge_{x \in L(V)} ctx^n(x) = I \sststile{}{L(V)} h(\psi)$
