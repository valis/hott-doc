...
\TrinaryInfC{$\Gamma \vdash Path(x.\, A, a, a')\ type$}
\DisplayProof
\end{center}
...
\AxiomC{$S$}
\AxiomC{$\Gamma \vdash j : I$}
\BinaryInfC{$\Gamma \vdash iso(A, B, x.\,f, y.\,g, x i.\,p, y i.\,q, j)\ type$}
\DisplayProof
\end{center}
...
For every model $M = (A,\alpha,\beta)$ of a theory $T$, we define a theory $Lang(M)$.
...
In particular, $M$ has a natural structure of a model of $Lang(M)$ defined as follows:
\begin{align*}
\alpha'(f)(O_a) & = a \\
