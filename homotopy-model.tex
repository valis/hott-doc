...
\item Отношение $\ll - \vdash - \red - \Downarrow - \red - \rr = -$ на классе $Ctx \times Term \times Term \times Term \times Term \times Sec$.
\end{itemize}
...
\item Если $\ll \Gamma \vdash A \red^* B \rr = \Gamma' \times I' \models H$, то $\Gamma \vdash A$, и $A \red^* B$.
\item Если $\ll \Gamma \vdash a \red b \Downarrow A \red B \rr = \Gamma' \times I' \models h : H$, то $\Gamma \vdash a \Downarrow A$, $a \red b$, и $A \red B$.
\end{enumerate}
\end{lem}
\begin{proof}
...
\medskip
\begin{center}
\AxiomC{$\Gamma \vdash p \Uparrow \Sigma (x : A) B$}
...
\AxiomC{$\ll \Gamma, x : A \vdash B \rr = A' \models B'$}
\AxiomC{$\ll \Gamma \vdash b \Downarrow B[x := a] \rr = \Gamma' \models b' : a'^*(B')$}
\TrinaryInfC{$\ll \Gamma \vdash (a, b)_{\Sigma (x : A) B} \Uparrow \Sigma (x : A) B \rr = \Gamma' \models pair(a', b') : \Sigma(A', B')$}
\DisplayProof
\end{center}

\medskip
\begin{center}
\AxiomC{$\ll \Gamma \vdash p \Uparrow \Sigma (x : A) B \rr = \Gamma' \models p' : \Sigma(A', B')$}
\AxiomC{$\ll \Gamma, x : A \vdash B \rr = A' \models B'$}
\BinaryInfC{$\ll \Gamma \vdash proj_1\ p \Uparrow A \rr = \Gamma' \models proj_1(p') : A'$}
\DisplayProof
\end{center}
...
\AxiomC{$\ll \Gamma \vdash p \Uparrow \Sigma (x : A) B \rr = \Gamma' \models p' : \Sigma(A', B')$}
\AxiomC{$\ll \Gamma, x : A \vdash B \rr = A' \models B'$}
\BinaryInfC{$\ll \Gamma \vdash proj_2\ p \Uparrow B[x := proj_1\ p] \rr = \Gamma' \models proj_2(p') : proj_1(p')^*(B')$}
\DisplayProof
\end{center}
...
\AxiomC{$\ll \Gamma, x : A \vdash B \rr = A' \models B'$}
\AxiomC{$\ll \Gamma \vdash b \Downarrow B[x := a] \rr = \Gamma' \models b' : a'^*(B')$}
\TrinaryInfC{$\ll \Gamma \vdash proj_1\ (a, b)_{\Sigma (x : A) B} \red a \Downarrow A \rr = \Gamma' \times I' \models proj_1.pair(a', b') : \pi_1^*(A')$}
\DisplayProof
\end{center}
...
\AxiomC{$\ll \Gamma, x : A \vdash B \rr = A' \models B'$}
\AxiomC{$\ll \Gamma \vdash b \Downarrow B[x := a] \rr = \Gamma' \models b' : a'^*(B')$}
\TrinaryInfC{$\ll \Gamma \vdash proj_2\ (a, b)_{\Sigma (x : A) B} \red b \Downarrow B[x := a] \rr = \Gamma' \times I' \models proj_2.pair(a', b') : \pi_1^* a'^*(B')$}
\DisplayProof
\end{center}
