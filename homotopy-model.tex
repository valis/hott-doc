...
\AxiomC{$\ll \Gamma \vdash A_i \red^* B \rr = h_i : \Gamma.A_i \to \Gamma.B$, $i \in \{1, 2\}$}
\RightLabel{, $B \in NF$}
\BinaryInfC{$\ll \Gamma \vdash a \Downarrow A_2 \rr = h_2^{-1} \circ h_1 \circ a : A_2 \dotsb \Gamma$}
\DisplayProof
\end{center}
\bigskip
...
\medskip
\begin{center}
\AxiomC{$\ll \Gamma \vdash A \red B \rr = h_1 : A \to B \dotsb \Gamma$}
\AxiomC{$\ll \Gamma \vdash B \red^* C \rr = h_2 : B \to C \dotsb \Gamma$}
\BinaryInfC{$\ll \Gamma \vdash A \red^* C \rr = h_2 \circ h_1 : A \to C \dotsb \Gamma$}
\DisplayProof
\end{center}
\bigskip
...
\medskip
\begin{center}
\AxiomC{$\ll \Gamma \vdash a \red a' : A \rr = h_1 : I \to A \dotsb \Gamma$}
\AxiomC{$\ll \Gamma \vdash a' \red^* a'' : A \rr = h_2 : I \to A \dotsb \Gamma$}
\BinaryInfC{$\ll \Gamma \vdash a \red^* a'' : A \rr = h_3 : I \to A \dotsb \Gamma$}
\DisplayProof
\end{center}
\bigskip
...
Пусть $\ll \Gamma \vdash B\rr = B \ldots \Gamma$ и $x \notin \Gamma$.
...
\begin{lem}[correctness][Corectness of interpretation]
Верны следующие утверждения:
\begin{itemize}
\item Если $\Gamma \vdash$, то существует $\Gamma'$ такой, что $\ll \Gamma \vdash \rr = \Gamma'$.
\item Если $\Gamma \vdash A$, то для любого $\Gamma'$ такого, что $\ll \Gamma \vdash \rr = \Gamma'$, существует $A'$ такой, что $\ll \Gamma \vdash A \rr = A' \dotsb \Gamma'$.
\item Если $\Gamma \vdash a : A$, то для любых $\Gamma', A'$ таких, что $\ll \Gamma \vdash A \rr = A' \dotsb \Gamma'$, существует $a'$ такой, что $\ll \Gamma \vdash a : A \rr = a' : A' \dotsb \Gamma'$.
\end{itemize}
\end{lem}
...
\begin{lem}[subst-corr][Корректность подстановки]
