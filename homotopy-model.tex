...
Если $\ll \Gamma, \Delta \vdash \rr$, то предыдущая лемма влечет, что существуют $A'_1 \dotsb \Gamma'$, \ldots $A'_n \dotsb \Gamma'.A'_1 \ldots A'_{n-1}$ такие, что $\ll \Gamma \vdash \Delta \rr = A'_n \ldots A'_1 \dotsb \Gamma'$.

\begin{lem}[correctness-subst][Correctness of the substitution]
Пусть $\ll \Gamma \vdash a \Downarrow A \rr = a' : A' \dotsb \Gamma$.
Тогда верны следующие утверждения:
\begin{enumerate}
\item Если $\ll \Gamma, x : A, \Delta \vdash \rr = \Gamma'.A'_1 \ldots A'_n$, то $\ll \Gamma, \Delta[x := a] \vdash \rr = \Gamma'.a^*(A'_1) \ldots (a.A'_1 \ldots A'_{n-1})^*(A'_n)$.
...
\item Если $\ll \Gamma, x : A, \Delta \vdash b \Uparrow B \rr = b' : B' \dotsb \Gamma'.A'.A'_1 \ldots A'_n$, то $\ll \Gamma, \Delta[x := a] \vdash b[x := a] \Downarrow B[x := a] \rr = (a.A'_1 \ldots A'_n)^*(b') : (a.A'_1 \ldots A'_n)^*(B') \dotsb \Gamma.a^*(A'_1) \ldots (a.A'_1 \ldots A'_{n-1})^*(A'_n)$.
\end{enumerate}
\end{lem}
