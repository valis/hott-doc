...
let $\{ P \sststile{}{} A\!\downarrow \}$ be the theory under $T_I$ with one predicate symbol $P : s_1 \times \ldots \times s_k$
and one function symbol $A : s_1 \times \ldots \times s_k \to (d_p,n)$ (where $d_{ty} = ctx$ and $d_{tm} = ty$) and one axiom:
\[ P(x_1, \ldots x_k) \sststile{}{x_1, \ldots x_k} A(x_1, \ldots x_k)\!\downarrow \]
If $a : s_1 \times \ldots \times s_k \to s$, then let $\{ P \sststile{}{} a\!\downarrow \}$ be the theory under with one predicate symbol $P$, one function symbol $a$, and one axiom:
\[ P(x_1, \ldots x_k) \sststile{}{x_1, \ldots x_k} a(x_1, \ldots x_k)\!\downarrow \]

Let $\I_f$ be the set of morphisms of the form $\{ P \sststile{}{} A\!\downarrow \} \to \{ P \sststile{}{} a\!\downarrow \}$ which send $P$ to $P$ and $A$ to $e_p(a)$.
Let $\I_p$ be the set of morphisms of the form $0 \to \{ P \}$, where $\{ P \}$ is a theory with one predicate symbol $P : s_1 \times \ldots \times s_k$ and no axioms.
The set of generating cofibrations $\I$ is the union of $\I_f$ and $\I_p$.

For every $i : U \to V \in \I$, we need to define a relative cylinder object $C_U(V)$.
First, let $C(\{ P \}) = \{ P \} \amalg \{ P \}$.
