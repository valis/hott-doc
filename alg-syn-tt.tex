...
The category $\algtt_0$ of algebraic dependent type theories is the under category $\mathbb{T}_0/\ThC$.
\end{defn}
...
The category $\algtt_1$ of algebraic dependent type theories with substitution is the under category $\mathbb{T}_1/\ThC$.
\end{defn}
...
\begin{prop}
Suppose that for every $t \in WTerm(V)_s$, $\sigma \in \mathcal{F}$, and $i$ such that the context in $(i+1)$-th argument of $\sigma$ can be omitted,
    the following sequents are theorems of $\mathbb{T}$:
\begin{align*}
& \sststile{}{V} t \cong \beta(t) \axtag{c1} \\
\sigma(\Gamma, x_1, \ldots x_k)\!\downarrow\ & \sststile{}{\Gamma, x_1, \ldots x_k} ctx_{p,n}(\sigma(\Gamma, x_1, \ldots x_k)) = \Gamma \axtag{c2} \\
\sigma(\Gamma, x_1, \ldots x_k)\!\downarrow\ & \sststile{}{\Gamma, x_1, \ldots x_k} ctx_{p_i,n_i}(x_i) = D_i(\sigma(\Gamma, x_1, \ldots x_{i-1})) \axtag{c3}
\end{align*}
...
\end{prop}
\begin{proof}
By \axref{c1}, we only need to show that such $r(t)$ exists for every $t \in Term(V)_s$.
...
If $i > 0$, then by \axref{c2}, $t$ is equivalent to $ctx^{i-1}(\Gamma)$, so we can take $r(t) = r(ctx^{i-1}(\Gamma))$.
So suppose that $i = 0$.
...
First, note that by \axref{c3}, $\sigma(\Gamma', t_1', \ldots t_k')\!\downarrow\ \sststile{}{V} t_i' = t_i''' \land \varphi_i'$ is derivable.
...
Then axiom~\axref{c3} follows from this sequent.
We will usually assume axioms \axref{c1} and \axref{c2} implicitly.
...
The following axioms (together with \axref{c1} and \axref{c2}) are equivalent to axioms \eqref{ax:def-subst}, \eqref{ax:type-subst-ty} and \eqref{ax:type-subst-tm}.
\begin{center}
...
In this section we will consider a particular kind of algebraic dependent type theories which we call \emph{regular}.
Also we will define the category $\algtt_{reg}$ of regular algebraic dependent type theories.
...
Let $\mathcal{F}$ be a set of function symbols.
Then we define another set of function symbols $reg(\mathcal{F})$ which consists of symbols of $\mathbb{T}_1$ together with symbols
    $\sigma_n : (ctx, n) \times (p_1, q_1 + n) \times \ldots \times (p_k, q_k + n) \to (p, q + n)$ for every
    $\sigma \in \mathcal{F}$, $\sigma : (p_1,q_1) \times \ldots \times (p_k,q_k) \to (p,q)$, and every $n \in \mathbb{N}$.
...
For every $t \in HTerm_{reg(\mathcal{F})}(V)_{(p,q)}$, $m \in \mathbb{N}$, and variable $\Gamma$ we define a term
    $L^m(\Gamma, t) \in HTerm_{reg(\mathcal{F})}(L^m(V \amalg \{ \Gamma : (ctx,0) \}))_{(p,q+m)}$ by induction on $t$:
\begin{align*}
& L^m(\Gamma, *) = * \\
...
\end{align*}
Note that sets $Term$, $WTerm$, $BTerm$, $LTerm$ and $STerm$ are closed under $L^m(\Gamma,-)$.

Let $\mathcal{P}$ be a set of relation symbols.
Then we define another set of relation symbols $reg(\mathcal{P})$ which consists of symbols
    $R_n : (p_1, q_1 + n) \times \ldots \times (p_k, q_k + n)$ for every
    $R \in \mathcal{P}$, $R : (p_1,q_1) \times \ldots \times (p_k,q_k)$, and every $n \in \mathbb{N}$.
Given a theory over $(WTerm_{reg(\mathcal{F})}, reg(\mathcal{P}))$, we will often omit subscripts $n$ in symbols $v_{n,i}$, $subst_{p,n,k}$, $\sigma_n$ and $R_n$.

For every formula $\varphi \in Form_{reg(\mathcal{P})}(V)$ we define a formula
    $L^m(\Gamma,\varphi) \in Form_{reg(\mathcal{P})} \allowbreak (L^m(V \amalg \{ \Gamma : (ctx,0) \}))$ as follows:
\begin{align*}
L^m(\Gamma, t = t') & = (L^m(\Gamma,t) = L^m(\Gamma,t')) \\
L^m(\Gamma, R_n(t_1, \ldots t_k)) & = R_{n+m}(L^m(\Gamma,t_1), \ldots L^m(\Gamma,t_k)) \\
L^m(\Gamma, \varphi_1 \land \ldots \land \varphi_n) & = L^m(\Gamma,\varphi_1) \land \ldots \land L^m(\Gamma,\varphi_n)
\end{align*}

\begin{defn}
We will say that a theory $\mathbb{T}$ over $(WTerm_{reg(\mathcal{F})}, reg(\mathcal{P}))$ is \emph{stable}
    if for every axiom (and hence for every theorem) $\varphi \sststile{}{V} \psi$ of $\mathbb{T}$ and every $m \in \mathbb{N}$,
    sequent $L^m(\Gamma,\varphi) \sststile{}{L^m(V \amalg \{ \Gamma : (ctx,0) \})} L^m(\Gamma,\psi)$ is a theorem of $\mathbb{T}$.
...
We will write $B[a_1, \ldots a_k]$ for $subst_{p,n,k}(*, B, a_1, \ldots a_k)$.

\begin{defn}
A \emph{regular algebraic dependent type theory} is a tuple $(\mathcal{F},\mathcal{P},\mathcal{A})$, where $\mathcal{A}$
    is a stable set of axioms over $(WTerm_{reg(\mathcal{F})}, reg(\mathcal{P}))$ such that axioms of $\mathbb{T}_1$ are derivable,
    and for every $\sigma \in \mathcal{F}$, $\sigma : (p_1,q_1) \times \ldots \times (p_m,q_m) \to (p,q)$, the following sequent is derivable:
\begin{equation}
\sststile{}{B', B, b_i, a_j} subst_{p,n,k}(B', \sigma_k(B, b_1, \ldots b_m), a_1, \ldots a_k) = \sigma_n(B', b_1', \ldots b_m') \axtag{r1}
\end{equation}
where $b_i' = subst_{p_i,n,k}^{q_i}(B', b_i, a_1, \ldots a_k)$.
\end{defn}
