...
\begin{align*}
x[\rho] & = \rho(x) \\
\sigma(t_1, \ldots t_k)[\rho] & = \sigma(t_1', \ldots t_k')|_{\varphi_1 \land \ldots \land \varphi_k} \text{, where } t'_i|_{\varphi_i} = t_i[\rho] \\
R(t_1, \ldots t_k)[\rho] & = R(t_1', \ldots t_k') \land \varphi_1 \land \ldots \land \varphi_k \text{, where } t'_i|_{\varphi_i} = t_i[\rho] \\
(\varphi_1 \land \ldots \land \varphi_n)[\rho] & = \varphi_1[\rho] \land \ldots \land \varphi_n[\rho] \\
...
If $(p,n) \in \mathcal{T}$, then let $d(p,n) = n$ if $p = ty$ and $d(p,n) = n+1$ if $p = tm$.
Let $V$ be a model of $\mathbb{T}_0$ and let $U(V)$ be its underlying $\mathcal{T}$-set.
Then a \emph{contextual term} of sort $s$ with free variables in $V$ is a pair $(\Gamma,t)$, where $\Gamma \in Ctx^{d(s)}_\mathcal{F}(U(V))$ and $t \in Term_\mathcal{F}(U(V))_s$.
The set of such terms will be denoted by $CTerm_\mathcal{F}(V)_s$.
Then $CTerm_\mathcal{F}(V)$ is a model of $\mathbb{T}_0$ in the obvious way, and $CTerm_\mathcal{F}$ is a functor $\Mod{\mathbb{T}_0} \to \Mod{\mathbb{T}_0}$.
Often $V \in \Mod{\mathbb{T}_0}$ will be a free model on some $\mathcal{T}$-set.
The left adjoint to the forgetful functor will be denoted by $F : \Set^\mathcal{T} \to \Mod{\mathbb{T}_0}$.

A \emph{contextual atomic formula} over $\Sigma$ with free variables in $V$ is an expression either of the form $t_1 = t_2$ where $t_1, t_2 \in CTerm_\Sigma(V)_s$ for some sort $s$
or of the form $R(t_1, \ldots t_n)$ where $R \in \mathcal{P}$, $R : s_1 \times \ldots \times s_n$ and $t_i \in CTerm_\Sigma(V)_{s_i}$.
Contextual Horn formulae are expressions of the form $\varphi_1 \land \ldots \land \varphi_n$ where $\varphi_i$ are contextual atomic formulae.
The set of contextual Horn formulae with free varibles in $V$ will be denoted by $CForm_\Sigma(V)$.
Sometimes we use contexts of length $n+1$ as contextual terms of sort $(ty,n)$ in the obvious way.
In particular, we can use formulae of the form $\Gamma = \Delta$ if $\Gamma$ and $\Delta$ are contexts of a positive length.
...
\begin{align*}
(\Gamma,t)|_\varphi = (\Gamma',t')|_{\varphi'} \text{ means } & (\Gamma,t) = (\Gamma',t') \land \varphi \land \varphi' \\
(\Gamma,t) \downarrow \text{ means } & (\Gamma,t) = (\Gamma,t) \\
(\Gamma,t)|_\varphi \downarrow \text{ means } & (\Gamma,t)\!\downarrow \land \varphi \\
...
In order to define a substituion operation on contextual terms, we need to introduce an additional structure on the set of function symbols.
First, if $(p,n) \in \mathcal{T}$, then let $d(p,n) = n$ if $p = ty$ and $d(p,n) = n+1$ if $p = tm$.
Now, for each $\sigma \in \mathcal{F}$, $\sigma : (p_1,n_1) \times \ldots \times (p_k,n_k) \to (p,n)$,
for each $A_i \in CTerm_\mathcal{F}(V)_{(ty,i)}$, $t_i \in CTerm_\mathcal{F}(V)_{(p_i,n_i)}$ and each $1 \leq i \leq k$,
let $D_i(A_1, \ldots A_{d(p,n)}, \sigma(t_1, \ldots t_k))$ be a context of length $d(p_i,n_i)$ with free variables in $V$.
The idea is that this context should be a context in which $t_i$ is typeable if $\sigma(t_1, \ldots t_k)$ is typeable in context $A_1, \ldots A_{d(p,n)}$.

For every $\rho : V \to PCTerm_\Sigma(V')$, substitution functions for contextual terms, contextual formulae and partial contextual terms are defined as follows:
\[ (A_1, \ldots A_n, x)[\rho] = (A_1', \ldots A_n', t)|_{\varphi_1 \land \ldots \land \varphi_n \land \psi \land (A_1', \ldots A_n') = \Delta } \]
where $A_i[\rho] = (\Gamma_i,A'_i)|_{\varphi_i}$ and $\rho(x) = (\Delta,t)|_\psi$.
\[ (A_1, \ldots A_n, \sigma(t_1, \ldots t_k))[\rho] = (A_1', \ldots A_n', \sigma(t_1', \ldots t_k'))|_{\varphi_1 \land \ldots \land \varphi_n \land \psi_1 \land \ldots \land \psi_k } \]
where $A_i[\rho] = (\Gamma_i,A'_i)|_{\varphi_i}$ and $t_i[\rho] = (\Delta_i,t_i')|_{\psi_i}$.
\[ R(t_1, \ldots t_k)[\rho] = R(t_1', \ldots t_k') \land \varphi_1 \land \ldots \land \varphi_k \]
where $t_i[\rho] = t_i'|_{\varphi_i}$.
\begin{align*}
(\varphi_1 \land \ldots \land \varphi_n)[\rho] & = \varphi_1[\rho] \land \ldots \land \varphi_n[\rho] \\
t|_\varphi[\rho] & = t[\rho]|_{\varphi[\rho]}
\end{align*}
If $x \in V_s$, $(A_1, \ldots A_{d(s)}, t) \in CTerm_\mathcal{F}(V)_s$ and $t' \in CTerm_\mathcal{F}(V)_{s'}$, then we write $t'[t/x]$ for contextual term $t'[\rho]$, where 
\[ \rho(y) = 
  \begin{cases}
      (A_1, \ldots A_{d(s)}, t) & \text{if } y = x \\
      (A_1, \ldots A_{n+1-i}) & \text{if } s = (ty,n) \text{ and } y = ft^i(x) \\
      (A_1, \ldots A_{n+1-i}) & \text{if } s = (tm,n) \text{ and } y = ft^i(ty(x)) \\
      y & \text{otherwise}
  \end{cases}
\]

Let $V$ be a model of $\mathbb{T}_0$ and $x$ an element of $V_s$ for some $s$.
The we define context $D(x)$ of length $d(s)$ as follows:
\[ D(x) = 
  \begin{cases}
      ft^n(x), \ldots ft(x) & \text{if } s = (ty,n) \\
      ft^n(ty(x)), \ldots ft(ty(x)) & \text{if } s = (tm,n)
  \end{cases}
\]

A \emph{contextual Horn sequent} consists of a $\mathcal{T}$-set $V$ and two contextual Horn formulae $\varphi$ and $\psi$ with free variables in $F(V)$.
Such sequent will be denoted by $\varphi \sststile{}{V} \psi$.
...
It has rules \axref{b1}-\axref{b6} as in partial Horn logic together with the following rules:
\medskip
\begin{center}
$\sststile{}{x} (D(x),x)\!\downarrow$ \axlabel{c1}
\qquad
$(D(x),x) = (D(y),y) \land \varphi \sststile{}{V} \varphi[(D(y),y)/x]$ \axlabel{c2}
\end{center}

\medskip
\begin{center}
\AxiomC{$\varphi \sststile{}{V} \psi$}
\RightLabel{, $x \in FV(\varphi)$, $x \in V_s$, $t \in CTerm_\mathcal{F}(V')_s$ \axlabel{c3}}
\UnaryInfC{$\varphi[t/x] \sststile{}{V'} \psi[t/x]$}
\DisplayProof
\end{center}
\medskip

\[ (D(x),x) = (D(y),y) \sststile{}{x : (p,n) ,y : (p,n)} D(x) = D(y) \text{, $n > 0$ \axlabel{c4}} \]

\subsection{The category of contextual Horn theories}

Here we will define categories of signatures (and contextual morphisms) and contextual Horn theories.

\begin{defn}
Let $\Sigma$ and $\Sigma'$ be a pair of signatures with $\mathcal{T}$ as the set of sorts.
A \emph{contextual morphism} of these signatures is a pair $(\alpha,\beta)$ where $\alpha$ assigns to every $V \in \Set^\mathcal{T}$
...
These morphisms must satisfy the following conditions:
\begin{enumerate}
\item For every $t \in PCTerm_\Sigma(V)_s$, $FV(t) \cap V \subseteq FV(\alpha_V(t))$.
\label{con-mor-sig-a-fv}
\item For every $\varphi \in CForm_\Sigma(V)$, $FV(\varphi) \cap V \subseteq FV(\beta_V(\varphi))$.
\label{con-mor-sig-b-fv}
\item For every $x \in V_s$, $\alpha_V((D(x),x)|_\top) = (D(x),x)|_\top$.
\label{con-mor-sig-a-var}
\item For every $t \in PCTerm_\Sigma(A)_s$ and $\rho : A \to CTerm_\Sigma(B)$, $\alpha_B(t[\rho]) = \alpha_A(t)[x \mapsto \alpha_B(\rho(x)|_\top)]$.
\label{con-mor-sig-a-subst}
\item For every atomic $\varphi \in CForm_\Sigma(A)$ and $\rho : A \to CTerm_\Sigma(B)$, $\beta_B(\varphi[\rho]) = \beta_A(\varphi)[x \mapsto \alpha_B(\rho(x)|_\top)]$.
\label{con-mor-sig-b-subst}
\item For every $t|_\varphi \in PCTerm_\Sigma(V)$, $\alpha_V(t|_\varphi) = \alpha_V(t|_\top)|_{\beta_V(\varphi)}$.
\label{con-mor-sig-a-op}
\item $\beta_V(\top) = \top$, $\beta_V(\varphi \land \psi) = \beta_V(\varphi) \land \beta_V(\psi)$, and $\beta_V(t = t') = (\alpha_V(t) = \alpha_V(t'))$.
\label{con-mor-sig-b-op}
\end{enumerate}
The identity and the composition of morphisms are defined in the obvious way.
The category of signatures and contextual morphisms is denoted by $\CSig$.
\end{defn}
...
\begin{itemize}
\item For every $\sigma \in \mathcal{F}$, $\sigma : s_1 \times \ldots \times s_k \to (ty,0)$,
    a partial term $\alpha(\sigma) \in PTerm_{\Sigma'}(UF(\{ x_1 : s_1, \ldots x_k : s_k \}))$ such that $\{ x_1, \ldots x_n \} \subseteq FV(\alpha(\sigma))$.
\item For every $\sigma \in \mathcal{F}$, $\sigma : s_1 \times \ldots \times s_k \to s$, $s \neq (ty,0)$
    a partial term $\alpha(\sigma) \in PTerm_{\Sigma'}(UF(\{ A : (ty,d(s)-1), x_1 : s_1, \ldots x_k : s_k \}))$ such that $\{ A, x_1, \ldots x_n \} \subseteq FV(\alpha(\sigma))$.
\item For every $R \in \mathcal{P}$, $R : s_1 \times \ldots \times s_k$,
    a formula $\beta(R) \in CForm_{\Sigma'}(F(\{ x_1 : s_1, \ldots x_k : s_k \}))$ such that $\{ x_1, \ldots x_k \} \subseteq FV(\beta(R))$.
\end{itemize}
Then there is a unique contextual morphism $f$ of these signatures such that \todo{Fix margins}
$f(ft^{d(s)-1}(A), \ldots A, \sigma(x_1, \ldots x_k)) = (ft^{d(s)-1}(A), \ldots A, \alpha(\sigma))$ and $f(R((D(x_1),x_1), \ldots (D(x_k),x_k))) = \beta(R)$.
\end{prop}
\begin{proof}
First, let us extend $\alpha$ to a morphism $CTerm_\mathcal{F}(V) \to PCTerm_{\Sigma'}(V)$.
By conditions \eqref{con-mor-sig-a-var} and \eqref{con-mor-sig-a-subst} it can be done in a unique way:
\begin{align*}
\alpha_V(x) & = x \\
\alpha_V(\sigma(a_1, \ldots a_n)) & = \alpha(\sigma)[x_i \mapsto \alpha_V(a_i)]
\end{align*}
Second, we extend $\beta$ to a morphism $Form_\Sigma(V) \to Form_{\Sigma'}(V)$.
By conditions \eqref{con-mor-sig-b-subst} and \eqref{con-mor-sig-b-op} it can be done in a unique way:
\begin{align*}
\beta_V(a = b) & = \alpha_V(a) = \alpha_V(b) \\
\beta_V(R(a_1, \ldots a_n)) & = \beta(R)[x_i \mapsto \alpha_V(a_i)] \\
\beta_V(\varphi_1 \land \ldots \land \varphi_n) & = \beta_V(\varphi_1) \land \ldots \land \beta_V(\varphi_n)
\end{align*}
Now, we can extend $\alpha_V$ to a morphism $PTerm_\Sigma(V) \to PTerm_{\Sigma'}(V)$.
By condition \eqref{con-mor-sig-a-op} it can be done in a unique way:
\[ \alpha_V(t|_\varphi) = \alpha_V(t)|_{\beta_V(\varphi)} \]

Conditions \eqref{con-mor-sig-a-fv} and \eqref{con-mor-sig-b-fv} follows from the conditions on $FV(\alpha(\sigma))$ and $FV(\beta(R))$.
Conditions \eqref{con-mor-sig-a-var}, \eqref{con-mor-sig-a-op} and $\eqref{con-mor-sig-b-op}$ hold by the definitions of $\alpha_V$ and $\beta_V$.
Conditions \eqref{con-mor-sig-a-subst} and $\eqref{con-mor-sig-b-subst}$ are easy to check by induction on the term and on the formula respectively.
\end{proof}
