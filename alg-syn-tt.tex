...
Let $\mathcal{S} = \{ Tm_n\ |\ n \in \mathbb{N} \} \cup \{ Ty_n\ |\ n \in \mathbb{N} \}$ be the set of sorts.
...
Moreover, we require that either $s = Tm_0$ or $s = Ty_0$.
Let $Var$ be a set of variables.
Then we define the sets of terms $Term$ and types $Type$ inductively:
\begin{itemize}
\item $v_i \in Term$ for every $i \in \mathbb{N}$.
\item If $k \in \mathbb{N}$, $f : s_1 \times \ldots \times s_n \to s$, $a_1, \ldots a_n \in Term \cup Type$, $a_i \in Term$ if $s_i = Tm_m$ for some $m$ and $a_i \in Type$ if $s_i = Ty_m$ for some $m$,
    then $f^k(a_1, \ldots a_n) \in Term$ if $s = Tm_0$ and $f^k(a_1, \ldots a_n) \in Type$ if $s = Ty_0$.
\item If $x \in Var$, $a_1, \ldots a_n \in Term$, then $x[a_1, \ldots a_n] \in Term$ and $x[a_1, \ldots a_n] \in Type$.
    If $n = 0$, then we write $x[]$ simply as $x$.
\end{itemize}
The set $Ctx$ of contexts is defined as the set of finite sequences of types.

We define the operation of substitution on terms and types.
If $t \in Term$ or $t \in Type$ and $a_1, \ldots a_n \in Term$, then we define $t[a_1, \ldots a_n]$ as follows:
\begin{itemize}
\item $v_i[a_1, \ldots a_n] = a_{n-i}$ if $i < n$.
\item $v_i[a_1, \ldots a_n] = v_{i-n}$ if $i \geq n$.
\item If $k \in \mathbb{N}$, $f : s_1 \times \ldots \times s_n \to s$, $a_1, \ldots a_n \in Term \cup Type$, $a_i \in Term$ if $s_i = Tm_m$ for some $m$ and $a_i \in Type$ if $s_i = Ty_m$ for some $m$,
    then $f^k(a_1, \ldots a_n) \in Term$ if $s = Tm_0$ and $f^k(a_1, \ldots a_n) \in Type$ if $s = Ty_0$.
\item If $x \in Var$, $a_1, \ldots a_n \in Term$, then $x[a_1, \ldots a_n] \in Term$ and $x[a_1, \ldots a_n] \in Type$.
    If $n = 0$, then we write $x[]$ simply as $x$.
...
where $\Gamma$ is a finite sequence of terms and $a,a',A,A'$ are terms.
An inference rule consists of finite set of judgements $J_1$, \ldots $J_n$ called premises and a judgement $J$ called conclusion.
...
\end{defn}
Let $F$ be a function that to each $f \in \Sigma_T$ of arity $(n_1, \ldots n_k)$ assigns a term in $Term_S$ with free variables $x_1$, \ldots $x_k$.
Then $F$ can be extended to a function $\widehat{F} : Term_T \to Term_S$ in the obvious way.
If $J$ is a judgement of $T$ then we can define a judgement $\widehat{F}(J)$ of $S$ by applying $\widehat{F}$ to every term in $J$.
If $R$ is an inference rule of $T$, then we can define an inference rule $\widehat{F}(R)$ of $S$ by applying $\widehat{F}$ to every judgement in $R$.

A morphism of theories $T = (\Sigma_T, \mathcal{I}_T)$ and $S = (\Sigma_S, \mathcal{I}_S)$ is a function $F$ as described above such that
$\widehat{F}(R)$ is a derived inference rules of $S$ for every $R \in \mathcal{I}_T$.
Identity morphism $id_T$ is defined as $id_T(f) = f(x_1, \ldots x_k)$.
Compositions of morphisms $F : T \to S$ and $G : S \to P$ is defined as follows: $(G \circ F)(f) = \widehat{G}(F(f))$.
