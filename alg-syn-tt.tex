...
Given a monad $T : \Set^\mathcal{S} \to \Set^\mathcal{S}$ and a left $T$-module $F : \Set^\mathcal{S} \to \Set$ with some additional structure on them, we define a monad $P : \Set^\mathcal{S} \to \Set^\mathcal{S}$.
...
Let $(T : \Set^\mathcal{S} \to \Set^\mathcal{S}, \eta, (-)^*)$ be a monad and let $(F : \Set^\mathcal{S} \to \Set, (-)^\circ)$ be a left $T$-module.
Let $FV_V$ be a function that to each element $\bigcup{s \in \mathcal{S}} T(V)_s \cup F(V)$ assigns a finite subset of $V$.
We say that $FV_V(t)$ is a set of free variables of $t$.
Suppose this function satisfies the following condition:
\begin{align*}
& \text{For every $a \in A$, $FV_A(\eta_A(a)) = \{ a \}$} \\
& \text{For every $\rho : A \to T(B)$ and $t \in T(A)_s$, $FV_B(\rho^*(t)) = \bigcup_{a \in FV_A(t)} FV(\rho(a))$} \\
& \text{For every $\rho : A \to T(B)$ and $\varphi \in F(A)$, $FV_B(\rho^\circ(\varphi)) = \bigcup_{a \in FV_A(\varphi)} FV(\rho(a))$}
\end{align*}
Then we define a monad $P : \Set^\mathcal{S} \to \Set^\mathcal{S}$ as follows:
\[ P(V)_s = \{\ t|_\Phi\ |\ t \in T(V)_s \text{, $\Phi$ is a finite set of elements of $F(V$})\ \} \]
First, for every $t|_\Phi \in P(V)_s$, let $\pi_1(t|_\Phi) = t$ and $\pi_2(t|_\Phi) = \Phi$.
Second, for every $t|_\Phi \in P(V)_s$ we can define the set of its free variables $FV_V(t|_\Phi)$ as $FV_V(t) \cup \bigcup_{\varphi \in \Phi} FV_V(\varphi)$.
For every $A \in \Set^\mathcal{S}$ we define $\theta_A : A \to P(A)$ as $\theta_A(x) = \eta_A(x)|_\varnothing$.
For every $\rho : A \to P(B)$ we define $\rho^+ : P(A) \to P(B)$ as follows:
\[ \rho^+(t|_\Phi) = (\pi_1 \circ \rho)^*(t)|_{\{ (\pi_1 \circ \rho)^\circ(\varphi) | \varphi \in \Phi \} \cup \bigcup_{a \in FV_A(t|_\Phi)} \pi_2(\rho(a))} \]
...
\begin{align*}
\theta_A^+(t|_\Phi) & = \\
(\pi_1 \circ \theta_A)^*(t)|_{\{ (\pi_1 \circ \theta_A)^\circ(\varphi) | \varphi \in \Phi \} \cup \bigcup_{a \in FV_A(t|_\Phi)} \pi_2(\theta_A(a))} & = \\
\eta_A^*(t)|_{\{ \eta_A^\circ(\varphi) | \varphi \in \Phi \}} & = \\
t|_\Phi
\end{align*}
\item For every $\rho : A \to P(B)$, $\rho^+ \circ \theta_A = \rho$:
\begin{align*}
\rho^+(\theta_A(x)) & = \\
(\pi_1 \circ \rho)^*(\eta_A(x))|_{\bigcup_{a \in FV_A(\eta_A(x))} \pi_2(\rho(a))} & = \\
\pi_1(\rho(x))|_{\pi_2(\rho(x))} & = \\
\rho(x)
\end{align*}
\item First, note that $\pi_1 \circ \sigma^+ = (\pi_1 \circ \sigma)^* \circ \pi_1$.
Now, we can prove that for every $\rho : A \to P(B)$, $\sigma : B \to P(C)$, $\sigma^+ \circ \rho^+ = (\sigma^+ \circ \rho)^+$:
\begin{align*}
\sigma^+(\rho^+(t|_\Phi)) & = \\
\sigma^+((\pi_1 \circ \rho)^*(t)|_{\{ (\pi_1 \circ \rho)^\circ(\varphi) | \varphi \in \Phi \} \cup \bigcup_{a \in FV_A(t|_\Phi)} \pi_2(\rho(a))}) & = \\
(\pi_1 \circ \sigma)^*((\pi_1 \circ \rho)^*(t))|_{\{ (\pi_1 \circ \sigma)^\circ(\varphi) | \varphi \in \Psi \} \cup \bigcup_{a \in FV_A((\pi_1 \circ \rho)^*(t)|_\Psi)} \pi_2(\sigma(a))} & = \\
(\pi_1 \circ \sigma^+ \circ \rho)^*(t)|_{\{ (\pi_1 \circ \sigma^+ \circ \rho)^\circ(\varphi) | \varphi \in \Phi \} \cup \bigcup_{a \in FV_A(t|_\Phi)} \pi_2(\sigma^+(\rho(a)))} & = \\
(\sigma^+ \circ \rho)^+(t|_\Phi)
\end{align*}
where $\Psi = \{ (\pi_1 \circ \rho)^\circ(\varphi) | \varphi \in \Phi \} \cup \bigcup_{a \in FV_A(t|_\Phi)} \pi_2(\rho(a))$.
\end{itemize}
\end{proof}
