...
\begin{align}
\setcounterref{equation}{ax:type-subst}
...
Function $subst_{n,k}(B, a, a_1, \ldots a_k)$ is defined whenever \[ Hom_{n,k}(B, ft_k(ty_k(a)), a_1, \ldots a_k) \] holds.
...
If $WTerm$ be a submonad of $HTerm$, then we can define Horn formulae, partial terms, inference rules, partial Horn theories, and the category of partial Horn theories
    in the same way as we did in \ref{sec:PHT}, but with $WTerm$ in place of $Term$.
We want to prove that if $WTerm$ satisfies some conditions, then the category of partial Horn theories over $Term$ and over $WTerm$ are equivalent.
...
\begin{itemize}
\item $H(\circ,t)$ for every $t \in Term(V)$.
