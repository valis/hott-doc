...
By proposition~2.3 of \cite{c-systems} it is enough to construct a map $s_{f'} : B \to A[f]$ for each $f' = (a_1, \ldots a_k, a_{k+1}) : B '\to A$ and prove a few properties of $s_{f'}$.
Since $s_{f'}$ must be a section of $p_B$, first $n$ components of $s_{f'} = (b_1, \ldots b_{n+1})$ must be equal to identity on $B$, that is $(v_{n,n-1}(B), \ldots v_{n,0}(B))$.
The last component $b_{n+1}$ of $s_{f'}$ must satisfy equation $ty_n(b_{n+1}) = A[f][b_1, \ldots b_n] = A[f][id_B] = A[f]$.
We define $b_{n+1}$ to be equal to $a_{k+1}$.
Axioms \eqref{ax:var-subst} and \eqref{ax:subst-subst} implies that $q(f, B) \circ s_{f'} = f$.

If $g : ft_k(A) \to ft_m(C)$ and $A = C[g]$, then we need to show that $s_{f'} = s_{q(g,C) \circ f'}$.
The first $n$ components of both morphisms is identity on $B$.
The last component of $q(g,C) \circ f'$ equals to $v_{n+1,0}(C[g])[f'] = a_{k+1}$.
Thus the last components of these morphisms also coincide.
This completes the proof that the square above is cartesian.

We are left to prove that operation $A[f]$ is functorial.
