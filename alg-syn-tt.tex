...
We say that an inference rule defines a symbol $(f : s_1 \times \ldots s_n \to (p,q)) \in \Sigma$ if its conclusion is
either of the form $\Gamma \vdash f(x_1, \ldots x_n)$ or of the form $\Gamma \vdash f(x_1, \ldots x_n) : A$ where $x_i$ are variables.
We say that such inference rule is \emph{regular} if the length of $\Gamma$ is $q$ and
if all variables in the list $x_1$, \ldots $x_n$ are distinct and this list contains all free variables of the rule.
A syntactic type theory $(\Sigma, \mathcal{I})$ is \emph{regular} if every function symbol in $\Sigma$ is
defined by a single regular inference rule in $\mathcal{I}$ and every inference rule in $\mathcal{I}$ is either of the form $a \deq b$ or defines some function symbol.
...
Function symbols of $E(T)$ are
\[ f_k : Ctx_k \times (p_1,q_1+k) \times \ldots \times (p_n,q_n+k) \to (p,q+k) \text{ for each } k \in \mathbb{N} \text{ and } (f : (p_1,q_1) \times \ldots \times (p_n,q_n) \to (p,q)) \in \Sigma \]
and $v_{n,i}$, $subst_{n,k}$ and $Subst_{n,k}$ with the signatures as in $T_1$.
