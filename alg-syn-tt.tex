...
Let $\algtt^0_{con}$ be the full subcategory of $\algtt^0$ which consists of theories of the form $((\mathcal{C}, \mathcal{F}_0 \amalg \mathcal{F}, \mathcal{P}), \mathcal{A})$
    such that $\mathcal{F}$ satisfies \rcond{ctx}, axioms of $\mathbb{T}_0$ and the following sequent are derivable:
\begin{equation}
...
\medskip
\begin{center}
\AxiomC{$(A_1, \ldots A_k, C_1, \ldots C_q \vdash \sigma_k(b_1, \ldots b_m))\!\downarrow$}
\AxiomC{$\Gamma \vdash ctx$}
\AxiomC{$\Gamma \vdash a_i : A_i'$}
\RightLabel{\quad\axlabel{r1}}
\TrinaryInfC{$\Gamma, C_1', \ldots C_q' \vdash subst^q(\sigma_k(b_1, \ldots b_m), a_1, \ldots a_k) \equiv \sigma_n(b_1', \ldots b_m')$}
\DisplayProof
\end{center}
where $A_i' = subst(A_i, a_1, \ldots a_{i-1})$, $C_i' = subst^{i-1}(C_i, a_1, \ldots a_k) $, and $b_i' = subst^{q+q_i} \allowbreak (b_i, a_1, \ldots a_k)$.
The full subcategory of $\algtt^1_{st}$ on regular theories will be denoted by $\algtt^1_{reg}$.
\end{defn}

\todo{Stabilization, regularization, colimits, examples}
