...
\begin{lem}
If $Term$ is the standard monad over $\mathcal{F}$, then categories of partial algebras over $Term$
...
\item For every partial function $f : V \to A$, $\beta_V(f)(\varphi \land \psi) = \top$ if and only if both $\beta_V(f)(\varphi)$ and $\beta_V(f)(\psi)$ equal to $\top$.
\end{itemize}
...
\begin{lem}
If $Term$ is the standard monad over $\mathcal{F}$ and $Form$ is the left module of Horn formulae over $\mathcal{P}$,
    then categories of partial algebras over $(Term,Form)$ and partial structures for signature $(\mathcal{S},\mathcal{F},\mathcal{P})$ are isomorphic.
\end{lem}
\begin{proof}
\todo{Write the proof}
\end{proof}
...
\begin{lem}
If $Term$ is the standard monad over $\mathcal{F}$ and $\mathbb{T} = (Term,\mathcal{P},\mathcal{A})$ is a partial Horn theory,
