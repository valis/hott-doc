...
This proposition is still not easy to use, so we prove another lemma which is useful for proving that a map is a monomorphism.
If $\mathbb{T} = ((\mathcal{S},\mathcal{F},\mathcal{P}),\mathcal{A})$ is a theory, then
let $\mathcal{S}(\mathbb{T})$ be the subset of $\mathcal{S}$ which consists of those sorts which are used in $\mathbb{T}$.
That is, $\mathcal{S}(\mathbb{T})$ is the smallest subset of $\mathcal{S}$ such that $(\mathcal{S}(\mathbb{T}),\mathcal{F},\mathcal{P})$ is a signature and $\mathcal{A}$ is a theory in it.

\begin{lem}
Let $\mathbb{T} = ((\mathcal{S},\mathcal{F},\mathcal{P}),\mathcal{A})$ be a theory
and let $\mathbb{T}' = ((\mathcal{S}, \mathcal{F} \amalg \{ c_s : s\ |\ s \in \mathcal{S} \}, \mathcal{P}), \mathcal{A} \cup \{ \sststile{}{} c_s\!\downarrow |\ s \in \mathcal{S} \} \cup \{ \sststile{}{x : s, y : s} x = y\ |\ s \in \mathcal{S} \setminus \mathcal{S}(\mathbb{T}) \})$ be its extension.
Then the natural map $\mathbb{T} \to \mathbb{T}'$ is a monomorphism.
\end{lem}
\begin{proof}
If $\varphi$ is a Horn formula in the signature of $\mathbb{T}'$, then we define a Horn formula $T(\varphi)$ in the signature of $\mathbb{T}$.
If $\varphi$ is atomic, then $T(\varphi) = \varphi[x_s/c_s]$, if $\varphi$ is a formula over
$(\mathcal{S}(\mathbb{T}), \mathcal{F} \amalg \{ c_s : s\ |\ s \in \mathcal{S}(\mathbb{T}) \}, \mathcal{P})$,
where $x_s$ is a variable of sort $s$ that is not a free variable of $\varphi$.
Otherwise $T(\varphi) = \top$.
For nonatomic formulae $T(-)$ is defined as follows:
\[ T(\varphi_1 \land \ldots \land \varphi_n) = T(\varphi_1) \land \ldots \land T(\varphi_n) \]
If $S$ is a sequent $\varphi \sststile{}{V} \psi$, then let $T(S)$ be the sequent $T(\varphi) \sststile{}{V \cup FV(T(\varphi)) \cup FV(T(\psi))} T(\psi)$.
Note that if $S$ is an axioms of $\mathbb{T}$, then $T(S) = S$.
Thus we only need to prove that if $S$ is a theorem of $\mathbb{T}'$, then $T(S)$ is a theorem of $\mathbb{T}$.

If $S$ is an axiom of $\mathbb{T}'$, then we need to consider the following cases:
\begin{itemize}
\item If $S = \varphi \sststile{}{V} \psi$ belongs to $\mathcal{A}$, then $T(\varphi) = \varphi$ and $T(\psi)$ is a sublist of $\psi$.
    Hence, $T(S)$ is a theorem of $\mathbb{T}$.
\item If $S = \sststile{}{} c_s\!\downarrow$, then $T(S) = \sststile{}{} x_s\!\downarrow$ which is an axiom of the logic.
\item If $S = \sststile{}{x : s, y : s} x = y$, then $T(S) = \sststile{}{x : s, y : s} \top$ which is an axiom of the logic.
\end{itemize}
Now, we need to prove that $T(-)$ preserves inference rules of the logic.
It is clearly preserves rules (b1)-(b6) and (a1).
\end{proof}
...
\begin{lem}
Let $\mathbb{T}' = ((\mathcal{S}',\mathcal{F}',\mathcal{P}'),\mathbb{T}')$ be a disjoint subtheory of $\mathbb{T} \in \Th_\mathcal{S}$.
Let 
If for every axiom $\varphi \sststile{}{V} \psi$ of $\mathbb{T}$ sequent $\mathbb{T}'(\varphi) \sststile{}{\mathbb{T}'(V)} \mathbb{T}'(\psi)$ is a theorem of $\mathbb{T}'$, then this is true for every theorem of $\mathbb{T}$.
\end{lem}
\begin{proof}
...
hence sequent $\mathbb{T}'(x = y \land \varphi) \sststile{}{x:s,y:s,V} \mathbb{T}'(\varphi[y/x])$ equals to $\top \sststile{}{\mathbb{T}'(V)} \top$ which is clearly derivable.
If $s \in \mathcal{S}'$, then this sequent equals either to $x = y \land \varphi \sststile{}{x:s,y:s,\mathbb{T}'(V)} \varphi[y/x]$ or to $x = y \sststile{}{x:s,y:s,\mathbb{T}'(V)} \top$, both of which are derivable.
