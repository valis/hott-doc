...
(A_1, \ldots A_n) \equiv (B_1, \ldots B_n) & \text{ means } \bigwedge\limits_{1 \leq i \leq n} A_1, \ldots A_{i-1} \vdash A_i \equiv B_i \\
\Gamma \vdash & \text{ means } \Gamma \equiv \Gamma
\end{align*}
...
To define analogs of other axioms we will need an auxiliary construction.
For every contextual term $t$ and contextual formula $\varphi$ we define their sets of free variables with contexts $CFV(t)$ and $CFV(\varphi)$.
These sets consist of pairs $(\Gamma,x)$, where $x$ is a variable of a sort $s$ and $\Gamma$ is a context of length $d(s)$.
\begin{align*}
CFV(\Gamma,x) & = \{ (\Gamma,x) \} \\
CFV(\Gamma, \sigma(t_1, \ldots t_k)) & = \bigcup_{1 \leq i \leq k} CFV(D_i(\Gamma,\sigma(t_1, \ldots t_k)), t_i) \\
CFV(\varphi_1 \land \ldots \land \varphi_k) & = \bigcup_{1 \leq i \leq k} CFV(\varphi_i) \\
CFV(R(t_1, \ldots t_k)) & = CFV(D_i(R(t_1, \ldots t_k)), t_i)
\end{align*}

Now, we can define analogs of axioms (a1)-(a2).
Note that since (c1) is much weaker than (a1) we need additional axioms (c4) and (c5).
\begin{align*}
& \sststile{}{A} E^0_{ty}(A,A) & \text{(c1)} \\
R(t_1, \ldots t_k) & \sststile{}{V} E^{n_i}_{p_i}(D_i(R(t_1, \ldots t_k)), t_i, t_i) & \text{(c4)} \\
E^n_p(\Gamma, \sigma(t_1, \ldots t_k), \sigma(t_1, \ldots t_k)) & \sststile{}{V} E^{n_i}_{p_i}(D_i(\Gamma, \sigma(t_1, \ldots t_k)), t_i, t_i) & \text{(c5)} \\
\bigwedge\limits_{(\Gamma,x) \in CFV(\varphi)} E^n_p(\Gamma, x, y) \land \varphi & \sststile{}{V} \varphi[y/x] & \text{(c2)}
\end{align*}
Finally, an analog of (a3) looks like this:
\begin{center}
\AxiomC{$\varphi \sststile{}{V} \psi$}
\RightLabel{, $x \in FV(\varphi)$ \!\qquad (c3)}
\UnaryInfC{$\bigwedge\limits_{(\Gamma',x) \in CFV(\varphi)} \Gamma[t/x] \equiv \Gamma'[t/x] \land \varphi[t/x] \sststile{}{V'} \psi[t/x]$}
\DisplayProof
\end{center}
where $(\Gamma,x) \in CFV(\varphi)$.
...
\bigwedge\limits_{(\Gamma,x) \in CFV(\varphi)} E^n_p(\Gamma, x, y) \land \varphi & \sststile{}{V} \varphi[y/x] & \text{(c2)} \\
\AxiomC{$\varphi \sststile{}{V} \psi$}
\RightLabel{, $x \in FV(\varphi)$}
\UnaryInfC{$\bigwedge\limits_{(\Gamma',x) \in CFV(\varphi)} \Gamma \equiv \Gamma' \land \varphi[t/x] \sststile{}{V'} \psi[t/x]$}
\DisplayProof
\!\!\kern-16em
& & \text{(c3)} \\
R(t_1, \ldots t_k) & \sststile{}{V} E^{n_i}_{p_i}(D_i(R(t_1, \ldots t_k)), t_i, t_i) & \text{(c4)} \\
...
If we let $R = E^n_p$, then (c4) implies that for every $\Gamma \in Ctx^n_\mathcal{F}(V)$, $a,a' \in Term_\mathcal{F}(V)_{(tm,n)}$ and $A,A' \in Term_\mathcal{F}(V)_{(ty,n)}$ the following sequents are derivable:
\begin{align*}
...
If we let $\varphi = E^n_{tm}(\Gamma, A, a, a')$, then (c2) implies well-known inference rules of type theory:
...
then for every $(\Gamma,x) \in CFV(\varphi)$ the following sequent is derivable:
