...
If $V \in \Set^\mathcal{S}$, then we write $\mathbb{T}'(V) \in \Set^{\mathcal{S}'}$ for the subset of $V$ which consists of variables of sorts from $\mathcal{S}'$.
If $\varphi$ is a Horn formula over $(\mathcal{S},\mathcal{F},\mathcal{P})$, then we define a formula $\mathbb{T}'(\varphi)$ over $(\mathcal{S}',\mathcal{F}',\mathcal{P}')$.
For atomic formulae it is defined as follows:
\begin{align*}
\mathbb{T}'(\varphi) & = \varphi \text{ if $\varphi$ is a formula over $(\mathcal{S}',\mathcal{F}',\mathcal{P}')$,} \\
\mathbb{T}'(\varphi) & = \top \text{ otherwise.}
\end{align*}
For other Horn formulae $\mathbb{T}'$ is defined as follows:
\[ \mathbb{T}'(\varphi_1 \land \ldots \land \varphi_n) = \mathbb{T}'(\varphi_1) \land \ldots \land \mathbb{T}'(\varphi_n) \]
We will say that a subtheory $\mathbb{T}'$ of $\mathbb{T}$ is \emph{disjoint} if for every $\sigma : s_1 \times \ldots \times s_n \to s$ such that $s \in \mathcal{S}'$, $\sigma \in \mathcal{F}'$.
