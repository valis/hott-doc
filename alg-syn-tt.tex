...
Now, we define a category $\Th_\mathcal{S}$ of standard partial Horn theories.
...
\begin{prop}
The inclusion $\Th_\mathcal{S} \to \Th^T_\mathcal{S}$ is an equivalence of categories.
\end{prop}
\begin{proof}
We only need to prove that this inclusion is essentially surjective on objects.
Let $(T,\mathcal{P},\mathcal{A})$ be a partial Horn theory.
Then we define the set of function symbols with signature $s_1 \times \to \times s_n \to s$ as $T(\{ x_1 : s_1, \ldots x_n : s_n \})_s$.
The set of all function symbols will be denoted by $\mathcal{F}$.
The set of relation symbols is the same, and the set of axioms $\mathcal{A}'$ consists of two kinds of axioms.
First, since $T$ is finitary, for every axiom from $\mathcal{A}$ there exists a finite set $V'$
    such that every term $t \in T(V)_s$ that appears in this axiom belongs to $T(V')_s$.
Thus we can replace every term in an axiom with a function symbol from $\mathcal{F}$.
This defines an axiom over $(Term_\mathcal{F},Form_\mathcal{P})$.
Second, for every $t \in T(\{ x_1 : s_1, \ldots x_n : s_n \})_s$ and every $t_1, \ldots t_n$ such that $t_i \in T(V)_{s_i}$,
    we have an axiom $\sststile{}{V} t(t_1, \ldots t_n) \cong t[x_i \mapsto t_i]$.

Now, we need to prove that monadic presentations $P(T,\mathcal{P},\mathcal{A})$ and $P(Term_\mathcal{F},\mathcal{P},\mathcal{A}')$ are isomorphic.
\end{proof}
