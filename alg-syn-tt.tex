...
\section{Modules over monads}

It is well-known that $\mathcal{S}$-sorted algebraic theories can be presented as finitary monads on $\Set^\mathcal{S}$.
We will show that essentially algebraic theories can be presented as finitary modules over finitary monads.
In this section we will review the theory of monads and modules over them.
One of the aims of this paper is to define a category of essentially algebraic theories (in the form of partial Horn theories).
We will do this in the next section using the category of modules over monads, which we define here.
...
\subsection{A monadic presentation of essentially algebraic theories}

There are several equivalent ways of defining essentially algebraic theories.
Here we will introduce yet another one.
We will show that it is equivalent to other definitions in the next section.
...
\begin{itemize}
\item $\land$ is commutative and idempotent.
\item For every $\rho : V \to T(V')$, $\top[\rho] = \top$.
\item For every $\rho : V \to T(V')$, $(\varphi \land \psi)[\rho] = \varphi[\rho] \land \psi[\rho]$.
\end{itemize}

Now, for every monad $T$ on $\Set^\mathcal{S}$ we define a left module $E$ with values in $\Set$.
...
\emph{A module of formulae with equality} over $T$ is a module $F$ of formulae over $T$ together with a morphism $E \to F$.

Now, we introduce a concept of ``partial terms''.
If an ordinary term represents a function, then a partial term represents a function with its domain restricted to a subset defined by a formula.
\emph{A monad of partial terms} is a triple $(P,F,\mu)$, where $P$ is a monad on $\Set^\mathcal{S}$,
    $F$ is a module of formulae with equality over $P$, and $\mu_{V,s} : P(V)_s \times F(V) \to P(V)_s$, satisfying the conditions below.
If $t \in P(V)_s$ and $\varphi \in F(V)$, then we write $t|_\varphi$ for $\mu_{V,s}(t,\varphi)$.
Triple $(P,F,\mu)$ must satsfy the following conditions:
\begin{itemize}
\item $t|_\top = t$.
\item $t|_{\varphi \land \psi} = t|_\varphi|_\psi$.
\item For every $\rho : V \to P(V')$, $(t|_\varphi)[\rho] = t[\rho]|_{\varphi[\rho]}$.
\item If $FV(V) = \{ x_1, \ldots x_n \}$, then for every $\rho : V \to P(V')$ and $\varphi_1$, \ldots $\varphi_n$,
    $t[x_i \mapsto \rho(x_i)|_{\varphi_i}] = t[\rho]|_{\varphi_1 \land \ldots \land \varphi_n}$.
\item If $FV(V) = \{ x_1, \ldots x_n \}$, then for every $\rho : V \to P(V')$ and $\varphi_1$, \ldots $\varphi_n$,
    $\varphi[x_i \mapsto \rho(x_i)|_{\varphi_i}] = \varphi[\rho] \land \varphi_1 \land \ldots \land \varphi_n$.
\end{itemize}
A monad of partial terms is \emph{finitary} if both $P$ and $F$ are finitary.
A morphism of monads of partial terms $(P,F,\mu)$ and $(P',F',\mu')$ is a morphism $(\alpha,\beta)$ of left modules $F$ and $F"$,
    such that $\alpha(t|_\varphi) = \alpha(t)|_{\beta(\varphi)}$.

Now, we construct a left adjoint to the forgetful functor $U : \PMnd \to \LMod$ from the category of monads of partial terms to the category of left modules.
...
In this section we will review necessary for our development parts of the theory of partial horn theories.
We will show that every partial Horn theory defines a monad of partial terms and that every monad of partial terms is obtained in this way from some partial Horn theory.
We will see that the category of models of a partial Horn theory is equivalent to the category of algebras of the corresponding monad of partial terms.
This shows that monads of partial terms give us an alternative equivalent way of working with essentially algebraic theories.
% Also this allows us to define a category of partial Horn theories which is equivalent to the category of monads of partial terms.
% We will also define algebraic type theories as certain partial horn theories.

\subsection{The monad of partial terms of a partial Horn theory}
