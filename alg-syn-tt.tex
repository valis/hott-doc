...
Let $\mathcal{F}_0$ be a set of function symbols of $\mathbb{T}_0$, and let $\mathcal{F}$ be a set of fucntion symbols such that
...
\item If $\sigma \in \mathcal{F}$, $\sigma : (ctx,n) \times (p_1,n_1) \times \ldots \times (p_k,n_k) \to (p,n)$, $\Gamma \in BTerm(V)_{(ctx,n)}$,
    $\Gamma$ has a well-defined context, and $t_i \in BTerm(V)_{(p_i,n_i)}$, then $\sigma(\Gamma, t_1, \ldots t_k) \in BTerm(V)_{(p,n)}$.
\item If $\sigma \in \mathcal{F}$, $\sigma : (ctx,n) \times (p_1,n_1) \times \ldots \times (p_k,n_k) \to (p,n)$, $t_i \in BTerm(V)_{(p_i,n_i)}$,
    then $\sigma(*, t_1, \ldots t_k) \in BTerm(V)_{(p,n)}$.
\end{itemize}
...
To define a function $\beta : WTerm(V)_s \to Term(V)_s$, we need an additional structure.
Suppose that for every function symbol $\sigma : (ctx,n) \times (p_1,n_1) \times \ldots \times (p_k,n_k) \to (p,n)$
    ...
    there is a term $D_i(\sigma(\Gamma, x_1, \ldots x_{i-1})) \in WTerm(\{ \Gamma, x_1, \ldots x_{i-1} \})_{(ctx_i,n_i)}$.
...
First, let $STerm$ be a subfunctor of $BTerm$ which consists of terms in which every subterm is
    either of the form $ctx^i(x)$, where $x$ is a variable, or of the form $ty(t)$, or of the form $\sigma(*, t_1, \ldots t_k)$.
...
\end{itemize}
Note that $LTerm$ is a subfunctor of $WTerm$, while $STerm$ is not.

\begin{prop}
Suppose that the following sequents are theorems of $\mathbb{T}$:
\begin{align*}
...
Let $t_i'' = t_i'$ and $\varphi_i' = \top$ if $t_i'$ is a term in $STerm$.
...
Note that $\sigma(\Gamma', t_1'', \ldots t_k'')$ is equivalent to $\sigma(\Gamma', t_1''', \ldots t_k''')$,
    where $t_i''' = t_i'$ if $t_i'$ is a term in $STerm$, $t_i''' = \tau(\Delta_i''', q_1, \ldots q_n)$ if $t_i' = \tau(\Delta_i, q_1, \ldots q_n)$,
    and $t_i''' = ty(\tau(\Delta_i''', q_1, \ldots q_n))$ if $t_i' = ty(\tau(\Delta_i, q_1, \ldots q_n))$,
...
\end{align*}
Finally, we will write a sequent $\varphi_1 \land \ldots \land \varphi_n \sststile{}{V} \varphi$ as
\begin{center}
\AxiomC{$\varphi_1$}
\AxiomC{$\ldots$}
\AxiomC{$\varphi_n$}
...
We define auxiliary derived operations in a theory under $\mathbb{T}_1$.
For every $m,n \in \mathbb{N}$ and $p \in \{ ty, tm \}$, we define $wk^m_{p,n} : (ctx,n+m) \times (p,n) \to (p,n+m)$ as follows:
\[ wk^m_{p,n}(B,a) = subst_{p,n+m,n}(B, a, v_{n+m-1}, \ldots v_m) \]
