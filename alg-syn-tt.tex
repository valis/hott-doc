...
If \todo{Replace with PTerm?} $\rho : V \to Term_\Sigma(V')$ is a (partial) function, then
\begin{align*}
...
\begin{comment}
The following lemma allows us to prove properties of $F(\Gamma,t)$ by induction on $t$.

\begin{lem}[F-char]
The following sequents are derivable:
\begin{itemize}
\item $\sststile{}{V} F(\Gamma,x) \cong x|_{e(x) = F(\Gamma)}$.
\item $\sststile{}{V} F(\Gamma,\sigma(t_1, \ldots t_k)) \cong \sigma(F(\Gamma), t_1', \ldots t_k')$,
    where for each $1 \leq i \leq k$, $t_i' = F(D_i(\Gamma, \sigma(t_1, \ldots t_k)), t_i)$.
\end{itemize}
\end{lem}
\begin{proof}
\end{proof}
...
Let us denote $\Gamma[(\Gamma',t')/x]$ by $\Gamma''$.
...
If $t \neq x$, then $F'(\Gamma,t)[F(\Gamma',t')/x] = t|_{e(t) = F(\Gamma)[F(\Gamma',t')/x]}$ and
    $\sststile{}{V} F((\Gamma,t)[(\Gamma',t')/x]) \cong t|_{e(t) = F(\Gamma'')}$.
Thus the assertion holds by induction hypothesis.

If $t = x$, then on the one hand
\[ F((\Gamma,x)[(\Gamma',t')/x]) = F((\Gamma'',t')|_{\Gamma'' = \Gamma'}) = F(\Gamma'',t')|_{F(\Gamma'' = \Gamma')}. \]
On the other hand,
\[ F'(\Gamma,x)[F(\Gamma',t')/x] = F(\Gamma',t')|_{e(F(\Gamma',t')) = F(\Gamma)[F(\Gamma',t')/x]}. \]
Hence $\sststile{}{V} F'(\Gamma,x)[F(\Gamma',t')/x] \cong F(\Gamma',t')|_{F(\Gamma') = F(\Gamma)[F(\Gamma',t')/x]}$.
By induction hypothesis $\sststile{}{V} F'(\Gamma,x)[F(\Gamma',t')/x] \cong F(\Gamma',t')|_{F(\Gamma'') = F(\Gamma')}$.
To prove that partial terms $F(\Gamma'',t')|_{F(\Gamma'' = \Gamma')}$ and $F(\Gamma',t')|_{F(\Gamma'') = F(\Gamma')}$ are equivalent,
    we need to show that $F(\Gamma'' = \Gamma')$ and $F(\Gamma'') = F(\Gamma')$ are equivalent and
    that $F(\Gamma'',t')\!\downarrow \land F(\Gamma'') = F(\Gamma') \sststile{}{V} F(\Gamma'',t') = F(\Gamma',t')$.
First, note that by \rlem{F-e}, $F(A_1, \ldots A_n) = F(A_1', \ldots A_n') \sststile{}{V} F(A_1, \ldots A_i) = F(A_1', \ldots A_i')$ for every $i \leq n$.
...
On the one hand,
\[ F'(\Gamma,t)[F(\Gamma',t')/x] = \sigma(F(\Gamma)[F(\Gamma',t')/x], t_1', \ldots t_k'), \]
    where $t_i' = F'(D_i(\Gamma, \sigma(y_1, \ldots y_k)), t_i)[F(\Gamma',t')/x][y_1 \mapsto t_1', \ldots y_{i-1} \mapsto t_{i-1}']$.
On the other hand,
\[ F((\Gamma,t)[(\Gamma',t')/x]) = F(\Gamma'',t[(\Gamma',t')/x]), \]
By \rlem{F-basic}~\eqref{F-F'}, \rlem{F-e} and induction hypothesis,
\[ \sststile{}{V} F(\Gamma'',t[(\Gamma',t')/x]) \cong \sigma(F(\Gamma''), t_1'', \ldots t_k''), \]
    where $t_i'' = F'(D_i(\Gamma, \sigma(y_1, \ldots y_k)), t_i[(\Gamma',t')/x])[y_1 \mapsto t_1'', \ldots y_{i-1} \mapsto t_{i-1}'']$.
By induction hypothesis, sequents $\sststile{}{V} t_i' \cong t_i''$ are derivable.
Thus we proved that sequents $\sststile{}{V} F'(\Gamma,t)[F(\Gamma',t')/x] \cong F((\Gamma,t)[(\Gamma',t')/x])$ are derivable.

Now, we can finish the proof of the proposition.
We do this by induction on the length of $\Gamma$.
By \rlem{F-basic}~\eqref{F-F'}, we have $F(A_1, \ldots A_n, t) = F'(x_1, \ldots x_n, t)[x_1 \mapsto F(A_1)] \ldots [x_n \mapsto F(A_1, \ldots A_n)]$.
Hence $\sststile{}{V} F(A_1, \ldots A_n, t)[F(\Gamma',t')/x] \cong F(x_1, \ldots x_n, t[(\Gamma',t')/x])[x_1 \mapsto F(A_1[(\Gamma',t')/x])] \ldots [x_n \mapsto F(A_1[(\Gamma',t')/x], \ldots A_n[(\Gamma',t')/x])]$.
For every $x_1$, \ldots $x_n$ and $t$, we have $\sststile{}{V} F(x_1, \ldots x_n, t) \cong F'(x_1, \ldots x_n, t)|_{ty(x_n) = x_{n-1} \land \ldots \land ty(x_2) = x_1}$.
Thus $\sststile{}{V} F(A_1, \ldots A_n, t)[F(\Gamma',t')/x] \cong F(x_1, \ldots A_n[(\Gamma',t')/x], t[(\Gamma',t')/x]) \ldots [x_1 \mapsto F'(A_1[(\Gamma',t')/x])]$.
\end{proof}
