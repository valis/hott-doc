...
Let $F$ be a function that to each $R \in \mathcal{P}$, $R : s_1 \times \ldots \times s_n$ assigns a Horn formula $\psi$ over $\Sigma'$ with free variables in $\{ x_1 : s_1, \ldots x_n : s_n \}$
and to each $\sigma \in \mathcal{F}$, $\sigma : s_1 \times \ldots \times s_n \to s$ assigns a term $t$
in $Term_{\mathcal{F}'}(V)_s$ and a Horn formula $\varphi$ over $\Sigma'$ with free variables in $V$ where $V = \{ x_1 : s_1, \ldots x_n : s_n \}$.
We will write such functions $F$ as $F(R(x_1, \ldots x_n)) = \psi$ and $F(\sigma(x_1, \ldots x_n)) = t|_\varphi$.
If $\varphi = \top$, then we will write $F(\sigma(x_1, \ldots x_n)) = t$.
Given such a function $F$ we can extend it to the whole set of terms $Term_\mathcal{F}(V)_s$:
\begin{itemize}
\item $F(x) = x$ if $x \in V_s$.
...
\begin{itemize}
\item If $\varphi$ is of the form $t_1 = t_2$ and $F(t_i) = t_i'|_{\psi_i}$, then let $F(\varphi)$ be equal to $t_1' = t_2' \land \psi_1 \land \psi_2$.
\item If $\varphi$ is of the form $R(x_1, \ldots x_n)$, then $F(\varphi)$ is already defined.
...
\item For every $\sigma \in \mathcal{F}_\Sigma$, $\sigma : s_1 \times \ldots \times s_n \to s$ if $F(\sigma(x_1, \ldots x_n)) = t|_\varphi$ and $G(\sigma(x_1, \ldots x_n)) = s|_\psi$,
    then sequents $t\!\downarrow \land \varphi \sststile{}{x_1, \ldots x_n} t = s \land \psi$ and \linebreak $s\!\downarrow \land \psi \sststile{}{x_1, \ldots x_n} t = s \land \varphi$ are theorems of $\mathbb{T}'$.
\end{itemize}
Note that if $F$ and $G$ are equivalent then $\sststile{}{V} F(t) \cong G(t)$ is a theorem of $\mathbb{T}'$ for each $t \in Term_\Sigma(V)_s$.
