...
\begin{itemize}
\item $x[\rho] = \rho(x)$.
\item $\sigma(a_1, \ldots a_k)[\rho] = \sigma(a_1[\rho], \ldots a_k[\rho])$.
\end{itemize}
...
If $\varphi$ is a formula with free variables in $V$ and $\rho : V \to Term_\mathcal{F}(V')$, then we will write $\varphi[\rho]$ for a formula defined as follows:
\begin{itemize}
\item $(t = s)[\rho] = (t[\rho] = s[\rho])$.
\item $R(a_1, \ldots a_k)[\rho] = R(a_1[\rho], \ldots a_k[\rho])$.
\item $(\varphi_1 \land \ldots \land \varphi_n)[\rho] = \varphi_1[\rho] \land \ldots \land \varphi_n[\rho]$.
\end{itemize}
...
For every $t|_\phi \in PTerm_\Sigma(A)_s$, let $term(t|_\phi) = t$ and $dom(t|_\phi) = \phi$.
For every $\rho : A \to PTerm_\Sigma(B)$ we define $t_\phi[\rho] \in PTerm_\Sigma(B)_s$ as follows:
\[ t|_\phi[\rho] = t[term \circ \rho]|_{\varphi[term \circ \rho] \land \bigwedge_{a \in FV(t|_\phi)} dom(\rho(a))} \]
