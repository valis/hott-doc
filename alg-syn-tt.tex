...
We will say that morphisms $(\alpha,\beta)$ and $(\alpha',\beta')$ are \emph{equivalent} if the following conditions hold:
\begin{itemize}
...
\end{itemize}
Note that if $F$ and $G$ are equivalent, then $\sststile{}{V} F(t) \cong G(t)$ are theorems of $\mathbb{T}'$ for each $t \in PTerm_\Sigma(V)_s$.
...
\begin{prop}
The data above describe a category.
\end{prop}
\begin{proof}
TODO
\end{proof}
