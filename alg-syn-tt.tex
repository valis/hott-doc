...
Now, for every set of variables $V$ we define a set $L(V)$ which contains a variable $x$ of sort $(p,q+1)$ for every variable $x$ of sort $(p,q)$ in $V$.
For every $t \in Term_{reg(\mathcal{F})}(V)_{(p,q)}$ we define a term $L(t) \in Term_{reg(\mathcal{F})}(L(V \amalg \{ \Gamma : (ctx,1) \}))_{(p,q+1)}$ by induction on $t$:
\begin{align*}
& L(*) = \Gamma \\
& L(x_i) = x_i \\
...
We define auxiliary derived operations in the signature of $T_1$.
...
such that for every $\sigma \in \mathcal{F}$, $\sigma : (p_1,q_1) \times \ldots \times (p_m,q_m) \to (p,0)$ the following sequents are derivable.
