...
For each $A \in M(Ctx_n)$, $B \in M(Ctx_k)$ morphisms from $A$ to $B$ are tuples $(a_1, \ldots a_k)$ such that $a_i \in M(Tm_n)$ and $M(Hom_{n,k})(A, B, a_1, \ldots a_k)$.
...
\[ (M(v_{n,n-1})(A), \ldots M(v_{n,0})(A)) \]
and $p_A : A \to ft(A)$ as tuple
\[ (M(v_{n,n-1})(A), \ldots M(v_{n,1})(A)). \]

Now, we introduce some notation.
If $B \in M(Ctx_n)$, $A \in M(Ctx_{k+1})$, and $f = (a_1, \ldots a_k) : B \to M(ft_k)(A)$ is a morphism, then we define $A[f] \in M(Ctx_{n+1})$ as $Subst_{n,k}(B, A, a_1, \ldots a_k)$.
If $a \in M(Tm_k)$ and $M(ty_k)(a) = A$, then we define $a[f] \in M(Tm_n)$ as $subst_{n,k}(B, a, a_1, \ldots a_k)$.
...
\begin{align*}
M(ty_n)(c_i[f]) & = \text{(by axiom~\eqref{ax:type-subst})} \\
M(ty_k)(c_i)[f] & = \text{(since $Hom_{k,m}(c_1, \ldots c_m)$)} \\
M(ft^{m-i}_i)(C)[c_1, \ldots c_{i-1}][f] & = \text{(by axiom~\eqref{ax:Subst-Subst})} \\
M(ft^{m-i}_i)(C)[c_1[f], \ldots c_{i-1}[f]] &
\end{align*}
...
M(ty_n)(a[f]) & = A[f] \\ \notag
\text{ for each } f : B \to M(ft_k)(A) & \text{ where } A = M(ty_k)(a) \\
A[id_{M(ft_n)(A)}] & = A \\
a[id_{M(ft_n)(M(ty_n)(a))}] & = a \\
\setcounter{equation}{\ref{ax:Subst-Subst}}
\addtocounter{equation}{-1}
A[g][f] & = A[g \circ f] \\ \notag
\text{ for each } f : C \to B \text{ and } & g : B \to M(ft_m)(A) \\
a[g][f] & = a[g \circ f] \\ \notag
\text{ for each } f : C \to B \text{ and } & g : B \to M(ft_m)(M(ty_m)(a))
\end{align}
...
If $A \in M(Ctx_{k+1})$, $B \in M(Ctx_n)$, and $f = (a_1, \ldots a_k) : B \to ft_k(A)$, then we define $f^*(B)$ as $A[f] = M(Subst_{n,k})(B, A, a_1, \ldots a_k)$.
Map $q(f,B)$ defined as the tuple with $i$-th component equals to
\[ \left\{
  \begin{array}{lr}
    a_i[M(v_{n+1,n})(f^*(B)), \ldots M(v_{n+1,1})(f^*(B))] & \text{ if } 1 \leq i \leq k \\
    M(v_{n+1,0})(f^*(B))                                  & \text{ if } i = k+1
  \end{array}
\right. \]
If $(a_1, \ldots a_{k+1}) : B \to A$ is a morphism, then axiom~\eqref{ax:var-subst} implies that $p_A \circ (a_1, \ldots a_{k+1})$ equals to $(a_1, \ldots a_k)$.
It follows that the following square commutes.
\[ \xymatrix{ f^*(B) \ar[r]^-{q(f,B)} \ar[d]_{p_B} & A \ar[d]^{p_A} \\
              B \ar[r]_-f                          & ft_k(A)
            } \]
Now we need to show that this square is cartesian.
By proposition~2.3 of \cite{c-systems} it is enough to construct a map $s_{f'} : B \to f^*(B)$ for each $f' = (a_1, \ldots a_k, a_{k+1}) : B '\to A$ and prove a few properties of $s_{f'}$.
Since $s_{f'}$ must be a section of $p_B$, first $n$ components of $s_{f'} = (b_1, \ldots b_{n+1})$ must be equal to identity on $B$, that is $(M(v_{n,n-1})(B), \ldots M(v_{n,0})(B))$.
The last component $b_{n+1}$ of $s_{f'}$ must satisfy equation $M(ty_n)(b_{n+1}) = f^*(B)[b_1, \ldots b_n] = f^*(B)[id_B] = f^*(B)$.
We define $b_{n+1}$ to be equal to $a_{k+1}$.
