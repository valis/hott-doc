...
If $B$ and $a$ are terms of sorts $Ctx_{n+1}$ and $Tm_n$ respectively, then we write $wk(B,a)$ for $subst_{n+1,n}(B, a, v_n(B), \ldots v_1(B))$.
If $A$ is a terms of sort $Ctx_{k+2}$, $B$ is a term of sort $Ctx_{n+1}$, and $a_1$, \ldots $a_k$ are terms of sort $Tm_n$, then we write $Subst^w_{n,k}(B, A, a_1, \ldots a_k)$ for
\[ Subst_{n+1,k+1}(B, A, wk(B, a_1), \ldots wk(B, a_k), v_0(B)). \]

If $B$ and $a$ are terms of sorts $Ctx_{n+2}$ and $Tm_n$ respectively, then we write $B[a]$ for
\[ Subst_{n,n+1}(ft^2(B), B, v_{n-1}(ft^2(B)), \ldots v_0(ft^2(B)), a). \]
If $b$ and $a$ are terms of sorts $Tm_{n+1}$ and $Tm_n$ respectively, then we write $b[a]$ for
\[ subst_{n,n+1}(ft^2(ty(b)), b, v_{n-1}(ft^2(ty(b))), \ldots v_0(ft^2(ty(b))), a). \]
...
& \top\text{-}elim(Subst^w_{n,k}(\top(B), D, a_1, \ldots a_k)), subst_{n,k}(B, d, a_1, \ldots a_k), subst_{n,k}(B, t, a_1, \ldots a_k))
\end{align*}
