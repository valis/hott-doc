...
A \emph{partial Horn theory} in a signature $\Sigma$ is a set of Horn sequents over $\Sigma$.
...
\sigma(t_1, \ldots t_k)\!\downarrow & \sststile{}{V} t_i = t_i \axtag{a5}
\end{align*}
...
\section{Algebraic type theories}
\label{sec:T1}

In this section we will define algebraic type theories and theories with substitution, describe two partial Horn theories $\mathbb{T}_0$, $\mathbb{T}_1$
    in terms of which algebraic type theories are defined, and prove that the category of models of $\mathbb{T}_1$ is equivalent to the category of contextual categories.
...
\begin{defn}
An \emph{algebraic type theory} is a theory $\mathbb{T} \in \ThC$ together with a morphism $\mathbb{T}_0 \to \mathbb{T}$.
The category of algebraic type theories is the under category $\mathbb{T}_0/\ThC$.
\end{defn}
...
\begin{defn}
An \emph{algebraic type theory with substitution} is a theory $\mathbb{T} \in \ThC$ together with a morphism $\mathbb{T}_1 \to \mathbb{T}$.
The category of algebraic type theories with substitution is the under category $\mathbb{T}_1/\ThC$.
\end{defn}
...
Let $u : \mathbb{T}_1 \to \mathbb{T}$ be an algebraic type theory with substitution.
...
In this section we show how to construct monadic presentations of a partial Horn theory which are isomorphic to the one defined in \rdefn{mon-pres} (which we call the standard presentation), but has less redundant information.
...
Let $WTerm$ be a submonad of $HTerm$.
...
\begin{center}
$\sststile{}{V} h_1 \cong h_2$ \axlabel{h1}
\end{center}
for every $h_1,h_2 \in WTerm(V)_s$ such that there exists a term $t \in Term(V)_s$ such that $H(h_1,t)$ and $H(h_2,t)$.

We want to prove that if $WTerm$ satisfies some conditions, then the categories of partial Horn theories over $Term$ and over $WTerm$ are equivalent.
Since they are both defined in terms of monadic presentations, we only need to prove that their essential images in the category of monadic presentations coincide.
Let $\mathbb{T}$ be a theory over $WTerm$.
If $h \in WTerm(V)_s$ and $t \in Term(V)_s$, then we will say that $t$ \emph{represents} $h$
    if $FV(t) \subseteq FV(h)$, $H(h,t)$, and for every $t' \in Term(V)_s$, if $H(h,t')$, then $t'\!\downarrow\ \sststile{}{V} t = t'$.

\begin{prop}
Suppose that every term with holes in $WTerm(V)_s$ represented by some term in $Term(V)_s$,
    and every term in $Term(V)_s$ represents some term with holes in $WTerm(V)_s$.
Then the categories of partial Horn theories over $Term$ and over $WTerm$ are equivalent.
\end{prop}
\begin{proof}
Note that if $t_1$ and $t_2$ represent some term with holes $h$, then $\sststile{}{V} t_1$
Let $\alpha : Term(V)_s \to WTerm(V)_s$ and $\beta : WTerm(V)_s \to Term(V)_s$ be any functions such that $t$ represents $f(t)$ and $g(h)$ represents $h$.
\end{proof}
