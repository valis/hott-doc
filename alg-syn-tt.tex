...
\AxiomC{$\Gamma \vdash ctx$}
\AxiomC{$\Gamma \vdash a_i : subst(A_i, a_1, \ldots a_{i-1})$}
\TrinaryInfC{$\Gamma \vdash subst(\sigma_k(b_1, \ldots b_m), a_1, \ldots a_{k+q}) \equiv \sigma_n(b_1', \ldots b_m')$}
\DisplayProof
\end{center}
...
Let $st^0(\mathbb{T}) = (\coprod\limits_{n \in \mathbb{N}} L^n(\mathbb{T}), \alpha)$, where $\alpha$ is the following composition:
    $L(\coprod\limits_{n \in \mathbb{N}} L^n(\mathbb{T})) = \coprod\limits_{n \in \mathbb{N}} L(L^n(\mathbb{T})) \hookrightarrow \coprod\limits_{n \in \mathbb{N}} L^n(\mathbb{T})$.
...
For every $(\mathbb{T}_a,\alpha) \in \algtt^0_{st}$, we define a left adjoint $st^0_{(\mathbb{T}_a,\alpha)} : \mathbb{T}_a/\algtt^0_{con} \to (\mathbb{T}_a,\alpha)/\algtt^0_{st}$
...
\todo{Stabilization, regularization, examples}
