...
First, let $STerm$ be a submonad of $BTerm$ which consists of terms in which every subterm is either a variable, or of the form $ty(t)$, or of the form $\sigma(*, t_1, \ldots t_k)$.
...
\end{align*}
Then for every $t \in WTerm(V)_s$, there is a partial term $\gamma(t) \in PLTerm(V)_s$ such that $\sststile{}{V} t \cong \gamma(t)$ is a theorem of $\mathbb{T}$.
\end{prop}
\begin{proof}
\todo{Write the proof}
\end{proof}
...
We already saw that we can work exclusively with $STerm$, but when applying axiom~\axref{a3} term $t$ which we sabstitute in formulae should be from $WTerm$.
Now, we will show that if the premise satisfies some conditions, then we can apply \axref{a3} to a term from $STerm$.
