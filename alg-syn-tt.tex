...
\begin{abstract}
% В этой статье мы предложим абстрактное определение теорий типов, которое обобщает существующие теории.
% Мы определим категорию теорий типов, что позволит композицировать их различным образом и рассматривать различного рода эквивалентности между теориями.
% Кроме того мы изучим модели таких теорий в общем случае и начальные в частности.
\end{abstract}
...
% Мартин-Лёф определил свою теорию типов тогда-то [ссылка].
% Даже у самого Мартин-Лёфа было несколько вариантов определения \cite{MLTT72,MLTT73,MLTT79}.
% Потом появилось еще несколько вариантов \cite{luo94,CoC}.
% С тех пор много чего было сделано на тему.
% Различные вариации и расширения теории были предложены [ссылки: calculus of construction, Luo, ? inductive data types].
% Но проблема в том, что нет никакого общего определения того, что такое теория типов.
% В этой статье мы предложим вариант такого определения, использующий (существенно) алгебраические теории.
% \begin{enumerate}
% \item Позволяет отделить синтаксические детали от существенных.
% \item Позволяет изучать конструкции отдельно друг от друга.
% \item Позволяет определить эквивалентность теорий.
% \item Позволяет изучать модели, в частности начальные.
% \end{enumerate}

% В частности модельки народ конструировал [насовать ссылок на Штрайхера, Питтса, и других].
% Конструировать модели сказочно сложно, а так как теории не композицируются, то выходит, что нужно повторять эти конструкции из раза в раз.
% Одна из проблем, возникающая при этом, -- это так называемая проблема когерентности.
% Существует несколько подоходов к ее решению: [ссылки на Хоффмана, Ввоеводского, локальные вселенные]
% Проблема в том, что отдельные конструкции нужно обрабатывать отдельно.

% Существуют различные синтаксические вопросы, связанные с определением теорий.
% Например, сколько нужно аргументов пихать конструкторам.
% [Пример с лямбдой и аппликацией, ссылка на Штрайхера]
% Или, например, вопрос с тем как определять judgemental equality.
% Алгебраический подход избавляет нас от этих вопросов.

% Другая проблема заключается в том, что различные конструкции можно описывать по разному.
% Даже такая простая штука как $\Sigma$-типы допускает различные описания (см. примеры X и Y).
% Соответственно, возникает вопрос когда два различных описания задают эквивалентные теории.
% Вопрос может осложняться тем, что в одном варианте пара термов равны judgementally, а в другом только пропозиционально.
% We define a category of theories which opens a posibility of considering different kinds of equivalences between them.
% We won't discuss this problem in this paper, оставим ее на будущее.

% В этой статье, используя формализм существенно алгебраических теорий, мы вводим понятие алгебраических теорий типов, который предоставляют вариант решения этих проблем.
% Копроизведения, и в более общем случае, копределы позволяют комбинировать отдельные простые теории в более сложные.
% Например, теория типов с $\Sigma$, $\Pi$ и $Id$ типами может быть описана как $T_\Sigma \amalg T_\Pi \amalg T_{Id}$,
% где $T_\Sigma$, $T_\Pi$, $T_{Id}$ -- это теории $\Sigma$, $\Pi$ и $Id$ типов соответственно.
% Теория со вселенными может быть описана при помощи пушаута (см. пример Z).

% Существует естественное понятие модели существенно алгебраической теории.
% Мы покажем, что модели алгебраических теорий типов дают хорошо известное понятие.
% Конретно, модель любой алгебраической теории типов $T$ является контекстуальной категорией с некоторой дополнительной структурой, зависящей от $T$.

% Мы определим категорию алгебраических теорий типов, что дает естественное понятие эквивалентности теорий, конкретно изоморфизм.
% В большинстве случаев это слишком сильное понятие эквивалентности, которое требуется ослабить.
% Мы оставим эти вопросы на будущее.
% В некоторых случаях изоморфизма всё же будет достаточно.
% Формализм алгебраических теорий типов лишен проблем с синтаксисом, описаных ранее.
% Judgemental equality в них можно вводить только одним образом,
% а вопрос с количеством аргументов хоть и допускает вариации, получившиеся теории всё равно будут изоморфны.

% Статья организована следующим образом ...
% Algebraic and parameterized theories
...
A morphism of these signatures is a pair $(\alpha,\beta)$ where $\alpha$ assigns to every $V \in \Set^\mathcal{S}$ a morphism $\alpha_V : PTerm_\Sigma(V) \to PTerm_{\Sigma'}(V)$
and $\beta$ assigns to every $V \in \Set^\mathcal{S}$ a morphism $Form_\Sigma(V) \to Form_{\Sigma'}(V)$.
...
\end{itemize}
Then there is a unique morphism $(\alpha,\beta)$ of these signatures such that $\alpha(\sigma(x_1, \ldots x_n)) = \alpha(\sigma)$ and $\beta(R(x_1, \ldots x_n)) = \beta(R)$.
\end{prop}
\begin{proof}
...
We say that a theory $\mathbb{T}$ is \emph{total} if for every function symbols $\sigma : s_1 \times \ldots \times s_n \to s$
sequent $\sststile{}{x_1, \ldots x_n} \sigma(x_1, \ldots x_n)\!\downarrow$ is a theorem of $\mathbb{T}$.
...
Let $(\alpha,\beta)$ be a morphism of $\Sigma$ and $\Sigma'$.
Let $\mathbb{T}$ and $\mathbb{T}'$ be Horn theories in signatures $\Sigma$ and $\Sigma'$ respectively.
Then we will say that $(\alpha,\beta)$ \emph{respects} a sequent $\varphi \sststile{}{V} \psi$ over $\Sigma$ if $\beta_V(\varphi) \sststile{}{V} \beta_V(\psi)$ is a theorem of $\mathbb{T}'$.
Note that if $(\alpha,\beta)$ respects all axioms of $\mathbb{T}$, then it respects all of its theorems too.

Let $T$ and $T'$ be theories over $\Sigma$ and $\Sigma'$ respectively.
Then we will say that morphisms $(\alpha,\beta)$ and $(\alpha',\beta')$ between $T$ and $T'$ are \emph{equivalent} if the following conditions hold:
\begin{enumerate}
\item For every $t \in PTerm_\Sigma(V)_s$ sequents $\sststile{}{V} \alpha_V(t) \cong \alpha'_V(t)$ are theorems of $T'$.
\label{mor-eq-a}
\item For every $\varphi \in Form_\Sigma(V)$ sequent $\beta_V(\varphi) \ssststile{}{V} \beta'_V(\varphi)$ is a theorem of $T'$.
\label{mor-eq-b}
\end{enumerate}

\begin{prop}[mor-eq]
Let $T = (\mathcal{F},\mathcal{P},\mathbb{T})$ and $T'$ be theories.
Morphisms $(\alpha,\beta)$ and $(\alpha',\beta')$ between $T$ and $T'$ are equivalent if and only if the following conditions hold:
\begin{itemize}
\item For every $\sigma \in \mathcal{F}$, $\sigma : s_1 \times \ldots \times s_n \to s$ sequents
    \[ \sststile{}{U} \alpha_U(\sigma(x_1, \ldots x_n)) \cong \alpha'_U(\sigma(x_1, \ldots x_n)) \]
are theorems of $T'$, where $U = \{ x_1 : s_1, \ldots x_n : s_n \}$.
\item For every $R \in \mathcal{P}$, $R : s_1 \times \ldots \times s_n$ sequent
    \[ \beta_U(R(x_1, \ldots x_n)) \ssststile{}{U} \beta'_U(R(x_1, \ldots x_n)) \]
is a theorem of $T'$, where $U = \{ x_1 : s_1, \ldots x_n : s_n \}$.
\end{itemize}
\end{prop}
\begin{proof}
...
If $t$ is a variable, then $\alpha_V(t) = \alpha'_V(t) = t$.
If $t = \sigma(t_1, \ldots t_n)$, then $\sststile{}{V} \alpha_V(t_i) \cong \alpha_V(t'_i)$
and $\sststile{}{U} \alpha_U(\sigma(x_1, \ldots x_n)) \cong \alpha'_U(\sigma(x_1, \ldots x_n))$ where $U = \{ x_1 : s_1, \ldots x_n : s_n \}$.
Since $FV(\alpha_U(\sigma(x_1, \ldots x_n))) = \{ x_1, \ldots x_n\}$, by (a3) we have
$\alpha(t)\!\downarrow\,\sststile{}{V} \alpha(t) = \alpha'_U(\sigma(x_1, \ldots x_n))[x_i \mapsto \alpha_V(t_i)]$.
We also have $\alpha_V(t)\!\downarrow\,\sststile{}{V} \alpha_V(t_i)\!\downarrow$, hence by \rlem{cong-a}
$\alpha(t)\!\downarrow\,\sststile{}{V} \alpha'_U(\sigma(x_1, \ldots x_n))[x_i \mapsto \alpha_V(t_i)] = \alpha'_V(t)$.
By transitivity, we conclude that $\alpha_V(t)\!\downarrow\,\sststile{}{V} \alpha_V(t) = \alpha'_V(t)$.
The same argument shows that $\alpha'_V(t)\!\downarrow\,\sststile{}{V} \alpha_V(t) = \alpha'_V(t)$.

Let us prove \eqref{mor-eq-b}.
It is enough to prove it for atomic formulae $\varphi$.
If $\varphi$ equals to $t = t'$, then $\beta_V(\varphi)$ equals to $\alpha_V(t) = \alpha_V(t')$ and $\beta'_V(\varphi)$ equals to $\alpha'_V(t) = \alpha'_V(t')$.
We know that $\sststile{}{V} \alpha_V(t) \cong \alpha'_V(t)$ and $\sststile{}{V} \alpha_V(t') \cong \alpha'_V(t')$.
Thus by transitivity and symmetry we can conclude that $\alpha_V(t) = \alpha_V(t)' \sststile{}{V} \alpha'_V(t) = \alpha'_V(t')$.

If $\varphi = R(t_1, \ldots t_n)$, then $\beta_V(\varphi) = \beta_U(R(x_1, \ldots x_n))[x_i \mapsto \alpha_V(t_i)]$
and $\beta'_V(\varphi) = \beta'_U(R(x_1, \ldots x_n))[x_i \mapsto \alpha'_V(t_i)]$ where $U = \{ x_1 : s_1, \ldots x_n : s_n \}$.
We know that $\beta_U(R(x_1, \ldots x_n)) \sststile{}{U} \beta'_U(R(x_1, \ldots x_n))$.
Since $FV(\beta_U(R(x_1, \ldots x_n))) = \{ x_1, \ldots x_n \}$, by (a3) we can conclude that $\beta_V(\varphi) \sststile{}{V} \beta'_U(R(x_1, \ldots x_n))[x_i \mapsto \alpha_V(t_i)]$.
Since $\beta'_U(R(x_1, \ldots x_n))[x_i \mapsto \alpha_V(t_i)] \sststile{}{V} \alpha_V(t_i)\!\downarrow$, by \rlem{cong-b} we can conclude that
$\beta'_U(R(x_1, \ldots x_n))[x_i \mapsto \alpha_V(t_i)] \sststile{}{V} \beta'_V(\varphi)$.
By (b2) we conclude that $\beta_V(\varphi) \sststile{}{V} \beta'_V(\varphi)$.
The same argument shows that $\beta'_V(\varphi) \sststile{}{V} \beta_V(\varphi)$.

Now, we can finish the proof of \eqref{mor-eq-a}.
We need to prove that $\sststile{}{V} \alpha_V(t|_\varphi) \cong \alpha'_V(t|_\varphi)$.
We already saw that $\alpha_V(t)\!\downarrow\,\sststile{}{V} \alpha_V(t) = \alpha'_V(t)$ and $\beta_V(\varphi) \sststile{}{V} \beta'_V(\varphi)$.
Hence $\alpha_V(t)\!\downarrow \land \beta_V(\varphi) \sststile{}{V} \alpha_V(t) = \alpha'_V(t) \land \beta'_V(\varphi)$.
The same argument shows that $\alpha'_V(t)\!\downarrow \land \beta'_V(\varphi) \sststile{}{V} \alpha_V(t) = \alpha'_V(t) \land \beta_V(\varphi)$.
\end{proof}
...
Here, we will show that morphisms of theories determine functors between models of these theories and prove the following proposition:
...
If $T$ is a theory over signature $\Sigma$, then category $\Mod(T)$ of models of $T$ is a full subcategory of $\PStr{\Sigma}$.
Let $T_i$ be theories over signatures $\Sigma_i$, $i = 1,2$.
If a morphism of signatures $f : \Sigma_1 \to \Sigma_2$ preserves axioms of $T_1$, then for every model $A$ of $T_2$, $f^*(A)$ is a model of $T_1$.
Indeed, if $\varphi \sststile{}{V} \psi$ is a sequent of $T_1$, then it holds in $f^*(A)$ if $f(\varphi) \sststile{}{V} f(\psi)$ holds in $A$.
If $\varphi \sststile{}{V} \psi$ is an axiom of $T_1$, then $f(\varphi) \sststile{}{V} f(\psi)$ is a theorem of $T_2$, hence it holds in $A$.
Thus $f^*(A)$ is a model of $T_1$ and $f^*$ restricts to a functor $\Mod{T_2} \to \Mod{T_1}$.

Now, let $f,g : T_1 \to T_2$ be morphisms of theories.
...
Indeed, we have $f^*(A)(R) = A(f(R))$ and $g^*(A)(R) = A(g(R))$, and $A(f(R)) = A(g(R))$ since $A$ is a model of $T_2$ and $f(R) \ssststile{}{V} g(R)$ is a theorem of $T_2$.
Analogous argument shows that $f^*(A)(\sigma) = g^*(A)(\sigma)$.

Summarizing, we saw that every morphism of theories $f : T_1 \to T_2$ defines a functor $f^* : \Mod{T_2} \to \Mod{T_1}$.
Moreover, $(-)^*$ is functorial, which proves \rprop{iso-mod}.
...
\begin{prop}
Category $\Th_\mathcal{S}$ is locally finitely presentable.
\end{prop}
\begin{proof}
...
\begin{prop}
A morphism of theories $(\alpha,\beta) : T \to S$ is a monomorphism if and only if for every sequent $\varphi \sststile{}{V} \psi$ of $T$
if $\beta_V(\varphi) \sststile{}{V} \beta_V(\psi)$ is a theorem of $S$, then $\varphi \sststile{}{V} \psi$ is a theorem of $T$.
\end{prop}
\begin{proof}
First, let us prove the ``if'' part.
Let $(\alpha_1,\beta_1), (\alpha_2,\beta_2) : P \to T$ be a pair of morphisms such that $(\alpha,\beta) \circ (\alpha_1,\beta_1) = (\alpha,\beta) \circ (\alpha_2,\beta_2)$.
If $t \in PTerm_\Sigma(V)_s$, then $\sststile{}{V} \alpha(\alpha_1(t)) \cong \alpha(\alpha_2(t))$; hence $\sststile{}{V} \alpha_1(t) \cong \alpha_2(t)$.
If $\varphi \in Form_\Sigma(V)$, then $\beta(\beta_1(\varphi)) \ssststile{}{V} \beta(\beta_2(\varphi))$; hence $\beta_1(\varphi) \ssststile{}{V} \beta_2(\varphi)$.
Thus $(\alpha_1,\beta_1) = (\alpha_2,\beta_2)$.

Now, let us prove the ``only if'' part.
Suppose that $(\alpha,\beta)$ is a monomorphism.
Let $\varphi \sststile{}{V} \psi$ be a sequent of $T$ such that $\beta_V(\varphi) \sststile{}{V} \beta_V(\psi)$ is a theorem of $S$.
Let $P$ be a theory which consists of a single predicate symbol $R : s_1 \times \ldots \times s_n \times s'_1 \times \ldots \times s'_k$
where $s_1, \ldots s_n$ are sorts of variables in $fv(\varphi)$ and $s'_1, \ldots s'_k$ are sorts of variables in $fv(\psi)$.
Let $f : P \to T$ be a morphism defined by $f(R(x_1, \ldots x_n, y_1, \ldots y_k)) = \varphi \land y_1\!\downarrow \land \ldots \land y_k\!\downarrow$ and
let $g : P \to T$ be a morphism defined by $g(R(x_1, \ldots x_n, y_1, \ldots y_k)) = \varphi \land \psi$.
By \rprop{mor-eq} $(\alpha,\beta) \circ f = (\alpha,\beta) \circ g$, hence $f = g$ which implies that $\varphi \sststile{}{V} \psi$.
\end{proof}

Let $T = (\mathcal{F},\mathcal{P},\mathbb{T})$ be a theory in $\Th_\mathcal{S}$ and $T' = (\mathcal{F}',\mathcal{P}',\mathbb{T}')$ a theory in $\Th_{\mathcal{S}'}$.
Then we say that $T'$ is a \emph{subtheory} of $T$ if $\mathcal{S}' \subseteq \mathcal{S}$, $\mathcal{F}' \subseteq \mathcal{F}$, $\mathcal{P}' \subseteq \mathcal{P}$ and $\mathbb{T}' \subseteq \mathbb{T}$.
If $T'$ is a subtheory of a theory $T$, then we often need to know when a theorem of $T$ is a theorem of $T'$.
The lemma below gives us a simple criterion for this.
First, we need to introduce a bit of notation.
If $V \in \Set^\mathcal{S}$, then we write $T'(V) \in \Set^{\mathcal{S}'}$ for the subset of $V$ which consists of variables of sorts from $\mathcal{S}'$.
If $\varphi$ is a Horn formula over $(\mathcal{S},\mathcal{F},\mathcal{P})$, then we define a formula $T'(\varphi)$ over $(\mathcal{S}',\mathcal{F}',\mathcal{P}')$.
For atomic formulae $T'$ is defined as follows:
\begin{align*}
T'(\varphi) & = \varphi \text{ if $\varphi$ is a formula over $(\mathcal{S}',\mathcal{F}',\mathcal{P}')$,} \\
T'(\varphi) & = \top \text{ otherwise.}
\end{align*}
For other Horn formulae $T'$ is defined as follows:
\[ T'(\varphi_1 \land \ldots \land \varphi_n) = T'(\varphi_1) \land \ldots \land T'(\varphi_n) \]

We will say that a subtheory $T' = (\mathcal{F}',\mathcal{P}',\mathbb{T}') \in \Th_{\mathcal{S}'}$ of $T$ is \emph{disjoint}
if for every $\sigma : s_1 \times \ldots \times s_n \to s$ such that $s \in \mathcal{S}'$, $\sigma \in \mathcal{F}'$.

\begin{lem}
Let $T' = (\mathcal{F}',\mathcal{P}',\mathbb{T}') \in \Th_{\mathcal{S}'}$ be a disjoint subtheory of $T \in \Th_\mathcal{S}$.
If for every axiom $\varphi \sststile{}{V} \psi$ of $T$ sequent $T'(\varphi) \sststile{}{T'(V)} T'(\psi)$ is a theorem of $T'$, then this is true for every theorem of $T$.
\end{lem}
\begin{proof}
We need to check that every inference rule preserves this property.
This is clearly true for rules (b1)-(b6) and (a1).
Let us consider sequent $x = y \land \varphi \sststile{}{x:s,y:s,V} \varphi[y/x]$.
We can assume that $\varphi$ is atomic.
If $x \notin FV(\varphi)$, then this sequent is a special case of (b5).
Suppose $x \in FV(\varphi)$.
Note that $\varphi[y/x]$ is a formula of $(\mathcal{S}',\mathcal{F}',\mathcal{P}')$ if and only if $\varphi$ is.
If $s \notin \mathcal{S}'$, then $\varphi$ is not a formula of $(\mathcal{S}',\mathcal{F}',\mathcal{P}')$;
hence sequent $T'(x = y \land \varphi) \sststile{}{x:s,y:s,V} T'(\varphi[y/x])$ equals to $\top \sststile{}{T'(V)} \top$ which is clearly derivable.
If $s \in \mathcal{S}'$, then this sequent equals either to $x = y \land \varphi \sststile{}{x:s,y:s,T'(V)} \varphi[y/x]$ or to $x = y \sststile{}{x:s,y:s,T'(V)} \top$, both of which are derivable.
...
First, note that $F$ is full.
Indeed, given a morphism $f : \mathbb{T}_{-1} \amalg X \to \mathbb{T}_{-1} \amalg Y$,
