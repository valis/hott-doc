...
\item For every $x \in V_s$, $\alpha_V((\Gamma,x)|_\top) = (\Gamma',x)|_\varphi$,
...
\item If $\rho : A \to Term_\Sigma(B)$ is a function and $t \in PCTerm_\Sigma(A)_s$ is such that for every $x \in A$ there is a unique context $\Gamma_x$ in $FCV_x(t)$,
    then $\alpha_B(t[\rho]) = \alpha_A(t)[x \mapsto \alpha_B((\Gamma_x,\rho(x))|_\top)]$.
\label{con-mor-sig-a-subst}
\item If $\rho : A \to Term_\Sigma(B)$ is a function and $\varphi \in CForm_\Sigma(A)_s$ is an atomic formula such that for every $x \in A$
    there is a unique context $\Gamma_x$ in $FCV_x(\varphi)$,
    then $\beta_B(\varphi[\rho]) = \beta_A(\varphi)[x \mapsto \alpha_B((\Gamma_x,\rho(x))|_\top)]$.
...
\todo{Fix the characterization of premorphisms}
\begin{itemize}
