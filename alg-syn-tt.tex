...
\[ \alpha_{V,p,n,k} : Term_{E(\Sigma)}(V)_{Ctx_{n+k}} \times Term_{\Sigma}(V)_{(p,k)} \times Env_{p,n} \to Term_{E(\Sigma)}(V)_{(p,n+k)} \]
and
\[ \beta_{V,n,k} : Term_{E(\Sigma)}(V)_{Ctx_n} \times Ctx_\Sigma(V)_k \times Env_{ty,n} \to Term_{E(\Sigma)}(V)_{Ctx_{n+k}} \]
where $Ctx_\Sigma(V)_k$ is the set of contexts of length $k$ and $Env_{p,n}$ is the set of functions $V_{(p,m)} \to Term_{E(\Sigma)}(V)_{p,n+m}$ for all $m \in \mathbb{N}$.
\begin{itemize}
\item $\beta_{V,n,0}(\Gamma, (), \rho) = \Gamma$.
\item $\beta_{V,n,k+1}(\Gamma, (\Delta,A), \rho) = \alpha_{V,ty,n,k}(\beta_{V,n,k}(\Gamma, \Delta, \rho), A, \rho)$.
\item $\alpha_{V,p,n,k}(\Gamma, v_{k,i}, \rho) = v_{n+k,i}(\Gamma)$.
\item $\alpha_{V,p,n,k}(\Gamma, x, \rho) = \rho(x)$ where $x$ is a variable.
\item $\alpha_{V,p,n,k}(\Gamma, subst_{ty,n,k}(x, a_1, \ldots a_k)) = Subst_{n,k}(\Gamma, \rho(x), \alpha_{V,p,n,k}(\Gamma, a_1), \ldots \alpha_{V,p,n,k}(\Gamma, a_k))$ where $x$ is a variable.
\item $\alpha_{V,p,n,k}(\Gamma, subst_{tm,n,k}(x, a_1, \ldots a_k)) = subst_{n,k}(\Gamma, \rho(x), \alpha_{V,p,n,k}(\Gamma, a_1), \ldots \alpha_{V,p,n,k}(\Gamma, a_k))$ where $x$ is a variable.
\item $\alpha_{V,p,n,k+q}(\Gamma, f_k(a_1, \ldots a_m)) = f_{n+k}(\Gamma, a_1', \ldots a_m'))$.
    where $f : (p_1,q_1) \times \ldots \times (p_m,q_m) \to (p,q)$ and $a_i' = \alpha_{V,p_i,n,k+q_i}(\beta_{V,n,k}(\Gamma, \Delta_i), a_i)$.
\end{itemize}
These functions are well-defined by the assumption of regularity.
