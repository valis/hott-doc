...
\item Список конструкторов и их типов: $c_i : \Pi (b_1 : B_1) \ldots \Pi (b_k : B_k) \to D\ x_1 \ldots x_n$.
    Мы требуем, чтобы типы конструкторов имели указаный вид, более того, для каждого из $B_j$ должно быть верно,
    что либо $D \notin FV(B_j)$, либо $B_j$ имеет вид $\Pi (e_1 : E_1) \ldots \Pi (e_m : E_m) \to D\ a_1 \ldots a_n$, и $D \notin FV(E_i)$ для всех $i$.
    Другими словами, мы требуем, чтобы определение типа данных было строго положительным.
...
Если мы хотим подчеркнуть, что вывод происходит в сигнатуре $\Sigma$, мы будем писать $\Gamma \vdash_\Sigma$, $\Gamma \vdash_\Sigma A$ и $\Gamma \vdash_\Sigma a : A$.
...
\RightLabel{,}
\BinaryInfC{$\Gamma \vdash D\ a_1 \ldots a_n$}
\DisplayProof
\end{center}
где $D$ - тип данных в $\Sigma$ с параметрами $x_1 : A_1, \ldots x_n : A_n$.
...
\[ f\ (p_1[x^1_1 := t^1_1] \ldots [x^1_{k_1} := t^1_{k_1}]) \ldots (p_n[x_n := t^n_1] \ldots [y^n_{k_n} := s^n_{k_n}]) \to_\beta t[x^i_j := t^i_j] \]
...
Мы будем говорить, что объявление типа данных $D$ в сигнатуре $\Sigma$ корректно, если выполнены следующие условия:
\begin{itemize}
\item $\Gamma \vdash_\Sigma$, где $\Gamma$ - параметры $D$.
\item $\Gamma \vdash_{\Sigma,D} C_i$, где $C_i$ - тип конструктора $c_i$.
