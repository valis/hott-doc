...
But if all objects are fibrant, then we can give a simple explicit construction of a set of generating trivial cofibrations (see \rprop{min-cof-gen}).
...
we just need to prove that maps in $\Icell[F(\J)]$ are weak equivalences, where $\J$ is a set of generating cofibrations of $\C$.
...
Thus the following proposition implies that if a map of such a model category is a weak equivalence and belongs to $\Jinj_[\I]$, then it is a trivial fibration.
...
\item[(A3)] Since $\I$ is a set of generating cofibrations, every map in $\Iinj$ is a weak equivalence.
\item[(A4)] A map $\cyli_0 : V \to C_U(V)$ in $\J_\I$ is the composition of maps $V \to V \amalg_U V \to C_U(V)$,
where the first map is a pushout of a cofibration $U \to V$ and the second map is a cofibration by definition.
Moreover, $C_U(V) \to V$ is a weak equivalence; hence, $\cyli_0$ is also a weak equivalence by 2-out-of-3.
...
\begin{prop}
The class of weak $\I$-cofibrations is closed under pushouts, compositions and weak retracts.
\end{prop}
\begin{proof}
...
Maps in $\Jcell[_\I]$ are split monomorphisms.
...
Thus maps in $\Jcell[_\I]$ are pure with respect to all maps.
...
\begin{cor}[main-path]
Let $\C$ be a category that satisfies the conditions of \rthm{main}.
Then there exists a model structure on $\C$ with $\Icof$ as the class of cofibrations and $\Jcof[_\I]$ as the class of trivial cofibrations
