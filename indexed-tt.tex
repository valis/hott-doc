...
Let us consider the following version of the empty type:
\begin{center}
\AxiomC{}
\UnaryInfC{$\Gamma \mid \Delta \vdash 0\ \ob$}
\DisplayProof
\qquad
...
\DisplayProof
\end{center}
Note that we can eliminate $0$ only into a type which have only one free variable of type $0$.

\begin{prop}
If an indexed dependent type theory has extensional identity types and $\Sigma$-types, then it has the $0$ type if and only if it has an initial type.
\end{prop}
\begin{proof}
Suppose that the theory has the $0$ type.
Then it is initial.
Indeed, we can define a map $\lambda x.\,0\text{-}\fs{elim}(y.D,x) : \Hom(0,D)$.
If $f,g : \Hom(0,D)$, then we can define a homotopy between them as $\fs{hap^{-1}}(x.\,0\text{-}\fs{elim}(y.\,\Id_D(f\,y,g\,y),x)) : \Id(f,g)$.

Suppose that the theory has an initial type.
Then it is the $0$ type.
Indeed, we can define $0\text{-}\fs{elim}(x.D,a)$ as $\fs{hap}(h,a)_*(\pi_2(f\,a))$, where $f : \Hom(0,\Sigma_{x : 0} D)$ is any map and $h$ is any homotopy between $\pi_1 \circ f$ and $\id_0$.
\end{proof}

We will say that the initial type $0$ in an indexed unary type theory is \emph{stable under products} if, for every type $B$ such that the product $B \times 0$ exists, this product is initial.
...
\item \label{it:init-fourth} The type $0$ is a stable under products initial type.
\end{enumerate}
\end{prop}
\begin{proof}
...
% 3. Доказать, что в indexed dependent type theories есть конечные произведение и бесконечные тоже, если предположить их должны образом.
% 4. Сравнить внешнее и внутренее определение копределов.
% 5. (Локальная) декартовая замкнутость.
% 6. Вселенная Prop и прочие вселенные.
% 7. Regular categories and propositional truncation?
% 8. Other factorization systems, modalities, and localizations?
