...
The \emph{exponent over $\Delta$} of $p_A$ and a map $p_B : \Hom(B,\Delta)$ is a map $p : \Hom(B^A,\Delta)$ together with a map $\fs{ev} : \Hom_\Delta(B^A \times_\Delta A, B)$ such that the following function is an equivalence for every indexed type $X$:
...
\AxiomC{$\Gamma \mid \Delta, x : A \vdash p : \Id_B(f\,x,g\,x)$}
\AxiomC{$\Gamma \mid \Delta \vdash a : A$}
\BinaryInfC{$\Gamma \mid \Delta \vdash \fs{funext_h'}(x.p) : \Id(\pmap(f.\,f\,a,\fs{funext}(x.p)), p[a/x])$}
\DisplayProof
\end{center}
...
The left and the right functions are equivalences since identity maps are extensional.
To prove that the top map is an equivalence, it is enough to show that the following function is:
...
Thus, the bottom function is also an equivalence.
...
\eqref{it:exp-non-dep} $\Rightarrow$ \eqref{it:exp-exp}.
% TODO

\eqref{it:exp-exp} $\Rightarrow$ \eqref{it:exp-non-dep}.
% TODO
\end{proof}
