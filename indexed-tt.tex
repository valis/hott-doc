...
We will say that the theory has \emph{weak dependent pushouts} if the following rules are derivable:
\begin{center}
...
\Axiom$\fCenter \Gamma \mid \Delta, x : B, E[\fs{inl}(x)/z] \vdash d_1 : D[\fs{inl}(x)/z]$
\noLine
...
\def\extraVskip{2pt}
\BinaryInfC{$\Gamma \mid \Delta, E[t/z] \vdash \amalg\text{-}\fs{elim}(z E.D, x E.d_1, y E.d_2, w E.d_3, t) : D[t/z]$}
\DisplayProof
\end{center}
\medskip
...
\[ \xymatrix{ \fs{glue}(a)_*(\amalg\text{-}\fs{elim}(\fs{inl}(f\,a))[\rho(\fs{glue}(a))]) \ar@{=}[r] \ar@{=}[d] & d_1'[a/w] \ar@{=}[d]^{d_3[a/w]} \\
              \amalg\text{-}\fs{elim}(\fs{inr}(g\,a)) \ar@{=}[r]_-{h_2(g\,a)} & d_2[g\,a/y]
            } \]
The last square must commute up to a homotopy $h_3(a)$.
The left arrow is
\[ J(\fs{inl}(f\,a), v p.\,\Id(p_*(\amalg\text{-}\fs{elim}(\fs{inl}(f\,a))[\rho(p)]), \amalg\text{-}\fs{elim}(v)), \refl, \fs{inr}(g\,a), \fs{glue}(a)). \]
...
\item The theory has local unstable eliminator.
\item The theory has global unstable eliminator.
\end{enumerate}
...
\emph{Weak dependent products} is a version of this construction in which the last two equations hold propositionally.
We will denote these homotopies by $\beta$ and $\eta$:
\begin{align*}
\beta(i.b,j) & : \Id((\lambda i.\,b)\,j,b[j/i]) \\
\eta(f) & : \Id(\lambda i.\,f\,i,f)
\end{align*}

We will say that a dependent product $\prod_{i : I} B$ is \emph{extensional} if we have the following constructions:
\begin{center}
\AxiomC{$\Gamma, i : I \mid \Delta \vdash p : \Id_B(f\,i,g\,i)$}
\RightLabel{, $i \notin \mathrm{FV}(\Delta)$}
\UnaryInfC{$\Gamma \mid \Delta \vdash \fs{ext}(i.p) : \Id_{\prod_{i : I} B}(f,g)$}
\DisplayProof
\end{center}
\medskip

\begin{center}
\AxiomC{$\Gamma \mid \Delta \vdash p : \Id_{\prod_{i : I} B}(f,g)$}
\UnaryInfC{$\Gamma \mid \Delta \vdash \fs{ext_h}(p) : \Id(\fs{ext}(i.\pmap(f.\,f\,i,p)), p)$}
\DisplayProof
\end{center}
\medskip
...
\RightLabel{, $i \notin \mathrm{FV}(\Delta)$}
\AxiomC{$\Gamma \vdash j : I$}
\BinaryInfC{$\Gamma \mid \Delta \vdash \fs{ext_h'}(i.p,j) : \Id(\pmap(f.\,f\,j,\fs{ext}(i.p)), p[j/i])$}
\DisplayProof
\end{center}

A (weak) non-dependent product is a special case of (weak) dependent products.
If identity types are extensional, then a product is extensional if and only if it is extensional in the sense defined above.
...
We can think of (weak) dependent products as wide pullbacks.
Thus, to construct them from ordinary products, we also need to assume the existence of $\Sigma$-types and identity types:

\begin{prop}
If a theory has $\Sigma$-types, extensional identity types, and weak extensional products, then it has weak unstable extensional dependent products.
\end{prop}
\begin{proof}
Let $\Gamma, i : I \mid \Delta \vdash B_i \ob$ be an indexed type.
If $\Delta$ is the empty context, then $\prod_{i : I} B_i$ is the usual weak product.
Thus, we may assume that it is non-empty.
Since we have $\Sigma$-types, we may assume that $\Delta$ consists of exactly one type, that is $\Delta = (x : A)$.
...
Let $\Gamma, i : I \mid x : A \vdash b : B_i$ be a term.
Then we define $\Gamma \mid x : A \vdash \lambda i.\,b : \prod_{i : I} B_i$ as follows:
\[ \Gamma \mid x : A \vdash (\lambda i.(x,b), \fs{ext}(i. h(b,i))), \]
where $h(b,i) = \beta(i.\,\pi_1\,((\lambda i.(x,b))\,i),i) \ct \pmap(p.\,\pi_1(p), \beta(i.(x,b),i)) \ct \sym{\beta(i.x,i)}$.
If $\Gamma \mid x : A \vdash p : \prod_{i : I} B_i$ and $\Gamma \vdash j : I$, then we define $\Gamma \mid x : A \vdash p\,j : B_j$ as follows:
\[ \Gamma \mid x : A \vdash h'(p,j)_*(\pi_2(\pi_1(p)\,j)) : B_j, \]
where $h'(p,j) = \sym{\beta(i.\,\pi_1\,(\pi_1(p)\,i),j)} \ct \pmap(f.\,f\,j,\pi_2(p)) \ct \beta(i.x,j)$.

Let us prove that $(\lambda i.b)\,j = h'(\lambda i.b,j)_*((\lambda i.(x,b))\,j)$ is homotopic to $b[j/i]$.
First, note that we have the following homotopy:
\[ \beta(i.(x,b),j) : \Id_{\Sigma_{x : A} B_j}((\lambda i.(x,b))\,j,(x,b[j/i])). \]
A standard argument implies that $\pmap(p.\,\pi_1(p),\beta(i.(x,b),j))_*((\lambda i.(x,b))\,j)$ is homotopic to $b[j/i]$.
Thus, we just need to prove that $\pmap(p.\,\pi_1(p),\beta(i.(x,b),j))$ is homotopic to $h'(\lambda i.b,j)$:
\begin{align*}
h'(\lambda i.b,j) & = \\
\sym{\beta(i.\,\pi_1\,((\lambda i.(x,b))\,i),j)} \ct \pmap(f.\,f\,j,\fs{ext}(i. h(b,i))) \ct \beta(i.x,j) & \sim \\
\sym{\beta(i.\,\pi_1\,((\lambda i.(x,b))\,i),j)} \ct h(b,j) \ct \beta(i.x,j) & \sim \\
\pmap(p.\,\pi_1(p), \beta(i.(x,b),j)) & .
\end{align*}
The first homotopy follows from $\fs{ext_h'}$ and the second one from the definition of $h(b,j)$.

Now, let $(f,q)$ be a term of type $\prod_{i : I} B_i$.
Let us prove that $\lambda i.\,(f,q)\,i$ is homotopic to $(f,q)$.
Let $b = h'((f,q),i)_*(\pi_2(f\,i))$.
Then
\[ \lambda i.\,(f,q)\,i = (\lambda i.(x,b),\fs{ext}(i.h(b,i))). \]
By $\Sigma$-extensionality, it is enough to construct a homotopy $t$ between $f$ and $\lambda i.(x,b)$ such that $t_*(q)$ is homotopic to $\fs{ext}(i.h(b,i))$.
We can define $t$ as follows:
\[ \fs{ext}(i. \Sigma \fs{ext}(h'((f,q),i),\refl) \ct \sym{\beta(i.(x,b),i)}). \]
Note that $t_*(q)$ is homotopic $\sym{\fs{ext}(i.s)} \ct q$, where
\[ s = \beta(i.\,\pi_1(f\,i),i) \ct \pmap(p.\,\pi_1(p\,i),t) \ct \sym{\beta(i.\,\pi_1((\lambda i.(x,b))\,i),i)}. \]
This is easy to prove by the path induction on $t$.
Thus, we just need to prove that $q$ is homotopic to $\fs{ext}(i.\,s \ct h(b,i))$.
By $\fs{ext_h}$, it is enough to prove that $\pmap(f.\,f\,i,q)$ is homotopic to $s \ct h(b,i)$.

We have the following sequence of homotopies:
\begin{align*}
\pmap(p.\,\pi_1(p\,i),t) & \sim \\
\pmap(p.\,\pi_1(p),\pmap(f.\,f\,i,t)) & = \\
\pmap(p.\,\pi_1(p),\pmap(f.\,f\,i, \fs{ext}(i. \Sigma \fs{ext}(h'((f,q),i),\refl) \ct \sym{\beta(i.(x,b),i)}))) & \sim \\
\pmap(p.\,\pi_1(p), \Sigma \fs{ext}(h'((f,q),i),\refl) \ct \sym{\beta(i.(x,b),i)}) & \sim \\
h'((f,q),i) \ct \sym{\pmap(p.\,\pi_1(p), \beta(i.(x,b),i))} & .
\end{align*}
This implies the existence of the required homotopy:
\begin{align*}
s \ct h(b,x) & \sim \\
\beta(i.\,\pi_1(f\,i),i) \ct \pmap(p.\,\pi_1(p\,i),t) \ct \pmap(p.\,\pi_1(p), \beta(i.(x,b),i)) \ct \sym{\beta(i.x,i)} & \sim \\
\beta(i.\,\pi_1(f\,i),i) \ct h'((f,q),i) \ct \sym{\beta(i.x,i)} & \sim \\
\pmap(f.\,f\,i,q) & .
\end{align*}
The first homotopy easily follows from the definitions of $s$ and $h(b,i)$.
The second homotopy follows from the sequence above.
The last homotopy follows from the definition of $h'((f,q),i)$.
\end{proof}
...
\AxiomC{$\Gamma, i : I \mid \Delta \vdash B \ob$}
\RightLabel{, $i \notin \mathrm{FV}(\Delta)$}
\UnaryInfC{$\Gamma \mid \Delta \vdash \coprod_{i : I} B \ob$}
\DisplayProof
\qquad
\AxiomC{$\Gamma \vdash j : I$}
...
\begin{center}
\AxiomC{$\Gamma \mid \Delta, z : \coprod_{i : I} B, E \vdash D \ob$}
\AxiomC{$\Gamma, i : I \mid \Delta, x : B, E[(i,x)/z] \vdash d : D[(i,x)/z]$}
\AxiomC{$\Gamma \mid \Delta \vdash t : \coprod_{i : I} B$}
\TrinaryInfC{$\Gamma \mid \Delta, E[t/z] \vdash \coprod\text{-}\fs{elim}(z.D, i x.d, t) : D[t/z]$}
\DisplayProof
\end{center}
\medskip
...
\begin{prop}
Suppose that we have stable constructions $\coprod$ and $(-,-)$ as described above.
Then the following conditions are equivalent:
\begin{enumerate}
\item These constructions have the universal property of coproducts.
\item The theory has local unstable eliminator.
\item The theory has global unstable eliminator.
\end{enumerate}
If these equivalent conditions hold, then coproducts are stable under pullbacks.
\end{prop}
\begin{proof}
% TODO
\end{proof}
...
We can define locally reflective classes of fibration in a more type-theoretic way using the following constructions:
\begin{center}
\def\extraVskip{1pt}
\Axiom$\fCenter \Gamma \mid \Delta, z : \| A \|, E \vdash D \fib$
\noLine
\UnaryInf$\fCenter \Gamma \mid \Delta, x : A, E[|x|/z] \vdash d : D[|x|/z]$
\Axiom$\fCenter \Gamma \mid \Delta \vdash a : \| A \|$
\def\extraVskip{2pt}
\BinaryInfC{$\Gamma \mid \Delta, E[a/z] \vdash \| A \|\text{-}\fs{elim}(z E .D, x E.d, a) : D[a/z]$}
\DisplayProof
\end{center}
\medskip
...
\Axiom$\fCenter \Gamma \mid \Delta \vdash a : A$
\def\extraVskip{2pt}
\BinaryInfC{$\Gamma \mid \Delta, E[|a|/z] \vdash \| A \|\text{-}\fs{elim_h}(z E.D, x E.d, a) : \Id(\| A \|\text{-}\fs{elim}(z E.D, x E.d, |a|), d[a/x])$}
\DisplayProof
\end{center}
...
\item \label{it:fib-dep} $\Fib$ is closed under identity types and unstable constructions $\| A \|\text{-}\fs{elim}$ and $\| A \|\text{-}\fs{elim_h}$ are definable.
\item \label{it:fib-dep-local} $\Fib$ is closed under identity types and unstable constructions $\| A \|\text{-}\fs{elim}$ and $\| A \|\text{-}\fs{elim_h}$ are definable when $E$ is the empty context.
\end{enumerate}
\end{prop}
\begin{proof}
\eqref{it:fib-non-dep} $\Rightarrow$ \eqref{it:fib-dep}
By \rprop{fib-refl-id} and \rlem{fib-pullback}, a stably locally reflective class of fibrations is closed under identity types.
Consider the following pullback square:
\[ \xymatrix{ \Sigma(\Delta, x : A, E[|x|/z]) \ar[r] \ar[d]_j \pb   & \Sigma(\Delta, x : A) \ar[d]^j \\
...
Since $\Fib$ is closed under $\Sigma$-types, \rprop{fib-sigma} implies that it is also closed under compositions.
...
Let $B = \Sigma(\Delta, z : \| A \|, E)$ and $\pi_1 : \Hom(T,B)$ be the fibration corresponding to $D$.
...
This lift determines a term $\Delta, z : \| A \|, E \vdash d : D$ and a homotopy $\Delta, x : A, E[|x|/z] \vdash h : \Id(d[|x|/z],d)$.
Now, we can define $\| A \|\text{-}\fs{elim}(z E .D, x E.d, a)$ and $\| A \|\text{-}\fs{elim_h}(z E .D, x E.d, a)$ as $d[a/z]$ and $h[a/x]$, respectively.

\eqref{it:fib-dep} $\Rightarrow$ \eqref{it:fib-dep-local} Obvious.

\eqref{it:fib-dep-local} $\Rightarrow$ \eqref{it:fib-non-dep}
...
We can define a lift in this diagram as $\lambda z.\,\| A \|\text{-}\fs{elim}(z.B, x.b x, z)$.
The commutativity of the triangle is witnessed by the term $\lambda x.\,\| A \|\text{-}\fs{elim_h}(z.B, x.b x, x)$.
...
Since $\Fib$ is closed under identity types, the type $\Id(f_1,f_2)$ is fibrant over $\Delta, y : \| A \|, z : \| A \|$.
Thus, we have a homotopy between $f_1$ and $f_2$:
\[ \Delta, y : \| A \| \vdash \| A \|\text{-}\fs{elim}(z.\Id(f_1,f_2), x. h_1 \ct \sym{h_2}, y) : \Id(f_1[y/z], f_2[y/z]). \]
Let us denote this homotopy by $H$.
We need to prove that $\sym{H[|x|/y]} \ct h_1$ is homotopic to $h_2$.
This homotopy can be constructed from the following one:
\[ \Delta, x : A \vdash \| A \|\text{-}\fs{elim_h}(z.\Id(f_1,f_2), x. h_1 \ct \sym{h_2}, x) : \Id(H[|x|/y], h_1 \ct \sym{h_2}). \]
\end{proof}
