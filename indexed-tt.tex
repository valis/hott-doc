...
We will say that an indexed unary type theory \emph{has terminal (resp., initial) types} if, for every context $\Gamma$, there is a terminal (resp., initial) type $\Gamma \mid \cdot \vdash T\ \ob$ and substitution preserves terminal (resp., initial) types.
...
is contractible.
An indexed unary type theory \emph{has binary products} if a binary product exists for every pair of types in every context and binary products are stable under substitution.
...
An indexed unary type theory \emph{has equalizers} if an equalizer exists for every parallel pair of maps in every context and equalizers are stable under substitution.
...
An indexed unary type theory \emph{has pullbacks} if a pullback exists for every pair of maps with a common codomain in every context and pullbacks are stable under substitution.
...
We can define an extension of the basic locally small indexed unary type theory which makes it into a theory with finite limits.
First, we define canonical terminal types:
\begin{center}
\AxiomC{}
...
These constructions imply that $\top$ is contractible.
Moreover, since $\top$ is stable under substitution, every terminal type is equivalent to $\top$, and every type which is equivalent to $\top$ is terminal, it follows that terminal types are also stable under substitution.
Theories of canonical products, equalizers, and pullbacks are defined similarly and the same argument shows that these theories have corresponding categorical constructions.

\subsection{(Co)products}

In this subsection, we assume that the base theory has $\Pi$-types.

A \emph{product} of an indexed type $\Gamma, i : I \mid \cdot \vdash B\ \ob$ is an indexed type $\Gamma \mid \cdot \vdash P\ \ob$ together with an indexed term $\Gamma, i : I \mid x : P \vdash p : B$
such that, for all $\Gamma \mid \cdot \vdash P'\ \ob$ and $\Gamma, i : I \mid x : P' \vdash p' : B$, the following type is contractible:
\[ \sum_{h : \Hom(P',P)}  \]
