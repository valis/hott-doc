...
If the base theory has $\Sigma$-types, then we can define types expressing the property of a map $f : \Hom(A,B)$ of being an equivalence.
In this section, we define two such types and prove that they are equivalent.
...
Suppose that the base theory has $\Sigma$-types.
...
Indeed, the concatenation of both of these paths with $\eta$ is homotopic to $\eta * \eta$.
Thus, if we cancel $\eta$, this gives us a homotopy between the original paths.

It follows that $\refl(f) * \epsilon'$ is homotopic to the concatenation of paths 
$\refl(f \circ g \circ f) * \sym(\epsilon)$, $\eta * \refl(f \circ g \circ f)$, and $\refl(f) * \epsilon$.
This path is homotopic to the concatenation of $\eta * \refl(f) * \sym(\epsilon)$ and $\refl(f) * \epsilon$,
which is homotopic to the concatenation of $\eta * \refl(f)$, $\refl(f) * \sym(\epsilon)$, and $\refl(f) * \epsilon$,
which is homotopic to $\eta * \refl(f)$.
\end{proof}
