...
Finally, $f$ is a fibration since $f = q' \circ \langle \id_C, \id_C \rangle \circ f = q' \circ f' \circ r$.
...
The factorization $A \to C \to B$ will be called \emph{the universal factorization} of $f$.
...
\begin{lem}[fib-refl]
If $A \xrightarrow{i} C \xrightarrow{p} B$ is the universal factorization of $f$, then the following conditions are equivalent:
\begin{enumerate}
...
\end{lem}
\begin{proof}
Let $A \xrightarrow{i} B \xrightarrow{p} A$ be the universal factorization of $\id_A$.
...
Let $A \xrightarrow{i} C \xrightarrow{p} B$ be the universal factorization of a map $f : \Hom(A,B)$.
Then every commutative square as below has a lift if $v$ factors through $p$.
...
\begin{lem}[fib-refl-id]
Any locally reflective class of fibrations closed under pullbacks satisfies the equivalent conditions of \rprop{fib-id}.
\end{lem}
\begin{proof}
Let $f : \Hom(A,B)$ be a fibration and let $d = \langle \id_A, \id_A \rangle : \Hom(A, A \times_B A)$.
We need to prove that $d$ is a fibration.
Let $A \xrightarrow{i} C \xrightarrow{p} A \times_B A$ be the universal factorization of $d$.
By \rlem{fib-refl}, it is enough to prove that $i$ has a retraction over $A \times_B A$.
We define the retraction $r$ as the composite $C \xrightarrow{p} A \times_B A \xrightarrow{\pi_1} A$.
We need to prove that the following diagram commutes and the induced homotopy between $p \circ i$ and $d$ is homotopic to the canonical homotopy obtained from the factorization.
\[ \xymatrix{ A \ar@{=}[r] \ar[d]_i & A \ar[d]^d \\
              C \ar[r]_-p \ar[ur]^r & A \times_B A
            } \]
The top triangle commutes because $r \circ i = \pi_1 \circ p \circ i \sim \pi_1 \circ d \sim \id_A$.
By the universal property of pullbacks, to prove that the bottom triangle commutes, it is enough to prove that
triples $(r,r,\refl)$ and $(\pi_1 \circ p, \pi_2 \circ p, p * \pi_3)$ are homotopic as elements of $\Sigma_{s_1 s_2 : \Hom(C,A)} \Id(f \circ s_1, f \circ s_2)$.
The first components are homotopic trivially.
The second components are homotopic since they are both lifts in the following square:
\[ \xymatrix{ A \ar@{=}[r] \ar[d]_i             & A \ar[d]^f \\
              C \ar[r]_{f \circ \pi_2 \circ p}  & B
            } \]
Lifts in this square are unique by \rlem{fib-refl-lift}.
This means that we have a homotopy $h : \Id(r, \pi_2 \circ p)$ such that $h * f \sim p * \pi_3$.
Then $h$ is the second component in the homotopy between $(r,r,\refl)$ and $(\pi_1 \circ p, \pi_2 \circ p, p * \pi_3)$ and $h * f \sim p * \pi_3$ gives us the third component.
Moreover, $i * h$ is homotopic to the homotopy $r \circ i = \pi_1 \circ p \circ i \sim \pi_1 \circ d \sim \id_A \sim \pi_2 \circ d \sim \pi_2 \circ p \circ i$
and this homotopy can be combined with the homotopy $h * f \sim p * \pi_3$ to give another coherence homotopy.
These homotopies can be used to construct a coherence homotopy for the lift in the original square.
\end{proof}
...
Then the corresponding non-dependent class of fibrations is locally reflective if and only if we have the following \emph{unstable} constructions:
% TODO
