...
\[ \lambda z. \coprod\text{-}\fs{elim}(z. \sum_{d : D} \coprod_{i : I} B, i p. (\pi_1(p),\fs{in}_i(x)[\pi_2(p)/x])) : \Hom(\coprod_{i : I} \sum_{d : D} B, \sum_{d : D} \coprod_{i : I} B). \]
\end{prop}
\begin{proof}
The inverse of this map is defined as follows:
...
% \[ \pi_1 \circ t \sim [\pi_1 \circ t \circ \fs{in}_i]_{i : I} \sim [\lambda x.\,\fs{in}_i(x)]_{i : I} \sim \id. \]
% If we denote this homotopy by $h$, then we can define the eliminator $\coprod\text{-}\fs{elim}(z.D, i x.d)$ as $h_*(\pi_2(t\,z))$.

% TODO: Construct \coprod\text{-}\fs{elim_h}.
\end{proof}
...
\end{prop}
\begin{proof}
If the theory has weak dependent coproducts, then it has coproducts by \rlem{coprod-char}.
Conversely, suppose that it has coproducts.
Let $\Gamma, i : I \mid \Delta \vdash B_i \ob$ be a dependent type.
If $\Delta$ is empty, then the eliminator is definable by \rlem{coprod-char}.
...
By \rlem{coprod-char}, we just need to prove that $\fs{in}_i'$ makes its codomain into a coproduct.
...
\begin{cor}
A class of fibrations closed under substitutions and $\Sigma$-types is locally reflective if and only if it extends to a higher modality.
\end{cor}
