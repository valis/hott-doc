...
% Мы определяем аналог этой штукенции на языке теории типов.
% В отличие от обычной HoTT это позволяет нам обсуждать свойства произвольных категорий, а не только lccc.
% Даже для топосов это может быть полезно. Например, классификатор объектов нормально можно только здесь определить.
% Другой пример -- это локально ортогональные системы факторизации. В HoTT условие может быть слишком сильным.
% Все эти проблемы возникают когда мы пытаемся определить направленную HoTT.

% Мы определим два вида индексированных теорий типов: унарные и зависимые.
% Первые позволяют обсуждать произвольные категориальные конструкции.
% Вторые имеют более привичный теоретико-типов синтаксис, но подразумевают существование конечных пределов.
% Мы определим ряд различных категориальных конструкций в обоих стилях:
% 1. Finite limits / $\Sigma$-types, unit type, and identity types.
% 2. Finite colimits / finite HITs.
% 3. Exponents / $\Pi$-types.
% 4. Object classifiers / univalent universes.
% 5. Orthogonal factorization systems / higher modalities.
% Мы докажем эквивалентность всех этих пар.

% Рассказать подробнее про проблемы со вселенными и системами факторизаций.
...
\begin{prop}
If a theory has $\Sigma$-types, extensional identity types, and weak extensional products, then it has weak unstable dependent products.
\end{prop}
\begin{proof}
