...
% Сказать еще где-то, что работа в индексированной теории типов похожа на работу в категории, обагащенной в типах.
...
The theory of \emph{dependent coproducts} consists of the following unstable rules:
\begin{center}
...
\[ \coprod\text{-}\fs{elim}(z.D, i x.d)[(j,b)/z] = d[j/i,b/x] \]
...
Then the eliminator is definable if and only if the induced map $\Hom(\Sigma(\Delta,B_j), \Sigma(\Delta, \coprod_{i : I} B_i))$ makes $\Sigma(\Delta, \coprod_{i : I} B_i)$ into the coproduct of the family $\Gamma, i : I \mid \cdot \vdash \Sigma(\Delta,B_i)$.
\end{prop}
\begin{proof}
First, suppose that the eliminator is definable.
If $\Gamma, i : I \vdash f : \Hom(B_i,C)$, then we define $\Gamma \vdash [f]_{i : I} : \Hom(\coprod_{i : I} B_i, C)$ as follows:
\[ [f]_{i : I} = \lambda z.\,\coprod\text{-}\fs{elim}(z.C, i x.\,f\,x). \]
Let us prove that $[f]_{i : I} \circ \fs{in}_i \sim f$:
\[ [f]_{i : I} \circ \fs{in}_i = \lambda x.\,[f]_{i : I}\,(i,x) = \lambda x.\,\coprod\text{-}\fs{elim}(z.C, i x.\,f\,x)[(i,x)/z] \sim \lambda x.\,f\,x \sim f. \]
Finilly, for every $g : \Hom(\coprod_{i : I} B_i, C)$, we need to prove that $[g \circ \fs{in}_i]_{i : I}$.
It is enough to prove that, for every $z : \coprod_{i : I} B_i$, there is a homotopy between $[g \circ \fs{in}_i]_{i : I}\,z$ and $g\,z$.
To do this, we can use the eliminator for $\coprod$.
Then we just need to construct a homotopy between $[g \circ \fs{in}_i]_{i : I}\,(i,x)$ and $g\,(i,x)$:
\[ [g \circ \fs{in}_i]_{i : I}\,(i,x) = \coprod\text{-}\fs{elim}(z.C, i x.\,g\,(i,x))[(i,x)/z] \sim g\,(i,x). \]

Now, suppose that $\Sigma(\Delta, \coprod_{i : I} B_i)$ is a coproduct.
\end{proof}
...
% \BinaryInfC{$\Gamma \mid \Delta, z : \coprod_{i : I} B, E \vdash \coprod\text{-}\fs{elim}(z E. D, i x E. d) : D$}
% \DisplayProof
% \end{center}
% \medskip
...
\[ \lambda z. \coprod\text{-}\fs{elim}(z. \sum_{d : D} \coprod_{i : I} B, i p. (\pi_1(p),(i,\pi_2(p)))) : \Hom(\coprod_{i : I} \sum_{d : D} B, \sum_{d : D} \coprod_{i : I} B). \]
\end{prop}
\begin{proof}
The inverse of this map is defined as follows:
\[ \lambda p. \coprod\text{-}\fs{elim}(z. \coprod_{i : I} \sum_{d : D} B, i b. (i,(\pi_1(p),b)))[\pi_2(p)/z] : \Hom(\sum_{d : D} \coprod_{i : I} B, \coprod_{i : I} \sum_{d : D} B). \]
...
\begin{prop}
If the equivalent conditions of \rprop{coprod-char} hold, then coproducts are stable under pullbacks.
\end{prop}
\begin{proof}
Let $\Gamma, i : I \mid \cdot \vdash B \ob$ be a dependent type and let $p : \Hom(\coprod_{i : I} B, D)$ and $r : \Hom(E,D)$ be maps.
We define $\Gamma, i : I \mid d : D \vdash B' \ob$ as the fiber of $B$ over $d$.
Then we have the following pullback squares:
\[ \xymatrix{ \coprod_{i : I} r^*(B) \ar[r] \ar[d]_\simeq \pb & \coprod_{i : I} B \ar[d]^\simeq \\
              \coprod_{i : I} \sum_{e : E} B'[r\,e/d] \ar[r] \ar[d]_\simeq \pb & \coprod_{i : I} \sum_{d : D} B' \ar[d]^\simeq \\
              \sum_{e : E} \coprod_{i : I} B'[r\,e/d] \ar[r] \ar[d] \pb & \sum_{d : D} \coprod_{i : I} B' \ar[d] \\
              E \ar[r] & D
            } \]
...
Vertical maps in the first row are equivalences since $B \simeq \Sigma_{d : D} B'$ and $r^*(B) \simeq B'[r\,e/d]$.
\end{proof}
...
\end{prop}
\begin{proof}
The equivalence between \eqref{it:fib-non-dep} and \eqref{it:fib-dep} holds by \rprop{fib-dep-unst}.
