...
\begin{prop}
If a theory has $\Sigma$-types, extensional identity types, and extensional products, then it has weak unstable extensional dependent products.
\end{prop}
\begin{proof}
Let $\Gamma, i : I \mid \Delta \vdash B \ob$ be an indexed type.
If $\Delta$ is the empty context, then $\prod_{i : I} B$ is the usual weak product.
...
We define $\Gamma \mid x : A \vdash \prod_{i : I} B$ as follows:
\[ \Gamma \mid x : A \vdash \sum_{f : \prod_{i : I} \Sigma_{x : A} B_i} \Id_{A^I}(\lambda i.\,\pi_1(f\,i), \lambda i.\,x). \]

Let $\Gamma, i : I \mid x : A \vdash b : B$ be a term.
Then we define $\Gamma \mid x : A \vdash \lambda i.\,b : \prod_{i : I} B$ as follows:
\[ \Gamma \mid x : A \vdash (\lambda i.(x,b),h), \]
where $h$ is the homotopy $\lambda i.\,\pi_1((\lambda i.(x,b))\,i) \sim \lambda i.\,\pi_1(x,b) = \lambda i.\,x$.
This homotopy exists since products are extensional.
If $\Gamma \mid x : A \vdash p : \prod_{i : I} B$ and $\Gamma \vdash j : I$, then we define $\Gamma \mid x : A \vdash p\,j : B[j/i]$ as follows:
\[ \Gamma \mid x : A \vdash h'_*(\pi_2(\pi_1(p)\,j)) : B_j, \]
where $h'$ is the homotopy $\pi_1(\pi_1(p)\,j) \sim (\lambda i.\,\pi_1(\pi_1(p)\,i))\,j \sim (\lambda i.\,x)\,j \sim x$.
The second homotopy is $\pmap(f.\,f\,j, \pi_2(p))$ and the other two homotopies are just propositional $\beta$-equivalences.
...
\end{align*}
\emph{Functional extensionality} for (weak) function types and $\Pi$-types is defined as usual.
...
Then the following conditions are equivalent:
\begin{enumerate}
\item Weak unstable extensional $\Pi$-types $\Pi_{x : A} B$ exist for all families $\Gamma \mid \Delta, x : A \vdash B \ob$.
\item Weak unstable extensional function types $A \to B$ exist for all types $\Gamma \mid \Delta \vdash B \ob$.
\item $A$ is exponentiable.
\end{enumerate}
If these conditions holds, then $\Pi$-types $\Pi_{x : A} B$ are stable if and only if function types $A \to B$ are stable.
\end{prop}
\begin{proof}
Obviously, if weak unstable (resp., stable) $\Pi$-types $\Pi_{x : A} B$ exist, then weak unstable (resp., stable) function types $A \to B$ also exist.
Let us prove that the existence of weak stable extensional function types $A \to B$ implies the existence of weak stable extensional $\Pi$-types $\Pi_{x : A} B$.
...
\[ (\lambda x.(x,b), \fs{funext}(x.\,\pmap(p.\,\pi_1(p), \beta(x.(x,b),x)))). \]
If $\Gamma \mid \Delta \vdash p : \Pi_{x : A} B$ and $\Gamma \mid \Delta \vdash a : A$, then we define $f\,a$ as follows:
\[ (\sym{\beta(x.\,\pi_1\,(\pi_1(p)\,x),a)} \ct \pmap(f.\,f\,a,\pi_2(p)) \ct \beta(x.x,a))_*(\pi_2(\pi_1(p)\,a)). \]
\end{proof}
...
(\ref{it:fib-dep} $\Rightarrow$ \ref{it:fib-dep-local}) Obvious.

(\ref{it:fib-dep-local} $\Rightarrow$ \ref{it:fib-non-dep})
