...
Ordinary homotopy type theory is an internal language of $\infty$-categories with some additional structure dependeing on constructions that we assume in the theory.
...
For example, we could try to add a universe $\mathcal{U}_\mathrm{cov}$ of discrete Segal types and covariant maps between them to the theory decsribed in \cite{riehl-dhott}.
...
Indexed unary type theories allow us to discuss properties of aribtrary categories.
...
\begin{enumerate}
\item Finite limis in unary theories and $\Sigma$-types, unit types, and identity types in dependent theories.
