...
Then the theory has weak stable extensional $\Pi$-types $\Pi_{x : A} B$ for all dependent types $B$ if and only if it has weak stable extensional function types $A \to B$ for all types $B$.
\end{prop}
\begin{proof}
The ``only if'' direction holds for obvious reasons.
Let us prove the converse.
