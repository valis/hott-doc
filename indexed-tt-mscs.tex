...
Since $\fs{linv}(f)$ and $\fs{rinv}(f)$ are fibers of these maps over the identity morphisms, \cite[Theorem~4.2.3 and Theorem~4.2.6]{hottbook} imply that these types are contractible.
...
For every sufficiently nice model of the base theory, there is a model of the indexed theory with zero types.
Namely, if the model is constructed out of a Cartesian model category, then we can interpret indexed types over $\Gamma$ as pointed types over $\Gamma$ in which the point is a cofibration.
...
This base type is pointed with the base point given by the zero map, that is the map $S^0 \to 0 \to A$, where $0$ is the zero type.
...
Thus, by the definition of pullbacks, we have the following equivalence:
\begin{align*}
...
A \emph{product} of an indexed type $\Gamma, i : I \mid \cdot \vdash B \ob$ is an indexed type $\Gamma \mid \cdot \vdash P \ob$ together with a term $\Gamma, i : I \vdash \pi_i : \Hom(P,B)$
such that the function $\pi_i \circ -$ has an inverse in the sense that there is a rule of the form
\begin{center}
...
\begin{align*}
\Gamma, i : I & \vdash \Id(\pi_i \circ \langle f \rangle_{i : I}, f), \\
\Gamma & \vdash \Id(\langle \pi_i \circ g \rangle_{i : I}, g).
\end{align*}

The theory of coproducts is defined dually.
A \emph{coproduct} of an indexed type $\Gamma, i : I \mid \cdot \vdash B \ob$ is an indexed type $\Gamma \mid \cdot \vdash C \ob$ together with a term $\Gamma, i : I \vdash \fs{in}_i : \Hom(B,C)$
such that the function $- \circ \fs{in}_i$ has an inverse in the sense that there is a rule of the form
\begin{center}
...
and the following types are inhabited:
\begin{align*}
\Gamma, i : I & \vdash \Id([ f ]_{i : I} \circ \fs{in}_i, f) \\
\Gamma & \vdash \Id([ g \circ \fs{in}_i ]_{i : I}, g).
\end{align*}

If the $\Pi$-type $\Pi_{i : I} \Hom(P',B)$ exists for all indexed types $\Gamma \mid \cdot \vdash P' \ob$, then a pair $(P, \pi_i)$ is a product of a family $B$ if and only if the following function is an equivalence for every indexed type $P'$:
\[ \lambda h.\,\lambda i.\,\pi_i \circ h : \Hom(P',P) \to \prod_{i : I} \Hom(P',B). \]
Dually, if the $\Pi$-type $\Pi_{i : I} \Hom(B,C')$ exists for all indexed types $\Gamma \mid \cdot \vdash C' \ob$, then a pair $(C, \fs{in}_i)$ is a coproduct of a family $B$ if and only if the following function is an equivalence for every indexed type $C'$:
\[ \lambda h.\,\lambda i.\,h \circ \fs{in}_i : \Hom(C,C') \to \prod_{i : I} \Hom(B,C'). \]
...
Explicitly, this means that we have the following rules:
\begin{center}
\AxiomC{$\Gamma, i : I \vdash p : \Id_B(\pi_i \circ f, \pi_i \circ g)$}
\UnaryInfC{$\Gamma \vdash \fs{funext}(i.p) : \Id_{\Hom(P, \prod_{i : I} B)}(f,g)$}
\DisplayProof
\end{center}
\medskip

\begin{center}
\AxiomC{$\Gamma \vdash p : \Id_{\Hom(P, \prod_{i : I} B)}(f, g)$}
\UnaryInfC{$\Gamma \mid \Delta \vdash \fs{funext_h}(p) : \Id(\fs{funext}(i.\,\pmap(f.\, \pi_i \circ f, p)), p)$}
\DisplayProof
\end{center}
\medskip
...
\begin{center}
\AxiomC{$\Gamma, i : I \vdash p : \Id_B(\pi_i \circ f, \pi_i \circ g)$}
\AxiomC{$\Gamma \mid \Delta \vdash j : I$}
\BinaryInfC{$\Gamma \mid \Delta \vdash \fs{funext_h'}(i.p,j) : \Id(\pmap(f.\, \pi_j \circ f, \fs{funext}(i.p)), p[j/i])$}
\DisplayProof
\end{center}
...
The difference between products and strict products is that the former requires types $\Hom(P',\prod_{i : I} B)$ and $\Pi_{i : I} \Hom(P',B)$ to be equivalent while the latter requires them to be \emph{isomorphic}.
...
If $\Gamma \mid x : A \vdash f : \prod_{i : I} B$ and $\Gamma \vdash j : I$, then we define $f\,j$ as $\pi_j\,f$.
The $\beta$ and $\eta$ equations hold for these definitions:
\begin{align*}
& \lambda x.\,(\lambda i.b)\,j = \lambda x.\,\pi_j\,(\langle \lambda x.b \rangle_{i : I}\,x) = (\pi_i \circ \langle \lambda x.b \rangle_{i : I})[j/i] \sim (\lambda x.b)[j/i] = \lambda x.\,b[j/i] \\
& \lambda x.\,\lambda i.\,f\,i = \lambda x.\,\langle \lambda x.\,\pi_i\,f \rangle_{i : I}\,x = \langle \lambda x.\,\pi_i\,f \rangle_{i : I} = \langle \pi_i \circ (\lambda x.f) \rangle_{i : I} \sim \lambda x.f
\end{align*}

Now, suppose that $\prod_{i : I} B$ is a weak product.
We define
\[ \Gamma, i : I \vdash \pi_i : \Hom(\prod_{i : I} B, B) \]
as $\lambda f.\,f\,i$.
...
\begin{align*}
& \pi_i \circ \langle g \rangle_{i : I} = \lambda x.\,(\lambda i.\,g\,x)\,i \sim \lambda x.\,g\,x = g \\
& \langle \pi_i \circ g \rangle_{i : I} = \lambda x.\,\lambda i.\,g\,x\,i \sim \lambda x.\,g\,x = g
\end{align*}
\end{proof}
...
The following proposition shows that the (co)product of a contractibly-indexed family of types is any type of this family:
...
Clearly, this is an inverse to $\pi_i \circ -$.
Since $(B[i_0/i],\pi_i)$ is a product and products are unique up to equivalence, it follows that $B[i_0/i]$ is equivalent to $\prod_{i : I} B$.
...
If we want to work with more complicated constructions which are not stable under pullbacks such as pushouts, then the type former itself should also be restricted to the empty context.
...
It allows us to define the following analogues of transport and $\pmap$:
\begin{center}
\AxiomC{$\Gamma \vdash p : \Id_A(a,a')$}
\AxiomC{$\Gamma \mid \Delta \vdash$}
\AxiomC{$\Gamma, x : A \mid \Delta \vdash D \ob$}
\AxiomC{$\Gamma \mid \Delta \vdash d : D[a/x]$}
\QuaternaryInfC{$\Gamma \mid \Delta \vdash p_*(d) : D[a'/x]$}
\DisplayProof
\end{center}

\[ p_*(d) = J(a, x p \Delta. D, d, a', p) \]
\medskip

\begin{center}
\AxiomC{$\Gamma \vdash p : \Id_A(a,a')$}
\AxiomC{$\Gamma \mid \Delta \vdash B \ob$}
\AxiomC{$\Gamma, x : A \mid \Delta \vdash b : B$}
\TrinaryInfC{$\Gamma \mid \Delta \vdash \pmap(x.b, p) : \Id_B(b[a/x], b[a'/x])$}
\DisplayProof
\end{center}

\[ \pmap(x.b, p) = J(a, x p \Delta. \Id_B(b[a/x], b), \refl(b[a/x]), a', p) \]
...
(that is, there is a forgetful functor from the category of models of an indexed dependent theory $T$ to the category of models of the underlying indexed unary theory of $T$).
...
A standard way to construct a quasicategory out of a homotopical category is to apply the simplicial localization functor, the fibrant replacement functor, and the simplicial nerve functor.
...
It is straightforward to check that if $\Hom$-types are extensional, then (weak) dependent $\Hom$-types are also extensional.
That is, the following function is an equivalence:
\[ \lambda p.\,\lambda \overline{x}.\,\fs{hap}(p, \overline{x}) : \Id_{\Hom(\overline{x}.B)}(f,g) \to \Hom(\overline{x}.\,\Id_B(f\,\overline{x},g\,\overline{x})). \]
...
We will say that two dependent types $\Delta \vdash A$ and $\Delta' \vdash A'$ are equivalent if there is an equivalence of contexts $\Delta$ and $\Delta'$ which extends to an equivalence of contexts $\Delta, x : A$ and $\Delta', x : A'$.
...
Now, for every closed indexed type $C$ and every pair $x,y : C$, we can define the type of morphisms of $C$ between $x$ and $y$ as $\Sigma_{f : \Delta^1 \to C} \Id(f\,l, x) \times \Id(f\,r, y)$.
We will denote this type by $\chom_C(x,y)$.
Now, we can define the following predicate:
\begin{align*}
& \fs{isTerm} : \Ob(C) \to \fs{Prop} \\
& \fs{isTerm}(x) = \Pi_{y : \Ob(C)} \fs{isContr}(\Ob(\chom_C(y,x)))
\end{align*}
...
\begin{example}
Both models $\qCat$ and $\sSpace$ satisfy the equivalent conditions of \rprop{initial}.
\end{example}

A theory with a zero type cannot satisfy the conditions of \rprop{initial} unless it is trivial (that is, unless all indexed types are equivalent in it).
...
\begin{prop}
Let $0$ be a type in an indexed dependent type theory with identity types, $\Sigma$-types, and extensional $\Hom$-types.
Then $0$ is initial in the underlying indexed unary type theory if and only if the following rule is derivable:
\begin{center}
...
The following proposition shows that stability under substitution implies stability under pullbacks.
The converse of this proposition does not hold because stability under pullbacks implies that coproducts are stable under substitution only up to equivalence.
...
See \cite[Example~5.16]{indexed-models}.
\end{example}
...
\begin{align*}
& \lambda e. \amalg\text{-}\fs{rec} \left(\sum_{d : D} B \amalg_A C, (d,y).(d,\fs{inl}(y)), (d,z).(d,\fs{inr}(z)), (d,x).d_3, e \right) : \\
& \quad \Hom_\Delta \left( \left(\sum_{d : D} B \right) \amalg_{(\sum_{d : D} A)} \left(\sum_{d : D} C \right), \sum_{d : D} B \amalg_A C \right),
\end{align*}
where $d_3 = (d,\pmap(w.(d,w),\fs{glue}(x)))$ and $\amalg\text{-}\fs{rec}$ is the recursor for pushouts, that is a non-dependent version of the eliminator.
\end{prop}
\begin{proof}
The inverse of this map is defined as follows:
\[ \lambda (d,e). \amalg\text{-}\fs{rec} \left(\left(\sum_{d : D} B \right) \amalg_{(\sum_{d : D} A)} \left(\sum_{d : D} C \right), y.\,\fs{inl}(d,y), z.\,\fs{inr}(d.z), x.\,\fs{glue}(d,x), e \right). \]
It is straightforward to check that these maps are mutually inverse using the eliminator for pushouts.
\end{proof}
...
To do this, we can assume that $\id$ is homotopic to $\lambda x.g$.
It follows that $x$ is homotopic to $g$ for every $x : \Hom(0,X)$.
That is, $\Hom(0,X)$ is contractible.
Since powers are extensional, functional extensionality holds for the type $\Hom(0,X) \to \Hom(0,X)$.
It follows that $\Hom(0,X) \to \Hom(0,X)$ is also contractible and hence a proposition.
...
A map is $(-2)$-truncated if and only if it is an equivalence.
This fact and \rlem{trunc-id} define a predicate on maps (by induction on $n$) that corresponds to the notion of $n$-truncated maps.
...
More examples of classes of fibrations in this model can be found in \cite{fib-inf-cat}.
\end{example}

For every dependent class of fibrations $\Fib$ (closed under pullbacks), we can add a new sort of dependent types $\Gamma \mid \Delta \vdash A \fib$ consisting of types satisfying the predicate $\Fib$:
\begin{center}
...
For example, we can assume that $\Fib$ contains contractible types.
This is true if and only if it contains all identity morphisms.
Similarly, $\Fib$ contains $n$-types (as a dependent class) if and only if it contains $n$-truncated maps (as a non-dependent class).
...
\begin{example}
We believe that the classes of small fibrations mentioned in \rexample{qcat-fib} should be classified by an object classifier.
...
An analogous result in the context of higher categories was proved in \cite[Corollary~3.10]{univalence-lccc} and \cite[Theorem~3.28]{rasekh-eht}.
...
\begin{example}[spectra-int-universe]
The theory of spectra does not have interesting object classifiers (and hence internal universes).
...
\Rexample{spectra-int-universe} shows that internal universes may not exist in some interesting theories.
To fix this problem, we will define the notion of an \emph{external universe}.
In an indexed unary type theory, an external universe consists of a base type $\mathcal{U}_A$ defined for every closed indexed type $A$ together with an indexed type $\El(c)$ and a map $p(c) : \Hom(\El(c),A)$ defined for every $c : \mathcal{U}_A$.
...
\DisplayProof
\end{center}
Indeed, $\El(c,a)$ can be defined as the fiber of $p(c)$ over $a$.
Conversely, $p(c)$ can be defined as $\pi_1 : \Hom(\Sigma_{x : A} \El(c,x), A)$.
...
\begin{defn}
We will say that a class $\Fib$ is \emph{locally small} if it is classified by a univalent external universe.
\end{defn}
...
Any locally reflective class of fibrations closed under pullbacks is closed under identity types.
\end{prop}
\begin{proof}
...
In this subsection, we defined connected maps and orthogonal factorization systems and prove that they are equivalent to locally reflective classes of fibrations closed under composition and pullbacks (see \rthm{refl-orth}).
...
\bibitem[Carboni \emph{et al.} 1997]{cjkp-loc}
A.~Carboni, George Janelidze, G.~M. Kelly, and R.~Par{\'{e}}, \emph{On
  localization and stabilization for factorization systems}, Applied
  Categorical Structures \textbf{5} (1997), no.~1, 1--58.
...
\bibitem[Gepner and Kock 2012]{univalence-lccc}
David {Gepner} and Joachim {Kock}, \emph{{Univalence in locally cartesian
  closed infinity-categories}},  (2012), \href {http://arxiv.org/abs/1208.1749}
  {\path{arXiv:1208.1749}}.
...
\bibitem[Schreiber 2014]{schreiber-linear}
U.~{Schreiber}, \emph{{Quantization via Linear homotopy types}},  (2014), \href
  {http://arxiv.org/abs/1402.7041} {\path{arXiv:1402.7041}}.
