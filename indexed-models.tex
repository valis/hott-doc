...
In this section, we describe two examples of contextually indexed contextual categories, based on quasicategories and on (complete) Segal objects.
...
\item $X$ has the right lifting propert with respect to the map $\Delta^1 \to \bDelta^1$.
\item $G(X) = X$.
...
\end{prop}
\begin{proof}
Let $J$ be the set of maps of the form $b(\Lambda^n_k) \to b(\Delta^n)$.
...
It is enough to prove that $Z \to Y$ has the right lifting property with respect to the map $b(\Lambda^n_k) \to D$.
If $n > 2$, then $D = b(\Delta^n)$ and $Z \to Y$ has this property by the construction.
If $n < 2$, then the map $D \to Y$ factors through the map $D \to b(\Delta^n)$ since every edge of $Y$ is invertible.
...
Now, we are ready to prove that $\Id^*(\sSet_J)$ has various categorical constructions:
...
Then the contextually indexed contextual category $\Id^*(\sSet_J)$ is locally small and Cartesian closed.
...
\end{thm}
\begin{proof}
First, let us show that $\Id^*(\sSet_J)$ has $\Pi$-types in the empty context.
...
The fact that $\Id^*(\sSet_J)$ has dependent $\Hom$-types, $\Sigma$-types, unit types, and extensional identity types follows from \rexample{cisinski}.
