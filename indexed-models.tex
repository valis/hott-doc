...
for every fibration $p : A \twoheadrightarrow \Gamma \times V$, the map $\Pi_{\pi_1}(p) \to \Pi_{\pi_1}(q)$ is a fibration,
where $q : A' \twoheadrightarrow \Gamma \times U$ is the pullback of $p$ along the map $\id_\Gamma \times j : \Gamma \times U \to \Gamma \times V$.

\begin{remark}
A category with fibrations $\scat{C}$ satisfies the pushout-product axiom with respect to a map $j : U \to V$ if the class of fibrations of $\scat{C}$ is saturated, objects $U$ and $V$ are exponentiable and,
for every trivial cofibration $i : A \to B$, the map $B \times U \amalg_{A \times U} A \times V \to B \times V$ is a trivial cofibration.
This conditions hold if $\scat{C}$ is a Cartesian closed model category (\rremark{cartesian}) and $j$ is a cofibration.
\end{remark}
...
If moreover $U = \fs{colim}_{i \in \scat{I}} U_i$ and functors $X \times -$ preserve this colimit for every object $X$, then $\scat{C}$ has the modified version of extension types base on this decomposition of $U$.
\end{prop}
\begin{proof}
% TODO
\end{proof}
...
If $\scat{C}$ is a locally Cartesian closed right proper type-theoretic model category which has good cylinder objects, then it satisfies the conditions of \rlem{pushouts}.
\end{lem}
\begin{proof}
