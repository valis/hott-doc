...
For every pair $(\psi,V)$ in $P_M$, if $\psi \sststile{T_0}{V} t\!\downarrow$, then $\psi \sststile{T_0}{V} \varphi_{t,s}$.
...
For example, consider the following reduction rule (this is a part of the theory of $\Pi$-types):
...
To check that this condition holds for this example, we need to prove the main property of confluent theories:
...
We will sketch a proof of this fact for the theory of $\Pi$-types.
We need to show that $\psi \sststile{}{V} t\!\downarrow$ implies $\psi \sststile{}{V} \varphi_{t,s}$.
The reduction relation in the theory of $\Pi$-types is strongly normalizable (that is, the relation $R_0$ is well-founded).
We proceed by induction on the length of the longest chain of reductions starting in $t$.
...
% TODO: We cannot apply it to $ty(b)$ :(

Now, let us discuss the second condition of \rdefn{confluent}.
We could just require the relation $R_0$ to be confluent, but in this case we can apply \rprop{conf-main} only to sequents of the form $\sststile{}{} t = s$, but we need to apply it to sequents with a more general left hand side.
The second condition of \rdefn{confluent} guarantees that we can do that.
Moreover, it often follows from the assumption that $R_0$ is confluent:
...
\begin{comment}
\subsection{Confluent Morita equivalence}

The first class of examples consists of Morita equivalences which are confluent.
If we have a confluent map $f : T_1 \to T_2$, then the second condition of \rlem{eq-char-eq} is true.
Thus we just need to check the first condition.
The conditions of this lemma are sufficient, but not necessary; so it might be the case that the first condition does not hold, but if it does, then it is easy to verify it.

For example, if we have a theory $T$ which contains a contractible type $C$, then we can extend this theory adding any number of terms of the type $C$ and any number of reduction rules on these terms.
Indeed, we can lift new terms to the center of contraction of $C$.
Since $C$ is contractible, the new terms are homotopic to the center of contraction; so the first condition of \rlem{eq-char-eq} holds.
If the resulting theory is confluent, then the second condition also holds and this extension is a Morita equivalence.
The main point of this example is that we can add new constructions with strict equalities on them.
For example, we can add a strictly associative binary operator $\sq$ on the interval to obtain a theory that contains the theory of cubical sets with connections:
\begin{align*}
\sq(\leftI,j) & \Rightarrow \leftI \\
\sq(\rightI,j) & \Rightarrow j \\
\sq(i,\leftI) & \Rightarrow \leftI \\
\sq(i,\rightI) & \Rightarrow i \\
\sq(\sq(i,j),k) & \Rightarrow \sq(i,\sq(j,k))
\end{align*}

Let us give another example of a confluent Morita equivalence.
If $T$ is a theory with identity types, then we can define an operation of path concatenation which is associative only up to homotopy.
We can add a new such operation to $T$ with the strict associativity rules:
\begin{align*}
\conc(x,y,y,p,\refl(y)) & \Rightarrow p \\
\conc(x,z,w,\conc(x,y,z,p,q),r) & \Rightarrow \conc(x,y,w,p,\conc(y,z,w,q,r))
\end{align*}
If the resulting theory is confluent, then this extension is a Morita equivalence since any to concatenation functions are homotopic.

\subsection{Injectivity of function symbols}

Sometimes we need to prove that some function symbols are injective.
As an example consider a theory $T$ with $\Sigma$-types with projections that has the following reduction rules:
\begin{align*}
\pi_1(B,\pair(B,a,b)) & \Rightarrow a \\
\pi_2(B,\pair(B,a,b)) & \Rightarrow b
\end{align*}
We can define another theory $T'$ that has another set of reduction rules:
\begin{align*}
\pi_1(B',\pair(B,a,b)) & \Rightarrow a \\
\pi_2(B',\pair(B,a,b)) & \Rightarrow b
\end{align*}
There is an obvious map $T \to T'$, but there is no map in the other direction since $\Sigma(B) = \Sigma(B')$ does not imply $B = B'$; we need to know that $\Sigma$ is injective in some sence.
If the theory $T'$ is confluent and there are no reduction rules of the form $\Sigma(t) \Rightarrow s$, then this map is a Morita equivalence.
To prove this, it is enough to show that every sequent which is derivable in $T'$ is also derivable in $T$.
To do this, it is enough to show that the reduction rules of $T$ apply whenever the reduction rules of $T'$ do.
Indeed, both of the rules of $T'$ apply only if $\Sigma(B) = \Sigma(B')$, which implies that there is a term $C$ such that $\Sigma(B) \Rightarrow^* C$ and $\Sigma(B)' \Rightarrow^* C$.
Since there are no rules of the form $\Sigma(t) \Rightarrow s$, term $C$ must be of the form $\Sigma(B'')$ and both $B$ and $B'$ must reduce to $B''$.
This implies that $B = B'$, so the rules of $T$ apply.
