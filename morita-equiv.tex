...
Let $(\varphi,V)$ be a pair in $P_M$.
Suppose that the abstract reduction system $(\Term_{T,\varphi}^s, \Rightarrow_{sf} \Rightarrow_s^\nf)$ is confluent.
Then, for every derivable sequent $\varphi \sststile{}{V} t = s$, the terms $t$ and $s$ are joinable under the relation $\Rightarrow_{R_T,\varphi}$.
\end{thm}
...
Let $\mathcal{F}_0'$ be the set consisting of elements of $\mathcal{F}_0$, function symbols $v_i$ for all $i \in \mathbb{N}$, and function symbols $\subst_{p,m,k}$ for all $k > 0$.
...
\begin{align*}
\subst_{p,n,k}(v_i, a_1, \ldots a_n) & \Rightarrow a_{n-i} \\
\subst_{\tm,n,n}(a, v_{n-1}, \ldots v_0) & \Rightarrow a \\
\subst_{p,n,m}(\sigma(b_1, \ldots b_k), a_1, \ldots a_m) & \Rightarrow \sigma(b_1', \ldots b_k') \\
...
\[ \subst_{p_i,n+n_i,m+k}(b_i, \wk^{n_i}_{\tm,n}(a_1), \ldots \wk^{n_i}_{\tm,n}(a_m), v_{n_i-1}, \ldots v_0), \]
where $\wk^k_{p,n}(a) = \subst_{p,n+k,n}(a, v_{n+k-1}, \ldots v_k)$ if $n > 0$ and $\wk^k_{p,0}(a) = a$.
