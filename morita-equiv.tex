...
% Сказать про гипотезу, что модели HoTT = категориям.
% Сказать, что логично было бы посравнивать и модели различных теорий.

% Сказать, что еще одна причина заключается в том, что существуют примеры теорий, которые должны быть эквивалентны в каком-то смысле, но мы даже не можем проинтерпретировать одну в другой.
% Привести несколько примеров: стягиваемые и юнит-типы, два вида сигма-типов, пи-типы и типы независимых функций, кофибрантные замены, вселенные, теория с интервалом и без.

% Во всех этих примерах мы можем проинтерпретировать одну теорию в другой, но не наоборот, что показывает, что эквивалентность теорий должна быть слабой эквивалентностью.
% Чтобы определить их, для начала нам нужна категория теорий типов.
% Определение такой категории было предложено в \cite{alg-tt}.
% В этой статье мы определим понятие Морита эквивалентностей таких теорий и более слабое понятие синтаксических эквивалентностей.

% Сказать, что существует отличный кандидат на роль модельной структуры на категориях моделей.
% Если у нас есть интервал, то модельная структура на моделях всегда существует.
% Мы покажем, что в этом случае можно определить модельную категорию теорий.

% Сказать, что мы определяем Морита эквивалентность как мапу $f : T_1 -> T_2$, такую что единица $\eta_X : X \to f^*(f_!(X))$ порожденного ей сопряжения $f_! \dashv f^* : \Mod{T_2} \to \Mod{T_1}$ является слабой эквивалентностью в этой модельной структуре для любого корасслоенного $X$.
% Если модельная структура существует, то это верно тогда и только тогда, когда сопряжение является Квиллен эквивалентностью.
% Мапа является синтаксической эквивалентностью, если начальные модели теорий эквивалентны, точнее если уникальный морфизм $0 \to f^*(0)$ является слабой эквивалентностью.
% Один из ранних примеров синтаксических эквивалентностей, появлявшихся в литературе, -- это эквивалентность экстенсиональной и интенсиональной теорий типов в тезисе Хофманна.

% Мы также дадим характеризацию обоих видов слабых эквивалентностей исключительно в синтаксических терминах.
% Это наиболее удобная характеризация для проверки того, что конкретные примеры морфизмов являются эквивалентностями.
% Для работы с таким определением удобно предполагать, что теории являются конфлюэнтными.
% Условно это означает, что можно выбрать направление для аксиом теории таким образом, чтобы получившееся отношение стало конфлюэнтным.
% Мы дадим формальное определение этого понятия в сеттинге алгебраических теорий типов.

% К сожалению, нам не удалось на данный момент доказать, что многие приведенные ранее примеры являются Морита (или даже синтаксическими) эквивалентностями.
% Тем не менее, мы докажем, что пример со стягиваемым типом и юнит-типом является, и приведем еще несколько простых примеров.
...
Then condition~\eqref{it:dir-first} of \rdefn{directed} implies that $\varphi \sststile{T}{V} t = s$.

If $t$ and $s$ are terms such that $\varphi \sststile{T}{V} t = s$, then \rlem{der-eq} and \rprop{der-separated} imply that
...
Condition~\eqref{it:dir-second} of \rdefn{directed} implies that $t_i$ and $t_{i+1}$ are equivalent in the system $(\Term_{T,\varphi}^d,\Rightarrow_{T,\varphi})$.
\end{proof}
...
A \emph{confluent} type theory is a type theory $T$ with a reduction system such that equivalent conditions of \rcor{conf-main} hold for every pair $(\varphi,V)$.
\end{defn}
...
By \rprop{der-separated}, it is also derivable in $T_1$.
\end{example}
...
\begin{center}
\AxiomC{}
\UnaryInfC{$\Gamma \vdash c_0 : C$}
\DisplayProof
\qquad
\AxiomC{$\Gamma \vdash c : C$}
...
Suppose that $T$ is a theory under $\coeT_1 + \sigma + \PathT$ with well-defined function symbols which satisfies the conditions of \rlem{context}.
Then the first condition of \rlem{eq-char-fib} holds for the map $T \amalg T_0 \to T \amalg T_2$.
Thus, to prove that it is a Morita equivalence, it is enough to check that it satisfies the condition of \rlem{char-type-fib}.
...
By \rlem{context}, it is enough to prove the following facts:
\begin{enumerate}
...
\Rlem{context} implies that this is also true for the reduction system $\Rightarrow_{T \amalg T_0, \varphi}^c$
...
For example, if $T$ is defined in terms of a term rewriting system $R_T$ as in \rremark{trs-theory}, then we just need to assume that, for every reduction rule $(t,s) \in R_T$,
