...
\end{prop}
\begin{proof}
If $\Rightarrow_{tf}$ is confluent on $\Term_{T,\varphi}^t$, then it is easy to show that the same is true for $\Term_{T,\varphi}^c$.
...
Then the abstract reduction system $(\Term_{T,\varphi}^c,\Rightarrow_s)$ is confluent and weakly normalizing for every $(\varphi,V) \in P_M$.
\end{lem}
\begin{proof}
Let us prove that $(\Term_{T,\varphi}^c,\Rightarrow_s)$ is weakly normalizing.
First, let us prove that every term of the form $\subst_{p,n,k}(a, v_{i_1}, \ldots v_{i_k})$ is weakly normalizable for every normal form $a$ by induction on it.
If $a = v_i$ or $a = x$, then this is obvious.
If $a = \subst_{p,k,m}(a', a_1, \ldots a_m)$, then $a'$ is a variable since $a$ is a normal form.
Then $\subst_{p,n,k}(a, v_{i_1}, \ldots v_{i_k}) \Rightarrow_s \subst_{p,n,m}(a', a_1', \ldots a_m')$, where $a_i' = \subst_{\tm,n,k}(a_i, v_{i_1}, \ldots v_{i_k})$.
So $a_i'$ are weakly normalizable by the induction hypothesis and if $a_i''$ is a normal form of $a_i'$, then $\subst_{p,n,m}(a', a_1'', \ldots a_m'')$ is a normal form of $\subst_{p,n,k}(a, v_{i_1}, \ldots v_{i_k})$.
If $a = \sigma_k(b_1, \ldots b_m)$, then $\subst_{p,n,k}(a, v_{i_1}, \ldots v_{i_k}) \Rightarrow_s \sigma_n(b_1', \ldots b_m')$,
where $b_i'$ is a term such that $b_i' \Rightarrow_s \subst_{p_i,n+n_i,k+n_i}(b_i, v_{i_1'}, \ldots v_{i_{k+n_i}'})$ for some $i_1'$, \ldots $i_{k+n_i}'$.
Thus $b_i'$ is normalizable by the induction hypothesis.

Now, we can prove a stronger statement.
If $a$ is a normal form and $a_1$, \ldots $a_k$ are normalizable, then $\subst_{p,n,k}(a, a_1, \ldots a_k)$ is also normalizable.
The proof is done by induction on $a$ similarly to the previous one.
The only interesting case is when $a = \sigma_k(b_1, \ldots b_m)$.
Then $\subst_{p,n,k}(a, a_1, \ldots a_k) \Rightarrow_s \sigma_n(b_1', \ldots b_m')$, where $b_i' = \subst_{p_i,n+n_i,k+n_i}(b_i, \wk^{n_i}_{\tm,n}(a_1), \ldots \wk^{n_i}_{\tm,n}(a_k), v_{n_i-1}, \ldots v_0)$.
Since $a_j$ is normalizable, $\wk^{n_i}_{\tm,n}(a_j)$ is also normalizable.
The induction hypothesis implies that $b_i'$ is also normalizable.

Finally, we can show that every term is normalizable by induction on it.
The only nontrivial case is when the term is of the form $\subst_{p,n,k}(a, a_1, \ldots a_k)$.
Then $a$, $a_1$, \ldots $a_k$ are normalizable by the induction hypothesis.
If $a'$ is a normal form of $a$, then $\subst_{p,n,k}(a, a_1, \ldots a_k) \Rightarrow_s \subst_{p,n,k}(a', a_1, \ldots a_k)$ and the latter term is normalizable as we just proved.

Now, let us prove that $(\Term_{T,\varphi}^c,\Rightarrow_s)$ is confluent.
\end{proof}
