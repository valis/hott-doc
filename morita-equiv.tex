...
% TODO: blah-blah % Let us prove a technical lemma
...
% TODO: We didn't define confluent theories yet % The main property of confluent term rewriting systems theories is proved in the following proposition:

\begin{prop}[conf-main]
Let $T_0$ be a theory with directed axioms.
...
Let $(\varphi,V)$ be a pair in $P_M$ and let $\varphi \sststile{T_0}{V} t = s$ be a derivable sequent.
...
Moreover, any pair of equivalent elements of $A$ are joinable under $\Rightarrow_{R_1}^{nf} \Rightarrow_{R_0}^*$.
\end{lem}
\begin{proof}
...
Let us begin by showing that a term rewriting system is confluent whenever the subsystem on terms without $ty$ is.
To do this we need to know that reduction rules preserve types.
This condition is formalized in the following definition:
\begin{defn}
Let $T$ be a theory with directed axioms and let $R$ be its underlying term rewriting system.
We will say that the rules of $R$ \emph{preserve types} if there is a well-founded relation on the set of axioms of $T$ such that,
for every axiom $\varphi \sststile{}{V} t = s$ in $\mathcal{A}_e$ such that $t$ and $s$ are terms of a sort $(tm,n)$,
the sequent $\varphi \sststile{}{V} ty(t) = ty(s)$ is derivable from the axioms which are less than $\varphi \sststile{}{V} t = s$.
\end{defn}

We also need to separate typing reduction rules from ordinary reduction rules:
\begin{defn}
Let $T$ be a theory with directed axioms.
We will say that \emph{typing axioms are separated} if, for every reduction rule $(r,s) \in R_T$,
the function symbol $ty$ either does not appear in $t$ or appears only on the top level.
\end{defn}

Now, we can prove that theories satisfying these technical assumptions are confluent.

\begin{prop}
Let $T$ be a theory with directed axioms in which typing axioms are separated and such that its reduction rules preserve types.
Let $R_0$ be the set of reduction rules in which $ty$ does not appear in the left hand side and let $R_1 = R_T \setminus R_0$.
Suppose that closed defined terms are confluent under $R_0$ and confluent and weakly normalizing under $R_1$.
Then every pair of closed defined terms are joinable under $\Rightarrow_{R_1}^{nf} \Rightarrow_{R_0}^*$
\end{prop}
\begin{proof}
TODO
\end{proof}
...
Let $T_0$ be a theory with directed axioms and let $T_1$ be a theory as defined in \rprop{conf-main}.
Suppose that the conditions of the previous proposition hold.
Moreover, suppose that closed defined terms are weakly normalizable under $R_1 \cup R_{T_1}$.
Then closed defined terms of $T_1$ are confluent.
\end{cor}
