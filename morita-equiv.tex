...
\begin{lem}[lift-corr]
Let $V$ be a set of variables and let $\varphi$ be a formula with free variables in $V$.
Suppose that, for every pair of terms $A$ and $B$ of $T_1$ of sort $(ty,n)$ such that
$\varphi \sststile{T_1}{} A\!\downarrow \land\ B\!\downarrow$ and $f(\varphi) \sststile{T_2}{} f(A) = f(B)$, there exists a term $H$ of sort $(ty,n+1)$
such that $\varphi \sststile{T_1}{} left^*(H) = A \land right^*(H) = B$ and $f(H)$ is homotopic to the constant homotopy on $f(A)$.

If $a$ and $A$ are terms of $T_1$ such that 
\end{lem}
\begin{proof}
The ``only if'' direction follows from \rprop{morita-char}.
Let us prove the converse.
\end{proof}

\begin{prop}
Let $V$ be a set of variables and let $\varphi$ be a formula with free variables in $V$.
A morphism $f : T_1 \to T_2$ between quotients of cofibrant theories with identity types has the weak lifting property with respect to $V,\varphi$ if and only if,
