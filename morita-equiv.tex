...
For every sequence $(p_1,n_1), \ldots (p_{k+1},n_{k+1})$ of sorts, let $T_{(p_1,n_1), \ldots (p_{k+1},n_{k+1})}$ be the theory under the theory of identity types
...
Let $f : X \to Y$ be a pushout of $\cyli_0 : T_{l,(tm,n)} \to C_{T_{l,(ty,n)}}(T_{l,(tm,n)})$ (in the category of $I$-stable theories)
...
For every theory $T$ under the theory of identity types $T_{Id}$,
let $\I_T$ be the set of maps in $T/\algtt$ of the form $i \amalg_{id_{T_{Id}}} id_T : U \amalg_{T_{Id}} T \to V \amalg_{T_{Id}} T$ for every $i : U \to V$ in $\I$.
Analogously, we define a relative cylinder object for $i \amalg_{id_{T_{Id}}} id_T$ as $C_U(V) \amalg_{T_{Id}} T$.

\begin{thm}[theories-model-structure]
Let $T$ be an $I$-stable algebraic dependent type theory with identity types such that Morita equivalences between theories under $T$ satisfy the 2-out-of-3 property.
Then there exists a model structure on the category of $I$-stable algebraic dependent type theories under $T$
with $\I_T$ as the set of generating cofibrations, Morita equivalences as weak equivalences, and in which all objects are fibrant.
\end{thm}
\begin{proof}
Note that the set $\we_{\I_T}$ consists of Morita equivalences.
Since the codomains of maps in $\I$ are finite, Morita equivalences are closed under transfinite compositions.
Thus by \rthm{model-structure}, we just need to prove that pushouts of maps
$\cyli_0 \amalg id_T : T_{l,(p,n)} \amalg T \to C_{T_{l,(d_p,n)}}(T_{l,(p,n)}) \amalg T$ are Morita equivalences.
These pushouts are just pushouts of $\cyli_0$ in $\algtt$.
...
with $\I_{T_I}$ as the set of generating cofibrations, Morita equivalences as weak equivalences, and in which all objects are fibrant.
\end{cor}
\begin{proof}
By \rprop{morita-quillen}, Morita equivalences between theories under $T_I$ satisfy the 2-out-of-3 property since Quillen equivalences do.
\end{proof}
...
There is a model structure on $\algtt_f$ in which the classes of cofibrations, fibrations, and weak equivalences
are the intersections of the corresponding classes in $\algtt$ with the class of morphisms of $\algtt_f$.
This model category has the same sets of generating cofibrations and generating trivial cofibrations as $\algtt$.

\begin{prop}
The inclusion functor $i : \algtt_f \to \algtt$ has a right adjoint and this adjunction is a Quillen equivalence.
\end{prop}
\begin{proof}
...
there is a function symbol $\sigma : s_1 \times \ldots \times s_k \to s$ such that sequent $\sststile{}{x_1, \ldots x_k} t \simeq \sigma(x_1, \ldots x_k)$ is derivable.
...
Thus we may restrict and corestrict functor $i : \algtt_f \to \algtt$ to the full subcategories of $\algtt_f$ and $\algtt$ on theories with enough function symbols.
...
Functor $i' : \algtt'_f \to \algtt'$ preserves and reflects cofibrations and weak equivalences by definition.
Thus to prove that $i' \dashv r'$ is a Quillen equivalence, we just need to show that the counit $\epsilon_T : i'(r'(T)) \to T$ is a Morita equivalence.
But this is obvious since $i'(r'(T))$ and $T$ have the same sets of terms and theorems (which do not involve predicate symbols).
\end{proof}
...
For every sequence of sorts $s_1, \ldots s_k$, let $P_{s_1, \ldots s_k}$ be the theory with one predicate symbol $R : s_1 \times \ldots \times s_k$.
...
If $T$ is a theory with identity types, then the set $\I^P_T$ is defined analogously to $\I_T$.
If $T$ satisfies the conditions of \rthm{theories-model-structure}, then
to prove that there is a model structure on $T/\algtt$ with $\I^P_T$ as a set of generating cofibrations,
...
Of course, the identity functor determine a Quillen equivalence between these to model structures on $T/\algtt$.
...
Adjunction $Lang_{T_I} \dashv Syn_{T_I}$ is a Quillen equivalence between the model structure on $\Mod{T_I}$ and the syntactic model structure on $T_I/\algtt$.
To prove this, we just need to show that the unit of the adjunction is a weak equivalence.
But it is actually an isomorphism since $Lang_{T_I}$ is full and faithful.
Finally, note that the identity functor on $T_I/\algtt$ is a left Quillen functor from the syntactic model structure to the Morita model structure
