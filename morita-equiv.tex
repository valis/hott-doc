...
Let $T$ be a theory and let $(\varphi,V)$ be a pair in $P_M$.
Then we define a relation $\Rightarrow_{T,\varphi}^c$, which is a subset of $\Rightarrow_{T,\varphi}$.
\begin{lem}
...
Thus it is enough to prove that $g_n(t) \Rightarrow_{T \amalg T_2, f(\varphi)}^c g_n(s)$ whenever $t \Rightarrow_{T \amalg T_2, f(\varphi)}^c s$.
