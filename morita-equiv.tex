...
By \rlem{der-eq} and \rprop{der-separated}, there exists a sequence $t_1$, \ldots $t_n$ such that $t' = t_1$, $s' = t_n$, and, for every $1 \leq i < n$, either $t_i \Rightarrow_{R_{T_1}} t_{i+1}$ or $t_{i+1} \Rightarrow_{R_{T_1}} t_i$.
...
Let $T_0$ be a theory with directed axioms and let $T_1$ be a theory as defined in \rprop{conf-main}.
...
We will give another set of conditions that implies the same result in appendix~\ref{sec:app-conf} (see \rcor{ya-morita-conf}).

If we want to use \rprop{conf-main}, then we need to know that the reduction rules that were constructed in this proposition are derivable.
...
closed defined terms of the theory $T_1$ defined in \rprop{conf-main} are confluent and,
...
Let us begin by showing that a term rewriting system is confluent whenever the subsystem on terms without function symbols $ty$ and $ft$ is.
...
where $\sigma$ is neither $ft$ nor $ty$ and the function symbols $ft$ and $ty$ are applied only to variables in $s$ (if they appear in this term).
An axiom $\varphi \sststile{}{V} t = s$ is a \emph{pure axiom} if the function symbols $ft$ and $ty$ do not appear in $t$ and $s$.
We will say that a theory $T$ with directed axioms has \emph{separated typing axioms} if every rule in the underlying term rewriting system of $T$ is either a typing axiom or a pure axiom.
The sets of typing and pure axioms will be denoted by $\mathcal{A}_t$ and $\mathcal{A}_p$, respectively.
The sets of reduction rules corresponding to typing and pure axioms will be denoted by $R^t_T$ and $R^p_T$, respectively.

Let $T$ be a theory with separated typing axioms.
We will say that pure axioms of $T$ \emph{preserve types} if there is a well-founded relation on the set of pure axioms of $T$ such that,
for every pure axiom $\varphi \sststile{}{V} t = s$, the sequent $\varphi \sststile{}{V} e_p(t) = e_p(s)$ is derivable from the set $\mathcal{A}_d \cup \mathcal{A}_t \cup \mathcal{A}_p'$,
where $\mathcal{A}_p'$ is the set of pure axioms which are less than $\varphi \sststile{}{V} t = s$.
\end{defn}
...
If $ty(\sigma(t_1, \ldots t_k)) \Rightarrow_{R_T^t} s(ty(t_1), \ldots ty(t_k), t_1, \ldots t_k)$, then we need to show that $|ty(\sigma(t_1, \ldots t_k))| > |s(ty(t_1), \ldots ty(t_k), t_1, \ldots t_k)|$.
Let $\alpha_i = |t_i|$ and let $\alpha$ be the maximum of $\alpha_1$, \ldots $\alpha_k$.
Then $|ty(\sigma(t_1, \ldots t_k))| = \omega^{\alpha_1 \oplus \ldots \oplus \alpha_k \oplus 1} \geq \omega^{\alpha + 1}$.
Note that $ft$ and $ty$ do not occur in the term $s$.
It follows that
\begin{align*}
|s(ty(t_1), \ldots ty(t_k), t_1, \ldots t_k)| & = \omega^{\alpha_1} n_1 \oplus \ldots \oplus \omega^{\alpha_k} n_k \oplus \alpha_1 m_1 \oplus \ldots \oplus \alpha_k m_k \oplus c \\
...
Now we can prove that the confluence under $R^p_T$ implies the confluence under $R^p_T \cup R^t_T$:

\begin{prop}
Let $T$ be a theory with well-defined typing axioms such that its pure axioms preserve types.
Suppose that closed defined terms in which $ty$ does not occur are confluent under $R^p_T$.
...
Let $\psi = \psi_1 \land \ldots \land \psi_n$ be a closed formula such that if $\psi_i$ is of the form $t_1 = t_2$, then the terms $t_1$ and $t_2$ are joinable under $\Rightarrow_{R_T^t}^{nf} \Rightarrow_{R_T^p}^*$.
We prove that, for every such formula $\psi$, whenever a sequent $\psi \sststile{}{} t = s$ is derivable from the set $\mathcal{A}_d \cup \mathcal{A}_t \cup \mathcal{A}_p'$ where $\mathcal{A}_p'$ is a subset of $\mathcal{A}_p$
the terms $t$ and $s$ are joinable under $\Rightarrow_{R_T^t}^{nf} \Rightarrow_{R_T^p}^*$.
The statement of the proposition then follows if we take $\psi = \top$ and $\mathcal{A}_p' = \mathcal{A}_p$.

We proceed by double induction on the set $\mathcal{A}_p'$ (using the well-founded relation we just described) and (the size of) the derivation of $\psi \sststile{}{} t = s$.
By \rlem{der-eq}, there is a zig-zag of reductions of the form $a \Rightarrow_{R_2} b$ between $t$ and $s$ where $a \Rightarrow_{R_2} b$ if and only if $a = c[a'[\rho]/x]$, $b = c[b'[\rho]/x]$,
and either $a' = b'$ is a subformula of $\psi$ or $\varphi \sststile{}{V} a' = b'$ is an axiom in $\mathcal{A}_t \cup \mathcal{A}_p'$.
...
By assumption, closed defined terms in which $ty$ does not occur are confluent under $R^p_T$.
It is easy to see that all closed defined terms are confluent under $R^p_T$.
Moreover, the term rewriting system $R^t_T$ is normalizing and confluent by \rlem{types-red}.
Thus we can apply \rlem{conf-nf} to show that the terms $t$ and $s$ are joinable under $\Rightarrow_{R_T^t}^{nf} \Rightarrow_{R_T^p}^*$.
To do this, we need to prove that terms of the form $c[a'[\rho]/x]$ and $c[b'[\rho]/x]$ are joinable assuming that there is an axiom $\varphi \sststile{}{V} a' = b'$ in $\mathcal{A}_t \cup \mathcal{A}_p'$.

If $\varphi \sststile{}{V} a' = b'$ is an axiom in $\mathcal{A}_t$, then this is obvious since $c[a'[\rho]/x] \Rightarrow_{R_T^p} c[b'[\rho]/x]$, so they have the same $R_T^t$-normal form.
Assume that $\varphi \sststile{}{V} a' = b'$ is an axiom in $\mathcal{A}_p'$.
Since reduction rules in $R_T^p$ do not contain function symbols $ft$ and $ty$, a reduction rule from $R_T^p$ and a reduction rule from $R_T^t$ can overlap only in a term of the form $e_p(\sigma(t_1, \ldots t_k))$ as follows:
$e_p(\sigma(t_1, \ldots t_k)) \Rightarrow_{R_T^t} u[t_1/x_1, \ldots t_k/x_k]$ for some term $u$ and $e_p(\sigma(t_1, \ldots t_k)) \Rightarrow_{R_T^p} e_p(v)$ for some term $v$.
...
we will still have a reduction of the form $c'[a'[\rho']/x] \Rightarrow_{R_T^p} c'[b'[\rho']/x]$ for some $c'$ and $\rho'$ such that $c \Rightarrow_{R_T^t}^* c'$ and $\rho \Rightarrow_{R_T^p}^* \rho'$.
...
If there is no such redex, then we are done since these terms are $R_T^t$-normal forms and are joinable under $\Rightarrow_{R_T^p}^*$ since one of them reduces to the other.
Otherwise, there is a term $c''$ such that $c'[a'[\rho']/x] = c''[e_p(a'[\rho'])/x]$ and $c'[b'[\rho']/x] = c''[e_p(b'[\rho'])/x]$.
Since pure axioms of $T$ preserve types, the sequent $\varphi \sststile{}{V} e_p(a') = e_p(b')$ is derivable from the set $\mathcal{A}_d \cup \mathcal{A}_t \cup \mathcal{A}_p''$ for some $\mathcal{A}_p'' < \mathcal{A}_p'$,
...
Moreover, since $\mathcal{A}_p'' < \mathcal{A}_p'$, we can apply the induction hypothesis to conclude that $c''[e_p(a'[\rho'])/x]$ and $c''[e_p(b'[\rho'])/x]$ are joinable under $\Rightarrow_{R_T^t}^{nf} \Rightarrow_{R_T^p}^*$.
\end{proof}

\begin{cor}[ya-morita-conf]
Let $T_0$ be a theory with well-defined typing axioms such that its pure axioms preserve types and let $T_1$ be a theory as defined in \rprop{conf-main}.
Suppose that closed defined terms of $T_0$ in which $ty$ does not occur are confluent under $R^p_T$.
Then closed defined terms of $T_1$ are confluent.
\end{cor}
\begin{proof}
...
We can choose terms $t_i$ in such a way that they do not contain the function symbols $ft$ and $ty$.
Indeed, we can normalize $t_i$ by \rlem{types-red} so that these function symbols are applied only to $c_j$ (with $j < i$) and we can replace $e_p(c_j)$ with $t_j$.

Since the only new axioms of $T_1$ are typing axioms, pure axioms of this theory preserve types.
...
Let $T_1$ be the extension of $T_0$ defined in \rprop{conf-main} (for some pair $(\varphi,V) \in P_M$) and let $R$ be its underlying term rewriting system.
