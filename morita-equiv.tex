...
To define confluent theories, we restrict our attention to a certain kind of theories.
This is a very mild restriction; all standard examples satisfy it.
We will say that a theory $T$ \emph{has well-defined types and contexts} if $ty$ and $ft$ are function symbols of $T$ (and not arbitrary terms) and, there is a well-ordering on the set of function symbols such that,
for every function symbol $\sigma$ of $T$, there exists a term $t_\sigma$ consisting of function symbols strictly less than $\sigma$ such that the sequent $\sststile{}{x_1, \ldots x_k} e_p(\sigma(x_1, \ldots x_k)) \cong t_\sigma$ is derivable.
Note that such theories up to isomorphism are just quotients of relative $\I$-cell complexes.
We will denote by $\Term^e_T$ the set of terms of $T$ in which the function symbols $ty$ and $ft$ do not occur.
The main property of such theories is that the relation $\Rightarrow_{R_e}$ is strongly normalizable (that is, there is no infinite sequence of $\Rightarrow_{R_e}$-reductions),
where $R_e$ consists of pairs of the form $(e_p(\sigma(x_1, \ldots x_k)), t_\sigma)$.
This implies that every term is equivalent to a term in $\Term^e_T$.

Now, we can define theories with directed axioms:

\begin{defn}[directed]
An algebraic type theory $T$ with well-defined types and contexts has \emph{directed axioms} if the following conditions are satisfied:
\begin{itemize}
\item $T$ has separated axioms and every axiom in $\mathcal{A}_e$ is of the form $\varphi \sststile{}{FV(t)} t = s$, where $t,s \in \Term^e_T$.
...
\end{itemize}
Every type theory with directed axioms induces a term rewriting system $R_T$ on the set $\Term^e_T$: $R_T(t,s)$ is true if and only if there exists an axiom in $\mathcal{A}_e$ of the form $\varphi \sststile{}{FV(t)} t = s$.
\end{defn}

This definition requires some explanation.
A simple situation in which its conditions hold is when $\mathcal{A}_e$ consists of axioms of the form $\{ t\!\downarrow\ \sststile{}{FV(t)} t = s\ \}$, which looks rather natural, but sometimes we need the more general definition.
...
A \emph{confluent} type theory is a type theory $T_0$ with directed axioms such that its underlying term rewriting system is confluent.
% of the theory $T_1 = T_0 \cup \{ \sststile{}{} c_i\!\downarrow |\ 1 \leq i \leq n \} \cup \{ \varphi_i[c_1/x_1, \ldots c_i/x_i]\ |\ 1 \leq i \leq n \}$ is confluent.
\end{defn}
...
Let $T_0$ be a theory with directed axioms such that its underlying term rewriting system $R_0$ is confluent.
...
The first condition guarantees that the systems $R_0$ and $R_1$ are orthogonal to each other.
...
Thus we may assume that if $R_0(t,s)$ and either of $t$ or $s$ contains a subterm of the form $e_p(t')$, then $t'$ is not a variable.
...
We will write $t \Rightarrow_{R_1}^{nf} t'$ if $t \Rightarrow_{R_1}^* t'$ and $t'$ is in a normal form.
...
It follows that we have the following reduction diagram:
\[ \xymatrix@=1em{                              & \ar[ld]_{R_0^*} t \ar[rd]^{R_1^{nf}}  &                           \\
                   t_1 \ar@{-->}[rd]_{R_1^{nf}} &                                       & t_2 \ar@{-->}[ld]^{R_0^*} \\
                                                & t'                                    &
            } \]
Since $t'$ is in a normal form, we have the diagram on the left. This diagram together with the diagram above implies that we have diagram on the right:
\[ \xymatrix@=1em{                              & \ar[ld]_{R_1^*} t \ar[rd]^{R_1^{nf}}  &                       \\
                   t_1 \ar@{-->}[rd]_{R_1^{nf}} &                                       & t_2 \ar@{-->}[ld]^=   \\
                                                & t'                                    &
            }
\qquad
   \xymatrix@=1em{                              & \ar[ld]_{(R_0 \cup R_1)^*} t \ar[rd]^{R_1^{nf}}   &                           \\
                   t_1 \ar@{-->}[rd]_{R_1^{nf}} &                                                   & t_2 \ar@{-->}[ld]^{R_0^*} \\
                                                & t'                                                &
            } \]

The term rewriting system $R_1$ is not only confluent, but also (weakly) normalizable, that is, for every $t$, there is a term $t'$ such that $t \Rightarrow_{R_1}^{nf} t'$.
The following diagram finishes the proof:
\[ \xymatrix@=1em{                                  &                               & \ar[lldd]_{(R_0 \cup R_1)^*} t \ar@{-->}[dd]^{R_1^{nf}} \ar[rrdd]^{(R_0 \cup R_1)^*}  &                               &                               \\
                                                    &                               &                                                                                       &                               &                               \\
                   t_1' \ar@{-->}[rd]_{R_1^{nf}}    &                               & t' \ar@{-->}[ld]_{R_0^*} \ar@{-->}[rd]^{R_0^*}                                        &                               & t_2' \ar@{-->}[ld]^{R_1^{nf}} \\
                                                    & t_1'' \ar@{-->}[rd]_{R_0^*}   &                                                                                       & t_2'' \ar@{-->}[ld]^{R_0^*}   &                               \\
                                                    &                               & t''                                                                                   &                               &
            } \]
\end{proof}
...
where $T_1$ is the same as in \rdefn{confluent} and $R$ is its underlying term rewriting system.
\end{prop}
\begin{proof}
...
Since $R$ is confluent, this implies that $t' \Rightarrow_R^* r$ and $s' \Rightarrow_R^* r$ for some term $r$.
\end{proof}
...
In this appendix we show that basic properties of algebraic type theories such as confluence and normalization follow from these properties for corresponding ordinary type theories.
...
Terms of contextual theories are similar to terms of usual type theories, but they still differ from them.
...
We will show that basic properties of the set of terms of a contextual theory (such as confluence and normalization) is related to the properties of ordinary terms.

Let $T_0$ be a contextual theory and let $\mathcal{F}_0$ be the set of its basic function symbols.
...
\end{enumerate}
Throught this appendix, let $T_0$ be a fixed theory satisfying the above assumption.
