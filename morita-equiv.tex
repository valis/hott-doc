...
If $\varphi = \varphi_1 \land \ldots \land \varphi_n$ and $\varphi_i$ equals to $e_{p_i}(x_i) = t_i$, then $\Rightarrow_\varphi$ consists of pairs $(e_{p_i},t_i)$ for every $1 \leq i \leq n$.
We will denote by $\Rightarrow_{R_T,\varphi}$ the relation $\Rightarrow_{R_T} \cup \Rightarrow_\varphi$.
The following propositions shows that equivalent terms of a theory with directed axioms are joinable under this relation:

\begin{prop}[conf-main]
Let $T$ be a theory with directed axioms and let $(\varphi,V)$ be a pair in $P_M$ where $\varphi = \varphi_1 \land \ldots \land \varphi_n$.
Suppose that the system $(\Term_{T,\varphi}^d,\Rightarrow_{R_T,\varphi})$ is confluent.
Then any pair of terms $t$ and $s$ such that $\varphi \sststile{T}{V} t = s$ is joinable in this system.
\end{prop}
\begin{proof}
By \rlem{der-eq} and \rprop{der-separated}, there exists a sequence $t_1$, \ldots $t_n$ such that $t' = t_1$, $s' = t_n$, and, for every $1 \leq i < n$, either $t_i \Rightarrow_{R_T,\varphi} t_{i+1}$ or $t_{i+1} \Rightarrow_{R_T,\varphi} t_i$.
Since $T$ is confluent, this implies that $t' \Rightarrow_{R_T,\varphi}^* r$ and $s' \Rightarrow_{R_T,\varphi}^* r$ for some term $r$.
\end{proof}
...
Let $T$ be a theory with directed axioms and let $(\varphi,V)$ be a pair in $P_M$.
...
The only serious restriction of the previous lemma is the left-linearity of $R_T$.
...
A \emph{confluent} type theory is a type theory $T$ with directed axioms such that the following conditions hold for every pair $(\varphi,V)$ in $P_M$:
\begin{enumerate}
\item \label{it:conf-first} For every pair of terms $t$ and $s$ such that the sequent $\varphi \sststile{}{V} t = s$ is derivable, they are joinable under $\Rightarrow_{R_T,\varphi}$.
\item \label{it:conf-second} For every axiom $\psi \sststile{}{V'} t = s$ in $\mathcal{A}_e$ and every substitution $\rho$, if $\varphi \sststile{T}{V} t[\rho]\!\downarrow$, then $\varphi \sststile{T}{V} t[\rho] = s[\rho]$.
\end{enumerate}
\end{defn}

\begin{remark}
If the second condition of \rdefn{confluent} holds, then the converse of \rprop{conf-main} is true.
That is, for every confluent theory $T$, the abstract reduction system $(\Term_{T,\varphi}^d,\Rightarrow_{R_T,\varphi})$ is confluent.
Indeed, if $t$ is defined and $t \Rightarrow_{R_T,\varphi} s$, then \eqref{it:conf-second} implies that $\varphi \sststile{T}{V} t = s$.
Thus, if $t \Rightarrow_{R_T,\varphi} s_1$ and $t \Rightarrow_{R_T,\varphi} s_2$, then $\varphi \sststile{T}{V} s_1 = s_2$.
Now \eqref{it:conf-first} implies that $s_1$ and $s_2$ are joinable.
\end{remark}

We can also define a relative version of confluent theories:

\begin{defn}
A \emph{confluent extension} of a theory $T$ is a confluent theory $T'$ such that $T \subseteq T'$ and, for every term $t$ of $T$ and every term $s$ of $T'$ such that $(t,s) \in R_{T'}$,
...
\end{prop}
\begin{proof}
By \rprop{conf-main}, there exists a term $r$ such that $f(t) \Rightarrow_{R_T,\varphi}^* r$ and $f(s) \Rightarrow_{R_T,\varphi}^* r$.
...
More precisely, for every contextual theory $T$, we will define a set $\Term_{T,\varphi}^c$ which can be described informally as the set of defined terms
...
\begin{thm}[conf-comp]
Let $T$ be a contextual type theory with separated typing axioms, separated substitution axioms, and simple contexts such that $\ty$-free axioms preserve types.
For every pair $(\varphi,V) \in P_M$, if the induced reduction relation on $\Term_{T,\varphi}^c$ is confluent and a sequent $\varphi \sststile{}{V} t = s$ is derivable,
then terms $t$ and $s$ are joinable under the relation $\Rightarrow_{R_T,\varphi}$.
\end{thm}

The technical assumptions that we put on the theory will be defined in this section.
The theorem itself follows from \rprop{ty-elim}, \rprop{subst-elim}, and \rprop{ctx-elim}.
...
An axiom $\varphi \sststile{}{V} t = s$ is a \emph{$\ty$-free (and $\ft$-free) axiom} if the function symbols $\ft$ and $\ty$ do not appear in $t$.
...
\begin{lem}[conf-nf]
Let $\Rightarrow_0$, $\Rightarrow_1$, and $\Rightarrow_2$ be abstract reduction systems on the same set $A$.
...
Then any two elements $a,b \in A$ which are equivalent under $\Rightarrow_2$ (that is, there is a zig-zag of $\Rightarrow_2$-reductions between them) are joinable under $\Rightarrow_1^\nf \Rightarrow_0^*$.
\end{lem}
\begin{proof}
...
Now we can prove that the confluence under $\Rightarrow_{R^{tf}_T} \Rightarrow_{R^t_T,\varphi}^\nf$ implies the confluence under $\Rightarrow_{R_T,\varphi}$:

\begin{prop}[ty-elim]
Let $T$ be a theory with well-defined typing axioms such that its $\ty$-free axioms preserve types and let $(\varphi,V)$ be a pair in $P_M$.
Suppose that the abstract reduction system $(\Term_{T,\varphi}^t, \Rightarrow_{R^{tf}_T} \Rightarrow_{R^t_T,\varphi}^\nf)$ is confluent.
If a sequent $\varphi \sststile{}{V} t = s$ is derivable, then $t$ and $s$ are joinable under $\Rightarrow_{R_T,\varphi}$.
\end{prop}
\begin{proof}
...
Let us denote the relation $\Rightarrow_{R_T^t,\varphi}^\nf (\Rightarrow_{R_T^{tf}} \Rightarrow_{R^t_T,\varphi}^\nf)^*$ by $\Rightarrow_{tf}$
Let $\psi = \psi_1 \land \ldots \land \psi_n$ be a formula such that if $\psi_i$ is of the form $t_1 = t_2$, then the terms $t_1$ and $t_2$ are joinable under $\Rightarrow_{tf}$.
...
the terms $t$ and $s$ are joinable under $\Rightarrow_{tf}$.
The statement of the proposition then follows if we take $\psi = \varphi$ and $\mathcal{A}_{tf}' = \mathcal{A}_{tf}$.
...
By assumption, defined terms in which $\ty$ and $\ft$ do not occur are confluent under $\Rightarrow_{R^{tf}_T} \Rightarrow_{R^t_T}^\nf$.
This implies that all defined terms are confluent under this relation.
Moreover, the term rewriting system $R^t_T$ is normalizing and confluent by \rlem{types-red}.
Thus we can apply \rlem{conf-nf} to show that the terms $t$ and $s$ are joinable under $\Rightarrow_{tf}$.
...
we will still have a reduction of the form $c'[a'[\rho']/x] \Rightarrow_{R_T^{tf}} c'[b'[\rho']/x]$ for some $c'$ and $\rho'$ such that $c \Rightarrow_{R_T^t,\varphi}^* c'$ and $\rho \Rightarrow_{R_T^{tf}}^* \rho'$.

Since we reduce exactly one redex, the term $c'$ contains exactly one occurrence of the variable $x$.
Since $a'$, $b'$, $c'$, and $\rho'$ do not contain $\Rightarrow_{R_T^t,\varphi}$-redexes, such a redex can occur in terms $c'[a'[\rho']/x]$ and $c'[b'[\rho']/x]$ only when $c'$ contains a subterm of the form $e_p(x)$.
So there is at most one $\Rightarrow_{R_T^t,\varphi}$-redex in these terms.
If there is no such redex, then we are done since these terms are $\Rightarrow_{R_T^t,\varphi}$-normal forms and are joinable under $\Rightarrow_{R_T^{tf}}^*$ since one of them reduces to the other.
...
It follows that $\chi[\rho'] \sststile{}{V} c''[e_p(a'[\rho'])/x] = c''[e_p(b'[\rho'])/x]$ is also derivable from this set of axioms.
Since the formula $\chi[\rho]$ satisfies the hypothesis of our inductive proof and $\chi[\rho] \Rightarrow_{R_T^t}^* \chi[\rho']$, the latter formula also satisfies it.
Moreover, since $\mathcal{A}_{tf}'' < \mathcal{A}_{tf}'$, we can apply the induction hypothesis to conclude that $c''[e_p(a'[\rho'])/x]$ and $c''[e_p(b'[\rho'])/x]$ are joinable under $\Rightarrow_{tf}$.
\end{proof}
...
and function symbols $\subst^k_{p,n,m} : (ctx,n) \times (p,m+k) \times (tm,n)^m \to (p,n+k)$ are defined by induction on $k$.
Let $\subst^{-1}_{\ty,n,m}(\Gamma, A, a_1, \ldots a_m) = \Gamma$.
If $k \geq 0$, then let $\subst^k_{p,n,m}(\Gamma, a, a_1, \ldots a_m)$ be equal to
\[ \subst_{p,n+k,m+k}(\Gamma', a, \wk^k_{\tm,n}(\Gamma',a_1), \ldots \wk^k_{\tm,n}(\Gamma',a_m), v_{k-1}(\Gamma'), \ldots v_0(\Gamma')), \]
where $\Gamma' = \subst^{k-1}_{\ty,n,m}(\Gamma, \ctx_{m+k}(a), a_1, \ldots a_m)$ and
\[ \wk^k_{p,n}(\Gamma,a) = \subst_{p,n+k,n}(\Gamma, a, v_{n+k-1}(\Gamma), \ldots v_k(\Gamma)). \]
...
Then the abstract reduction system $(\Term_{T,\varphi}^t, \Rightarrow_{R^s_T,\varphi} \Rightarrow_{R^t_T,\varphi}^\nf)$ is confluent and strongly normalizing for every $(\varphi,V) \in P_M$.
\end{lem}
\begin{proof}
TODO
\end{proof}

Let $\Term_{T,\varphi}^s$ be the subset of $\Term_{T,\varphi}^t$ consisting of terms in which function symbols $\subst_{p,n,k}$ do not occur.

\begin{prop}[subst-elim]
Let $T$ be a theory with separated substitution axioms.
Then the abstract reduction system $(\Term_{T,\varphi}^t, \Rightarrow_{R^{tf}_T} \Rightarrow_{R^t_T,\varphi}^\nf)$ is confluent
if and only if the system $(\Term_{T,\varphi}^s,\Rightarrow_{R^{sf}_T} \Rightarrow_{R^t_T,\varphi}^\nf (\Rightarrow_{R^s_T} \Rightarrow_{R^t_T,\varphi}^\nf)^\nf)$ is confluent.
\end{prop}
\begin{proof}
Since $\Term_{T,\varphi}^s$ is a subset of $\Term_{T,\varphi}^t$ which is closed under $\Rightarrow_{R^{tf}_T} \Rightarrow_{R^t_T,\varphi}^\nf$, the ``only if'' direction is obvious.
...
Indeed, the conditions of \rdefn{confluent} involve only sequents of the form $\varphi \sststile{}{V} \psi$ where $(\varphi,V) \in P_M$
