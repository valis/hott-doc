...
Let $\I^{SP}$ be the union of $\I^S$ and maps of the form $0 \to P_{s_1, \ldots s_k}$.
Note that $\I \subseteq \I^S$ and $\I^P \subseteq \I^{SP}$.
...
where $\varphi$ is a formula and $A$ is a term such that every function symbol that appears in either $\varphi$ or $A$ is less than $\sigma$.
...
Let $t$ be a term of $T'$ of the form $\sigma(t_1, \ldots t_k)$, where $\sigma \in \mathcal{F}'$.
Then, for every formulas $\varphi$ and $\psi$ of $T$, if the sequent $f(\varphi)[t/x] \sststile{}{V} f(\psi)[t/x]$ is derivable,
then the sequent $f(\varphi)[t'/x] \sststile{}{V} f(\psi)[t'/x]$ is also derivable for every term $t'$ such that the sequent $f(\varphi) \sststile{}{V} e_p(t) = e_p(t')$ is derivable.
\end{lem}
\begin{proof}
We prove this by induction on $\sigma$.
For a fixed $t$, we proceed by induction on the derivation of $f(\varphi) \sststile{}{V} f(\psi)[t/x]$ in the natural deduction system.
Let us consider the rules \axref{nl} and \axref{na} since the rest are easy to prove.
First, 
\AxiomC{$f(\varphi) \sststile{}{V} a = b$}
\AxiomC{$f(\varphi) \sststile{}{V} \psi'[a/x]$}
\BinaryInfC{$f(\varphi) \sststile{}{V} \psi'[b/x]$}
\DisplayProof
