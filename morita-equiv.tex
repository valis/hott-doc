...
\item If $e_p(t')$ is a subterm of $t$, then $t'$ is not a variable.
\end{enumerate}
\end{defn}

The confluence property is usually defined for a term rewriting system, which is simply a set of function symbols together with a relation on the set of terms.
...
But we still can prove some simple general results about modularity of confluence for type theories as shown in the following lemmas:
...
\item If $t \Rightarrow_{R_0}^* t_1$ and $t \Rightarrow_{R_1} t_2$, then there is a term $s$ such that $t_1 \Rightarrow_{R_1} s$ and $t_2 \Rightarrow_{R_1}^* \Rightarrow_{R_0}^* \Rightarrow_{R_1}^* s$.
\end{enumerate}
...
\end{lem}
\begin{proof}
By \rlem{morita-simple}, $T_0 \cup \{ \sststile{}{} c_i\!\downarrow |\ 1 \leq i \leq n \} \cup \{ \varphi_i[c_1/x_1, \ldots c_i/x_i]\ |\ 1 \leq i \leq n \}$ has simple axioms.
...
Then, by \rlem{der-eq} and \rprop{der-simple}, there exists a sequence $t_1$, \ldots $t_n$ such that $t = t_1$, $s = t_n$, and, for every $1 \leq i < n$, either $t_i \Rightarrow_R t_{i+1}$ or $t_{i+1} \Rightarrow_R t_i$.
TODO
\end{proof}
