...
In this subsection we will prove that they are Morita equivalent.
...
Let us prove that $T \to T \amalg T_2$ is a Morita equivalence whenever $T \amalg T_2$ is confluent and satisfies some additional conditions.
Since $T \to T \amalg T_0$ is a Morita equivalence, it is enough to prove that $f : T \amalg T_0 \to T \amalg T_2$ is a Morita equivalence,
where $f(C) = \top$, $f(c_0) = \unit$, and $f(\Ceq) = \refl(\unit)$.

Suppose that $T$ is a theory under $\coeT_1 + \sigma + \PathT + \wUA$ with well-defined function symbols.
Then \rlem{eq-char-fib}, \rlem{char-type-fib}, and \rlem{eq-comp-char-fib} imply that $T \amalg T_0 \to T \amalg T_2$ is a Morita equivalence whenever it satisfies the condition of \rlem{char-type-fib}.
...
Moreover, terms $\Ceq(c)$ and $\refl(c)$ have the same type only when $c$ is equivalent to $c_0$ and, since $C$ is contractible, $\Ceq(c_0)$ and $\refl(c_0)$ are homotopic.
Formally, for every pair of terms $a$ and $b$ such that $\ctx^n(a) = \ctx^n(b) = \Gamma$ for some $n$ and $f(a) = f(b)$, 
we define a term $h(a,b)$ such that $\ctx^n(h(a,b)) = (\Gamma, i : I)$, $\leftI^*(h(a,b)) = a$, $\rightI^*(h(a,b)) = b$, and $f(h(a,b))$ is a constant homotopy.
