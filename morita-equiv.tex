...
Moreover, suppo that, for every function symbol $\sigma$ of $T_2$, either the first condition of \rlem{eq-char-fib} holds for $\sigma$ or,
...
The axioms of $(T,R)$ are the axioms of $T$ together with the axiom $t\!\downarrow\ \sststile{}{} t = s$ for every pair of terms $t$ and $s$ such that $R(t,s)$.
...
\item If $t \Rightarrow_R^* t_1$ and $t \Rightarrow_R^* t_2$, then there exist terms $s_1$ and $s_2$ such that $t_1 \Rightarrow_R^* s_1$, $t_2 \Rightarrow_R^* s_2$ and $\sststile{T}{} s_1 \cong s_2$.
\end{enumerate}
...
\item If $R(t,s)$, then $t\!\downarrow\ \sststile{T'}{} s\!\downarrow$.
\item If $R(t,s)$, then $t\!\downarrow\ \sststile{(T',R)}{} e_p(t) = e_p(s)$.
\end{enumerate}
...
\begin{lem}
If $T \to T' \to (T',R)$ is a confluent map, then a sequent of the form $\varphi \sststile{}{} t\!\downarrow$ is derivable in $(T',R)$ if and only if it is derivable in $T'$.
\end{lem}
\begin{proof}
The ``if'' direction is obvious since $T'$ is a subtheory of $(T',R)$.
To show the converse, we prove that if $\varphi \sststile{(T',R)}{} \psi$, then $\varphi \sststile{T'}{} t\!\downarrow$ for every subterm $t$ of $\psi$.
We proceed by induction on the derivation of the sequent $\varphi \sststile{(T',R)}{} \psi$ in the natural deduction system.
TODO
\end{proof}

\begin{lem}
Let $T \to T' \to (T',R)$ be a confluent map.
If $\varphi \sststile{}{} t_1 = t_2$ is derivable in $(T',R)$, then there exist terms $s_1$ and $s_2$ such that $t_1 \Rightarrow_R^* s_1$, $t_2 \Rightarrow_R^* s_2$, and $\varphi \sststile{T'}{} s_1 = s_2$.
\end{lem}
\begin{proof}
First, let us prove that the relation $\varphi \sststile{(T',R)}{} - = -$ is the smallest congruence containing $\varphi \sststile{T}{} - = -$ and $R$.
Indeed, the relation $\varphi \sststile{(T',R)}{} - = -$ contains $\varphi \sststile{T'}{} - = -$ and $R$ by definition and the rules of the logic implies that it is a congruence.
Conversely, TODO
\end{proof}

\begin{lem}
If $f : T \to (T',R)$ is a confluent map and $f(\varphi) \sststile{(T',R)}{} f(\psi)$, then $\varphi \sststile{T}{} \psi$.
\end{lem}
\begin{proof}
TODO
\end{proof}
