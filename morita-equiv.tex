...
\begin{enumerate}
\item \label{it:char-fun} For every function symbol $\sigma$ of $T_2$, there exist terms $A_1$, \ldots $A_k$, $t$ of $T_1$ such that $FV(A_i) = \{ x_1, \ldots x_{i-1} \}$ and the following sequents are derivable:
\begin{align*}
\bigwedge_{1 \leq i < k} e_{p_i}(x_i) = A_i & \sststile{}{x_1, \ldots x_k} t\!\downarrow \\
\sigma(x_1, \ldots x_k)\!\downarrow & \sststile{}{x_1, \ldots x_k} \sigma(x_1, \ldots x_k) = f(t) \land \bigwedge_{1 \leq i < k} e_{p_i}(x_i) = f(A_i)
\end{align*}
% \item \label{it:char-type} For every pair of terms $A$ and $B$ of $T_1$ of sort $(ty,n)$ such that
% $\varphi \sststile{T_1}{V} A\!\downarrow \land B\!\downarrow$ and $f(\varphi) \sststile{T_2}{V} f(A) = f(B)$,
% there exists a homotopy $H$ between $A$ and $B$ such that $f(H)$ equals (resp., homotopic to) the constant homotopy on $f(A)$.
\item \label{it:char-type} For every pair of terms $a$ and $B$ of $T_1$ such that $\varphi \sststile{}{V} a\!\downarrow \land B\!\downarrow$ and $f(\varphi) \sststile{}{V} e_p(f(a)) = f(B)$,
there exists a term $b$ such that $\varphi \sststile{}{V} e_p(b) = B$ and $f(\varphi) \sststile{}{V} f(a) = f(b)$.
\end{enumerate}
