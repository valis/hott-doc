...
there is a zig-zag of $\Rightarrow_{R_T}$-reductions between $t[\rho]$ and $s[\rho]$ such that,
for every term $q$ in this sequence of reductions, the sequent $\varphi \sststile{}{V} q\!\downarrow$ is derivable.
\end{enumerate}
% An algebraic type theory $T$ has \emph{directed axioms} if it has separated axioms and every axiom in $\mathcal{A}_e$ of the form $\psi \sststile{}{V'} t = s$ satisfies the following conditions:
% \begin{itemize}
...
% \end{itemize}
% Every type theory with directed axioms induces a term rewriting system $R_T$ on the set $\Term_T$: $R_T(t,s)$ is true if and only if there exists an axiom in $\mathcal{A}_e$ of the form $\varphi \sststile{}{V'} t = s$.
\end{defn}
...
If $t$ and $s$ are terms such that $\varphi \sststile{T}{V} t = s$, then it is enough to show that there is a zig-zag of $\Rightarrow_{R_T,\varphi}$-reductions consisting of $\varphi$-defined terms between them.
By \rlem{der-eq} and \rprop{der-separated}, there exists a sequence $t_1$, \ldots $t_n$ such that $t = t_1$, $s = t_n$, and, for every $1 \leq i < n$,
...
Condition~\eqref{it:dir-second} of \rdefn{directed} implies that in the latter case there is a zig-zag of $\Rightarrow_{R_T}$-reductions between $t_i$ and $t_{i+1}$.
Since $(\Term_{T,\varphi}^d,\Rightarrow_{R_T,\varphi})$ is confluent, this implies that the terms $t$ and $s$ are joinable.

The converse is easy.
If $t \Rightarrow_{R_T,\varphi} t_1$ and $t \Rightarrow_{R_T,\varphi} t_2$, then $\varphi \sststile{T}{V} t = t_1$ and $\varphi \sststile{T}{V} t = t_2$ which implies that $\varphi \sststile{T}{V} t_1 = t_2$.
Hence $t_1$ and $t_2$ are joinable.
\end{proof}
...
A \emph{confluent} type theory is a type theory $T$ with a reduction system such that equivalent conditions of \rprop{conf-main} hold.
\end{defn}
...
\begin{thm}[conf-comp]
Let $T$ be a contextual type theory with a reduction system in which typing axioms are separated, substitution axioms are separated, contexts are simples, and $\ty$-free axioms preserve types.
