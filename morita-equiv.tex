...
A morphism $f : T_1 \to T_2$ between theories with identity types is a strong Morita equivalence if and only if it has the weak lifting property with respect to every pair $V,\varphi$.
\end{prop}
\begin{proof}
...
Let $\I_S \subseteq \I$ be the subset which consists of the maps $T_{l, (d_p,n)} \to T_{l, (p,n)}$ such that $l$ is empty.
...
An analogous proof shows that a map has RLP with respect to $\I$ if and only if it has the lifting property with respect to the set of pairs described in \rprop{morita-char}.
Similarly, \rprop{syn-equiv-char} implies that a map has RLP (up to relative homotopy) with respect to $\I_S$ if and only if it has the (weak) lifting property with respect to the pair $\varnothing,\top$.
\end{remark}
...
The weak equivalences of this model structure are syntactic equivalences, $\I_S$ is a set of generating cofibrations, and every object is fibrant in this model structure.
...
\section{Quotients of cofibrant theories}

TODO
% In this section we consider theories which are quotients of cofibrant theories.
% We will give another characterization for Morita equivalences between such theories.

\begin{comment}
Let $T$ be a theory with identity types and let $\varphi$ be a formula of $T$.
A \emph{homotopy} between contexts $x_1 : A_1, \ldots x_n : A_n$ and $y_1 : B_1, \ldots a_n : B_n$ with respect to $\varphi$ is a sequence of relative homotopies $H_1,\ldots H_n$,
where $H_i = (f_i,g_i,p_i,g'_i,p'_i)$ is a relative homotopy between $x_1 : A_1, \ldots x_{i-1} : A_{i-1} \vdash A_i$ and $x_1 : A_1, \ldots x_{i-1} : A_{i-1} \vdash B_i[y_1 \repl f_1, \ldots y_{i-1} \repl f_{i-1}]$.
If $a : (p,n)$ and $B : (d_p,n)$ are terms and $H$ is a homotopy between $e_p(a)$ and $B$, then it is easy to construct a term $coe(H,a) : (p,n)$ such that $e_p(coe(H,a)) = B$.
Moreover, if $H$ is a constant homotopy, then $coe(H,a) = a$.
\end{comment}

By \cite[Lemma~4.6]{alg-tt}, every theory is isomorphic to a contextual theory in which every basic function symbol has a signature of the form
\[ \sigma : s_1 \times \ldots \times s_k \to (p,0), \]
where $p \in \{ ty, tm \}$.
Thus we can assume that all theories have this form without loss of generality.
...
\begin{enumerate}
\item \label{it:char-fun} For every basic function symbol $\sigma$ of $T_2$, there exist terms $A_1$, \ldots $A_k$, $t$ of $T_1$ such that $FV(A_i) = \{ x_1, \ldots x_{i-1} \}$ and the following sequents are derivable:
\begin{align*}
\bigwedge_{1 \leq i < k} e_{p_i}(x_i) = A_i & \sststile{T_1}{x_1, \ldots x_k} t\!\downarrow \\
\sigma(x_1, \ldots x_k)\!\downarrow & \sststile{T_2}{x_1, \ldots x_k} \bigwedge_{1 \leq i < k} e_{p_i}(x_i) = f(A_i)
\end{align*}
Moreover, $\sigma(x_1, \ldots x_k)$ and $f(t)$ are relatively homotopic with respect to $\sigma(x_1, \ldots x_k)\!\downarrow$.
...
% If $a = x$ is a variable, then temrs $e_p(x)$ and $A$ satisfy the conditions of \eqref{it:char-type}.
% Thus there exists a homotopy $H$ between $e_p(x)$ and $A$ such that $f(H)$ is the constant homotopy on $e_p(x)$.
% Then $coe(H,x)$ is the required lifting.
...
% First, let us show that, for every term $a$ of $T_2$ such that $f(\varphi) \sststile{}{V} a\!\downarrow$,
% there exists a term $a'$ of $T_1$ such that $\varphi \sststile{}{V} a'\!\downarrow$ and $f(\varphi) \sststile{}{V} a = f(a')$.
\end{proof}
...
\begin{align*}
\bigwedge_{1 \leq i < k} e_{p_i}(x_i) = A_i & \sststile{T_1}{x_1, \ldots x_k} t\!\downarrow \\
\sigma(x_1, \ldots x_k)\!\downarrow & \sststile{T_2}{x_1, \ldots x_k} f(t) = \sigma(x_1, \ldots x_k) \land \bigwedge_{1 \leq i < k} e_{p_i}(x_i) = f(A_i)
\end{align*}
\item \label{it:char-type} For every pair of terms $A$ and $B$ of $T_1$ of sort $(ty,n)$ such that
...
Since $f(d^*(\Delta))$ equals to $f(\Gamma)$, we have a term $\Gamma \vdash b : d^*(\Delta)$ such that $f(b)$ equals to $v_0(f(\Gamma))$ by \eqref{it:char-type}.
Thus, $d,b$ is the required morphism of contexts $\Gamma$ and $\Delta$.

Let us prove that \eqref{it:char-type} implies the following fact, which will be denoted by (2').
...
Since $f(B) = f(ty(a))$, we have a term $ty(a) \vdash b' : B$ such that $f(b')$ equals to $v_0(f(B))$ by \eqref{it:char-type}.
Then we can define $b$ as $b'[d^*(a)]$.
This complete the proof of (2').

Let $A$ and $a$ be terms such that $\varphi \sststile{}{V} A\!\downarrow$ and $f(\varphi) \sststile{}{V} e_p(a) = f(A)$.
Then we construct the required lifting by induction on $a$.
If $a = x$ is a variable, then (2') implies that that the required lifting exists.

Now, suppose that $a = \sigma(a_1, \ldots a_k)$.
The same proof as in the previous lemma shows that condition \eqref{it:char-fun} holds for all function symbols.
Let $A_1$, \ldots $A_k$, $t$ be terms as described in \eqref{it:char-fun}.
By induction hypothesis, there exist terms $a_1'$, \ldots $a_k'$ such that $\varphi \sststile{}{V} e_{p_i}(a_i') = A_i[x_1 \repl a_1', \ldots x_{i-1} \repl a_{i-1}']$ and $f(\varphi) \sststile{}{V} f(a_i') = a_i$.
Let $a' = t[x_1 \repl a_1', \ldots x_k \repl a_k']$.
Since $\varphi \sststile{}{V} a'\!\downarrow$ and $f(\varphi) \sststile{}{V} f(a') = f(a)$, (2') implies that there exists the required lifting.
\end{proof}
