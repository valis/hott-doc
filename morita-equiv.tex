...
Now, we can prove the special case of the lemma when $\varphi = \top$ and $V = \varnothing$.
...
Finally, we can prove the general case of the lemma.
Let $\varphi = \varphi_1 \land \ldots \land \varphi_n$ be a formula such that $\varphi_i$ equals to $e_{p_i}(x_i) = A_i$, where $A_i$ is a term such that $\varphi_1 \land \ldots \land \varphi_{i-1} \sststile{}{x_1, \ldots x_{i-1}} A_i\!\downarrow$.
Then, for each $0 \leq i \leq n$, we define a theory $T_i$ as $T \cup \{ \sststile{}{} c_j\!\downarrow |\ 1 \leq j \leq i \} \cup \{ \varphi_j[c_1/x_1, \ldots c_j/x_j]\ |\ 1 \leq j \leq i \}$.
By \rlem{mcf}, a sequent $\varphi \sststile{}{x_1, \ldots x_n} \psi$ is derivable in $T$ if and only if the sequent $\sststile{}{} \psi[c_1/x_1, \ldots c_n/x_n]$ is derivable in $T_n$.
Thus we just need to prove that $T_n$ has simple axioms.
We prove by induction on $i$ that all theories $T_i$ have simple axioms.
...
Since $T_i$ has simple axioms, it follows that $\sststile{}{} \varphi_\sigma[t_1/x_1, \ldots t_k/x_k]$ is derivable from $\mathcal{A}_d \cup \mathcal{A}_e$ in $T_i$ and hence in $T_{i+1}$.
\end{proof}

The previous lemma implies that the maps between theories with simple axioms corresponding to inclusions of subtheories are Morita equivalences.
...
Recall that a relation $R$ is confluent if $R^*(t,s_1)$ and $R^*(t,s_2)$ implies that there exists $s$ such that $R^*(s_1,s)$ and $R^*(s_2,s)$, where $R^*$ is the reflexive transitive closure of $R$.
Now, we can define confluent theories and prove their main property:
...
such that $\mathcal{A}_e = \{ t\!\downarrow\ \sststile{}{FV(t) \cup FV(s)} t = s\ |\ R(t,s) \}$.
\end{defn}
