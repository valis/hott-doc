...
We will show that the theory of $\Pi$-types is confluent in section~\ref{sec:applications}.
...
Since we often need to know that a certain theory is confluent, we first show that some standard theories are confluent.
...
In this appendix we construct several standard examples of type theories and prove that some of them are confluent.
The theories will be presented in a contextual form (see \cite{alg-tt} for a definition of a contextual theory).
...
Let $\Term_{\mathcal{F}_0}$ be the set of terms constructed from the function symbols in $\mathcal{F}_0$.
