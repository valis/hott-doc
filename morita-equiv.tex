...
A term $t$ in a term rewriting system $R$ is \emph{confluent} if $t \Rightarrow_R^* s_1$ and $t \Rightarrow_R^* s_2$ implies that there exists $s$ such that
...
A \emph{confluent} type theory is a type theory $T_0$ with directed axioms such that, for every terms $t$ and $s$ satisfying $R_0(t,s)$, if either $t$ or $s$ contains a subterm of the form $e_p(t')$, then $t'$ is not a variable.
\end{defn}
...
The condition on $R_0$ formulated in \rdefn{confluent} implies that $R_0$-reductions preserve $R_1$-normal forms (a term $t$ is in the $R_1$-normal form if there is no term $t'$ such that $t \Rightarrow_{R_1} t'$).
...
The term rewriting system $R_1$ is (weakly) normalizable, that is, for every $t$, there is a term $t'$ such that $t \Rightarrow_{R_1}^{nf} t'$.
...
Let $T_1$ be the extension of $T_0$ defined in \rlem{morita-conf} (for some pair $(\varphi,V) \in P_M$) and let $R$ be its underlying term rewriting system.
