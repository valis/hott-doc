...
Suppose that closed defined terms of the theory $T_1 = T_0 \cup \{ \sststile{}{} c_i\!\downarrow |\ 1 \leq i \leq n \} \cup \{ \varphi_i[c_1/x_1, \ldots c_i/x_i]\ |\ 1 \leq i \leq n \}$ are confluent.
...
if $a,b \in A$ are such that $a \Rightarrow_{R_0} b$, then $a$ and $b$ are joinable under the relation $\Rightarrow_{R_1}^{nf} \Rightarrow_{R_0}^*$.
Then the system $(A,\Rightarrow_{R_0 \cup R_1})$ is confluent.
Moreover, equivalent elements of $A$ are joinable under $\Rightarrow_{R_1}^{nf} \Rightarrow_{R_0}^*$.
\end{lem}
\begin{proof}
...
We will say that pure axioms of $T$ \emph{preserve types} if, for every pure axiom $\varphi \sststile{}{V} t = s$ of $T$, the sequent $\varphi \sststile{}{V} e_p(t) = e_p(s)$ is derivable from the set $\mathcal{A}_d \cup \mathcal{A}_p$.
\end{defn}

The main property of theories in which pure axioms preserve types is proved in the following lemmas:
...
\begin{lem}
Let $T$ be a theory with separated typing axioms in which pure rules preserve types.
If $t$ and $s$ are terms such that the sequent $\sststile{}{} t = s$ is derivable in $T$, then terms $e_p(t)$ and $e_p(s)$ have the same $R_T^t$-normal form.
\end{lem}
\begin{proof}
By \rlem{der-separated-closed}, the sequent $\sststile{}{} t = s$ is derivable from the set of axioms $\mathcal{A}_d \cup \mathcal{A}_e$.
We proceed by induction on (the size of) such a derivation of $\sststile{}{} t = s$ in the natural deduction system.
\end{proof}
...
Let $A$ be a subset of $\Term_T$ closed under $\Rightarrow_{R_0 \cup R_1}$.
Suppose that $(A,\Rightarrow_{R_0})$ is confluent, $(A,\Rightarrow_{R_1})$ is confluent and weakly normalizable, and
if $a,b \in A$ are such that $a \Rightarrow_{R_2} b$, then $a$ and $b$ are joinable under the relation $\Rightarrow_{R_1}^{nf} \Rightarrow_{R_0}^*$.
...
\begin{prop}
Let $T$ be a theory with well-defined typing axioms such that its reduction rules preserve types.
Suppose that closed defined terms in which $ty$ does not occur are confluent under $R^p_T$.
Then closed defined terms of $T$ are confluent.
\end{prop}
\begin{proof}
TODO % Prove that if terms without ty are confluent, then so are terms with ty.
\end{proof}

\begin{cor}[ya-morita-conf]
% TODO: Rewrite these conidtions
Let $T_0$ be a theory with directed axioms and let $T_1$ be a theory as defined in \rprop{conf-main}.
Suppose that the conditions of the previous proposition hold for $T_1$.
Moreover, suppose that closed defined terms are weakly normalizable under $R_{T_1} \setminus R^p_T$.
Then closed defined terms of $T_1$ are confluent.
\end{cor}
\begin{proof}
TODO
