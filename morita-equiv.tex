...
A \emph{confluent} type theory is a type theory $T_0$ with separated axioms together a term rewriting system $R_0$ such that the following conditions hold:
\begin{enumerate}
\item $\mathcal{A}_e = \{ \varphi_{t,s} \sststile{}{FV(t)} t = s\ |\ R_0(t,s) \}$, where the formulas $\varphi_{t,s}$ satisfy the following condition.
For every pair $(\psi,V)$ in $P_M$ and every substitution $\rho$, if $\psi \sststile{T_0}{V} t[\rho]\!\downarrow$, then $\psi \sststile{T_0}{V} \varphi_{t,s}[\rho]$.
\item For every pair $(\varphi_1 \land \ldots \land \varphi_n, V)$ in $P_M$, the term rewriting system $R = R_0 \cup R_1$ is confluent, where $R_1$ is defined as follows.
...
Every formula $\varphi_i$ equals to $e_{p_i}(x_i) = A_i$ for some term $A_i$.
We define $R_1$ as $\{ (e_{p_i}(c_i), A_i[c_1/x_1, \ldots c_{i-1}/x_{i-1}])\ |\ 1 \leq i \leq n \}$.
\end{enumerate}
\end{defn}

This definition requires some explanation.
A simple situation in which the first condition holds is when $\mathcal{A}_e = \{ t\!\downarrow\ \sststile{}{FV(t)} t = s\ |\ R_0(t,s) \}$, which looks rather natural, but sometimes we need the more general definition.
For example, consider the following reduction rule:
\[ \app(B, \lambda(b), a) \Rightarrow_{R_0} b[a] \]
...
The first condition of \rdefn{confluent} guarantees that it is always true.

Now, let us discuss the second condition of \rdefn{confluent}.
We could just require the relation $R_0$ to be confluent, but in this case we can apply \rprop{conf-main} only to sequents of the form $\sststile{}{} t = s$.
The second condition of \rdefn{confluent} guarantees that the proposition applies to sequents with a more general left hand side.
Moreover, the stronger condition often follows from the assumption that $R_0$ is confluent:

\begin{lem}[morita-conf]
Let $(T_0,R_0)$ be a theory with a confluent term rewriting system on it.
...
Since $t'$ is in the normal form, we have the diagram on the left. This diagram together with the diagram above implies that we have diagram on the right:
...
We will work with theories in a contextual form (see \cite{alg-tt} for a definition of a contextual theory).
...
We want to work with closed terms of $T$, but we still need to prove properties of arbitrary terms.
This problem can be solved using \rlem{mcf}.
We extend $T$ with a finite set of constants $\{ c_1, \ldots c_n \}$.
Then there is a bijection between closed terms of this extension and terms of $T$ with $n$ free variables.
...
So, $\Term_{\mathcal{F}_0}$ is the set of terms of $T \cup \{ c_1, \ldots c_n \}$ in which we omit contexts.
If $t$ is a term of $T$, then we can define a term $U(t) \in \Term_{\mathcal{F}_0}$ in the obvious way:
\begin{align*}
U(x) & = x \\
U(\sigma_m(\Gamma, t_1, \ldots t_k)) & = \sigma(U(t_1), \ldots U(t_k))
\end{align*}

Let $\Term_T$ be the set of closed defined terms of $T \cup \{ c_1, \ldots c_n \}$, that is the set of terms $t$ such that the sequent $\sststile{}{} t\!\downarrow$ is derivable.
...
Let $(T,R)$ be a contextual theory together with a term rewriting system on $\Term_T$ such that,
for every terms $t$ and $s$ such that $R(t,s)$, the multiset $C(t)$ consists of distinct variables and the multiset $C(s)$ consists of strongly normalizable terms.
Such a theory satisfies the following properties:
\begin{enumerate}
\item \label{it:red-fib} Let $t$ be a term of $T$ such that $U(t) \Rightarrow_R s$ for some term $s$.
...
\end{enumerate}
Throught this appendix, let $T$ be a fixed contextual theory satisfying the above assumption.
...
\begin{prop}
The reduction relation on $\Term_T$ generated by $R$ is strongly normalizing if and only if the same is true for the induced reduction relation on $\Term_T^c$.
\end{prop}
\begin{proof}
