...
Let $\varphi$ be a formula which does not contain any predicate symbols and let $A$ be a term of $T_{l,(p,n)}$ such that $\varphi \sststile{}{V} A\!\downarrow$.
Let $a$ be a term of $C_{T_{l,(d_p,n)}}(T_{l,(p,n)})$ such that $\cyli_0(\varphi) \sststile{}{V} e_p(a) = \cyli_0(A)$.
If we define $a'$ as $s(a)$, then $\varphi \sststile{}{V} e_p(a') = A$ and the fact that $\cyli_0(a')$ and $a$ are relatively homotopic follows from \rlem{jcell-morita}.
\end{proof}
...
There is another model structure on the category of $I$-stable theories under $T_I$ which has syntactic equivalences as weak equivalences.
