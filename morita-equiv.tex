...
A \emph{term rewriting system} is a binary relation $R$ on the set of defined closed terms of some theory such that the following conditions hold:
\begin{enumerate}
...
\begin{defn}[directed]
An algebraic type theory $T$ has \emph{directed axioms} if it has separated axioms and every axiom in $\mathcal{A}_e$ is of the form $\varphi \sststile{}{FV(t)} t = s$ and satisfies the following condition.
For every pair $(\psi,V)$ in $P_M$ and every substitution $\rho$, if $\psi \sststile{T}{V} t[\rho]\!\downarrow$, then $\psi \sststile{T}{V} \varphi[\rho]$.

Every type theory with directed axioms induces a term rewriting system $R_T$ on its terms: $R_T(t,s)$ is true if and only if there exists an axiom in $\mathcal{A}_e$ of the form $\varphi \sststile{}{FV(t)} t = s$.
\end{defn}
...
the underlying term rewriting system of the theory $T_1 = T_0 \cup \{ \sststile{}{} c_i\!\downarrow |\ 1 \leq i \leq n \} \cup \{ \varphi_i[c_1/x_1, \ldots c_i/x_i]\ |\ 1 \leq i \leq n \}$ is confluent.
\end{defn}
...
Let $T_1$ be the extension of $T_0$ defined in \rdefn{confluent} and let $R$ be its underlying term rewriting system.
