...
Now we want to prove that weak equivalences satisfy the 2-out-of-6 property.
To do this, we give a characterization of weak equivalences which generalizes \cite[Proposition~3.3]{kap-lum-model} and \cite[Proposition~3.11]{alg-models}.
The homotopy category $\Ho(M)$ of a model $M$ is defined as usual: its objects are contexts of $M$ and morphisms are context morphisms.
For every natural number $n$, let $\Ho_n(M)$ be the full subcategory of $\Ho(M)$ consisting of contexts of length $n$.

\begin{prop}
A map of models $f : M \to N$ is a weak equivalence if and only if $\Ho(f)$ and $\Ho_n(f)$ are equivalences of categories for all $n$.
\end{prop}
\begin{proof}
Suppose that $f$ is a weak equivalence.
Let $\Gamma$ and $\Delta$ be objects of $\Ho(M)$, and let $f(\Gamma) \vdash b : f(\Delta)$ be a morphism of $\Ho(N)$.
...
\begin{cor}
Weak equivalences of models of type theories satisfy the 2-out-of-6 property.
\end{cor}
...
We are interested in question when the classes of cofibrations and weak equivalences as defined above determine a model structure.
It was shown in \cite{alg-models} that this is true for theories under $\coeT_1 + \sigma + \PathT + \wUA$.
In general, this model structure might not exist, but a left semi-model structure still could exist as shown in \cite{kap-lum-model}.
We will use the definition of left semi-model structures given in Lemma~6.7 of this paper.
Assuming that such left semi-model structures exist, the following simple proposition characterizes Morita equivalences between such theories.
...
Let $T_1$ and $T_2$ be theories such that the categories of their models carry the left semi-model structures described above.
...
We can generalize this theorem using the following lemmas:
...
If $f : T_1 \to T_2$ is a strong Morita equivalence, then there is a model structure on $T_1$
with the class of cofibrations and weak equivalences as described above and such that $f_! \dashv f^*$ is a Quillen equivalence between $\Mod{T_1}$ and $\Mod{T_2}$.
\end{lem}
\begin{proof}
...
\begin{lem}
Let $T_1$ be a theory with $\Id$-types and $\Sigma$-types, and let $T_2$ be a theory such that there is a left semi-model structure on $\Mod{T_2}$
with cofibrations and weak equivalences as described above.
If $f : T_1 \to T_2$ is a Morita equivalence, then there is a left semi-model structure on $T_1$
with the class of cofibrations and weak equivalences as described above and such that $f_! \dashv f^*$ is a Quillen equivalence between $\Mod{T_1}$ and $\Mod{T_2}$.
\end{lem}
\begin{proof}
Since weak equivalences satisfy 2-out-of-3 and $\eta_X : X \to f^* f_!(X)$ is a weak equivalence for all $X$,
...
We will show that if a theory $T$ is nice enough, then the map $T \to T + \coeT_1 + \sigma + \PathT + \wUA$ is a Morita equivalence.
Since the category of models of the latter theory always carries a model structure, this implies that the category of models of every such theory $T$ carries a semi-model structure.
...
% T -> T + I + Path + coe is a Morita equivalence
...
% pulling the (semi-) model structure along Morita equivalences
