...
\begin{thm}[conf-comp]
Let $T$ be a contextual type theory with a reduction system in which typing rules are separated, substitution rules are well-behaved and stable, contexts are simples, and $\ty$-free rules preserve types.
...
We will say that $\subst$-free rules are \emph{well-behaved} if the following conditions hold:
\begin{itemize}
\item For every such $\subst$-free rule $(t,s)$ of sort $(p,n)$, terms $t$ and $s$ are $\Rightarrow_s$-normal forms and,
for every subterm of $t$ of the form $\subst_{p,n,k}(x, t_1, \ldots t_k)$, it is true that $k > n$ and $t_i = v_{k-i}$ for each $i \leq n$.
\item For every substitution $\rho$, consisting of $\Rightarrow_s$-normal forms, if $t[\rho] \in \Term_{T,\varphi}^c$,
then $t' (\Rightarrow_{sf} \Rightarrow_s^\nf)^* s'$, where $t'$ and $s'$ are $\Rightarrow_s$-normal forms of $t[\rho]$ and $s[\rho]$, respectively.
\end{itemize}

\begin{example}
If function symbols $\subst_{p,n,k}$ do not occur in $t$, then every rule of the form $(t,s)$ is well-behaved.
\end{example}
...
Let us define a lifting function for terms $L^i : \Term_{T,\varphi}^c \to \Term_{T,\varphi}^c$ for every $i \in \mathbb{N}$:
\begin{align*}
L^i(x) & = x \\
L^i(v_j) & = \\
L^i(\sigma_m(t_1, \ldots t_k)) & = \\
L^i(\subst_{p,n,k}(t, t_1, \ldots t_k)) & = \\
\end{align*}

We will say that $\subst$-free rules are \emph{stable} if the following conditions hold: % TODO

% Let $i$ be a natural number.
% We will say that $i$-th index does not occur in a term $t \in \Term_{T,\varphi}^s$ if the corresponding variable is not free in this term.
% Formally, this property is defined by induction on $t$.
% If $t = v_j$, then $i$-th index occurs in $t$ if and only if $i = j$.
% If $t = x$, then $i$-th index occurs in $t$.
% If $t = \subst_{p,n,k}(x, t_1, \ldots t_k)$, then $i$-th index occurs in $t$ if and only if it occurs in $t_j$ for some $1 \leq j \leq k$.
% If $t = \sigma_m(t_1, \ldots t_k)$ and $\sigma : (p_1,n_1) \times \ldots \times (p_k,n_k) \to (p,0)$,
% then $i$-th index occurs in $t$ if and only if there exists $1 \leq j \leq k$ such that $(i+n_j)$-th index occurs in $t_j$.

% If $i$-th index does not occur in a term $t$ of sort $(p,n+1)$, then we define a term $D^i(t)$ of sort $(p,n)$:
