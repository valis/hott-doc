...
If $T_2$ is a model theory and $F : T_1 \to T_2$ is a strong Morita equivalence, then $T_1$ is also model and $F_! \dashv F^*$ is a Quillen equivalence between $\Mod{T_1}$ and $\Mod{T_2}$.
\end{lem}
\begin{proof}
...
\subsection{The $\sigma$-rule}

We defined several theories with the interval type in \cite{alg-models} and it was conjectured that these theories are equivalent.
We will prove that, for every theory $T$ with an interval type satisfying some mild additional hypotheses, $T$ is equivalent to the theory with the $\sigma$-rule.
First, we need to establish a technical lemma:

\begin{lem}[hom-pres]
\end{lem}

\begin{prop}
Let $f : T \to T \cup \mathcal{A}$ be a map of theories such that $T \cup \mathcal{A}$ is a confluent theory and $T$ is a theory with separated axioms such that,
for every axiom $\varphi_\sigma \sststile{}{x_1, \ldots x_k} \sigma(x_1, \ldots x_k)\!\downarrow$ in $\mathcal{A}_d$, the pair $(\varphi_\sigma, \{ x_1, \ldots x_k \})$ belongs to $P_M$.
Suppose that, for every pair $(\varphi,V) \in P_M$, every substitution $\rho$, and every reduction rule $(t,s)$ of $T \cup \mathcal{A}$ such that $\varphi \sststile{T}{V} t[\rho]\!\downarrow$,
there exists a homotopy $h$ such that $\varphi \sststile{T}{V} h[\leftI] = t[\rho] \land h[\rightI] = s[\rho]$ and $h$ is a constant homotopy in $T \cup \mathcal{A}$ and $e_p(h)$ is a constant homotopy in $T$.
Then $f$ is a trivial fibration.
\end{prop}
\begin{proof}
We will verify conditions of \rlem{eq-char-fib}.
By \rprop{der-separated}, we may assume that $T$ is maximal.
The first condition follows from the fact $T$ is a maximal theory with separated axioms and that the $(\varphi_\sigma, \{ x_1, \ldots x_k \})$ belongs to $P_M$.
To prove the second condition, we need to verify conditions of \rlem{char-type-fib}.
If $\varphi \sststile{T}{V} \ft(A) = \ft(B)$ and $\varphi \sststile{T \cup \mathcal{A}}{V} A = B$, then there is a term $C$ such that $A \Rightarrow_{R_{T \cup \mathcal{A}},\varphi}^* C$ and $B \Rightarrow_{R_{T \cup \mathcal{A}},\varphi}^* C$.
By assumption and \rlem{hom-pres}, there exists a sequence of homotopies between $A$ and $B$, which are constant in $T \cup \mathcal{A}$.
This implies that there is a term $A \vdash b : B$ which is equal to $v_0(A)$ in $T \cup \mathcal{A}$.
\end{proof}

\begin{example}
Let $T$ be a theory with an interval type and let $T'$ be the extension of $T$ with the $\sigma$-rule.
If $T$ has a reduction system, then there is a natural reduction system on $T'$ which consists of the rules of $T$ together with the following rule:
\[ \coe_1(x. A, a, i) \Rightarrow a \]
for every term $A$ in which variable $x$ is not free.

Suppose that $T'$ is confluent and that $T$ satisfies the conditions of the previous proposition.
Then the map $T \to T'$ is a trivial fibration.
Indeed, we need to show that if $\varphi \sststile{T}{V} t[\rho]\!\downarrow$ and $(t,s) \in R_{T'}$, then $t[\rho]$ and $s[\rho]$ are relatively homotopic in $T$ and the homotopy is constant in $T'$.
If $(t,s) \in R_T$, then $t$ and $s$ are equal in $T$, so we may assume that $(t,s)$ is the $\sigma$-rule.
We can define the following relative homotopy between $\coe_1(x. A, a, i)$ and $a$:
\[ \coe_1(y.\,\coe_1(x.A, a, y) \idtype a, \refl(a), i) \]
This homotopy is constant in $T'$.
Thus the conditions of the previous proposition hold.
\end{example}
