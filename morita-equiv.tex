...
and, for every pair $(\varphi_1 \land \ldots \land \varphi_n, V)$ in $P_M$, the term rewriting system $R = R_0 \cup R_1$ is confluent, where $R_1$ is defined below.
...
If $\varphi_i$ equals to $e_{p_i}(x_i) = A_i$, then we define $R_1$ as $\{ (e_{p_i}(c_i), A_i[c_1/x_1, \ldots c_{i-1}/x_{i-1}])\ |\ 1 \leq i \leq n \}$.
\end{defn}

The following lemma shows that to prove that $(T_0,R_0)$ is confluent, it is often enough to show that $R_0$ is confluent.
...
By \rlem{der-eq} and \rprop{der-simple}, there exists a sequence $t_1$, \ldots $t_n$ such that $t' = t_1$, $s' = t_n$, and, for every $1 \leq i < n$, either $t_i \Rightarrow_R t_{i+1}$ or $t_{i+1} \Rightarrow_R t_i$.
Since $R$ is confluent, this implies that $t' \Rightarrow_R^* r$ and $s' \Rightarrow_R^* r$.
\end{proof}
