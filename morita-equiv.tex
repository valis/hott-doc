...
\UnaryInfC{$\varphi \sststile{}{V,x} x\!\downarrow$}
\DisplayProof
\qquad
...
Finally, for every axiom $\psi_1 \land \ldots \land \psi_n \sststile{}{x_1 : s_1, \ldots x_k : s_k} \chi_1 \land \ldots \land \chi_m$ and for all terms $t_1 : s_1$, \ldots $t_k : s_k$
such that $t_i = x_i$ if $x_i \notin FV(\psi_1 \land \ldots \land \psi_n)$, we have the following rules for all $1 \leq j \leq m$:
\medskip
\begin{center}
\AxiomC{$\varphi \sststile{}{V} \psi_1[t_1/x_1, \ldots t_k/x_k]$ \qquad \ldots \qquad $\Gamma \sststile{}{V} \psi_n[t_1/x_1, \ldots t_k/x_k]$}
...
\end{prop}
\begin{proof}
Let us first prove the ``if'' part.
The rule \axref{nv} follows from \axref{a1}, \axref{b3}, and \axref{b2}.
The rule \axref{ns} follows from \axref{a2}, \axref{a3}, and \axref{b2}.
The rule \axref{nh} follows from \axref{b1}, \axref{b4}, and \axref{b5}.
The rule \axref{nl} follows from \axref{a2}, \axref{a3}, and \axref{b6}.
The rules \axref{np} and \axref{nf} follow from \axref{a3}, \axref{a1}, \axref{b2}, and \axref{b3}.
The rule \axref{na} follows from \axref{a3}, \axref{b6}, \axref{b2}, and the axioms.

Now, let us prove the ``only if'' part.
The rules \axref{b1}, \axref{b4}, and \axref{b5} follow from \axref{nh}.
...
Every axiom is derivable from \axref{na}.
