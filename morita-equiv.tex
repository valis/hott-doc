...
Now we can prove that the confluence under $R^p_T$ implies the confluence under $R^p_T \cup R^t_T$ for closed defined terms (that is, terms $t$ such that the sequent $\sststile{}{} t\!\downarrow$ is derivable):
...
If $T$ is a contextual theory, then we have a set of function symbols $\mathcal{F}_0$ (which we call \emph{basic function symbols}) such that the set of function symbols of $T$ consists of function symbols $\ft_m$, $\ty_m$, and the following function symbols:
...
Let $\Term_T$ be the set of closed defined terms of $T$ in which $\ty_m$ and $\ft_m$ do not occur.
...
\begin{enumerate}
\item For all terms $t$ and $s$ in $\Term_T$ such that $(t,s) \in R_T$, the multiset $C(t)$ consists of distinct variables and the multiset $C(s)$ consists of strongly normalizable terms.
...
\item There is a well-founded relation on the set $\Term_T$ such that each element of $C(s)$ is less than $t$ whenever $t \Rightarrow_R^* s$.
\end{enumerate}
\end{defn}
...
Indeed, we can define a well-founded relation on the set $\Term_T$ as follows: $t_1 > t_2$ if $|t_1| > |t_2|$ where $|t_i|$ is the length of the longest sequence of reductions starting from $t_i$.
...
Let $T$ be a theory such that, for every pair of terms $t$ and $s$ in $\Term_T$ such that $(t,s) \in R_T$, the multiset $C(t)$ consists of distinct variables and $C(s) \subseteq C(t)$.
Then the first and the third conditions of \rdefn{simp-ctx} hold for $T$.
The first condition holds for obvious reasons.
To prove the third condition, we define a well-founded relation on the set $\Term_T$ as follows: $t_1 > t_2$ if the size of $t_1$ is greater than the size of $t_2$.
...
Then the reduction relation on $\Term_T$ is confluent if and only if corresponding reduction relation on $\Term_T^c$ is confluent.
\end{prop}
\begin{proof}
