...
\begin{enumerate}
\item For every pair of terms $t$ and $s$, if $R(t,s)$, then $FV(s) \subseteq FV(t)$.
\item For every pair of terms $t$ and $s$ such that $R(t,s)$, there is at most one occurence of each variable in $t$.
...
We will say that a map of theories is \emph{confluent} if it isomorphic to a composition $T \to T' \to (T',R)$ for some (presented) relative $\Icell$ complex $T \to T'$ and some confluent relation $R$ on $T'$ such that,
for every pair of terms $t$ and $s$, if $R(t,s)$, then $t$ consists of new function symbols that were added to $T'$.

We will say that a theory $T$ is \emph{confluent} if the map $0 \to T$ is confluent.
\end{defn}

TODO: Prove that confluent theories are conservative.
