...
We will give another set of conditions that implies the same result in \rcor{ya-morita-conf}.
...
We already say an examples of such a Morita equivalence in \rprop{der-separated}.
...
By \rcor{conf-th}, % TODO: another proposition?
the theory $T_\Pi'$ is confluent.
...
The second condition of \rcor{conf-th} coincides with the condition of \rprop{ext-morita} which we just proved and the first condition of \rcor{conf-th} we proved earlier.
...
An axiom $\varphi \sststile{}{V} t = s$ is a \emph{pure axiom} if the function symbols $\ft$ and $\ty$ do not appear in $t$ and $s$.
...
Suppose that closed defined terms in which $\ty$ does not occur are confluent under $R^p_T$.
...
By assumption, closed defined terms in which $\ty$ does not occur are confluent under $R^p_T$.
...
Since reduction rules in $R_T^p$ do not contain function symbols $\ft$ and $\ty$, a reduction rule from $R_T^p$ and a reduction rule from $R_T^t$ can overlap only in a term of the form $e_p(\sigma(t_1, \ldots t_k))$ as follows:
...
Suppose that closed defined terms of $T_0$ in which $\ty$ does not occur are confluent under $R^p_T$.
Then closed defined terms of $T_1$ are confluent.
\end{cor}
\begin{proof}
...
But if we know that the conditions of \rdefn{confluent} hold, then this is true:
\begin{cor}[conf-th]
Let $T$ be a theory with well-defined typing axioms such that its pure axioms preserve types.
Suppose that the following conditions hold:
\begin{itemize}
\item Closed terms of $T$ in which $\ty$ does not occur are confluent under $R^p_T$.
...
\end{cor}
\begin{proof}
By \rcor{ya-morita-conf}, we just need to show that closed \emph{defined} terms of $T$ in which $\ty$ does not occur are confluent under $R^p_T$.
...
Indeed, if $a \Rightarrow b$, then there is an axiom $\varphi \sststile{}{FV(t)} t = s$ in $\mathcal{A}_e$ and a substitution $\rho$ such that $a = t[\rho]$ and $b = s[\rho]$.
Since $\sststile{T}{} t[\rho]\!\downarrow$, it follows that $\sststile{T}{} \varphi[\rho]$, so $\sststile{T}{} s[\rho]\!\downarrow$.
\end{proof}
...
Let $\Term_{\mathcal{F}_0}$ be the set of terms constructed from the function symbols in $\mathcal{F}_0 \cup \{ \ty, \ft \} \cup \{ c_1, \ldots c_n \}$.
