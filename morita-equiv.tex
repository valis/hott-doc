...
\section{Theories with simple axioms}

The axioms of type theories that occur in practice often can be divided in two parts: the first part determines when function symbols are defined and the second part introduces some equivalence relation on terms.
In this section we study such theories.

\subsection{Theories with simple axioms}

We will say that a theory \emph{has simple axioms} if it is isomorphic to a theory in which the set of axioms consists of three disjoint parts $\mathcal{A}_d$, $\mathcal{A}'_d$, and $\mathcal{A}_e$ such that the following conditions hold:
\begin{enumerate}
\item The set $\mathcal{A}_d$ consists of axioms of the form $\varphi \sststile{}{x_1, \ldots x_k} \sigma(x_1, \ldots x_k)\!\downarrow$.
For every $\sigma$, there is exactly one axiom of this form.
We will denote the left hand side of this axiom by $\varphi_\sigma$.
\item The set $\mathcal{A}'_d$ is a subset of the set of axioms of the form $\sigma(x_1, \ldots x_k)\!\downarrow\ \sststile{}{x_1, \ldots x_k} \varphi_\sigma$.
\item For every axiom $\varphi \sststile{}{V} \psi$ in $\mathcal{A}_e$ and every subterm $t$ of the formula $\psi$, the sequent $\varphi \sststile{}{V} t\!\downarrow$ is derivable from $\mathcal{A}_d \cup \mathcal{A}'_d$.
\end{enumerate}
We will say that a theory with simple axioms is \emph{minimal} if $\mathcal{A}'_d$ is empty.
We will say that a theory with simple axioms is \emph{maximal} if $\mathcal{A}'_d$ contains all of the axioms of the form $\sigma(x_1, \ldots x_k)\!\downarrow\ \sststile{}{x_1, \ldots x_k} \varphi_\sigma$.
...
In this section we define confluent theories, which have a useful property that Morita equivalences between them satisfy several closure properties.
We also prove several useful lemmas that characterize trivial fibrations in various model structures that we considered in previous section.
...
TODO
% If a type theory has well-defined function symbols, then often it can be described in a simple way.
% We can divide its axioms in two parts: the first part defines function symbols as described in the previous subsection and the second part introduces some equivalence relation on terms.
% Moreover, this equivalence relation is often defined in terms of a reduction relation.

\begin{defn}
A \emph{simple} type theory is a type theory $T$ with well-defined function symbols together with a relation $R$ on the set of terms of $T$
such that the set of axioms of $T$ consists of axioms that define its function symbols and axioms of the form
\[ e_p(t) = e_p(s) \sststile{}{FV(t) \cup FV(s)} t = s \]
for every $t$ and $s$ such that $R(t,s)$.
\end{defn}

The main property of simple theories is proved in the following lemma:

\begin{lem}
Let $(T,R)$ be a simple type theory.
If $\varphi \sststile{}{V} t = s$ is derivable, then there exist terms $t_1, \ldots t_n$ such that $t = t_1$, $s = t_n$, and, for every $1 \leq i < n$, $t_i = c[a/x]$ and $t_{i+1} = c[b/x]$
for some terms $a$, $b$, and $c$ such that one of the following conditions hold:
\begin{enumerate}
\item The sequent $\varphi \sststile{}{V} e_p(a) = e_p(b)$ is derivable and there exist terms $a'$, $b'$, $d_1$, \ldots $d_m$
such that $a = a'[d_1/x_1, \ldots d_m/x_m]$, $b = b'[d_1/x_1, \ldots d_m/x_m]$, and either $R(a',b')$ or $R(b',a')$.
\item $\varphi = \varphi_1 \land \ldots \land \varphi_k$ and there exists $j$ such that $\varphi_j$ equals to either $a = b$ or $b = a$.
\end{enumerate}
% Let $(T,R)$ be a simple type theory.
% Let $\varphi$ be a formula of $T$, let $V$ be a set of variables, and let $A$ be a term of $T$.
% We denote by $T_{\varphi,V,A}$ the set of terms $a$ of $T$ such that the sequent $\varphi \sststile{}{V} e_p(a) = A$ is derivable.
% The restriction of $R$ to $T_{\varphi,V,A}$ is denoted by $R_{\varphi,V,A}$.
% Then the restriction of the relation $\varphi \sststile{}{V} - = -$ to $T_{\varphi,V,A}$ is the smallest congruence containing $R_{\varphi,V,A}$.
\end{lem}
\begin{proof}
% If $R_{\varphi,V,A}(a,a')$, then the sequents $\varphi \sststile{}{V} e_p(a) = A$ and $\varphi \sststile{}{V} e_p(a') = A$ are deirable.
% Hence the sequent $\varphi \sststile{}{V} e_p(a) = e_p(a')$ is also derivable.
% Since $R(a,a')$, it follows that the sequent $\varphi \sststile{}{V} a = a'$ is also derivable.
% Thus, the relation $\varphi \sststile{}{V} - = -$ contains $R_{\varphi,V,A}$.
% Also, if $a \in T_{\varphi,V,A}$, then $\varphi \sststile{}{V} e_p(a) = A$ and hence $\varphi \sststile{}{V} a\!\downarrow$.
% Thus, the relation $\varphi \sststile{}{V} - = -$ is reflexive and the rules of the logic imply that it is a congruence.
% 
% Conversely, ?
\end{proof}
...
Let $\Rightarrow_R^*$ be the reflextive transitive closure of $\Rightarrow_R$.
...
\item If $R(t,s)$, then there is at most one occurence of each variable in $t$.
...
% maybe work with more general congruent theories?
...
First, let us prove an analog of \rlem{eq-char-fib} for weak lifting properties:
...
% properness of the model categories of models
