...
\begin{defn}[directed]
An algebraic type theory $T$ has \emph{directed axioms} if the following conditions are satisfied:
\begin{itemize}
\item $T$ has separated axioms and every axiom in $\mathcal{A}_e$ is of the form $\varphi \sststile{}{FV(t)} t = s$, where $t,s \in \Term_T$ and $t$ is not a variable.
\item For every pair $(\psi,V)$ in $P_M$, every axiom $\varphi \sststile{}{FV(t)} t = s$ in $\mathcal{A}_e$, and every substitution $\rho$, if $\psi \sststile{T}{V} t[\rho]\!\downarrow$, then $\psi \sststile{T}{V} \varphi[\rho]$.
\end{itemize}
...
A simple situation when the conditions of \rdefn{directed} hold is when $\mathcal{A}_e$ consists of axioms of the form $\{ t\!\downarrow\ \sststile{}{FV(t)} t = s\ \}$, which looks rather natural, but sometimes we need the more general definition.
For example, consider the following reduction rule:
\[ \app(B, \lambda(b), a) \Rightarrow b[a] \]
The left hand side is defined if and only if $\Pi(ty(b)) = \Pi(B) \land ty(a) = ft(B)$, but usually this rule is defined in such a way that it applies only when a stronger condition holds, namely $ty(b) = B \land ty(a) = ft(B)$.
This formula cannot be arbitrary since we still need to know that it is derivable whenever $t\!\downarrow$ is derivable.
The conditions of \rdefn{directed} guarantee that this is always true.

Now, we can define confluent theories:

\begin{defn}[confluent]
A \emph{confluent} type theory is a type theory $T_0$ with directed axioms such that, for every pair $(\varphi_1 \land \ldots \land \varphi_n, V)$ in $P_M$,
closed defined terms of the theory $T_1 = T_0 \cup \{ \sststile{}{} c_i\!\downarrow |\ 1 \leq i \leq n \} \cup \{ \varphi_i[c_1/x_1, \ldots c_i/x_i]\ |\ 1 \leq i \leq n \}$ are confluent.
\end{defn}

Note that if $T_0$ has directed axioms, then so does $T_1$, but this is not true for the confluence.
It may be the case that the underlying term rewriting system of $T_0$ is confluent, but $R_{T_1}$ is not,
and we need the stronger condition since otherwise we can apply \rprop{conf-main} only to sequents of the form $\sststile{}{} t = s$.
...
Let $T_0$ be a theory with directed axioms and let $T_1$ be a theory as defined in \rdefn{confluent}.
...
\item For every $t$ and $s$ such that $R_0(t,s)$, if $t$ contains a subterm of the form $e_p(t')$, then $t'$ is not a variable.
\end{itemize}
Then $T_0$ is confluent.
\end{lem}
\begin{proof}
...
We will give another set of conditions that implies that $T_0$ is confluent in appendix~\ref{sec:app-conf}.
...
Let $T_0$ be a confluent theory and let $(\varphi,V)$ be a pair in $P_M$.
If a sequent $\varphi \sststile{}{V} t = s$ is derivable in $T_0$, then there exists a term $r$ of $T_1$ such that $t[c_1/x_1, \ldots c_n/x_n] \Rightarrow_R^* r$ and $s[c_1/x_1, \ldots c_n/x_n] \Rightarrow_R^* r$,
where $T_1$ is the same as in \rlem{morita-conf} and $R$ is its underlying term rewriting system.
\end{prop}
\begin{proof}
By \rlem{mcf}, the sequent $\sststile{}{} t' = s'$ is derivable in $T_1$, where $t' = t[c_1/x_1, \ldots c_n/x_n]$ and $s' = s[c_1/x_1, \ldots c_n/x_n]$.
By \rlem{der-eq} and \rprop{der-separated}, there exists a sequence $t_1$, \ldots $t_n$ such that $t' = t_1$, $s' = t_n$, and, for every $1 \leq i < n$, either $t_i \Rightarrow_R t_{i+1}$ or $t_{i+1} \Rightarrow_R t_i$.
By \rlem{morita-conf}, this implies that $t' \Rightarrow_R^* r$ and $s' \Rightarrow_R^* r$ for some term $r$.
\end{proof}
...
By \rprop{conf-main}, there exists a term $r$ such that $f(t) \Rightarrow_R^* r$ and $f(s) \Rightarrow_R^* r$.
...
% We proceed by induction on the length of the longest chain of reductions starting in $t$ ignoring the reductions of the form $ty(t_1) \Rightarrow t_2$ (we can do this since this relation is also strongly normalizable).
% The only interesting case is when $t = \app(B, \lambda(b), a)$.
% In this case, $\psi \sststile{}{V} \Pi(ty(b)) = \Pi(B)$ and we need to prove that $\psi \sststile{}{V} ty(b) = B$.
% \Rprop{conf-main} implies that $\Pi(ty(b))$ and $\Pi(B)$ reduce to the same term and, since there is no reduction rule of the form $\Pi(t) \Rightarrow s$, this implies that $ty(b)$ and $B$ also reduce to the same term.
% The induction hypothesis applies to $ty(b)$ and $B$ and implies that $\psi \sststile{}{V} ty(b) = B$.
...
\section{Union of conlfuent theories}

The aim of this appendix is to prove the following general lemma about term rewriting systems:

\begin{lem}[nf-conf]
Let $R_0$ and $R_1$ be term rewriting systems on the same theory $T$.
Let $A$ be a subset of $\Term_T$ closed under $\Rightarrow_{R_0 \cup R_1}$.
Suppose that $(A,\Rightarrow_{R_0})$ is confluent, $(A,\Rightarrow_{R_1})$ is confluent and weakly normalizable, and
if $a,b \in A$ are such that $a \Rightarrow_{R_0} b$, then there is a term $c$ such that $a \Rightarrow_{R_1}^{nf} \Rightarrow_{R_0}^* c$ and $b \Rightarrow_{R_1}^{nf} \Rightarrow_{R_0}^* c$.
Then the system $(A,\Rightarrow_{R_0 \cup R_1})$ is confluent.
Moreover, if $a,b \in A$ are equivalent, then there is a term $c$ such that $a \Rightarrow_{R_1}^{nf} \Rightarrow_{R_0}^* c$ and $b \Rightarrow_{R_1}^{nf} \Rightarrow_{R_0}^* c$.
\end{lem}
\begin{proof}
First, note that we have the following diagram:
\[ \xymatrix@=1em{                                      & \ar[ld]_{R_0} a \ar[rd]^{R_1^{nf}}    &                           \\
                   a_1 \ar@{-->}[rd]_{R_1^{nf} R_0^*}   &                                       & a_2 \ar@{-->}[ld]^{R_0^*} \\
                                                        & a'                                    &
                 } \]
This diagram means that, for every $a \in A$, if $a \Rightarrow_{R_0} a_1$ and $a \Rightarrow_{R_1}^{nf} a_2$,
then there is a term $a'$ such that $a_1 \Rightarrow_{R_1}^{nf} \Rightarrow_{R_0}^* a'$ and $a_2 \Rightarrow_{R_0}^* a'$.
Indeed, by assumption, there are elements $a'$ and $a_2'$ such that $a_1 \Rightarrow_{R_1}^{nf} \Rightarrow_{R_0}^* a'$, $a \Rightarrow_{R_1}^{nf} a_2'$, and $a_2' \Rightarrow_{R_0}^* a'$.
Since $R_1$ is confluent, $a_2 = a_2'$.

It follows that we have the following diagram:
\[ \xymatrix@=1em{                                      & \ar[ld]_{R_0^*} t \ar[rd]^{R_1^{nf} R_0^*}    &                           \\
                   t_1 \ar@{-->}[rd]_{R_1^{nf} R_0^*}   &                                               & t_2 \ar@{-->}[ld]^{R_0^*} \\
                                                        & t'                                            &
                 } \]
Indeed, we can prove this by induction on the length of the top left reduction sequence.
This is obvious if it is empty.
Otherwise, this follows from the following diagram:
\[ \xymatrix@=1em{                                      &                                                       & \ar[ld]_{R_0} a \ar[rd]^{R_1^{nf}}    &                                               &                               \\
                                                        & a_1 \ar[ld]_{R_0^*} \ar@{-->}[rd]_{R_1^{nf} R_0^*}    &                                       & a_1' \ar@{-->}[ld]^{R_0^*} \ar[rd]^{R_0^*}    &                               \\
                   a_1' \ar@{-->}[rd]_{R_1^{nf} R_0^*}  &                                                       & a' \ar@{-->}[ld]^{R_0^*}              &                                               & a_2' \ar@{-->}[lldd]^{R_0^*}  \\
                                                        & a_1'' \ar@{-->}[rd]_{R_0^*}                           &                                       &                                               &                               \\
                                                        &                                                       & a''                                   &                                               &
            } \]

It is also easy to see that we have the diagram on the left.
It follows that we also have the diagram on the right.
\[ \xymatrix@=1em{                                      & \ar[ld]_{R_1^*} a \ar[rd]^{R_1^{nf} R_0^*}    &                   \\
                   a_1 \ar@{-->}[rd]_{R_1^{nf} R_0^*}   &                                               & a_2 \ar@{=}[ld]   \\
                                                        & a_2                                           &
                 }
\qquad
   \xymatrix@=1em{                                      & \ar[ld]_{(R_0 \cup R_1)^*} a \ar[rd]^{R_1^{nf} R_0^*} &                           \\
                   a_1 \ar@{-->}[rd]_{R_1^{nf} R_0^*}   &                                                       & a_2 \ar@{-->}[ld]^{R_0^*} \\
                                                        & a'                                                    &
                 } \]

Since $R_1$ is weakly normalizing, we have the following diagram, which proves that the system $(A,\Rightarrow_{R_0 \cup R_1})$ is confluent:
\[ \xymatrix@=1em{                                      &                               & \ar[lldd]_{(R_0 \cup R_1)^*} a \ar@{-->}[dd]^{R_1^{nf}} \ar[rrdd]^{(R_0 \cup R_1)^*}  &                               &                                       \\
                                                        &                               &                                                                                       &                               &                                       \\
                   a_1' \ar@{-->}[rd]_{R_1^{nf} R_0^*}  &                               & a' \ar@{-->}[ld]_{R_0^*} \ar@{-->}[rd]^{R_0^*}                                        &                               & a_2' \ar@{-->}[ld]^{R_1^{nf} R_0^*}   \\
                                                        & a_1'' \ar@{-->}[rd]_{R_0^*}   &                                                                                       & a_2'' \ar@{-->}[ld]^{R_0^*}   &                                       \\
                                                        &                               & a''                                                                                   &                               &
            } \]

Finally, let $a,b \in A$ be equivalent terms.
Then there is a term $c$ such that $a \Rightarrow_{R_0 \cup R_1}^* c$ and $b \Rightarrow_{R_0 \cup R_1}^* c$.
The following diagram prove the last statement:
\[ \xymatrix@=1em{                              & \ar@{-->}[ld]_{R_1^{nf}} a \ar[rd]    &                                                   & \ar[ld] b \ar@{-->}[rd]^{R_1^{nf}}    &                           \\
                   a' \ar@{-->}[rdd]_{R_0^*}    &                                       & c \ar@{-->}[d]^{R_1^{nf}}                         &                                       & b' \ar@{-->}[ldd]^{R_0^*} \\
                                                &                                       & c' \ar@{-->}[ld]_{R_0^*} \ar@{-->}[rd]^{R_0^*}    &                                       &                           \\
                                                & a_1'' \ar@{-->}[rd]_{R_0^*}           &                                                   & a_2'' \ar@{-->}[ld]^{R_0^*}           &                           \\
                                                &                                       & a''                                               &                                       &
            } \]
\end{proof}
...
It is well-known that standard examples of type theories (such as theories of identity types, $\Sigma$-types, and $\Pi$-types) are confluent and strongly normalizing.
...
In this appendix we prove that it is often possible to show that an algebraic type theory is confluent if corresponding usual type theory is.

We will work with contextual theories with directed axioms (see \cite{alg-tt} for a definition of a contextual theory).
If $T$ is a contextual theory, then we have a set of function symbols $\mathcal{F}_0$ such that the set of function symbols of $T$ consists of the following function symbols:
...
First, we will show that a theory is confluent whenever the subsystem on terms without $ty$ is.
To do so, we need to put a few natural conditions on $T$.
We assume that, for every reduction rule $R(t,s)$, the following conditions hold:
\begin{itemize}
\item The function symbol $ty$ either does not appear in $t$ or appears only on the top level.
\item if $s$ has a subterm of the form $ty(s')$, then $s'$ is a variable.
\end{itemize}
Moreover, we need to assume that the term rewriting system consisting of the rules of this form is weakly normalizing.

Throught this appendix, let $T_0$ be a fixed contextual theory satsfying these conditions.

\begin{prop}
Let $R_1$ be the set of reduction rules of the form $ty(t) \Rightarrow s$ and let $R_0$ be the rest of reduction rules.
If the term rewriting system $(\Term_{T \setminus \{ ty \}}, R_0)$ is confluent, then $T$ is also confluent.
\end{prop}

\begin{lem}[ya-morita-conf]
Let $T_0$ be a confluent theory and let $(\varphi_1 \land \ldots \land \varphi_n, V)$ be a pair in $P_M$.
Then the theory $T_1 = T_0 \cup \{ \sststile{}{} c_i\!\downarrow |\ 1 \leq i \leq n \} \cup \{ \varphi_i[c_1/x_1, \ldots c_i/x_i]\ |\ 1 \leq i \leq n \}$ is also confluent.
\end{lem}
\begin{proof}
Let $R_1 = R_{T_1} \setminus R_0$.
It is easy to see that $R_1$ is confluent and strongly normalizing.
The condition on $R_0$ formulated in \rdefn{confluent} implies that $R_0$-reductions preserve $R_1$-normal forms.
...
\[ \xymatrix@=1em{                              & \ar[ld]_{R_0} t \ar[rd]^{R_1^{nf}}    &                           \\
                   t_1 \ar@{-->}[rd]_{R_1^{nf}} &                                       & t_2 \ar@{-->}[ld]^{R_0}   \\
                                                & t'                                    &
                 } \]
Since the systems are orthogonal, $R_0$-redexes and $R_1$-redexes do not overlap.
Thus we may assume that all $R_1$-redexes occur inside the $R_0$-redex.
...
Since $\Rightarrow_{R_0}$-reductions preserve $R_1$-normal forms, $t'$ is in a normal form, so $t_1 \Rightarrow_{R_1}^{nf} t'$.
\Rlem{nf-conf} implies that $T_1$ is confluent.
\end{proof}

Let $\mathcal{F}_0$ be the set of basic function symbols of the theory $T_0$.
Let $T_1$ be the extension of $T_0$ defined in \rdefn{confluent} (for some pair $(\varphi,V) \in P_M$) and let $R$ be its underlying term rewriting system.
Let $\Term_{\mathcal{F}_0}$ be the set of terms constructed from the function symbols in $\mathcal{F}_0 \cup \{ ty, ft \} \cup \{ c_1, \ldots c_n \}$.
So, $\Term_{\mathcal{F}_0}$ is the set of terms of $T_1 = T_0 \cup \{ c_1, \ldots c_n \}$ in which we omit contexts.
If $t$ is a term of $T_1$, then we can define a term $U(t) \in \Term_{\mathcal{F}_0}$ in the obvious way:
\begin{align*}
U(x) & = x \\
U(\sigma_m(\Gamma, t_1, \ldots t_k)) & = \sigma(U(t_1), \ldots U(t_k))
\end{align*}

Let $\Term_T$ be the set of closed defined terms of $T_1$, that is the set of terms $t$ such that the sequent $\sststile{}{} t\!\downarrow$ is derivable.
Let $\Term_T^c$ be the image of $\Term_T$ under $U$.
The reduction relation on $\Term_T$ induces a reduction relation on $\Term_T^c$ in the obvious way.
If $t \in \Term_T$, then let $C(t)$ be the multiset of terms discarded by $U$.
...
Let $T_0$ be a contextual theory with directed axioms such that, for all terms $t$ and $s$ in $\Term_T$ such that $R(t,s)$,
the multiset $C(t)$ consists of distinct variables and the multiset $C(s)$ consists of strongly normalizable terms.
Such a theory satisfies the following properties:
\begin{enumerate}
\item \label{it:red-fib} Let $t$ be a term in $\Term_T$ such that $U(t) \Rightarrow_R s$ for some term $s$.
...
\end{enumerate}
This is a very mild assumption that holds for all theories that occur in practice.

Now, we can show that $R$ is normalizing if and only if it becomes normalizing after applying $U$:
\begin{prop}
The reduction relation on $\Term_T$ is strongly normalizing if and only if the same is true for the induced reduction relation on $\Term_T^c$.
\end{prop}
\begin{proof}
If there is an infinite sequence $t_1 \Rightarrow_R t_2 \Rightarrow_R \ldots$ in $\Term_T^c$, then there is a term $t_1'$ such that $U(t_1)$ and \eqref{it:red-fib} implies that there is an infinite sequence of reductions starting with $t_1'$.
Conversely, suppose that $\Term_T^c$ is strongly normalizing.
We prove by induction on a term $t_1 \in \Term_T$ that there is no infinite reduction sequences starting with $t_1$.
Let $t_1 \Rightarrow_R t_2 \Rightarrow_R \ldots$ be such a sequence.
Note that, for every $i \geq 1$, every $\Gamma \in C(t_i)$ is strongly normalizable.
Indeed, by the induction hypothesis, this is true for $i = 1$.
Every term $\Gamma$ in $C(t_{i+1})$ belongs to either $C(t_i)$ or $C(s)$ for some term $s$ such that $R(t,s)$.
In both cases, $\Gamma$ is strongly normalizable.
This implies that the sequence $U(t_1) \Rightarrow_R^* U(t_2) \Rightarrow_R^* \ldots$ contains an infinite subsequence of reductions.
\end{proof}

% 1. Get rid of ft
% 2. Get rid of contexts
% 3. Get rid of ty -- ???
% 4. Get rid of subst

It is harder to prove analogous result for confluence and we need an additional assumption for that:
\begin{prop}
If the reduction relation on $\Term_T$ is confluent, then the same is true for the induced reduction relation on $\Term_T^c$.
The converse holds if there is a well-founded relation on the set $\Term_T$ such that each element of $C(s)$ is less than $t$ whenever $t \Rightarrow_R^* s$.
\end{prop}
\begin{proof}
If $\Rightarrow_R$ is confluent on $\Term_T$, then it is easy to show that the same is true for $\Term_T^c$.
Indeed, if we have a term $t \in \Term_T^c$ such that $t \Rightarrow_R^* t_1$ and $t \Rightarrow_R^* t_2$,
then there is a term $t' \in \Term_T$ and \eqref{it:red-fib} implies that there are terms $t_1'$ and $t_2'$ such that $t' \Rightarrow_R^* t_1'$, $t' \Rightarrow_R^* t_2'$, $U(t_1') = t_1'$, and $U(t_2') = t_2'$.
...
Conversely, let $t_1$ and $t_2$ be equivalent terms in $\Term_T$.
...
TODO
Let us show that $j$-th elements in sequences $C(s_1)$ and $C(s_2)$ are equivalent for every $j$.
The first elements of $C(s_i)$ are equivalent to $ctx(s_i)$ and they are equivalent since $s_1$ and $s_2$ are.

The first assumption of this proposition implies that contexts of $s_1$ and $s_2$ are equivalent.
The second assumption implies that every term in $C(s_1)$ is less than $t_1$, so we can apply the induction hypothesis to conclude that $j$-th terms in $C(s_1)$ and $C(s_2)$ reduce to the same term for every $j$.
This implies that $s_1$ and $s_2$ also reduce to the same term.
\end{proof}

% For every term $t = \sigma_m(\Gamma, t_1, \ldots t_k)$ and every $1 \leq i \leq k$, there exists a term $C_i$ such that there is a zig-zag of reductions between $ctx(t_i)$ and $C_i$.

% We also want to construct a term $F(\Gamma,t) \in \Term_\mathcal{F}(V)_{(p,n)}$ for every pair of terms $\Gamma \in \Term_\mathcal{F}(V)_{(ctx,n)}$ and $t \in \Term_{\mathcal{F}_0}(V)_{(p,n)}$.
% To do this, we assume that every function symbol $\sigma \in \mathcal{F}_0$, $\sigma : (p_1,n_1) \times \ldots \times (p_k,n_k) \to (p,0)$ is equipped 

% We prove by induction on t and s that if t ~ s, then there is a term r such that t => r and s => r.
% Indeed, we have U(t) => r' <= U(s). Then t => r1 and s => r2 such that U(r1) = U(r2) = r.
% Now, we just need to show that, for every position in $r$, the corresponding contexts in $r1$ and $r2$ reduce to the same term.
% We know that they are equivalent since r1 and r2 are.
% Every context in $r1$ comes from a context in $t$, so we can apply the induction hypothesis.
