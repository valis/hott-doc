...
\begin{lem}
For every well-behaved term $t$ of sort $(p,n)$,
\[ \subst_{p,m,n}(t, a_1, \ldots a_n) \Rightarrow_s^\nf L^m(t)[\rho], \]
where $\rho$ replaces every subterm of the form $\subst_{q,m+l,m+i}(x, v_{m+l-1}, \ldots v_l, t_1, \ldots t_i)$ with the term $\subst_{q,m+l,n+i}(x, \wk_{\tm,m}^l(a_1), \ldots \wk_{\tm,m}^l(a_n), t_1, \ldots t_i)$
and every variable $y$ with $\subst_{q,m+j,n+j}(y, \wk_{\tm,m}^j(a_1), \ldots \wk_{\tm,m}^j(a_n), v_{j-1}, \ldots v_0)$.
\end{lem}
\begin{proof}
It is easy to prove by induction on a subterm $s$ of $t$ that
\[ \subst_{q,m+l,n+l}(s, \wk_{\tm,m}^l(a_1), \ldots \wk_{\tm,m}^l(a_n), v_{l-1}, \ldots v_0) \Rightarrow_s^\nf L^m(s)[\rho]. \]
...
Let $c'$ be the $\Rightarrow_s$-normal form of $U(c)$ and let $\rho'$ be the $\Rightarrow_s$-normal form of $U(\rho)$.
Note that $U(c)$ contains at most one occurrence of $x$, but $c'$ may contain several such occurrences.
Let us show that $\Rightarrow_s$-redexes in the term $c'[U(a)[\rho']/x]$ that do not belong to $U(a)$ do not overlap.
...
Now, a $\Rightarrow_s$-redex in the term $c'[U(a)[\rho']/x]$ either belongs to $U(a)$ or it comes from a subterm of $c'$ of the form $\subst_{p,n,k}(x, a_1, \ldots a_k)$ and the fact that we just proved implies that $a_1, \ldots a_k$ do not contain such subterms.

Since $\Rightarrow_s$-redexes in the term $c'[U(a)[\rho']/x]$ do not overlap, we may consider them one at a time.
Thus we just need to prove that every pair of terms of the form $\subst_{p,n,k}(U(a)[\rho'], a_1, \ldots a_k)$ and $\subst_{p,n,k}(U(\nf^t(b))[\rho'], a_1, \ldots a_k)$ is joinable under $\Rightarrow_s^\nf (\Rightarrow_{sf} \Rightarrow_s^\nf)^*$.
Since $\subst$-free rules are well-behaved, it is true that $a' (\Rightarrow_{sf} \Rightarrow_s^\nf)^* b'$, where $a'$ and $b'$ are $\Rightarrow_s$-normal forms of $U(a)[\rho']$ and $U(\nf^t(b))[\rho']$, respectively.
\end{proof}
