...
Now we can prove that the confluence under $R^{tf}_T$ implies the confluence under $R^{tf}_T \cup R^t_T$ for closed defined terms (that is, terms $t$ such that the sequent $\sststile{}{} t\!\downarrow$ is derivable):

\begin{prop}
Let $T$ be a theory with well-defined typing axioms such that its $\ty$-free axioms preserve types.
Suppose that closed defined terms in which $\ty$ does not occur are confluent under $R^{tf}_T$.
...
By assumption, closed defined terms in which $\ty$ does not occur are confluent under $R^{tf}_T$.
...
Thus we can apply \rlem{conf-nf} to show that the terms $t$ and $s$ are joinable under $\Rightarrow_{R_T^t}^{nf} \Rightarrow_{R_T^{tf}}^*$.
...
Suppose that closed defined terms of $T_0$ in which $\ty$ does not occur are confluent under $R^{tf}_T$.
Then closed defined terms of $T_1$ are confluent.
\end{cor}
\begin{proof}
...
Suppose that the following conditions hold:
\begin{itemize}
\item Closed terms of $T$ in which $\ty$ does not occur are confluent under $R^{tf}_T$.
...
\end{cor}
\begin{proof}
By \rcor{ya-morita-conf}, we just need to show that closed \emph{defined} terms of $T$ in which $\ty$ does not occur are confluent under $R^{tf}_T$.
...
\subst_{\tm,n,n}(\Gamma, a, v_{k-1}(\Gamma), \ldots v_0(\Gamma)) & \Rightarrow a \\
...
Let $\Term_T$ be the set of closed defined terms of $T$ in which function symbols $\ty_m$ and $\ft_m$ do not occur
and let $\Term_T^s$ be the subset of $\Term_T$ consisting of terms in which function symbols $\subst_{p,n,k}$ do not occur.

\begin{prop}
Let $T$ be a theory in which axioms are well-behaved with respect to substitution.
Then the reduction relation on $\Term_T$ is confluent if and only if the induced reduction relation on $\Term_T^s$ is confluent.
\end{prop}
\begin{proof}
...
If $T$ has separated typing axioms, then $\ty$-free axioms of $T$ induces a reduction relation on $\Term_T^c$ in the obvious way.
